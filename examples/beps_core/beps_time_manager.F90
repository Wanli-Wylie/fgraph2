...
! Public methods
   ...
   !*************************************************************************** lm
      timemgr_diff_secs,        &! calculate difference between to times in seconds
   !*************************************************************************** lm
      ...
      get_curr_date,            &! return date components at end of current tartimestep
...
! Public parameter data
   ...
   integer,parameter :: isecspday = 86400
...
!=========================================================================================
   ...
   subroutine get_timemgr_defaults( calendar_out,      start_ymd_out,     start_tod_out, ref_ymd_out,        &
      ...
      !
      character(len=*), parameter :: sub = 'beps::get_timemgr_defaults'
...
!=========================================================================================
   ...
   subroutine set_timemgr_init( calendar_in,      start_ymd_in,     start_tod_in, ref_ymd_in,        &
      ...
      !
      character(len=*), parameter :: sub = 'beps::set_timemgr_init'
...
!=========================================================================================
   ...
   subroutine timemgr_init( )
      ...
      !---------------------------------------------------------------------------------
!!  call timemgr_spmdbcast( )    offline run not used @J.Wang
...
!=========================================================================================
   ...
   subroutine init_clock( start_date, ref_date, curr_date, stop_date )
      ...
      tm_clock = ESMF_ClockCreate(name="BEPS Time-manager clock", timeStep=step_size, startTime=start_date, &
...
!=========================================================================================
   ...
   subroutine timemgr_restart( )
      ...
      !
      character(len=*), parameter :: sub = 'beps::timemgr_restart'
      integer :: rc                            ! return code
      integer :: yr, mon, day, tod             ! Year, month, day, and second as integers
      type(ESMF_Time) :: start_date            ! start date for run
...
!=========================================================================================
   ...
   subroutine calc_nestep()
      ...
      !
      character(len=*), parameter :: sub = 'beps::calc_nestep'
...
!=========================================================================================
   ...
   subroutine init_calendar( )
      ...
      ! Local variables
      !
      character(len=*), parameter :: sub = 'beps::init_calendar'
...
!=========================================================================================
   ...
   subroutine timemgr_print()
      ...
      !---------------------------------------------------------------------------------
      character(len=*), parameter :: sub = 'beps::timemgr_print'
      integer :: rc
      integer :: yr, mon, day
      ...
      type(ESMF_TimeInterval) :: t_step ! Time-step @J.Wang change step into t_step, because step is already used in module beps_par
      !---------------------------------------------------------------------------------

      call ESMF_ClockGet( tm_clock, startTime=start_date, currTime=curr_date, &
         refTime=ref_date, stopTime=stop_date, timeStep=t_step, &
         advanceCount=step_no, rc=rc )
      call chkrc(rc, sub//': error return from ESMF_ClockGet')
      nstep = step_no

      write(iulog,*)' ******** BEPS Time Manager Configuration ********'

      call ESMF_TimeIntervalGet( t_step, s=step_sec, rc=rc )
...
!=========================================================================================
   ...
   subroutine advance_timestep()
      ...
      character(len=*), parameter :: sub = 'beps::advance_timestep'
      integer :: rc
...
!=========================================================================================
   ...
   subroutine get_clock( clock )
      ...
      character(len=*), parameter :: sub = 'beps::get_clock'
...
!=========================================================================================
   ...
   function get_curr_ESMF_Time( )
      ...
      character(len=*), parameter :: sub = 'beps::get_curr_ESMF_Time'
      integer :: rc
...
!=========================================================================================
   ...
   integer function get_step_size()
      ...
      character(len=*), parameter :: sub = 'beps::get_step_size'
      type(ESMF_TimeInterval) :: step_size       ! timestep size
      integer :: rc
...
!=========================================================================================
   ...
   integer function get_nstep()
      ...
      character(len=*), parameter :: sub = 'beps::get_nstep'
      integer :: rc
      integer(ESMF_KIND_I8) :: step_no
...
!=========================================================================================
   ...
   subroutine get_curr_date(yr, mon, day, tod, offset)
      ...
      character(len=*), parameter :: sub = 'beps::get_curr_date'
      integer :: rc
      type(ESMF_Time) :: date
...
! Local variables
      character(len=*), parameter :: sub = 'beps::get_prev_date'
      integer :: rc
      type(ESMF_Time) :: date
!-----------------------------------------------------------------------------------------
...
! Local variables
      character(len=*), parameter :: sub = 'beps::get_driver_start_ymd'
!-----------------------------------------------------------------------------------------
...
! Local variables
      character(len=*), parameter :: sub = 'beps::get_ref_date'
      integer :: rc
      type(ESMF_Time) :: date
!-----------------------------------------------------------------------------------------
...
! Local variables
      character(len=*), parameter :: sub = 'beps::get_prev_time'
      integer :: rc
...
! Return value
      ...
      real(r8) :: get_curr_calday_r8 ! copy from iLab 2025/02/17

! Local variables
      character(len=*), parameter :: sub = 'beps::get_curr_calday'
      integer :: rc
      type(ESMF_Time) :: date
...
!-----------------------------------------------------------------------------------------
      ...
      call ESMF_TimeGet( date, dayOfYear_r8=get_curr_calday_r8, rc=rc )
      call chkrc(rc, sub//': error return from ESMF_TimeGet')
      !----------------------------------------------------------------------------------------!
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      if ( (get_curr_calday_r8 > 366.0) .and. (get_curr_calday_r8 <= 367.0) .and. &
         (trim(calendar) == GREGORIAN_C) )then
         get_curr_calday_r8 = get_curr_calday_r8 - 1.0_r8
      end if
      !!!!!!!!!!!!!! END HACK TO ENABLE Gregorian CALENDAR WITH SHR_ORB !!!!!!!!!!!!!!!!!!!!!!!!
      !----------------------------------------------------------------------------------------!
      if ( (get_curr_calday_r8 < 1.0) .or. (get_curr_calday_r8 > 366.0) )then
         write(iulog,*) sub, ' = ', get_curr_calday_r8
         if ( present(offset) ) write(iulog,*) 'offset = ', offset
         call endrun( sub//': error get_curr_calday out of bounds' )
      end if

      get_curr_calday = int(get_curr_calday_r8) ! 2025/02/17 copy from iLab
...
! Local variables
      character(len=*), parameter :: sub = 'beps::get_calday'
      integer :: rc                 ! return code
      type(ESMF_Time) :: date
!-----------------------------------------------------------------------------------------
...
!=========================================================================================
   ...
   integer function get_days_per_year( offset )
      ...
      character(len=*), parameter :: sub = 'beps::get_days_per_year'
...
!=========================================================================================
   ...
   subroutine set_nextsw_cday( nextsw_cday_in )
      ...
      character(len=*), parameter :: sub = 'beps::set_nextsw_cday'
...
!-----------------------------------------------------------------------------------------
      ...
      call get_curr_date(yr, mon, day, tod)
      is_end_curr_year = (mon==12 .and. day == 31  .and.  tod == 82800)
...
! Local variables
      character(len=*), parameter :: sub = 'beps::is_first_step'
      integer :: rc
      !-- iLab::fixed typing
      integer(ESMF_KIND_I8) :: nstep
      !integer :: nstep
      integer(ESMF_KIND_I8) :: step_no
!-----------------------------------------------------------------------------------------
...
! Local variables
      character(len=*), parameter :: sub = 'beps::is_last_step'
...
! Local variables
      character(len=*), parameter :: sub = 'beps::timemgr_datediff'
      integer :: rc   ! return code
...
!***************************************************************************************************  lm
   subroutine timemgr_diff_secs(ymd1, tod1, ymd2, tod2, secs)

      ! Calculate the difference (ymd2,tod2) - (ymd1,tod1) and return the result in seconds.
      ...
      real(kind=8), intent(out) :: secs ! (ymd2,tod2)-(ymd1,tod1) in seconds

      ! Local variables
      character(len=*), parameter :: sub = 'beps::timemgr_diff_secs'
      integer :: rc   ! return code

      type(ESMF_Time) :: date1
      type(ESMF_Time) :: date2
      type(ESMF_TimeInterval) :: diff
      integer(ESMF_KIND_I8) :: isecs
      !-----------------------------------------------------------------------------------------

      date1 = TimeSetymd( ymd1, tod1, "date1" )
      date2 = TimeSetymd( ymd2, tod2, "date2" )
      diff = date2 - date1
      call ESMF_TimeIntervalGet( diff, s_i8=isecs, rc=rc )
      call chkrc(rc, sub//': error return from ESMF_TimeIntervalGet')
      ! write(*,*)'debug2 secs=',secs

      !-- expicit cast of integer8 to real8 (*should* be safe)
      secs = real(isecs, kind=8)
      ! write(*,*)'debug3 secs=',secs
   end subroutine timemgr_diff_secs
   !***************************************************************************************************  lm
...
!  use clm_varctl, only : nsrest, nsrContinue
      use beps_par, only : nsrest, nsrContinue   !! @J.Wang
