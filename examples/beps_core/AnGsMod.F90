...
subroutine Vcmax_Jmax(lai_o, clumping, Vcmax0, slope_Vcmax_N, leaf_N, CosZs,  &
...
implicit none
    ...
    ! Prevent negative values for Trans
...
subroutine photosynthesis(LC,temp_leaf_p,rad_leaf,e_air,g_lb_w,vc_opt,f_soilwater,b_h2o, &
                          m_h2o,cii,temp_leaf_c,LH_leaf,Gs_w,gs_h2o_mole,aphoto,ci,ffpa,xSIF,&
                          f_feileaf,fw_flag ) ! 2025/02/16
...
!*************************************************************************
 implicit none
    ...
    real(r8)  :: kt25,VT,RT,KT,Alpha,Qp, c4_a, c4_b, c4_c,c4_d,product
...
! 2024/01/07 flag for choosing the form of water stress on stomatal conductance
    ...
    ca=CO2_air
    iphoton = 4.55*0.5*rad_leaf ! 0.5 could be set as parameter: f_leaf
...
!       iphoton = 4.55*0.5*rad_leaf
        ...
        iphoton = 4.55*0.5*rad_leaf   ! replace 0.5 to a coefficient, f_leaf, leaf respiration rate, for optimization purpose, @MOUSONG WU, 2020-09-14
