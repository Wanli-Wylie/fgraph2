...
type,public::Phen
    ...
    real(r8) :: dvi  ! crop development index     !DVI需要分配npoints个元素的数组来存储各格网的值,此时的npoints= 10011！！！
    real(r8) :: rootc  ! root biomass (kg m-2)
    ! real(r8),DIMENSION(10011,PFT) :: harvc  ! crop yield (kg m-2)
...
! Initialization process
 ...
 case(1004)    !rice
      param%t_bse= 8+273.15                                                                    
			...
			param%t_max= 42+273.15                                                                    
			  ! Maximum temp for development (K).
			param%tt_emr= 60  +273.15      !注意检查单位看是否需要转化成开式度，加上273.15                                                         
			  ! Thermal time for emergence (degree days).
			param%crit_pp= 24.                                                                 
			  ! Critical daylength for photoperiod sensitivity (hours).
			param%pp_sens= 0.                                                                 
 ...
 subroutine beps_crop_Development(i,j,param,phenp)
  ...
  if ( phenp%temp_daily <= param%t_opt .AND. phenp%temp_daily >= param%t_bse ) then
	...
	if ( bound%phot_type(i,j)==1.0 .AND. phenp%dvi < 1.0 ) then
			...
			phenp%dvi = phenp%dvi + phenp%rpe * (phenp%teff) / bound%tt_veg(i,j) 
			! write(*,*)"1:teff,tt_veg",phenp%teff,bound%tt_veg(i,j)
			!write(*,*)'dvi = ',phenp%dvi
			...
			phenp%dvi = phenp%dvi + (phenp%teff)/ bound%tt_rep(i,j)
			! write(*,*)"2:teff,tt_rep",phenp%teff,bound%tt_rep(i,j)
			!write(*,*)'dvi = ',phenp%dvi
...
!------------------------------------------------------------------------------------------------
 ...
 subroutine beps_crop_pool_alloc(param,phenp) 
  ...
  ! Calculate Partition coefficients
	...
	! write(*,*)"phenp%npp_ft_acc = ",phenp%npp_ft_acc
	! write(*,*)"phenp%reservec = ",phenp%reservec
	! write(*,*)"phenp%leafc", phenp%leafc
	! write(*,*)'1 harv = ', phenp%harvc
	...
	if ( phenp%dvi > param%sen_dvi ) then   !param%sen_dvi= 0, (phenp%sen_fac * phenp%leafc) == 0 always
		
			!phenp%sen_fac = param%mu * (  - param%sen_dvi )** param%nu
	...
	!no_carbon_pools_below_minimum(i,param,phenp) 
 ...
 subroutine carbon_fraction_from_dvi(param,phenp)
  ...
  REAL(r8) :: denom
	...
	! write(*,*)"f_root = ",phenp%f_root
	! write(*,*)"f_stem = ",phenp%f_stem
	! write(*,*)"f_leaf = ",phenp%f_leaf
	! write(*,*)"f_harv = ",phenp%f_harv
