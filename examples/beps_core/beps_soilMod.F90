...
!    real(r8)  :: soil_r     ! not used
    ...
    real(r8)  :: f_soilwater
    real(r8)  :: f_feileaf   ! water stress factor from leaf water potential
    real(r8)  :: Sp          ! initial plant water storage

!!! Properties belong to each soil horizon
    real(r8)  :: d_soil(0:MAX_LAYERS-1)   ! soil depth for each layer
    real(r8)  :: f_root(0:MAX_LAYERS-1)   !root weight/root fraction for each layer
   ...
   ! read from patameters
    ...
    real(r8)  :: psi_sat(0:MAX_LAYERS-1)       ! soil matric water potential in sat
...
!!   ! derived variables
    ...
    real(r8)  :: psim_prev(0:MAX_LAYERS-1)     ! for updating psim
...
! I have considered using struct pointer in bepstype and bepstypeInit, but there were some bugs with passing values
! 2023/12/15, failed to pass on the value of theta_Amin, so I used struct @Lu HU
...
! Initialization process
 ...
 subroutine Init_planthydraulics_para(phydra)
    ...
    phydra%p_delta=0.5
    phydra%ppslh=0.5/1000.  ! may be  x/10^5
 ...
 p%Zsp   = snowdepth
 p%r_rain_g    = 0.0
 ! initialized plant water storage
 p%Sp = 1.e-6
 ...
 ! 2024/01/07 initialized the factor of leaf water potential
 p%f_feileaf = 1.0
 !p%f_Tleaf = 1.0
 ! 2025/01/24
 p%KK(0:4)=1.e-12
...
! 2023/12/13 after checked and add to update thetam and psim
 ...
 !real(r8)   :: Qupt1(0:MAX_LAYERS-1),Qupt2(0:MAX_LAYERS-1),Qupt3(0:MAX_LAYERS-1)
 ...
 !Qupt1(0:4) = (/0.,0.,0.,0.,0./)       ! m/s
 !Qupt2(0:4) = (/0.,0.,0.,0.,0./)       ! m/s
 !Qupt3(0:4) = (/0.,0.,0.,0.,0./)       ! m/s
...
! 6: fws = 1.0, vcmax = vcmax*min(f_soilwater,f_feileaf)
 ...
 ! for the first layer
    ...
    p%Ett(0) = qupt(0) + Evap_soil/rho_w
...
! 2024/01/07 modified
 ...
 integer,intent(in)   :: fw_flag
 ! 2024/07/26
 !real(r8), intent(out) :: Qupt1(0:MAX_LAYERS-1),Qupt2(0:MAX_LAYERS-1),Qupt3(0:MAX_LAYERS-1)
...
! Estimate water uptake for each layer using the Darcy's approach
 ...
 do i = 0,p%n_layer-1
    ...
    ! avoid the floating-point overflow
    if ((fei_c/p%psim_prev(i)) >1.0) then
       f_theta(i) = min(1.0,ftheta)
    else
        ! a**b = exp(b*log(a))
       f_theta(i) = min(exp(max((spac_p1*Etp_mm+spac_p2)*log(fei_c/p%psim_prev(i)),-40.0)),ftheta)
       ! exp(-40.)=4.24e.-18., avoid floating-point overflow
    end if
    f_theta(i) = max(f_theta(i), 0.01) ! avoid too small
    ...
    q1=max(0.,q1)
    !write(* ,*)  "q1 = ",q1
    q2 = p%f_root(i)*(Etp/rho_w + p_excess)
    q2 = max(0.,q2)
    !write(*,*)  "q2 = ",q2
    q3 = p%f_root(i)*(fpmax - p%Sp)/kkstep
    !write(*,*)  "q3 = ",q3
    q3=max(0.,q3)
    qupt(i) = min(q1,q2)
...
! 2023/12/13
 ...
 if(dtt_sum < 1.e-6) then
   ...
   ! 2024/01/07 add
   p%f_soilwater  = min(1.0,p%f_soilwater)
 end if
 ...
 !end if
 ! 2024/05/15 adjustment of rs from Yang caused larger soil evaporation under non-growing period when I tested at US-Moz @LuHu
 Gwater_g       = 1./(4.0*exp(8.2-4.2*swc_g/porosity_g)) ! Sellers,1996, SiB2
