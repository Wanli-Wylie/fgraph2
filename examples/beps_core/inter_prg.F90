!! A inter-program between main program and moduls
!! Last update: July,2015
!! Fortran version: 3/5/2017 @J.Wang

subroutine inter_prg(lai,lai_input0,lc,clumping,Vcmax0,bh2o,mh2o,vod1,vod2,vod3,PHydra,&
   ...
   real(r8),intent(in)  ::clumping,Vcmax0,CosZs,daylen   !!J.Wang
   real(r8),intent(in)  :: bh2o,mh2o,vod1,vod2,vod3 ! 2025/02/17
   real(r8),intent(in)  ::param(0:49)
   type(climatedata),intent(in) ::meteo
   real(r8),intent(in)  ::var_o(0:40)
   real(r8),intent(out) ::var_n(0:40)
   real(r8),intent(out)  :: lai
   type(soil)           :: soilp
   type(results)        :: mid_res
   type(surf), pointer  :: bfields
...
!if(landcover == 25 .or. landcover ==40) then
   if(landcover == 14 .or. landcover == 15 .or. landcover == 40 .or. landcover == 41 .or. landcover ==1001 &
      .or. landcover ==1002 .or. landcover ==1003.or. landcover ==1004) then   !14->grass 15->crop @JWang
      lai_u = 0.01
   else
...
! vcmax jmax module  by L. He
   ...
   call Vcmax_Jmax(lai_o,clumping,Vcmax0,slope_Vcmax_N,leaf_N,CosZs,Vcmax_sunlit,Vcmax_shaded,Jmax_sunlit,Jmax_shaded)
...
!    write(*,*) "DG01: Ac_snow_o(kkk) =",Ac_snow_o(kkk)
      ...
      do while (.True.)
         ...
         if(CosZs>0.) then
            call photosynthesis(landcover,Tc_o_sunlit_old,R_o_sunlit,e_a10,G_o_b,Vcmax_sunlit,f_soilwater,b_h2o,m_h2o,&
               Ci_o_sunlit_old,temp_air,leleaf_o_sunlit,Gs_o_sunlit_new,gs_h2o,Ac_o_sunlit,&
               Ci_o_sunlit_new,ffpa,lSIF_o_sunlit,f_feileaf,fw_flag)
            call photosynthesis(landcover,Tc_o_shaded_old,R_o_shaded,e_a10,G_o_b,Vcmax_shaded,f_soilwater,b_h2o,m_h2o,&
               Ci_o_shaded_old,temp_air,leleaf_o_shaded,Gs_o_shaded_new,gs_h2o,Ac_o_shaded,&
               Ci_o_shaded_new,ffpa,lSIF_o_shaded,f_feileaf,fw_flag)
            call photosynthesis(landcover,Tc_u_sunlit_old,R_u_sunlit,e_a10,G_u_b,Vcmax_sunlit,f_soilwater,b_h2o,m_h2o,&
               Ci_u_sunlit_old,temp_air,leleaf_u_sunlit,Gs_u_sunlit_new,gs_h2o,Ac_u_sunlit,&
               Ci_u_sunlit_new,ffpa,lSIF_u_sunlit,f_feileaf,fw_flag)
            call photosynthesis(landcover,Tc_u_shaded_old,R_u_shaded,e_a10,G_u_b,Vcmax_shaded,f_soilwater,b_h2o,m_h2o,&
               Ci_u_shaded_old,temp_air,leleaf_u_shaded,Gs_u_shaded_new,gs_h2o,Ac_u_shaded,&
               Ci_u_shaded_new,ffpa,lSIF_u_shaded,f_feileaf,fw_flag)
         else
...
!    stSIF_u_shaded = 0.0
   ...
   mid_res%gpp_o_sunlit = GPP_o_sunlit*12.*1.e-6*1.e-3    ! umol CO2/m2/s -> kg/m2/s/
   ...
   if(lai_input0 < 0) then
      ...
      theta_day = theta_day + mid_res%thetam_surf
...
!          write(*,*) 'fAPAR = ', mid_res%fAPAR
   ...
   cosa = COS_air * 1.e-12                       ! convert ppt to mol

   call cos_calc(cosa,gs_h2o,G_o_b,Vcmax0,soilp,mid_res)
   mid_res%COS_flux      = mid_res%COS_plant + mid_res%COS_grnd
!    write(*,*) 'COS_flux = ', mid_res%COS_flux
   return
end subroutine
