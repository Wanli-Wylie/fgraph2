...
! for DALEC2
   real(r8),pointer :: noPF_POOLS(:)

contains

   subroutine init_noPF_POOLS()
      implicit none
      allocate(noPF_POOLS(6))
      noPF_POOLS(:) = (/970.5762,1045.6867,980.1575,47757.2765,964.9985,102436.9932/)
   end subroutine init_noPF_POOLS
   ...
   subroutine  plant_resp(f_q10,lc,mid_res,lai_yr,lai,temp_air,temp_soil,CosZs)
      ...
      if(lc == 25 .or. lc ==40 .or. lc==41) then
         lai_u  = 0.01
      else if(lc == 1001 .or. lc ==1002 .or. lc==1003 .or. lc == 1004) then
         lai_u  = 0.01
      else
...
!q10  = 3.22 - 0.046*temp_air
      ...
      if(lc >=1 .and. lc <= 5) then
         ...
         lai_max_o=3.3  !  /*   LAI_max_overstory     */
         lai_max_u=0.01 !/*  LAI_max_understroy     */  !/* leaf resp co.  kg C-1 d-1 kg-1    */
      else  ! other land cover type, SAV or WSA
         ! using the same parameters with lc = 13
         biomass=1.545*lai_yr+0.183*lai_yr*lai_yr
         biomass_leaf_o=0.1*biomass    !/* leaf C of overstory */
         biomass_stem_o=0.90*biomass    !/* stem C of overstory */
         biomass_root_o=1.432*biomass**0.639    !/* root C of overstory  Kurz 1996 */
...
! write(*,*) "mid_res%npp_o =", mid_res%npp_o
...
!NEP
   ...
   subroutine DALEC2_resp(days, lc, soiltype, lai_yr, lai, temp_soil, &
      CosZs, temp_air, pars, Cpools,T_Step, soilp, mid_res)
...
! we also need to change the POOLS to the initial conditions of the pools, to be optimized
      ...
      real(r8), intent(inout) :: Cpools(nopools) ! vector of ecosystem pools
      ...
      real(r8) :: mm, Mopt, Msat, B1, B2, B3, B   ! @LuHu
      ...
      !   POOLS(1,1)=pars(19) ! labile
      !   POOLS(1,2)=pars(20) ! foliar
      !   POOLS(1,3)=pars(21) ! roots
      !   POOLS(1,4)=pars(22) ! wood
      !   POOLS(1,5)=pars(23) ! litter
      !   POOLS(1,6)=pars(24) ! som
      !endif

      !if (isfirst_step) then
      ! assign initial conditions
      !POOLS(1)=pars(19) ! labile
      !POOLS(2)=pars(20) ! foliar
      !POOLS(3)=pars(21) ! roots
      !POOLS(4)=pars(22) ! wood
      !POOLS(5)=pars(23) ! litter
      !POOLS(6)=pars(24) ! som
      !endif
      !write(*,*) "POOLS(1)=",POOLS(1)
      !write(*,*) "POOLS(2)=",POOLS(2)
      !write(*,*) "POOLS(3)=",POOLS(3)
      !write(*,*) "POOLS(4)=",POOLS(4)
      !write(*,*) "POOLS(5)=",POOLS(5)
      !write(*,*) "POOLS(6)=",POOLS(6)
      ...
      !do ii=1,layer
      !if(soiltype >=6) then
      !lambda_w(ii) = 5.44*soilp%thetam_prev(ii-1)/soilp%fei(ii-1)-5.03*(soilp%thetam_prev(ii-1)/soilp%fei(ii-1))**2-0.472
      !else
      !lambda_w(ii) = 5.63*soilp%thetam_prev(ii-1)/soilp%fei(ii-1)-4.64*(soilp%thetam_prev(ii-1)/soilp%fei(ii-1))**2-0.710
      !end if
      !lambda_w(ii)=max(0.3,lambda_w(ii))
      !end do

      ! updating the MOIST function, @LuHU ref 'Potential Net Primary Productivity in South America: Application of a Global Model'
      ! mm, Mopt, Msat are fitted parameters provided by Prof.Ju
      do ii=1,layer
         mm = 0.00003*(soilp%theta_vfc(ii-1)*100.)**3 - 0.0063*(soilp%theta_vfc(ii-1)*100.)**2 + &
            0.3886*(soilp%theta_vfc(ii-1)*100.) - 6.9874
         Mopt = -0.0062*(soilp%theta_vfc(ii-1)*100.)**2 + 1.0745*(soilp%theta_vfc(ii-1)*100.)+26.515
         Msat = -0.00006*(soilp%theta_vfc(ii-1)*100.)**2 + 0.0124*(soilp%theta_vfc(ii-1)*100.) + 0.0994
         B3 = 100.
         B1 = (100.*soilp%thetam_prev(ii-1)/soilp%fei(ii-1))**mm - Mopt**mm
         B2 = Mopt**mm -B3**mm
         !write (*,*), 'B1',B1
         !write (*,*), 'B2',B2
         B = (B1/B2)**2
         lambda_w(ii) = 0.8* Msat**B + 0.2
         !write (*,*),'lambda_w', lambda_w(ii)
      ...
      q10  = 3.22 - pars(18)*temp_air      ! f_q10 default value 0.046
      ...
      FLUXES(1) = mid_res%GPP*1000.*86400.  ! we need to convert to gC.m-2.day-1
      ...
      FLUXES(3) = mid_res%res_A*1000.*86400.  ! we need to convert to gC.m-2.day-1
      ...
      FLUXES(8) = Cpools(1)*(1.-(1.-FLUXES(16))**T_Step)/T_Step
      ...
      FLUXES(10) = Cpools(2)*(1.-(1.-FLUXES(9))**T_Step)/T_Step

      ! total wood production
      !FLUXES(n,11) = POOLS(n,4)*(1.-(1.-pars(6))**deltat(n))/deltat(n)
      FLUXES(11) = Cpools(4)*(1.-(1.-pars(6))**T_Step)/T_Step

      ! total root litter production
      !FLUXES(n,12) = POOLS(n,3)*(1.-(1.-pars(7))**deltat(n))/deltat(n)
      FLUXES(12) = Cpools(3)*(1.-(1.-pars(7))**T_Step)/T_Step
      ...
      FLUXES(13) = lam_d*Cpools(5)*(1.-(1.-FLUXES(2)*pars(8))**T_Step)/T_Step

      ! respiration heterotrophic som
      !FLUXES(n,14) = POOLS(n,6)*(1.-(1.-FLUXES(n,2)*pars(9))**deltat(n))/deltat(n)
      FLUXES(14) = lam_d*Cpools(6)*(1.-(1.-FLUXES(2)*pars(9))**T_Step)/T_Step

      ! litter to som
      !FLUXES(n,15) = POOLS(n,5)*(1.-(1.-pars(1)*FLUXES(n,2))**deltat(n))/deltat(n)
      FLUXES(15) = lam_d*Cpools(5)*(1.-(1.-pars(1)*FLUXES(2))**T_Step)/T_Step
      ...
      !��������
      mid_res%res_H = (FLUXES(13)+FLUXES(14))/86400.0/1000. ! kgC/m2/s
