...
!_ ================================================================================================================================
  ...
  SUBROUTINE activate_sub_models()
    ...
    IF (ok_stomate) THEN
       ...
       !
       !Config Key   = HARVEST_BE
       !Config Desc  = Harvest model for bioenergy grass PFTs.
       !Config If    = OK_STOMATE 
       !Config Def   = y
       !Config Help  = Compute harvest above ground biomass for bioenergy
       !Config         Change daily turnover.
       !Config Units = [FLAG]
       CALL getin_p('HARVEST_BE', harvest_be)
       !
       ...
       CALL getin_p('SPINUP_ANALYTIC',spinup_analytic)
           !
           !Config Key   = SPINUP_CNP
           !Config Desc  = Identifier of simulations which serve to spinup the biogeochemical cycles
           !Config If    = OK_STOMATE
           !Config Def   = n
           !Config Help  = Activate this option if you are spining up nutrient cycles
           !Config Units = BOOLEAN    
           CALL getin_p('SPINUP_CNP',spinup_cnp)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE veget_config
    ...
    IF (impveg) THEN
       ...
       !Config Desc  = Should the soil texture class be prescribed ? -i DSG: unfortunaltey it is only texture
       !Config Def   = n
       !Config If    = IMPOSE_VEG
       !Config Help  = This flag allows the user to impose a soil texture distribution.
       ...
       !Config Key   = IMPOSE_SOILS
       !Config Desc  = Should the soil order be prescribed?
       !Config Def   = n
       !Config If    = IMPOSE_VEG
       !Config Help  = This flag allows the user to impose a soil order.
       !Config         It is espacially interesting for 0D
       !Config         simulations. On the globe it does not make too much sense as
       !Config         it imposes the same soil everywhere
       !Config Units = [FLAG]
       CALL getin_p('IMPOSE_SOILS', impsoils)     
    ENDIF
    ...
    !Config Key   = IMPOSE_NINPUT
    !Config Desc  = Should the Nutrients inputs be prescribed ?
    !Config Def   = n
    !Config If    = -
    !Config Help  = This flag allows the user to impose N and P inputs 
    !Config         It is espacially interesting for 0D
    !Config         simulations. On the globe it does not make too much sense as
    !Config         it imposes the same N inputs everywhere
    !Config Units = [FLAG]
    CALL getin_p('IMPOSE_NINPUT', impose_Ninput)     
    IF(impose_cn) impose_Ninput=.TRUE.

    IF(impose_Ninput) THEN
       !Config Key   = READ_PWEAT
       !Config Desc  = Should the P release inputs be read from file?
       !Config Def   = n
       !Config If    = -
       !Config Help  = This flag allows the user to read in P release from
       !Config         weathering from run.def.
       !Config Units = [FLAG]
       CALL getin_p('READ_PWEAT', read_pweat)     

       !Config Key   = IMPOSE_NMAP
       !Config Desc  = Should the N/P input maps be prescribed ?
       !Config Def   = n
       !Config If    = -
       !Config Help  = This flag allows the user to impose N/P inputs from netcdf map
       !Config         It is espacially interesting for 2D
       !Config         simulations. People can prepare their own maps of N/P input
       !Config Units = [FLAG]
       CALL getin_p('IMPOSE_NMAP', impose_Nmap)

       !Config Key   = READ_BNF
       !Config Desc  = Should the BNF input be read from file?
       !Config Def   = n
       !Config If    = -
       !Config Help  = This flag allows the user to impose BNF inputs from run.def
       !map
       !Config         It is espacially interesting for 0D
       !Config Units = [FLAG]
       CALL getin_p('READ_BNF', read_bnf)
    ENDIF
    ...
    ENDIF
        ...
        !Config If    = -
        !Config Def   = y
        !Config Help  = The parameter is used to bypass ninput_year count 
        !Config         and reinitialize it with NINPUT_YEAR parameter.
        ...
        !Config         If NINPUT_YEAR is set to 0, this means there is no time axis.
        !Config Units = [FLAG] 
        CALL getin_p('NINPUT_YEAR', ninput_year_orig)
        !
        !Config Key   = ALLOW_AGRI_FERT
        !Config Desc  = whether N P input map to be read
        !Config If    = -
        !Config Def   = .FALSE.
        !Config Help  = .TRUE. for reading N P fertilization maps
        !Config         If NINPUT_YEAR is set to 0, this means there is no time
        !axis.
        !Config Units = [FLAG]
        CALL getin_p('ALLOW_AGRI_FERT', allow_agri_fert)        
...
!!$
       ...
       !Config Key   = ENHANCED_WEAT
       !Config Desc  = do we consider enhanced weathering treatment
       !Config Def   = n
       !Config If    = -
       !Config Help  =  .TRUE. if we condsider enhanced weathering application
       !Config         
       !Config Units = [FLAG]
       CALL getin_p('ENHANCED_WEAT', enhanced_weat)     

       !Config Key   = EW_FORCE_STOCK
       !Config Desc  = Do we force a fixed mineral stock size (instead of application rate)
       !Config Def   = n
       !Config If    = -
       !Config Help  =  .TRUE. if we force a fixed mineral stock size (instead of application rate)
       !Config         
       !Config Units = [FLAG]
       CALL getin_p('EW_FORCE_STOCK', EW_force_stock)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE config_sechiba_parameters
    ...
    !! 0.4 Local variables
    REAL :: nudge_tau_mc     !! Temporary variable read from run.def
    REAL :: nudge_tau_snow   !! Temporary variable read from run.def
...
!_ ================================================================================================================================
  ...
  SUBROUTINE config_stomate_parameters
   ...
   !-
    ...
    !
    !Config Key   = FRAC_SOIL_STRUCT_AB 
    ...
    CALL getin_p('FRAC_TURNOVER_DAILY',frac_turnover_daily)   
       ...
       !Config Desc  = Clay-dependant fraction from Active to Passive pool
       !Config if    = OK_STOMATE 
       !Config Def   = 0.032
       !Config Help  =
       !Config Units = [-]
       ...
       !
       !Config Key   = NP_TARGET_IACTIVE_REF
       !Config Desc  = NP target ratio of active pool for soil min N = 0
       !Config if    = OK_STOMATE 
       !Config Def   = 6.
       !Config Help  = 
       !Config Units = [-] 
       CALL getin_p('NP_TARGET_IACTIVE_REF',NP_target_iactive_ref)
       !
       !Config Key   = NP_TARGET_ISLOW_REF
       !Config Desc  = NP target ratio of slow pool for soil min N = 0
       !Config if    = OK_STOMATE 
       !Config Def   = 14.
       !Config Help  = 
       !Config Units = [-] 
       CALL getin_p('NP_TARGET_ISLOW_REF',NP_target_islow_ref)
       !
       !Config Key   = NP_TARGET_IPASSIVE_REF
       !Config Desc  = NP target ratio of passive pool for soil min N = 0
       !Config if    = OK_STOMATE 
       !Config Def   = 6.
       !Config Help  = 
       !Config Units = [-] 
       CALL getin_p('NP_TARGET_IPASSIVE_REF',NP_target_ipassive_ref)
       !
       !
       !Config Key   = BCM_TURN_ISURFACE
       !Config Desc  = BCM turnover of P in surface pool
       !Config if    = OK_STOMATE 
       !Config Def   = 6.0
       !Config Help  =
       !Config Units =  [year-1] 
       CALL getin_p('BCM_TURN_ISURFACE',bcm_turn_isurface)
       !
       !Config Key   = BCM_TURN_IACTIVE
       !Config Desc  = BCM turnover of P in active pool
       !Config if    = OK_STOMATE 
       !Config Def   = 7.3
       !Config Help  =
       !Config Units =  [year-1] 
       CALL getin_p('BCM_TURN_IACTIVE',bcm_turn_iactive)
       !
       !Config Key   = BCM_TURN_ISLOW
       !Config Desc  = BCM turnover of P in slow pool
       !Config if    = OK_STOMATE 
       !Config Def   = 0.2
       !Config Help  =
       !Config Units = [year-1]
       CALL getin_p('BCM_TURN_ISLOW',bcm_turn_islow)
       !
       !Config Key   = BCM_TURN_IPASSIVE
       !Config Desc  = BCM turnover of P in passive pool
       !Config if    = OK_STOMATE 
       !Config Def   = 0.0
       !Config Help  = 
       !Config Units = [year-1] 
       CALL getin_p('BCM_TURN_IPASSIVE',bcm_turn_ipassive)
       !
       !Config Key   = TAU_SORBED 
       !Config Desc  = 
       !Config if    = OK_STOMATE 
       !Config Def   = 18250.0
       !Config Help  = 
       !Config Units = days
       CALL getin_p('TAU_SORBED',tau_sorbed)
       !
       !Config Key   = SORB_TUNE
       !Config Desc  = TUNE THE SORBED FRACTION
       !Config if    = OK_STOMATE 
       !Config Def   = 1.0
       !Config Help  = 
       !Config Units = 
       CALL getin_p('SORB_TUNE',sorb_tune)
       !
       !Config Key   = BNF_SCAL
       !Config Desc  = scaling factor for BNF from NPP
       !Config if    = OK_STOMATE 
       !Config Def   = 0.967
       !Config Help  = 
       !Config Units = 
       CALL getin_p('BNF_SCAL',bnf_scal)
    ...
    CALL getin_p('FLUX_TOT_COEFF',flux_tot_coeff)
       ...
       !-
       !Config Key   = H_SAXTON
       !Config Desc  = Coefficient h for defining maximum porosity
       !Config If    = OK_STOMATE 
       !Config Def   = 0.332
       !Config Help  =
       !Config Units = [-]  
       ...
       !-
       !Config Key   = H_SAXTON
       !Config Desc  = Coefficient j for defining maximum porosity
       !Config If    = OK_STOMATE 
       ...
       !-
       !Config Key   = K_SAXTON
       !Config Desc  = Coefficient k for defining maximum porosity
       !Config If    = OK_STOMATE 
       !Config Def   = O.1276
       !Config Help  =
       !Config Units = [-]  
       ...
       !-
       !Config Key   = Rn2oN
       !Config Desc  = Reference n2o production per N-NO3 produced g N-N2O XuRi
       !and Prentice 2008
       !Config If    = OK_STOMATE
       !Config Def   = 0.0008
       !Config Help  =
       !Config Units = [day**-1]
       CALL getin_p('RN2ON',Rn2oN)
       !-
       !Config Key   = RnoN
       !Config Desc  = Reference NO production per N-NO3 produced g N-N2O XuRi
       !and Prentice 2008
       !Config If    = OK_STOMATE
       !Config Def   = 0.02
       !Config Help  =
       !Config Units = [day**-1]
       CALL getin_p('RNON',RnoN)
       !-
       !Config Key   = scal_anvf
       !Config Desc  = critical moisture threshold to have strongly varying
       ! anox behaviour at air entry
       !Config If    = OK_STOMATE
       !Config Def   = 0.8
       !Config Help  =
       !Config Units = [day**-1]
       CALL getin_p('SCAL_ANVF',scal_anvf)
       !-
       !Config Key   = scal_ph
       !Config Desc  = pH scalling factor
       !Config If    = OK_STOMATE
       !Config Def   = 0.00
       !Config Help  =
       !Config Units = -
       CALL getin_p('SCAL_PH',scal_ph)
       !-
       ...
       !-
       !-
       !Config Key   = VMAX_N_UPTAKE
       !Config Desc  = Vmax of nitrogen uptake by plants for Ammonium (ind.1) and Nitrate (ind.2)
       !Config If    = OK_STOMATE 
       !Config Def   = 5.4 5.4
       !Config Help  =
       !Config Units = [umol (g DryWeight_root)-1 h-1)]  
       CALL getin_p('VMAX_N_UPTAKE',vmax_n_uptake)
       !-
       !Config Key   = K_N_MIN
       ...
       !-
       !Config Key   = VMAX_P_UPTAKE
       !Config Desc  = Vmax of phosphorus uptake by plants 
       !Config If    = OK_STOMATE 
       !Config Def   = 3.96
       !Config Help  =
       !Config Units = [umol (g DryWeight_root)-1 h-1)]  
       CALL getin_p('VMAX_P_UPTAKE',vmax_p_uptake)
       !-
       !Config Key   = K_P_MIN
       !Config Desc  = linear increase in uptake for high P concentrations
       !Config If    = OK_STOMATE 
       !Config Def   = 5.
       !Config Help  =
       !Config Units = [umol per litter]  
       CALL getin_p('K_P_min',K_P_min)
       !-
       !Config Key   = LOW_K_P_MIN
       !Config Desc  = dissolved P concentraion for which the Puptake equals vmax/2.   (umol per litter)
       !Config If    = OK_STOMATE 
       !Config Def   = 0.01
       !Config Help  =
       !Config Units = [ ]  
       CALL getin_p('LOW_K_P_min',low_K_P_min)
       ...
       !-
       !Config Key   = BLYPASS_CC_BUG
       !Config Desc  = There is a bug somewhere which allows the circumference classes to
       !               deviate from monotonically increasing order, which can cause a problem.
       !               Setting this to TRUE disables the check for this condition and impliments
       !               a small patch for the one problem which has manifested.  This needs to
       !               be found.
       !Config If    = OK_STOMATE, functional allocation 
       !Config Def   = FALSE 
       !Config Help  = 
       !Config Units = [-]
       CALL getin_p('BYPASS_CC_BUG',lbypass_cc)  
 ...
 !DSG      CALL getin_p('LD_FAKE_HEIGHT', ld_fake_height)

      CALL getin_p('LD_ALLOC', ld_alloc)
       !
       ...
       ! JC MOD045 wstress threshold for grasses only
       !
       !Config Key   = WSTRESS_LOWGRASS
       !Config Desc  = minimum water stress impact on grass leaf/root ratio LF
       !               too low wstress (severe stress) may cause huge root but
       !               few leaf for photosynthesis, the direct effect on LF is 
       !               too stronge, it sould be conservatively limited to an
       !               'realistic' range 
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = 0.1
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('WSTRESS_LOWGRASS',wstress_lowgrass)       
       !
       !Config Key   = SSTRESS_LOWGRASS
       !Config Desc  = minimum total stress impact on grass leaf/root ratio LF
       !               too low sstress (severe stress) may cause huge root but
       !               few leaf for photosynthesis, the direct effect on LF is
       !               too stronge, it sould be conservatively limited to an
       !               'realistic' range
       !Config If    = OK_STOMATE, functional allocation
       ...
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('SSTRESS_LOWGRASS',sstress_lowgrass)
       ! JC MOD050 root resp suppression
       !
       !Config Key   = COEFF_SUPPRESS_RESP
       !Config Desc  = the fraction of root maint_resp we want to suppress to
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = 0.1
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('COEFF_SUPPRESS_RESP',coeff_suppress_resp)
       !
       !Config Key   = DAYS_SENESC_CRIT 
       !Config Desc  = days start to suppress maint_resp for dormancy
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = 10.0
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('DAYS_SENESC_CRIT',days_senesc_crit)
       !
       !Config Key   = GRM_RtoL_turn
       !Config Desc  = relative grass root turnover compared to leaf turnover
       !               during senescence (1.0 for annual grasses, and 0.0 for
       !               perennial grasses)
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = 0.0
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('GRM_RtoL_turn',GRM_RtoL_turn)
!       !
!       !Config Key   = FIXED_FSOILN
!       !Config Desc  = Fixed factor for CN target ratios of soil pools
!       !Config If    = OK_STOMATE, functional allocation
!       !Config Def   = 1.0
!       !Config Help  =
!       !Config Units = [-]
!       CALL getin_p('FIXED_FSOILN',fixed_fsoiln)
       ...
       !Config Desc  = activate GRM module?
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = n
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('GRM_ENABLE_GRAZING',GRM_enable_grazing)
       !
       !Config Key   = GRM_allow_BNF
       !Config Desc  = allowed legume BNF?
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = n
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('GRM_ALLOW_BNF',GRM_allow_BNF)
       !
       !Config Key   = GRM_BNF_newmethod
       !Config Desc  = use new method (Lazzarotto et al. 2009) for BNF?
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = n
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('GRM_BNF_NEWMETHOD',GRM_BNF_newmethod)
       !
       !Config Key   = GRM_allow_DEVSTAGE 
       !Config Desc  = allow grass allocation follows exactly devstage as PaSim?
       !(should not activate)
       !Config If    = OK_STOMATE, functional allocation
       !Config Def   = n
       !Config Help  =
       !Config Units = [-]
       CALL getin_p('GRM_allow_DEVSTAGE',GRM_allow_DEVSTAGE)
       !Config Key   = FIX_LEGUME_FRAC
       !Config Desc  = read legume fraction for site simulation
       !Config If    =
       !Config Def   = 0.0
       !Config Help  =
       !Config Units =
       CALL getin_p('FIX_LEGUME_FRAC',fix_legume_frac)
