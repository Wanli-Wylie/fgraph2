...
!_ ================================================================================================================================
  ...
  SUBROUTINE config_soil_parameters()
    ...
    INTEGER, PARAMETER      :: error_level = 3         !! Switch to 2 to turn fatal errors into warnings.(1-3, unitless)
...
!DSG: fail-safe
          CALL ipslerr_p(error_level, "config_soil_parameters.", &
               &     "Wrong parameter value for HYDROL_CWRR.", &
               &     "Must be yes, 2 layer scheme is not supported anymore ", &
               &     "Please, check parameter value in run.def. ")
!DSG: fail-safe
