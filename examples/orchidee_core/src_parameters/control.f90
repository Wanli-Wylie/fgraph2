...
!_ ================================================================================================================================
  ...
  SUBROUTINE control_initialize
    ...
    INTEGER                             :: jv            !! Local index variable
    INTEGER                             :: ier           !! Error handeling

    ! Start reading options from parameter file 

    ! 
    !Config key   = ENABLE_NC_RESTART_COMPRESSION  
    ...
    !Config Units = [FLAG] 
    ! 
    NC_COMPRESSION_ENABLE = .TRUE. 
    CALL getin_p('ENABLE_NC_RESTART_COMPRESSION', NC_COMPRESSION_ENABLE) 
    WRITE(numout,*) "Netcdf compression is activated : ",NC_COMPRESSION_ENABLE 
    ...
    ELSE
       ...
       !Config If    = OK_SECHIBA 
       !Config Def   = y if OK_STOMATE else n
    ...
    WRITE(numout,*) 'N cycle is activated: ',ok_ncycle  
    ...
    ELSE
       impose_cn = .TRUE. 
       WRITE(numout,*) 'We force impose_cn to TRUE as N cycle is inactive'
    ENDIF
    WRITE(numout,*) 'CN ratio is imposed: ',impose_cn  

    !
    !Config Key   = IMPOSE_CN_RESTART
    !Config Desc  = Impose the CN ratio of leaves from restart
    !Config If    = OK_STOMATE
    !Config Def   = n
    !Config Help  = set to TRUE if leaves CN ration will be forced as
    !cn_leaf_avg_season from restart file
    !Config Units = [FLAG]
    !
    IF (ok_ncycle) THEN
       impose_cn_restart = .FALSE.
       CALL getin_p('IMPOSE_CN_RESTART',impose_cn_restart)
    ELSE
       impose_cn_restart = .FALSE.
       WRITE(numout,*) 'We force impose_cn_restart to FALSE as N cycle is inactive'
    ENDIF
    WRITE(numout,*) 'CN ratio from restart is imposed: ',impose_cn_restart

    IF (impose_cn_restart .AND. (.NOT. impose_cn .OR. .NOT. ok_ncycle) ) THEN
       WRITE(numout,*) 'ERROR : Incoherence between impose_cn_restart activated and either impose_cn or ok_ncycle not activated'
       CALL ipslerr_p(3,'control_initialize','Flags conflict','Check ok_ncycle impose_cn and impose_cn_restart','')
    ENDIF

    !
    !Config Key   = STOMATE_OK_PCYCLE
    !Config Desc  = Activate dynamic P cycle?
    !Config If    = OK_STOMATE
    !Config Def   = n
    !Config Help  = set to TRUE if P cycle is to be activated
    !Config Units = [FLAG]
    !

    ok_pcycle = .TRUE. 
    ! CALL getin_p('STOMATE_OK_PCYCLE',ok_pcycle) 
    WRITE(numout,*) 'P cycle is activated: ',ok_pcycle  

    !
    !Config Key   = STOMATE_IMPOSE_NP
    !Config Desc  = Impose the NP ratio of leaves 
    !Config If    = OK_STOMATE 
    !Config Def   = n 
    !Config Help  = set to TRUE if IMPOSE_NP is to be activated 
    !Config Units = [FLAG] 
    ! 
    IF (ok_pcycle) THEN
       impose_np = .FALSE. 
       CALL getin_p('STOMATE_IMPOSE_NP',impose_np) 
    ELSE
       impose_np = .TRUE. 
       WRITE(numout,*) 'We force impose_np to TRUE as P cycle is inactive'
    ENDIF
    WRITE(numout,*) 'NP ratio is imposed: ',impose_np  

    !
    !Config Key   = IMPOSE_NP_RESTART
    !Config Desc  = Impose the NP ratio of leaves from restart
    !Config If    = OK_STOMATE
    !Config Def   = n
    !Config Help  = set to TRUE if leaves NP ration will be forced as
    !np_leaf_avg_season from restart file
    !Config Units = [FLAG]
    !
    IF (ok_pcycle) THEN
       impose_np_restart = .FALSE.
       CALL getin_p('IMPOSE_NP_RESTART',impose_np_restart)
    ELSE
       impose_np_restart = .FALSE.
       WRITE(numout,*) 'We force impose_np_restart to FALSE as P cycle is inactive'
    ENDIF
    WRITE(numout,*) 'NP ratio from restart is imposed: ',impose_np_restart

    IF (impose_np_restart .AND. (.NOT. impose_np .OR. .NOT. ok_pcycle) ) THEN
       WRITE(numout,*) 'ERROR : Incoherence between impose_np_restart activated and either impose_np or ok_pcycle not activated'
       CALL ipslerr_p(3,'control_initialize','Flags conflict','Check ok_pcycle impose_np and impose_np_restart','')
    ENDIF

    !
    !Config Key   = STOMATE_MASS_CONSERVATION
    !Config Desc  = check mass conservation, stop model when violated
    !Config If    = OK_STOMATE 
    !Config Def   = n 
    !Config Help  = set to TRUE if MASS_CONSERVATION is to be activated 
    !Config Units = [FLAG] 
    ! 
    mass_conservation = .FALSE. 
    CALL getin_p('STOMATE_MASS_CONSERVATION',mass_conservation) 
    WRITE(numout,*) 'Violation of mass conservation leads to model stop: ',mass_conservation

    !
    !Config Key   = STOMATE_DSG_DEBUG
    !Config Desc  = check mass conservation, negative biomass, and stoichiometry
    !Config If    = OK_STOMATE 
    !Config Def   = n 
    !Config Help  = set to TRUE if DSG_DEBUG is to be activated 
    !Config Units = [FLAG] 
    ! 
    dsg_debug = .FALSE. 
    CALL getin_p('STOMATE_DSG_DEBUG',dsg_debug) 
    WRITE(numout,*) 'Mass conservation, negative biomass, and stoichiometry are checked: ',dsg_debug
