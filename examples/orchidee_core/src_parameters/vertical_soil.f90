...
!
!>\BRIEF   Initialization of vertical discretization in the soil for hydrology and thermodynamics   
!!
...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_parameters/vertical_soil.f90 $
...
!_ ================================================================================================================================
  ...
  SUBROUTINE vertical_soil_init
    ...
    !! 0.1 Local variables
    INTEGER, PARAMETER            :: nblayermax=500    !! Preset 500 for max. number of soil layers.
    INTEGER                       :: i, irev, iref, ntmp, ier
    REAL                          :: zgeoend           !! The node position where the geometrical increases of nodes stopped. (m)
    REAL                          :: dgeoend           !! The distance of the node at zgeoend and the node above. (m)
    REAL                          :: dcst              !! Related to refine at bottom. Work in progress...
    REAL                          :: cstint            !! Related to refine at bottom. Work in progress...
    REAL                          :: hh                !! Temporay variable, to calculate the layer thickness for temperature at zmaxh. 
    REAL                          :: ratio             !! The ratio of geometric increas.
    INTEGER                       :: nbrefine          !! Related to refine at bottom. Work in progress...
    INTEGER                       :: nbcst             !! Related to refine at bottom. Work in progress...
    INTEGER                       :: igeo              !! Related to refine at bottom. Work in progress...
    REAL, DIMENSION(nblayermax+1) :: ztmp              !! Depth at the node of each layer (m)
    REAL, DIMENSION(nblayermax+1) :: zint              !! Depth at the interface of each layer (m)
    REAL, DIMENSION(nblayermax+1) :: dtmp              !! Distance between the current node and the one above (m)
    REAL, DIMENSION(nblayermax+1) :: drefine           !! Related to refine at bottom. Work in progress...
    INTEGER, DIMENSION(1)         :: imin              !! Related to refine at bottom. Work in progress...

    !! Variables controling the vertical discretiztaion
    REAL                          :: depth_topthickness   !! Thickness of the top Layer (m)
    REAL                          :: depth_cstthickness   !! Depth at which constant layer thickness start (m)
    REAL                          :: depth_geom           !! Depth at which we resume geometrical increases for temperature (m)
    REAL                          :: ratio_geom_below     !! Ratio of the geometrical series defining the thickness below DEPTH_GEOM.
    ...
    !Config Units = m
    ! Default value for CWRR (only CWRR enters this subroutine)
    zmaxh=2.0
...
!!$  CALL getin_p("REFINEBOTTOM",refinebottom)
    ...
    IF (printlev >=2) THEN
       WRITE(numout,*) "=== Vertical discretization for hydrology ==================================="
       WRITE(numout,*) "Number of layers in hydrology: nslm=", nslm
       WRITE(numout,*) "Total depth in hydrology (DEPTH_MAX_H) (m): znh(nslm)=", znh(nslm)
       WRITE(numout,*) "Depth of nodes in hydrology (m): znh=", znh
       WRITE(numout,*) "Depth of lower layer-interface for hydrology (m): zlh=", zlh
       WRITE(numout,*) "Layer thickness for hydrology (m): dlh=",dlh
       WRITE(numout,*) "Internode distance for hydrology (m): dnh=",dnh
       WRITE(numout,*) "=== Vertical discretization for thermosoil =================================="
       WRITE(numout,*) "Number of layers in thermosoil: ngrnd=", ngrnd
       WRITE(numout,*) "Total depth for thermosoil (DEPTH_MAX_T) (m): zlt=", zlt(ngrnd)
       WRITE(numout,*) "to be compared with calculated total depth for thermosoil (not used) zint=", zint(ngrnd)
       WRITE(numout,*) "Depth of the nodes in thermosoil (m): znt=",znt
       WRITE(numout,*) "Depth of lower layer-interface for thermosoil (m): zlt=", zlt(1:ngrnd)
       WRITE(numout,*) "Layer thickness for thermosoil (m): dlt=", dlt
       WRITE(numout,*) "============================================================================="
    END IF
