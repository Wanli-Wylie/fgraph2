MODULE orchoasis_tools
  ...
  ! INPUT
  INTEGER, SAVE :: il_part_id, tair_id, qair_id, zlevtq_id, zlevuv_id
  INTEGER, SAVE :: rainf_id, snowf_id, lwdown_id, swnet_id, solarang_id
  INTEGER, SAVE :: u_id, v_id, ps_id, cdrag_id
  ! OUTPUT
  INTEGER, SAVE :: vevapp_id, fluxsens_id, fluxlat_id, coastal_id, river_id
  INTEGER, SAVE :: netco2_id, carblu_id, tsolrad_id, tsolnew_id, qsurf_id
  INTEGER, SAVE :: albnir_id, albvis_id, emis_id, z0_id
  !
...
!
  ...
  SUBROUTINE orchoasis_defvar (mpi_rank, kjpindex)
    ...
    ! ARGUMENTS
    !
    INTEGER, INTENT(in) :: mpi_rank, kjpindex
    !
    ! LOCAL
    !
    INTEGER               :: ierror, tot_error
  ...
  SUBROUTINE orchoasis_getvar (itau, dt, kjpindex, landindex, zlev_tq, zlev_uv, temp_air, qair, precip_rain, &
    ...
    ! ARGUMENTS
    !
    INTEGER, INTENT(in) :: itau, kjpindex
    INTEGER, DIMENSION(kjpindex), INTENT(in) :: landindex
    REAL, INTENT(in)    :: dt
    !
    REAL, DIMENSION(kjpindex), INTENT(out) :: zlev_tq, zlev_uv, temp_air, qair, precip_rain
    REAL, DIMENSION(kjpindex), INTENT(out) :: precip_snow, swnet, lwdown, sinang, u, v, pb
    REAL, DIMENSION(kjpindex), INTENT(out) :: cdrag
    !
    ! LOCAL
    !
    INTEGER :: oasis_info
  ...
  SUBROUTINE orchoasis_putvar(itau, dt, kjpindex, landindex, vevapp, fluxsens, fluxlat, coastalflow, riverflow, &
    ...
    REAL, INTENT(in)    :: dt
    !
    REAL, DIMENSION(kjpindex), INTENT(in) :: vevapp, fluxsens, fluxlat, coastalflow, riverflow
    REAL, DIMENSION(kjpindex), INTENT(in) :: netco2, carblu, tsol_rad, temp_sol_new, qsurf, emis, z0
    REAL, DIMENSION(kjpindex,2), INTENT(in) :: albedo
    !
    ! LOCAL
    !
    INTEGER :: oasis_info
