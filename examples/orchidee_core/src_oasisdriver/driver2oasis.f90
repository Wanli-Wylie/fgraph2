PROGRAM driver2oasis
  ...
  INTEGER, ALLOCATABLE, DIMENSION(:) :: kindex_g
  ...
  REAL    :: date_start, dt
  REAL    :: timestep_interval(2), timestep_int_next(2), julian
...
!-
  ...
  REAL, ALLOCATABLE, DIMENSION (:)             :: pb            !! Surface pressure
  REAL, ALLOCATABLE, DIMENSION (:)             :: zlev_tq       !! Height of layer for T and q
  REAL, ALLOCATABLE, DIMENSION (:)             :: zlev_uv       !! Height of layer for u and v
  ...
  REAL, ALLOCATABLE, DIMENSION (:)             :: swnet         !! Net surface short-wave flux
  REAL, ALLOCATABLE, DIMENSION (:)             :: solarang      !! Cosine of solar zenith angle
  !-
  !- output fields
  !-
  ...
  !-
  INTEGER :: glo_rank, glo_size ! rank and  number of pe
  INTEGER :: loc_rank, loc_size ! rank and  number of pe
  INTEGER :: LOCAL_OASIS_COMM  ! local MPI communicator and Initialized
  ...
  ! OASIS Output
  INTEGER :: il_part_id, tair_id, qair_id, zlevtq_id, zlevuv_id
  INTEGER :: rainf_id, snowf_id, swnet_id, lwdown_id, solarang_id
  INTEGER :: u_id, v_id, ps_id, cdrag_id
  ! OASIS Input
  INTEGER :: vevapp_id, fluxsens_id, fluxlat_id, coastal_id, river_id
  INTEGER :: netco2_id, carblu_id, tsolrad_id, tsolnew_id, qsurf_id
  INTEGER :: albnir_id, albvis_id, emis_id, z0_id
  !
  INTEGER, DIMENSION(2) :: var_nodims, var_shape
  INTEGER :: nbarg, iret, helpmsg = 0
  CHARACTER(LEN=10) :: arg
  LOGICAL :: initmode = .FALSE.
  !-
  INTEGER :: freq_diag=10, debug_lev=1
  INTEGER :: w_unit=737
  !
  ! Timer variables
  !
  LOGICAL, PARAMETER :: timemeasure=.TRUE.
  REAL :: waitput_cputime=0.0, waitget_cputime=0.0, preparation_cputime=0.0
  REAL :: waitput_walltime=0.0, waitget_walltime=0.0, preparation_walltime=0.0
  !
  ! Print point
  !
...
!!  REAL, DIMENSION(2) :: testpt=(/-5.25,41.25/)
