...
PROGRAM orchidedriver
  ...
  INTEGER :: nb_forcefile
  CHARACTER(LEN=8)  :: model_guess
  INTEGER    :: iim_glo, jjm_glo, file_id
  !- 
  INTEGER    :: nbseg
  REAL, ALLOCATABLE, DIMENSION(:,:) :: lon_glo, lat_glo, area_glo
  REAL, ALLOCATABLE, DIMENSION(:,:) :: mask_glo
  REAL, ALLOCATABLE, DIMENSION(:,:,:,:) :: corners_glo
  REAL, ALLOCATABLE, DIMENSION(:,:,:) :: corners_lon, corners_lat
  INTEGER :: nbindex_g, kjpindex
  INTEGER, ALLOCATABLE, DIMENSION(:) :: kindex, kindex_g
  REAL, DIMENSION(2) :: zoom_lon, zoom_lat
  !
  ! Variables for the global grid available on all procs and used
  ! to fill the ORCHIDEE variable on the root_proc
  !
  REAL, ALLOCATABLE, DIMENSION(:,:)    :: lalo_glo
  REAL, ALLOCATABLE, DIMENSION(:)      :: contfrac_glo
  CHARACTER(LEN=20)                           :: calendar
  !-
  !- Variables local to each processors.
  !-
  INTEGER :: i, j, ik, nbdt, first_point
  INTEGER :: itau, itau_offset, itau_sechiba
  REAL    :: date0, date0_shifted, dt, dt_sto, julian
  REAL    :: date0_tmp, dt_tmp
  INTEGER :: nbdt_tmp
  REAL    :: timestep_interval(2), timestep_int_next(2)
  !
  INTEGER :: rest_id, rest_id_stom
  INTEGER ::  hist_id, hist2_id, hist_id_stom, hist_id_stom_IPCC
  REAL, ALLOCATABLE, DIMENSION(:,:) :: lalo_loc
  INTEGER :: iim, jjm, ier
  ...
  !- input fields
  !-
  REAL, ALLOCATABLE, DIMENSION (:)             :: u             !! Lowest level wind speed
  REAL, ALLOCATABLE, DIMENSION (:)             :: v             !! Lowest level wind speed 
  REAL, ALLOCATABLE, DIMENSION (:)             :: zlev_uv       !! Height of first layer
  REAL, ALLOCATABLE, DIMENSION (:)             :: zlev_tq       !! Height of first layer
  REAL, ALLOCATABLE, DIMENSION (:)             :: qair          !! Lowest level specific humidity
  REAL, ALLOCATABLE, DIMENSION (:)             :: precip_rain   !! Rain precipitation
  REAL, ALLOCATABLE, DIMENSION (:)             :: precip_snow   !! Snow precipitation
  REAL, ALLOCATABLE, DIMENSION (:)             :: lwdown        !! Down-welling long-wave flux 
  REAL, ALLOCATABLE, DIMENSION (:)             :: swdown        !! Downwelling surface short-wave flux
  REAL, ALLOCATABLE, DIMENSION (:)             :: sinang        !! cosine of solar zenith angle
  REAL, ALLOCATABLE, DIMENSION (:)             :: temp_air      !! Air temperature in Kelvin
  REAL, ALLOCATABLE, DIMENSION (:)             :: epot_air      !! Air potential energy
  REAL, ALLOCATABLE, DIMENSION (:)             :: ccanopy       !! CO2 concentration in the canopy
  REAL, ALLOCATABLE, DIMENSION (:)             :: petAcoef      !! Coeficients A from the PBL resolution
  REAL, ALLOCATABLE, DIMENSION (:)             :: peqAcoef      !! One for T and another for q
  REAL, ALLOCATABLE, DIMENSION (:)             :: petBcoef      !! Coeficients B from the PBL resolution
  REAL, ALLOCATABLE, DIMENSION (:)             :: peqBcoef      !! One for T and another for q
  REAL, ALLOCATABLE, DIMENSION (:)             :: cdrag         !! Cdrag
  REAL, ALLOCATABLE, DIMENSION (:)             :: pb            !! Lowest level pressure
  !-
  !- output fields
  !-
  REAL, ALLOCATABLE, DIMENSION (:)            :: z0m           !! Surface roughness for momentum (m)
  REAL, ALLOCATABLE, DIMENSION (:)            :: z0h           !! Surface roughness for heat (m)
  REAL, ALLOCATABLE, DIMENSION (:)            :: coastalflow   !! Diffuse flow of water into the ocean (m^3/dt)
  REAL, ALLOCATABLE, DIMENSION (:)            :: riverflow     !! Largest rivers flowing into the ocean (m^3/dt)
  REAL, ALLOCATABLE, DIMENSION (:)            :: tsol_rad      !! Radiative surface temperature
  REAL, ALLOCATABLE, DIMENSION (:)            :: vevapp        !! Total of evaporation
  REAL, ALLOCATABLE, DIMENSION (:)            :: temp_sol_new  !! New soil temperature
  REAL, ALLOCATABLE, DIMENSION (:)            :: qsurf         !! Surface specific humidity
  REAL, ALLOCATABLE, DIMENSION (:,:)          :: albedo        !! Albedo
  REAL, ALLOCATABLE, DIMENSION (:)            :: fluxsens      !! Sensible chaleur flux
  REAL, ALLOCATABLE, DIMENSION (:)            :: fluxlat       !! Latent chaleur flux
  REAL, ALLOCATABLE, DIMENSION (:)            :: emis          !! Emissivity
  REAL, ALLOCATABLE, DIMENSION (:)            :: netco2        !! netco2flux
  REAL, ALLOCATABLE, DIMENSION (:)            :: carblu        !! fco2_land_use
  REAL, ALLOCATABLE, DIMENSION (:,:)          :: veget         !! Fraction of vegetation type (unitless, 0-1)
  REAL, ALLOCATABLE, DIMENSION (:,:)          :: lai           !! Leaf area index (m^2 m^{-2}
  REAL, ALLOCATABLE, DIMENSION (:,:)          :: height        !! Vegetation Height (m)
  !-
  !-
  !-
  REAL :: atmco2
  REAL, ALLOCATABLE, DIMENSION (:)  :: u_tq, v_tq, swnet
  ...
  REAL :: waitput_cputime=0.0, waitget_cputime=0.0, orchidee_cputime=0.0
  REAL :: waitput_walltime=0.0, waitget_walltime=0.0, orchidee_walltime=0.0
  !
  !
  ! Print point
  !
!!  REAL, DIMENSION(2) :: testpt=(/44.8,-25.3/)
!!  REAL, DIMENSION(2) :: testpt=(/44.8,-18.3/)
!!  REAL, DIMENSION(2) :: testpt=(/-60.25,-5.25/)
!!  REAL, DIMENSION(2) :: testpt=(/46.7,10.3/)
!!  REAL, DIMENSION(2) :: testpt=(/0.25,49.25/)
  ! Case when no ouput is desired.
  REAL, DIMENSION(2) :: testpt=(/9999.99,9999.99/)
  INTEGER :: ktest
...
!!  CALL getin_name("run.def")
  ...
  !-
  !- Set the global calendar and get the time period for the run
  !-
  ...
  ! Note that currently itau_offset is always 0
  ...
  ! It is not currently possible to read the date from the restart file
  ...
  WRITE(numout,*) "HISTORY : Defined for ", itau_sechiba, date0, dt
  !
  !-
  !---------------------------------------------------------------------------------------
  !-
...
!_ ================================================================================================================================
     SUBROUTINE orchideedriver_clear
      ...
      ! Deallocate and reset variables in forcing_tools module
      CALL forcing_tools_clear()

      ! Deallocate and reset variables in sechiba module and underlaying modules
      CALL sechiba_clear()

      WRITE(numout,*) "Memory deallocated in orchideedriver_clear"
