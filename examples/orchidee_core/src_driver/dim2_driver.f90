...
!!   
!! SVN     :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_driver/dim2_driver.f90 $ 
!! $Date: 2019-04-16 14:01:52 +0200 (二, 2019-04-16) $
!! $Revision: 5907 $
!! \n
!_ ================================================================================================================================
...
!- Variables which are forcings for SECHIBA
 ...
 & fluxsens, fluxlat, emis, z0
  ...
  CHARACTER(LEN=80) :: driv_restname_in, driv_restname_out
  ...
  INTEGER :: itau_dep, itau_dep_rest, itau_fin, itau_skip, glo_itau_len
  ...
  INTEGER :: forcing_idx !  select which file use in forcing_list
  LOGICAL :: forcing_cycle ! FORCING_CYCLE flag

  TYPE(t_forcing_list) :: forcing_list

  fill_init=nf90_fill_real
...
!- 
 !Config Key   = FORCING_CYCLE
 !Config Desc  = Enable the use of cyclic forcing files when using a list
 !Config If    = [FORCING_FILE=somefile.list]
 !Config Def   = n
 !Config Help  = Enable the use of cyclic forcing files from the list.
 !Config         A list file must be specified so it is used. 
 !Config Units = [y/n] 
 forcing_cycle=.FALSE.
 CALL getin_p('FORCING_CYCLE',forcing_cycle)
 ...
 WRITE(*,*) "dim2_driver:: filename=", filename

!- 
 
 forcing_list = T_FORCING_LIST(filename, forcing_cycle)
 filename = forcing_list%get(1)
!-
...
!-
  ...
  glo_itau_len = itau_fin-itau_dep
  ...
  !Config Units = [seconds, days, months, years]
!-
  WRITE(time_str,'(I10)') glo_itau_len
  CALL getin_p('TIME_LENGTH', time_str)
!-
! Transform into itau
!-
  CALL tlen2itau (time_str, dt_force, date0, glo_itau_len)
!-
  itau_fin = itau_dep+glo_itau_len
!-
  IF (printlev_loc >= 1) THEN
     ...
     WRITE(numout,*) '>> Length of the simulation is thus :', glo_itau_len
...
!-----
      ...
      ! Change filename when using a forcing file list
      IF (forcing_list%from_list()) THEN
        forcing_idx = ((it - (itau_dep)-1) / tm) + 1

        IF (forcing_idx .GT. forcing_list%total()) THEN
          ! is forcing cycle enabled? 
          IF (forcing_cycle) THEN
            !Yes, make sure to loop
            forcing_idx = MOD( forcing_idx-1, forcing_list%total()) + 1
          ELSE
            CALL ipslerr(3,'dim2_driver','No more forcing files to read','(FORCING_CYCLE is disabled)','')
          ENDIF
        ENDIF

        filename = TRIM(forcing_list%get(forcing_idx))
      ENDIF
      ! istp is ignored when forcing file is provided
...
!-----
   ...
   ENDDO ! DO WHILE (is <= split)
    ...
    IF (it==itau_fin-1) THEN
     ...
     CALL Write_Load_Balance(REAL(Get_cpu_time(timer_mpi)))
