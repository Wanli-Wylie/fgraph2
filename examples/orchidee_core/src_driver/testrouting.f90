...
PROGRAM testrouting
  ...
  INTEGER                                      :: nbseg                            !! 
  !
  INTEGER                                      :: iim                              !! Size in longitude of coarser grid
  INTEGER                                      :: jjm                              !! Size in latitude of coarser grid
  INTEGER                                      :: i, j                             !! Integer variable for loops 
  INTEGER                                      :: ibegt, iendt
  INTEGER                                      :: ni                               !! For checking nbindex
  REAL                                         :: nbyears                          !! Lenght of simulation in years 
  INTEGER                                      :: simlen                           !! Lenght of simulation: simlen = 365*48*nbyears
  REAL                                         :: dx, dy                           !! Lon/Lat resolution of coarser grid
  REAL, ALLOCATABLE, DIMENSION (:,:)           :: lon, lat                         !! Lon/lat of coarser grid
  REAL, ALLOCATABLE, DIMENSION (:,:)           :: orog                             !! New orography after interpolation
  REAL, ALLOCATABLE, DIMENSION (:)             :: orog_land, orog_loc
  REAL, ALLOCATABLE, DIMENSION (:,:)           :: lalo_land
  REAL, ALLOCATABLE, DIMENSION (:)             :: contfrac_land, contfrac_loc
  REAL, ALLOCATABLE, DIMENSION (:,:)           :: contfrac_2d
  !
  REAL, DIMENSION (1)                          :: lev                              !! Number of level (requested by restini routine) (unitless)
  ...
  INTEGER                                      :: hist_id                          !! History file identification for ???
  INTEGER                                      :: rest_id                          !! ID of the restart file
  !
  REAL                                         :: date0                            !! Initial date
  REAL                                         :: date0_rest                       !! Initial date from restart file
  REAL                                         :: date                             !! Current date
  REAL                                         :: dt                               !! Same as dtradia ???
  REAL                                         :: dw                               !! 86400. ???
  REAL                                         :: one_day_loc
  !
  ...
  !
  INTEGER                                      :: kjit                             !! Time step number
  INTEGER                                      :: nbindex                          !! Number of local continental points 
  REAL                                         :: dtradia                          !! Timestep length
  !
  INTEGER, ALLOCATABLE, DIMENSION (:)          :: kindex_g                         !! Index of land point on 2D map (in local position)
  INTEGER, ALLOCATABLE, DIMENSION (:)          :: kindex                           !! index of land point per proc
  REAL, ALLOCATABLE, DIMENSION (:)             :: runoff                           !! Grid-point runoff (kg/m^2/dt)
  REAL, ALLOCATABLE, DIMENSION (:)             :: drainage                         !! Grid-point drainage (kg/m^2/dt)
  REAL, ALLOCATABLE, DIMENSION (:)             :: humrel                           !! Soil moisture stress, root extraction potential (unitless)
  REAL, ALLOCATABLE, DIMENSION (:,:)           :: stempdiag                        !! Diagnostic soil temperature profile
  ...
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)       :: precip_rain                      !! Rainfall (kg/m^2/dt)
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)       :: k_litt                           !! Averaged conductivity for saturated infiltration in the 'litter' layer (kg/m^2/dt)
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)       :: reinf_slope                      !! Coefficient which determines the reinfiltration ratio in the grid box due to flat areas (unitless;0-1)
  INTEGER                                      :: hist2_id                         !! Access to history file 2 (unitless)
  !
  REAL, ALLOCATABLE, DIMENSION (:)             :: evapot_corr                      !! Soil Potential Evaporation
  !
  ! Output from routing_main
  !
  REAL, ALLOCATABLE, DIMENSION (:)             :: returnflow                       !! The water flow from lakes and swamps which returns to the grid box (kg/m^2/dt)
  REAL, ALLOCATABLE, DIMENSION (:)             :: irrigation                       !! This is the water taken from the reservoirs and beeing put into the upper layers of the soil (kg/m^2/dt)
  REAL, ALLOCATABLE, DIMENSION (:)             :: riverflow                        !! Outflow of the major rivers, will be located on the continental grid but this should be a coastal point (kg/dt)
  REAL, ALLOCATABLE, DIMENSION (:)             :: coastalflow                      !! Outflow on coastal points by small basins, the water which flows in a disperse way into the ocean (kg/dt)
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)       :: reinfiltration                   !! Water flow from ponds and floodplains which returns to the grid box (kg/m^2/dt)
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)       :: flood_res                        !! Diagnostic of water amount in the floodplains reservoir (kg)
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)       :: flood_frac                       !! Flooded fraction of the grid box (unitless;0-1)
  !
  !
!_ ================================================================================================================================
  !
