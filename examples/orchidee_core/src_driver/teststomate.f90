...
PROGRAM teststomate
  ...
  USE grid
  USE time, ONLY : time_initialize, time_nextstep
  USE slowproc
  USE stomate
  USE intersurf, ONLY: lstep_init_intersurf
...
!-
  ...
  INTEGER                            :: kjpij,kjpindex                       !! # of total/land points
  REAL                               :: dtradia,dt_force                     !! time step of sechiba and stomate components 
                                                                                    !! read into the forcing file [second] 
 
  INTEGER,DIMENSION(:),ALLOCATABLE   :: indices                              !! index over land points
  INTEGER,DIMENSION(:),ALLOCATABLE   :: indexveg
  REAL,DIMENSION(:,:),ALLOCATABLE    :: soiltile, veget_x                    !! texture and fraction of vegetation type 
                                                                                    !! (accounts for LAI dynamic) 
  REAL,DIMENSION(:),ALLOCATABLE      :: totfrac_nobio                        !! Total fraction of ice+lakes+cities etc. in 
                                                                                    !! the mesh 
  REAL,DIMENSION(:,:),ALLOCATABLE    :: frac_nobio                           !! Fraction of ice, lakes, cities etc. in the 
                                                                                    !! mesh 
  REAL,DIMENSION(:,:),ALLOCATABLE    :: veget_max_x                          !! Fraction of vegetation type
  INTEGER, DIMENSION(:),ALLOCATABLE  :: njsc
  REAL,DIMENSION(:,:),ALLOCATABLE    :: lai_x                                !! Leaf Area Index as calculated in STOMATE 
                                                                                    !! [m2/m2] 
  REAL,DIMENSION (:,:,:), ALLOCATABLE:: frac_age_x                           !! Age efficacity from STOMATE for isoprene 
  REAL,DIMENSION(:,:),ALLOCATABLE    :: veget_force_x                        !! Fraction of vegetation of PFTs in each grid 
                                                                                    !! cell (read in the forcinng file) [-] 
  REAL,DIMENSION(:,:),ALLOCATABLE    :: veget_max_force_x                    !! fraction of maximum vegetation of PFTs in 
                                                                                    !! each grid cell (read in the forcinng file) 
                                                                                    !! [-] 
  REAL,DIMENSION(:,:),ALLOCATABLE    :: lai_force_x                          !! Leaf Area Index as read in the forcing file 
                                                                                    !! [m2/m2] 
  REAL,DIMENSION(:),ALLOCATABLE      :: reinf_slope
  REAL,DIMENSION(:),ALLOCATABLE      :: t2m,t2m_min,temp_sol                 !! 2 m air temperature, min. 2 m air temp. 
                                                                                    !! during forcing time step and Surface 
                                                                                    !! temperature [K] 
  REAL,DIMENSION(:,:),ALLOCATABLE    :: soiltemp,soilhum                     !!  Soil temperature and Relative soil moisture
  REAL,DIMENSION(:,:),ALLOCATABLE    :: humrel_x
  REAL,DIMENSION(:),ALLOCATABLE      :: litterhum                            !! Litter humidity
  REAL,DIMENSION(:),ALLOCATABLE      :: precip_rain,precip_snow              !! Rainfall, Snowfall
  REAL,DIMENSION(:,:),ALLOCATABLE    :: gpp_x                                !! GPP (gC/(m**2 of total ground)/time step)
  REAL,DIMENSION(:),ALLOCATABLE      :: deadleaf_cover                       !! fraction of soil covered by dead leaves
  REAL,DIMENSION(:,:,:),ALLOCATABLE  :: assim_param_x                        !! min/max/opt temperature for photosynthesis 
                                                                                    !! and vcmax/vjmax parameters 
  REAL,DIMENSION(:,:),ALLOCATABLE    :: height_x                             !! height (m)
  REAL,DIMENSION(:,:),ALLOCATABLE    :: qsintmax_x                           !! Maximum water on vegetation for interception
  REAL,DIMENSION(:,:),ALLOCATABLE    :: co2_flux                             !! CO2 flux in gC/m**2 of ground/second
  REAL,DIMENSION(:),ALLOCATABLE      :: fco2_lu                              !! Land Cover Change CO2 flux (gC/m**2 of 
  REAL,DIMENSION(:),ALLOCATABLE      :: temp_growth                          !! Growth temperature - Is equal to t2m_month (°C)
                                                                                    !! average ground/s) 
  REAL,DIMENSION(:),ALLOCATABLE      :: tot_bare_soil                        !! Total evaporating bare soil fraction 

  INTEGER,DIMENSION(:),ALLOCATABLE   :: indices_g                            !! Vector of indeces of land points, only known by root proc
  REAL,DIMENSION(:),ALLOCATABLE      :: x_indices_g                          !! Temporary vector of indeces of land points
  REAL,DIMENSION(:,:),ALLOCATABLE    :: x_neighbours_g                       !! Indeces of neighbours for each land point
  ...
  INTEGER                            :: iim,jjm,llm                          !! # of lon,lat and time-step in the restart 
                                                                                    !! files 
  ...
  REAL, ALLOCATABLE, DIMENSION(:)           :: lon_rect, lat_rect                   !! Vectors of Longitude and latitude (iim or 
                                                                                    ...
                                                                                    !! rectilinear grid 
  REAL                               :: dt                                   !! Time step of sechiba component read in the 
                                                                                    !! restart file [seconds] 
  INTEGER                            :: itau_dep,itau_fin,itau               !! First, last and current time step of SECHIBA 
                                                                                    !! component 
  INTEGER                            :: itau_len,itau_step                   !! Total length of the simulation and length 
                                                                                    !! between 2 calls of slowproc (expreseed in 
                                                                                    !! time steps of SECHIBA component) 
  REAL                               :: date0                                !! Initial date
  INTEGER                            :: rest_id_sec,rest_id_sto              !! ID's of the restart files for the SECHIBA 
                                                                                    !! and STOMATE components 
  INTEGER                            :: hist_id_sec,hist_id_sec2             !! ID's of the history files of SECHIBA 
                                                                                    !! component (required by the interface of 
                                                                                    !! slowproc but not used) 
  INTEGER                            :: hist_id_stom,hist_id_stom_IPCC       !! ID's of the history files of STOMATE 
                                                                                    !! component 
  ...
  REAL                                      :: hist_dt_stom,hist_dt_stom_ipcc       !! Time frequency at which variables are 
                                                                                    ...
                                                                                    !! [second] 
  REAL, ALLOCATABLE, DIMENSION(:)    :: hist_PFTaxis                         !! Vector with PFT indeces used as axis in the 
                                                                                    !! history file 
  REAL,DIMENSION(10)                 :: hist_pool_10axis                     !! Vector with 10-year indeces used as axis in 
                                                                                    !! the history file (for land-use change) 
  REAL,DIMENSION(100)                :: hist_pool_100axis                    !! Vector with 100-year indeces used as axis in 
                                                                                    !! the history file (for land-use change) 
  REAL,DIMENSION(11)                 :: hist_pool_11axis                     !! Vector with 11-year indeces used as axis in 
                                                                                    !! the history file (for land-use change) 
  REAL,DIMENSION(101)                :: hist_pool_101axis                    !! Vector with 101-year indeces used as axis in 
                                                                                    !! the history file (for land-use change) 
  ...
  INTEGER                            :: i,j,iv                               !! used as counters
  ...
  INTEGER,PARAMETER                  :: nbvarmax=200                         !! Maximum number of variables assumed in the 
                                                                                    !! restart file of SECHIBA component used as 
                                                                                    !! input 
  INTEGER                            :: nbvar                                !! Number of variables present in the restart 
  ...
  CHARACTER(LEN=50),DIMENSION(nbvarmax)     :: varnames                             !! Name of the variables present in the restart 
                                                                                    !! file of SECHIBA component used as input 
  INTEGER                            :: varnbdim                             !! Number of dimensions of a variable 
                                                                                    !! varnames(i) 
  INTEGER,PARAMETER                  :: varnbdim_max=20                      !! Maximum number of dimensions of a variable 
                                                                                    !! varnames(i) 
  ...
  INTEGER                                   :: nsfm,nsft
  INTEGER                                   :: iisf,iiisf
  INTEGER                            :: max_totsize,totsize_1step
  INTEGER                            :: totsize_tmp  
  INTEGER                                   :: vid
  CHARACTER(LEN=100)                        :: forcing_name
  REAL                                      :: x
  REAL,DIMENSION(:,:),ALLOCATABLE    :: var_3d
  REAL                               :: var_1d(1)
  REAL                               :: time_sec,time_step_sec
  REAL                               :: time_dri,time_step_dri
  REAL,DIMENSION(1)                  :: r1d
  REAL                               :: julian,djulian

  INTEGER                            :: ji,jv,l
  INTEGER                            :: printlev_loc
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)   :: max_eau_var_x       !! Maximum water content of the soil @tex $(kg m^{-2})$ @endtex
  INTEGER                            :: ierr
...
!  CALL init_data_para(iim_g,jjm_g,nbp_glo,indices_g)
  ...
  ! index_g is declared in mod_orchidee_para and allocated by grid_allocate_glo
  ...
  !---
  CALL grid_init( kjpindex, 4, "RegLonLat", "ForcingGrid" ) 
...
!-
  ...
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (soiltemp(kjpindex,nslm),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (soilhum(kjpindex,nslm),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  ...
  a_er = a_er .OR. (ier.NE.0)
  ALLOCATE (max_eau_var_x(kjpindex),stat=ier)
  a_er = a_er .OR. (ier.NE.0)
  IF (a_er) THEN
     CALL ipslerr (3,'teststomate', &
          &        'PROBLEM WITH ALLOCATION', &
  ...
  totsize_1step = SIZE(soiltile(:,3))*KIND(soiltile(:,3)) + &
       ...
       SIZE(max_eau_var_x)*KIND(max_eau_var_x)
  ...
  !
  CALL time_initialize(itau_dep, date0, dt_sechiba, "START")
  !-
  ...
  itau_len = NINT( MAX(1.0,FLOAT(NINT(itau_len*dtradia/dt_force))) &
       &                *dt_force/dtradia)
  !-
...
!- declare this axis
 ...
 & hist_pool_11axis_id, hist_pool_101axis_id)
  ...
  DO l = 1, nslm-1
     ...
     diaglev(l) = zmaxh/(2**(nslm-1) -1) * ( ( 2**(l-1) -1) + ( 2**(l) -1) ) / 2.0
  ENDDO
  diaglev(nslm) = zmaxh
  !
!-
...
!-
  ...
  !-
  CALL slowproc_initialize (itau,        kjpij,       kjpindex,                          &
                            rest_id_sec, rest_id_sto, hist_id_stom,   hist_id_stom_IPCC, &
...
! &   rest_id_sec, hist_id_sec, hist_id_sec2, rest_id_sto, hist_id_stom, hist_id_stom_IPCC, co2_flux, fco2_lu, temp_growth)
  ...
  CALL time_nextstep( itau_dep+itau_step )
...
!---- start again at first forcing state
     ...
     max_eau_var_x(:) = max_eau_var_daily_fm(:,iisf)
...
! &     t2m, t2m_min, temp_sol, soiltemp, &
! &     humrel_x, soilhum, litterhum, precip_rain, precip_snow, PB_HERE gpp_x, & !<-------- TO ADD here pb
! &     tmc_pft, drainage_pft, runoff_pft, swc_pft, !<------TO ADD
! &     deadleaf_cover, assim_param_x, &
! &     max_eau_var_x, &
! &     lai_x, frac_age_x, height_x, veget_x, &
! &     frac_nobio, veget_max_x, totfrac_nobio, qsintmax_x, &
! &     rest_id_sec, hist_id_sec, hist_id_sec2, rest_id_sto, hist_id_stom, hist_id_stom_IPCC, &
