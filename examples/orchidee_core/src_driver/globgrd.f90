...
!!
  ...
  SUBROUTINE globgrd_getdomsz(filename, iim, jjm, nbland, model_guess, fid, forcingfile, zoom_lon, zoom_lat)
    ...
    REAL, DIMENSION(2), INTENT(in), OPTIONAL :: zoom_lon, zoom_lat
    !
    ! OUTPUT
    !
    INTEGER, INTENT(out)    :: fid
    INTEGER, INTENT(out)    :: iim, jjm, nbland
    CHARACTER(LEN=*), INTENT(out)  :: model_guess
    !
    ! LOCAL
    !
    INTEGER :: iret, ndims, nvars, nb_atts, id_unlim, iv, lll
    INTEGER :: iim_full, jjm_full, nbland_full
    CHARACTER(LEN=20) :: axname, varname
    CHARACTER(LEN=120) :: tmpfile
    REAL, DIMENSION(2) :: loczoom_lon, loczoom_lat
...
!!
  ...
  SUBROUTINE globgrd_getgrid(fid, iim, jjm, nbland, model_guess, lon, lat, mask, area, corners, &
    ...
    ! OUTPUT
    !
    REAL,DIMENSION(iim,jjm), INTENT(out)     :: lon, lat, mask, area
    REAL,DIMENSION(iim,jjm,4,2), INTENT(out) :: corners
    INTEGER, DIMENSION(nbland), INTENT(out)  :: lindex
    REAL,DIMENSION(nbland), INTENT(out)      :: contfrac
...
!!
  SUBROUTINE globgrd_getregular(fid, iim, jjm, nbland, lon, lat, mask, area, corners, &
    ...
    ! LOCAL
    !
    INTEGER :: iret, iv, nvars, varndim
    INTEGER :: i, j
    CHARACTER(LEN=20) :: varname
    INTEGER, DIMENSION(4) :: vardims
    REAL :: dx
    !
...
!!
  SUBROUTINE globgrd_getwrf(fid, iim, jjm, nbland, lon, lat, mask, area, corners, &
    ...
    ! LOCAL
    !
    INTEGER :: i, ip, jp, k, iret, iv, nvars, varndim
    INTEGER :: imm1, imp1
    CHARACTER(LEN=20) :: varname
    REAL,DIMENSION(iim,jjm) :: mapfac_x, mapfac_y
    INTEGER, DIMENSION(4) :: vardims
    INTEGER, DIMENSION(8) :: rose
    !
    !
    ! Set some default values agains which we can check 
    !
...
!
  SUBROUTINE globgrd_writegrid (gridfilename)
    ...
    !
    INTEGER :: iret, fid, i
    INTEGER :: lonid, latid, landdimid, resid, neighid, maskid, nbcornersid
    INTEGER :: londimid, latdimid, contfracid, resolutionid, neighbourid
    INTEGER :: landindexid, areaid, cornerid
    !
...
!
  SUBROUTINE globgrd_writevar(iim, jjm, lon, lat, nbpt, lalo, var, varname, filename)
    ...
    ! ARGUMENTS
    !
    INTEGER, INTENT(in) :: iim, jjm, nbpt
    REAL, INTENT(in)    :: lon(iim,jjm), lat(iim,jjm)
    REAL, INTENT(in)    :: lalo(nbpt,2)
    REAL, INTENT(in)    :: var(nbpt)
    CHARACTER(LEN=*), INTENT(in) :: varname
    CHARACTER(LEN=*), INTENT(in) :: filename
    !
    ! LOCAL
    !
    INTEGER :: iret, fid, i, ii, jj, nlonid, nlatid, varid
    INTEGER, DIMENSION(2) :: lolaid 
    REAL, ALLOCATABLE, DIMENSION(:,:) :: varfull, dist
    INTEGER, DIMENSION(2)             :: closest
    !
    !
    ...
    DO i=1,nbpt
       ...
       DO ii=1,iim
          DO jj=1,jjm
             IF ( ABS(lalo(i,1)-lat(ii,jj)) < 0.25 .AND. ABS(lalo(i,2)-lon(ii,jj)) < 0.25 ) THEN
                closest(1) = ii
                closest(2) = jj
             ENDIF
          ENDDO
