MODULE getlandseamask
  ...
  INTEGER, SAVE                           :: iim, jjm, nbland
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:) :: lon, lat
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:) :: contfrac, orog
...
CONTAINS
  ...
  SUBROUTINE getlandseamask_init(iim_out, jjm_out, nbland_out)
    ...
    ! ARGUMENTS
    !
    INTEGER, INTENT(out) :: iim_out, jjm_out, nbland_out
    !
    ! LOCAL
    !
    ...
    INTEGER                                      :: iim_full, jjm_full      !! Lon/Lat dimensions for ROSE in etopo20.nc 
    REAL, ALLOCATABLE, DIMENSION (:,:)           :: lon_full, lat_full      !! Lon/Lat array for ROSE
    REAL, ALLOCATABLE, DIMENSION (:,:)           :: etopo, contfrac_full    !! Array for reading ROSE in etopo20.nc
    INTEGER, DIMENSION(1)                        :: ibeg, iend
    INTEGER, DIMENSION(1)                        :: jbeg, jend
    INTEGER                                      :: fid, i, j
    REAL                                         :: dx, dy
    !
    REAL, DIMENSION(2)                           :: zoom_lon, zoom_lat               !! For crop region when reading
    !
  ...
  SUBROUTINE getlandseamask_read(lon_out, lat_out, mask_out, orog_out)
    ...
    ! ARGUMENTS
    !
    REAL, DIMENSION(:,:), INTENT(out) :: lon_out, lat_out
    REAL, DIMENSION(:,:), INTENT(out) :: mask_out, orog_out
    !
    ! LOCAL
    !
    lon_out(:,:) = lon(:,:)
  ...
  SUBROUTINE opencontfrac(filename, iim_full, jjm_full, fid)
    ...
    INTEGER, INTENT(out) :: iim_full, jjm_full, fid
    !
    INTEGER :: iret, i, lll, ndims, nvars
  ...
  SUBROUTINE readcontfrac(fid, filename, iimf, jjmf, lon, lat, contf)
    ...
    CHARACTER(LEN=*), INTENT(in)             :: filename
    INTEGER, INTENT(in)               :: iimf, jjmf
    REAL, DIMENSION(:,:), INTENT(out) :: lon, lat
    REAL, DIMENSION(:,:), INTENT(out) :: contf
    !
    ! LOCAL
    !
    INTEGER :: iret, ndims, nvars, xid, yid, vid, wid
    INTEGER :: xndims, yndims, vndims, wndims
    INTEGER :: iv, ndimsvar, i, j
  ...
  SUBROUTINE topo_getsize(filename, iim_full, jjm_full)
    ...
    !
    CHARACTER(LEN=*), INTENT(in) :: filename
    INTEGER, INTENT(out)  :: iim_full, jjm_full
    !
    ! LOCAL
    !
    INTEGER :: fid, iret, xid, yid, ndims, nvars, i, lll
    CHARACTER(LEN=20) :: dimname
    !
    iret = NF90_OPEN(filename, NF90_NOWRITE, fid)
    IF (iret /= NF90_NOERR) THEN
  ...
  SUBROUTINE topo_getvar(filename, iim, jjm, lon, lat, etopo)
    ...
    REAL, DIMENSION(iim,jjm), INTENT(out) :: lon 
    REAL, DIMENSION(iim,jjm), INTENT(out) :: lat
    REAL, DIMENSION(iim,jjm), INTENT(out) :: etopo
    !
    ! LOCAL
    !
    INTEGER :: fid, iret, xid, yid, vid
    INTEGER :: i, j, iv, ndimsvar, ndims, nvars
    CHARACTER(LEN=20) :: varname, units
    INTEGER, DIMENSION(1) :: il
    REAL, ALLOCATABLE, DIMENSION(:)   :: lon_loc, lon_read, lat_read
    REAL, ALLOCATABLE, DIMENSION(:,:) :: etopo_loc
  ...
  SUBROUTINE interpol(iim_fine, jjm_fine, lon_fine, lat_fine, orog, iim, jjm, lon, lat, neworog)
    ...
    IMPLICIT NONE
    ! Parameter
    !INTEGER, PARAMETER                                  :: nbvmax = 800
    INTEGER, PARAMETER                                  :: nbvmax = 30000
    !
    INTEGER                                             :: iim_fine, jjm_fine, iim, jjm
    REAL, DIMENSION (iim_fine,jjm_fine)                 :: lon_fine
    REAL, DIMENSION (iim_fine,jjm_fine)                 :: lat_fine
    REAL, DIMENSION (iim_fine,jjm_fine)                 :: orog
    REAL, DIMENSION (iim,jjm)                           :: lon
    REAL, DIMENSION (jjm,iim)                           :: lat
    REAL, DIMENSION (iim,jjm)                           :: neworog
    !
    REAL, DIMENSION (nbvmax)                            :: area, topo
    !
    INTEGER                                             :: ip, jp, ig, jg, i, fopt, lastjp
    REAL                                                :: dx, dy, ax, ay, ox, lon_up, lon_low, lat_up, lat_low, sgn
    REAL                                                :: totarea, landarea, height
    REAL, ALLOCATABLE, DIMENSION (:,:)                  :: lon_ful, lat_ful, laup_rel, loup_rel, lalow_rel, lolow_rel
    REAL                                                :: lonrel, lolowrel, louprel, coslat
    !
    ! Allocate arrays
    !
