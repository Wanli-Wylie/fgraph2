...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_sechiba/explicitsnow.f90 $
...
!================================================================================================================================
  SUBROUTINE explicitsnow_initialize( kjit,     kjpindex, rest_id,    snowrho,   &
    ...
    REAL, DIMENSION (kjpindex,nsnow), INTENT(out)  :: snowrho        !! Snow density (Kg/m^3)
    ...
    REAL, DIMENSION (kjpindex,nsnow), INTENT(out)  :: snowheat       !! Snow heat content (J/m^2)
    REAL, DIMENSION (kjpindex,nsnow), INTENT(out)  :: snowgrain      !! Snow grainsize (m)
...
!================================================================================================================================
  SUBROUTINE explicitsnow_main(kjpindex,    precip_rain,  precip_snow,   temp_air,    pb,       & ! in
    ...
    REAL, DIMENSION (kjpindex), INTENT(in)            :: precip_snow      !! Snowfall
    ...
    REAL, DIMENSION (kjpindex), INTENT(in)            :: temp_sol_new     !! Surface temperature
    ...
    REAL, DIMENSION (kjpindex,nnobio), INTENT(out)    :: subsnownobio     !! Sublimation of snow on other surface types (ice, lakes, ...)
    REAL, DIMENSION (kjpindex), INTENT(out)           :: grndflux         !! Net flux into soil [W/m2]
    REAL, DIMENSION (kjpindex), INTENT(out)           :: snowmelt         !! Snow melt [mm/dt_sechiba]
    REAL, DIMENSION (kjpindex), INTENT(out)           :: tot_melt         !! Total melt from ice and snow
    REAL, DIMENSION (kjpindex), INTENT(out)           :: subsinksoil      !! Excess of sublimation as a sink for the soil
    ...
    REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)   :: snowrho          !! Snow density (Kg/m^3)
    REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)   :: snowgrain        !! Snow grainsize (m)
    REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)   :: snowdz           !! Snow layer thickness [m]
    REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)   :: snowtemp         !! Snow temperature
    REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)   :: snowheat         !! Snow heat content (J/m^2)
    REAL, DIMENSION (kjpindex), INTENT(inout)         :: snow             !! Snow mass [Kg/m^2]
    REAL, DIMENSION (kjpindex), INTENT(inout)         :: temp_sol_add     !! Additional energy to melt snow for snow ablation case (K)

 
    !! 0.4 Local declaration
    INTEGER                                           :: ji, iv, jj,m,jv
    REAL,DIMENSION  (kjpindex)                        :: snow_depth_tmp
    REAL,DIMENSION  (kjpindex)                        :: snowmelt_from_maxmass
    REAL                                              :: snowdzm1
    REAL, DIMENSION (kjpindex)                        :: thrufal          !! Water leaving snow pack [kg/m2/s]
    REAL, DIMENSION (kjpindex)                        :: d_age            !! Snow age change
    REAL, DIMENSION (kjpindex)                        :: xx               !! Temporary
    REAL, DIMENSION (kjpindex)                        :: snowmelt_tmp,snowmelt_ice,icemelt,temp_sol_new_old
    REAL, DIMENSION (kjpindex,nsnow)                  :: snowdz_old
    REAL, DIMENSION (kjpindex)                        :: ZLIQHEATXS
    REAL, DIMENSION (kjpindex)                        :: ZSNOWEVAPS, ZSNOWDZ,subsnowveg,ZSNOWMELT_XS
    REAL                                              :: maxmass_snowdepth, snow_d1k
    REAL, DIMENSION (kjpindex,nsnow)                  :: WSNOWDZ,SMASS
    REAL, DIMENSION (kjpindex)                        :: SMASSC,snowacc,snow_remove
    INTEGER :: locjj
    REAL                                              :: grndflux_tmp
    REAL, DIMENSION (nsnow)                           :: snowtemp_tmp
    REAL                                              :: s2flux_tmp,fromsoilflux
    REAL, DIMENSION (kjpindex,nsnow)                  :: pcapa_snow 
    REAL, DIMENSION (kjpindex)                        :: psnowhmass
    REAL, PARAMETER                                   :: XP00 = 1.E5
    REAL, DIMENSION (kjpindex)                        :: snowheattot_begin,snowheattot_end,del_snowheattot !![J/m2]
...
!    snowmelt_from_maxmass(:) = 0.0
    ...
    CALL xios_orchidee_send_field("snowliqtot",SUM(snowliq(:,:),2))
    CALL xios_orchidee_send_field("snowrho",snowrho)
    CALL xios_orchidee_send_field("snowheat",snowheat)
    CALL xios_orchidee_send_field("snowgrain",snowgrain)
...
!================================================================================================================================
  SUBROUTINE explicitsnow_finalize ( kjit,     kjpindex, rest_id,    snowrho,   &
    ...
    INTEGER, INTENT(in)                           :: kjpindex       !! Domain size
    INTEGER,INTENT (in)                           :: rest_id        !! Restart file identifier
    REAL, DIMENSION (kjpindex,nsnow), INTENT(in)  :: snowrho        !! Snow density (Kg/m^3)
    ...
    REAL, DIMENSION (kjpindex,nsnow), INTENT(in)  :: snowheat       !! Snow heat content (J/m^2)
    REAL, DIMENSION (kjpindex,nsnow), INTENT(in)  :: snowgrain      !! Snow grainsize (m)
...
!================================================================================================================================
 ...
 SUBROUTINE explicitsnow_grain(kjpindex,snowliq,snowdz,gtemp,snowtemp,pb,snowgrain)
      ...
      !! 0.1 Input variables
      INTEGER,INTENT(in)                                  :: kjpindex         !! Domain size
      REAL,DIMENSION(kjpindex,nsnow),INTENT(in)           :: snowliq          !! Liquid water content
      ...
      REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)        :: snowgrain        !! Snow grain size (m)

      !! 0.4 Local variables
      REAL,DIMENSION(kjpindex,nsnow)                      :: zsnowdz,zdz,ztheta
      REAL,DIMENSION(kjpindex,0:nsnow)                    :: ztemp,zdiff,ztgrad,zthetaa,zfrac,&
                                                                    zexpo,zckt_liq,zckt_ice,zckt
      REAL,DIMENSION(kjpindex,nsnow)                      :: zrhomin,zgrainmin
      INTEGER :: ji,jj

      !! 0.5 Local parameters
      REAL, PARAMETER                                     :: ztheta_crit = 0.02     !! m3 m-3
      REAL, PARAMETER                                     :: zc1_ice     = 8.047E+9 !! kg m-3 K
      REAL, PARAMETER                                     :: zc1_liq     = 5.726E+8 !! kg m-3 K
      REAL, PARAMETER                                     :: zdeos       = 0.92E-4  !! effective diffusion
                                                                                           !! coef for water vapor in snow
                                                                                           !! at 0C and 1000 mb (m2 s-1)
      REAL, PARAMETER                                     :: zg1         = 5.0E-7   !! m4 kg-1
      REAL, PARAMETER                                     :: zg2         = 4.0E-12  !! m2 s-1
      REAL, PARAMETER                                     :: ztheta_w      = 0.05   !! m3 m-3
      REAL, PARAMETER                                     :: ztheta_crit_w = 0.14   !! m3 m-3
      REAL, PARAMETER                                     :: zdzmin        = 0.01   !! m : minimum thickness
...
!================================================================================================================================
   ...
   SUBROUTINE explicitsnow_compactn(kjpindex,snowtemp,snowrho,snowdz)
         ...
         REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)       :: snowrho          !! Snow density (Kg/m^3)
         REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)       :: snowdz           !! Snow depth

         !! 0.4 Local variables

         REAL,DIMENSION(kjpindex,nsnow)                     :: zwsnowdz,zsmass,zsnowrho2,zviscocity,zsettle 
         REAL,DIMENSION(kjpindex)                           :: zsmassc          !! cummulative snow mass (kg/m2)
         REAL,DIMENSION(kjpindex)                           :: snowdepth_crit
         INTEGER                                            :: ji,jj
...
!================================================================================================================================
   ...
   SUBROUTINE explicitsnow_transf(kjpindex,snowdz_old,snowdz,snowrho,snowheat,snowgrain)
     ...
     INTEGER,INTENT(in)                                           :: kjpindex         !! Domain size
     REAL,DIMENSION(kjpindex,nsnow),INTENT(in)                    :: snowdz_old       !! Snow depth at the previous time step 
     ...
     REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)                 :: snowrho          !! Snow density (Kg/m^3)
     REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)                 :: snowgrain        !! Snow grain size (m)
     REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)                 :: snowdz           !! Snow depth (m)
     REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)                 :: snowheat         !! Snow heat content/enthalpy (J/m2)
                                                                        
     !! 0.4 Local varibles                                              
      
     REAL,DIMENSION(kjpindex,0:nsnow)                             :: zsnowzo
     REAL,DIMENSION(kjpindex,0:nsnow)                             :: zsnowzn 
     REAL,DIMENSION(kjpindex,nsnow)                               :: zsnowddz 
     REAL,DIMENSION(kjpindex,nsnow)                               :: zdelta
     REAL,DIMENSION(kjpindex,nsnow)                               :: zsnowrhon,zsnowheatn,zsnowgrainn
     REAL,DIMENSION(kjpindex)                                     :: zsnowmix_delta
     REAL,DIMENSION(kjpindex)                                     :: zsumheat, zsumswe, zsumgrain
     INTEGER,DIMENSION(nsnow,2)                                   :: locflag
     REAL                                                         :: psnow
     INTEGER                                                      :: ji,jj,jjj
...
!================================================================================================================================
   ...
   SUBROUTINE explicitsnow_fall(kjpindex,precip_snow,temp_air,u,v,totfrac_nobio,snowrho,snowdz,&
       ...
       INTEGER,INTENT(in)                              :: kjpindex            !! Domain size
       REAL,DIMENSION(kjpindex),INTENT(in)             :: precip_snow         !! Snow rate (SWE) (kg/m2 per dt_sechiba)
       REAL,DIMENSION(kjpindex),INTENT(in)             :: temp_air            !! Air temperature
       REAL,DIMENSION(kjpindex),INTENT(in)             :: u,v                 !! Horizontal wind speed
       REAL,DIMENSION(kjpindex,nsnow),INTENT(in)       :: snowtemp            !! Snow temperature
       REAL,DIMENSION(kjpindex),INTENT(in)             :: totfrac_nobio
       !! 0.2 Output variables

       REAL, DIMENSION(kjpindex),INTENT(out)           :: psnowhmass          !! Heat content of snowfall (J/m2)

       !! 0.3 Modified variables
 
       REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)    :: snowrho             !! Snow density profile (kg/m3)
       REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)    :: snowdz              !! Snow layer thickness profile (m)
       REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)    :: snowheat            !! Snow heat content/enthalpy (J/m2)
       REAL,DIMENSION(kjpindex,nsnow),INTENT(inout)    :: snowgrain           !! Snow grain size (m)

       !! 0.4 Local variables

       REAL, DIMENSION(kjpindex)                       :: rhosnew             !! Snowfall density
       REAL, DIMENSION(kjpindex)                       :: dsnowfall           !! Snowfall thickness (m)
       REAL, DIMENSION(kjpindex,nsnow)                 :: snowdz_old
       REAL, DIMENSION(kjpindex)                       :: snow_depth,snow_depth_old,newgrain
       REAL                                            :: snowfall_delta,speed
       INTEGER                                         :: ji,jj
...
!================================================================================================================================
   ...
   SUBROUTINE explicitsnow_gone(kjpindex,pgflux,&
     ...
     INTEGER, INTENT(in)                                 :: kjpindex     !! Domain size
     REAL,DIMENSION (kjpindex), INTENT (in)              :: pgflux       !! Net energy into snow pack(w/m2)
     REAL,DIMENSION (kjpindex,nsnow),INTENT(in)          :: snowheat     !! Snow heat content (J/m^2)
     ...
     REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)     :: snowdz       !! Snow depth
     REAL,DIMENSION (kjpindex,nsnow), INTENT(inout)      :: snowrho      !! Snow density (Kg/m^3)
     REAL,DIMENSION (kjpindex,nsnow), INTENT(inout)      :: snowliq      !! Liquid water content
     REAL,DIMENSION(kjpindex), INTENT(inout)             :: grndflux     !! Soil/snow interface heat flux (W/m2)
     REAL,DIMENSION(kjpindex),INTENT(inout)              :: snowmelt     !! Snow melt
     REAL,DIMENSION(kjpindex)                            :: thrufal      !! Water leaving snowpack(kg/m2/s)

     !! 0.4 Local variables

     INTEGER                                             :: ji,jj
     REAL,DIMENSION(kjpindex)                            :: snowgone_delta
     REAL,DIMENSION (kjpindex)                           :: totsnowheat  !!snow heat content at each layer 
     REAL,DIMENSION(kjpindex)                            :: snowdepth_crit
...
!================================================================================================================================
   ...
   SUBROUTINE explicitsnow_melt_refrz(kjpindex,precip_rain,pgflux,soilcap,&
      ...
      INTEGER, INTENT (in)                             :: kjpindex     !! Domain size
      REAL,DIMENSION (kjpindex,nsnow)                  :: pcapa_snow   !! Heat capacity for snow
      REAL,DIMENSION (kjpindex), INTENT(in)            :: precip_rain  !! Rainfall      
      REAL,DIMENSION (kjpindex), INTENT(in)            :: temp_air     !! Air temperature
      REAL,DIMENSION (kjpindex),INTENT(in)             :: pgflux       !! Net energy into snowpack(w/m2)
      REAL,DIMENSION (kjpindex),INTENT(in)             :: soilcap      !! Soil heat capacity
      ...
      REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)  :: snowtemp     !! Snow temperature 
      REAL, DIMENSION (kjpindex,nsnow), INTENT(inout)  :: snowdz       !! Snow depth
      REAL,DIMENSION (kjpindex,nsnow), INTENT(inout)   :: snowrho      !! Snow layer density (Kg/m^3)
      REAL,DIMENSION (kjpindex,nsnow), INTENT(inout)   :: snowliq      !! Liquid water content
      REAL,DIMENSION(kjpindex),INTENT(inout)           :: grndflux     !! Net energy input to soil 
      REAL,DIMENSION (kjpindex), INTENT(inout)         :: snowmelt     !! Snowmelt

      !! 0.4 Local variables

      REAL,DIMENSION (kjpindex)                        :: meltxs       !! Residual snowmelt energy applied to underlying soil
      REAL                                             :: enerin,melttot,hrain 
      REAL,DIMENSION (nsnow)                           :: zsnowlwe
      REAL,DIMENSION (nsnow)                           :: flowliq
      REAL,DIMENSION (kjpindex)                        :: snowmass
      REAL,DIMENSION (nsnow)                           :: zphase       !! Phase change (from ice to water) (J/m2)
      REAL,DIMENSION (nsnow)                           :: zphase2      !! Phase change (from water to ice)
      REAL,DIMENSION (nsnow)                           :: zphase3      !! Phase change related with net energy input to snowpack
      REAL,DIMENSION (nsnow)                           :: zsnowdz      !! Snow layer depth
      REAL,DIMENSION (nsnow)                           :: zsnowmelt    !! Snow melt (liquid water) (m)
      REAL,DIMENSION (nsnow)                           :: zsnowtemp
      REAL,DIMENSION (nsnow)                           :: zmeltxs      !! Excess melt
      REAL,DIMENSION (nsnow)                           :: zwholdmax    !! Maximum liquid water holding (m)
      REAL,DIMENSION (nsnow)                           :: zcmprsfact   !! Compression factor due to densification from melting
      REAL,DIMENSION (nsnow)                           :: zscap        !! Snow heat capacity (J/m3 K)
      REAL,DIMENSION (nsnow)                           :: zsnowliq     !! (m)
      REAL,DIMENSION (nsnow)                           :: snowtemp_old
      REAL,DIMENSION (0:nsnow)                         :: zflowliqt    !!(m)
      REAL                                             :: zrainfall,zpcpxs
      REAL                                             :: ztotwcap
      REAL,DIMENSION(kjpindex,nsnow)                   :: snowdz_old,snowliq_old
      INTEGER                                          :: jj,ji, iv
      REAL,DIMENSION(nsnow)                            :: snowdz_old2
      REAL,DIMENSION(nsnow)                            :: zsnowrho 
...
!================================================================================================================================ 
  SUBROUTINE explicitsnow_levels( kjpindex,snow_thick, snowdz)
    ...
    !! 0.1 Input variables
    INTEGER, INTENT(in)                                     :: kjpindex     !! Domain size
    REAL,DIMENSION (kjpindex),   INTENT (in)                :: snow_thick   !! Total snow depth
    ...
    INTEGER                                                 :: il,it,ji
    INTEGER                                                 :: i,j, ix
    REAL, DIMENSION(kjpindex)                               :: xi, xf
    REAL, PARAMETER, DIMENSION(3)                           :: ZSGCOEF1  = (/0.25, 0.50, 0.25/) !! Snow grid parameters 
    REAL, PARAMETER, DIMENSION(2)                           :: ZSGCOEF2  = (/0.05, 0.34/)       !! Snow grid parameters 
    REAL, PARAMETER                                         :: ZSNOWTRANS = 0.20                !! Minimum total snow depth at which surface layer thickness is constant (m)
...
!================================================================================================================================
  ...
  !! 0. Variables and parameter declaration
    ...
    REAL,DIMENSION (kjpindex), INTENT (in)            :: temp_sol_new !! skin temperature
    REAL, DIMENSION (kjpindex,nsnow),INTENT (in)      :: cgrnd_snow   !! Integration coefficient for snow numerical scheme
    REAL, DIMENSION (kjpindex,nsnow),INTENT (in)      :: dgrnd_snow   !! Integration coefficient for snow numerical scheme
    REAL, DIMENSION (kjpindex),INTENT(in)             :: lambda_snow  !! Coefficient of the linear extrapolation of surface temperature 
    REAL, DIMENSION (kjpindex,nsnow),INTENT(in)       :: snowdz       !! Snow layer thickness
    ...
    !! 0.3 Modified variables
    REAL,DIMENSION (kjpindex,nsnow), INTENT (inout)   :: snowtemp
