...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_sechiba/diffuco.f90 $
!! $Date: 2018-05-18 14:12:34 +0200 (五, 2018-05-18) $
!! $Revision: 5245 $
!! \n
!_ ================================================================================================================================
...
MODULE diffuco
  ...
  !
  REAL, ALLOCATABLE, SAVE, DIMENSION (:)      :: wind                      !! Wind module (m s^{-1})
...
!_ ==============================================================================================================================
  SUBROUTINE diffuco_initialize (kjit,    kjpindex, index,                  &
    ...
    REAL,DIMENSION (kjpindex,2),   INTENT (in)  :: lalo             !! Geographical coordinates
    INTEGER,DIMENSION (kjpindex,NbNeighb),INTENT (in):: neighbours  !! Vector of neighbours for each 
    REAL,DIMENSION (kjpindex,2), INTENT(in)     :: resolution       !! The size in km of each grid-box in X and Y
    
    !! 0.2 Output variables
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: rstruct          !! Structural resistance for the vegetation
    
    !! 0.3 Modified variables
    REAL,DIMENSION (kjpindex), INTENT (in)      :: q_cdrag          !! Surface drag coefficient  (-)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE diffuco_main (kjit, kjpindex, index, indexveg, indexlai, u, v, &
     & zlev, z0m, z0h, roughheight, temp_sol, temp_air, temp_growth, rau, q_cdrag, qsurf, qair, q2m, t2m, pb, &
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER, INTENT(in)                         :: kjit             !! Time step number (-) 
    INTEGER, INTENT(in)                         :: kjpindex         !! Domain size (-)
    INTEGER,INTENT (in)                         :: hist_id          !! _History_ file identifier (-)
    INTEGER,INTENT (in)                         :: hist2_id         !! _History_ file 2 identifier (-)
    INTEGER,DIMENSION (kjpindex), INTENT (in)     :: index          !! Indeces of the points on the map (-)
    INTEGER,DIMENSION (kjpindex*(nlai+1)), INTENT (in) :: indexlai  !! Indeces of the points on the 3D map
    INTEGER,DIMENSION (kjpindex*nvm), INTENT (in) :: indexveg       !! Indeces of the points on the 3D map (-)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: u                !! Eastward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: v                !! Northward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: zlev             !! Height of first layer (m)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: z0m              !! Surface roughness Length for momentum (m)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: z0h              !! Surface roughness Length for heat (m)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: roughheight      !! Effective height for roughness (m)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: temp_sol         !! Skin temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: temp_air         !! Lowest level temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: temp_growth      !! Growth temperature (°C) - Is equal to t2m_month
    REAL,DIMENSION (kjpindex), INTENT (in)      :: rau              !! Air Density (kg m^{-3})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: qsurf            !! Near surface air specific humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: qair             !! Lowest level air specific humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: q2m              !! 2m air specific humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: t2m              !! 2m air temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: snow             !! Snow mass (kg)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: flood_frac       !! Fraction of floodplains
    REAL,DIMENSION (kjpindex), INTENT (in)      :: flood_res        !! Reservoir in floodplains (estimation to avoid over-evaporation)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: pb               !! Surface level pressure (hPa)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: rsol             !! Bare soil evaporation resistance (s m^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: evap_bare_lim    !! Limit to the bare soil evaporation when the 
                                                                           !! 11-layer hydrology is used (-)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: evapot           !! Soil Potential Evaporation (mm day^{-1}) 
                                                                           !! NdN This variable does not seem to be used at 
                                                                           !! all in diffuco
    REAL,DIMENSION (kjpindex), INTENT (in)      :: evapot_corr      !! Soil Potential Evaporation
    REAL,DIMENSION (kjpindex,nnobio), INTENT (in) :: frac_nobio     !! Fraction of ice,lakes,cities,... (-)
    REAL,DIMENSION (kjpindex,nnobio), INTENT (in) :: snow_nobio     !! Snow on ice,lakes,cities,... (kg m^{-2})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: totfrac_nobio    !! Total fraction of ice+lakes+cities+... (-)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: swnet            !! Net surface short-wave flux (W m^{-2})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: swdown           !! Down-welling surface short-wave flux (W m^{-2})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: coszang          !! Cosine of the solar zenith angle (unitless)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: ccanopy          !! CO2 concentration inside the canopy (ppm)
    ...
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: qsintveg         !! Water on vegetation due to interception (kg m^{-2})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: qsintmax         !! Maximum water on vegetation for interception 
                                                                           !! (kg m^{-2})
    REAL,DIMENSION (kjpindex,nvm,npco2), INTENT (in) :: assim_param !! referenced vcmax, nue, leaf N 
                                                                           !! for photosynthesis
    REAL,DIMENSION (kjpindex,2),   INTENT (in)  :: lalo               !! Geographical coordinates
    INTEGER,DIMENSION (kjpindex,NbNeighb),INTENT (in):: neighbours    !! Vector of neighbours for each 
                                                                             !! grid point (1=N, 2=E, 3=S, 4=W)
    REAL,DIMENSION (kjpindex,2), INTENT(in)     :: resolution         !! The size in km of each grid-box in X and Y
    REAL,DIMENSION (kjpindex), INTENT (in)      :: ptnlev1            !! 1st level of soil temperature (Kelvin)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: precip_rain        !! Rain precipitation expressed in mm/tstep
    REAL,DIMENSION (kjpindex,nvm,nleafages), INTENT (in)  :: frac_age !! Age efficiency for isoprene emissions (from STOMATE)
    REAL,DIMENSION (kjpindex), INTENT(in)       :: tot_bare_soil      !! Total evaporating bare soil fraction 
    REAL,DIMENSION (kjpindex), INTENT(in)       :: frac_snow_veg      !! Snow cover fraction on vegeted area
    REAL,DIMENSION (kjpindex,nnobio), INTENT(in):: frac_snow_nobio    !! Snow cover fraction on non-vegeted area

    !! 0.2 Output variables

    REAL,DIMENSION (kjpindex), INTENT (out)     :: vbeta            !! Total beta coefficient (-)
    REAL,DIMENSION (kjpindex), INTENT (out)     :: vbeta1           !! Beta for sublimation (-)
    REAL,DIMENSION (kjpindex), INTENT (out)     :: vbeta4           !! Beta for bare soil evaporation (-)
    REAL,DIMENSION (kjpindex), INTENT (out)     :: vbeta5           !! Beta for floodplains
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: gsmean           !! Mean stomatal conductance to CO2 (mol m-2 s-1) 
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: vbeta2           !! Beta for interception loss (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: vbeta3           !! Beta for transpiration (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: vbeta3pot        !! Beta for potential transpiration
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: rveget           !! Stomatal resistance for the whole canopy (s m^{-1})
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: rstruct          !! Structural resistance for the vegetation
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: cimean           !! Mean leaf Ci (ppm)  
    REAL,DIMENSION (kjpindex,nvm,2), INTENT(out)  :: gpp            !! Assimilation ((gC m^{-2} s^{-1}), total area)
                                                                           !! third dimension: actual GPP=1 ; potential GPP=2

    !! 0.3 Modified variables
 
    REAL,DIMENSION (kjpindex, nvm), INTENT (inout) :: humrel        !! Soil moisture stress (within range 0 to 1)
    REAL,DIMENSION (kjpindex), INTENT (inout)      :: q_cdrag       !! Surface drag coefficient  (-)

    !! 0.4 Local variables

    REAL,DIMENSION (kjpindex)         :: VPD              !! Vapor Pressure Deficit (kPa)
    REAL,DIMENSION (kjpindex,nvm)     :: vbeta23            !! Beta for fraction of wetted foliage that will
                                                                   !! transpire once intercepted water has evaporated (-)
    REAL,DIMENSION (kjpindex)         :: raero              !! Aerodynamic resistance (s m^{-1})
    INTEGER                           :: ilai
    CHARACTER(LEN=4)                         :: laistring
    CHARACTER(LEN=80)                        :: var_name           !! To store variables names for I/O
    REAL,DIMENSION(kjpindex)          :: qsatt              !! Surface saturated humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex,nvm)     :: cim                !! Intercellular CO2 over nlai 
...
!DSGpot: we call the routine twice 
      !! calling twice works as we do not have an INTENT(INOUT) variables !!
      ! First, we diagnose the potential GPP for the maximum leaf nutrient concentration (given by cn_leaf_min and np_leaf_min):
      CALL diffuco_trans_co2 (kjpindex, swdown, pb, qsurf, q2m, t2m, temp_growth, rau, u, v, q_cdrag, humrel, &
                              assim_param, ccanopy, &
                              veget, veget_max, lai, qsintveg, qsintmax, vbeta3, vbeta3pot, &
                              rveget, rstruct, cimean, gsmean, gpp(:,:,2), vbeta23, VPD, hist_id, indexveg, & 
                              indexlai, index, kjit, cim, 1) ! DSGpot: the "1" here triggers pot GPP calculation 

      ! Second, the usual way to get actual GPP (and all the other stuff) for the actual leaf nutrient concentration:
      CALL diffuco_trans_co2 (kjpindex, swdown, pb, qsurf, q2m, t2m, temp_growth, rau, u, v, q_cdrag, humrel, &
                              assim_param, ccanopy, &
                              veget, veget_max, lai, qsintveg, qsintmax, vbeta3, vbeta3pot, &
                              rveget, rstruct, cimean, gsmean, gpp(:,:,1), vbeta23, VPD, hist_id, indexveg, &
                              indexlai, index, kjit, cim)


!DSG: extra output for AMAZON FACE MEI:
       CALL histwrite_p(hist_id, 'VPD', kjit, VPD, kjpindex, index)
!DSG
    ...
    CALL diffuco_bare (kjpindex, u, v, q_cdrag, rsol, evap_bare_lim, humrel, &
         veget, veget_max, tot_bare_soil, vbeta2, vbeta3, vbeta4)
  ...
  !! 6. Combine the alpha and beta coefficients
    ...
    CALL diffuco_comb (kjpindex, humrel, rau, u, v, q_cdrag, pb, qair, temp_sol, temp_air, snow, &
       & veget, lai, tot_bare_soil, vbeta1, vbeta2, vbeta3, vbeta4, vbeta, qsintmax)
...
!!DSG: added
!      CALL histwrite_p(hist_id, 'PSurf', kjit, pb, kjpindex, index)
!      CALL histwrite_p(hist_id, 'CCO2', kjit, ccanopy, kjpindex, index) ! equalsatm CO2
!DSG: added
!DSG: AMAFACE
...
!_ ==============================================================================================================================
  SUBROUTINE diffuco_finalize (kjit, kjpindex, rest_id, rstruct )
    ...
    INTEGER, INTENT(in)                         :: kjpindex         !! Domain size (-)
    INTEGER,INTENT (in)                         :: rest_id          !! _Restart_ file identifier (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: rstruct          !! Structural resistance for the vegetation
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER, INTENT(in)                          :: kjpindex, kjit   !! Domain size
    REAL,DIMENSION (kjpindex), INTENT (in)       :: u                !! Eastward Lowest level wind speed (m s^{-1}) 
    REAL,DIMENSION (kjpindex), INTENT (in)       :: v                !! Northward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)       :: zlev             !! Height of first atmospheric layer (m)
    REAL,DIMENSION (kjpindex), INTENT (in)       :: z0h               !! Surface roughness Length for heat (m)
    REAL,DIMENSION (kjpindex), INTENT (in)       :: z0m               !! Surface roughness Length for momentum (m)
    REAL,DIMENSION (kjpindex), INTENT (in)       :: roughheight      !! Effective roughness height (m)
    REAL,DIMENSION (kjpindex), INTENT (in)       :: temp_sol         !! Ground temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)       :: temp_air         !! Lowest level temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)       :: qsurf            !! near surface specific air humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)       :: qair             !! Lowest level specific air humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)       :: snow             !! Snow mass (kg)

    !! 0.2 Output variables
    
    REAL,DIMENSION (kjpindex), INTENT (out)      :: q_cdrag          !! Surface drag coefficient  (-)
    ...
    INTEGER                                      :: ji, jv
    REAL                                         :: speed, zg, zdphi, ztvd, ztvs, zdu2
    REAL                                         :: zri, cd_neut, zscf, cd_tmp
!_ ================================================================================================================================
...
SUBROUTINE diffuco_snow (kjpindex, qair, qsatt, rau, u, v,q_cdrag, &
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER, INTENT(in)                           :: kjpindex       !! Domain size (-)
    REAL,DIMENSION (kjpindex), INTENT (in)        :: qair           !! Lowest level specific air humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: qsatt          !! Surface saturated humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: rau            !! Air density (kg m^{-3})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: u              !! Eastward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: v              !! Northward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: q_cdrag        !! Surface drag coefficient  (-)
    REAL,DIMENSION (kjpindex), INTENT (in)        :: snow           !! Snow mass (kg m^{-2})
    REAL,DIMENSION (kjpindex,nnobio), INTENT (in) :: frac_nobio     !! Fraction of ice, lakes, cities etc. (-)
    REAL,DIMENSION (kjpindex,nnobio), INTENT (in) :: snow_nobio     !! Snow on ice, lakes, cities etc. (-)
    REAL,DIMENSION (kjpindex), INTENT (in)        :: totfrac_nobio  !! Total fraction of ice, lakes, cities etc. (-)
    REAL,DIMENSION (kjpindex), INTENT(in)         :: frac_snow_veg  !! Snow cover fraction on vegeted area
    REAL,DIMENSION (kjpindex,nnobio), INTENT(in)  :: frac_snow_nobio!! Snow cover fraction on non-vegeted area
    
    !! 0.2 Output variables

    REAL,DIMENSION (kjpindex), INTENT (out)       :: vbeta1         !! Beta for sublimation (dimensionless ratio) 
    
    !! 0.3 Modified variables

    !! 0.4 Local variables

    REAL                                          :: subtest        !! Sublimation for test (kg m^{-2})
    REAL                                          :: zrapp          !! Modified factor (ratio)
    REAL                                          :: speed          !! Wind speed (m s^{-1})
    REAL                                          :: vbeta1_add     !! Beta for sublimation (ratio)
...
!_ ================================================================================================================================
  ...
  SUBROUTINE diffuco_flood (kjpindex, qair, qsatt, rau, u, v, q_cdrag, evapot, evapot_corr, &
    ...
    ! input scalar 
    INTEGER, INTENT(in)                               :: kjpindex   !! Domain size
    REAL,DIMENSION (kjpindex), INTENT (in)            :: qair       !! Lowest level specific humidity
    REAL,DIMENSION (kjpindex), INTENT (in)            :: qsatt      !! Surface saturated humidity
    REAL,DIMENSION (kjpindex), INTENT (in)            :: rau        !! Density
    REAL,DIMENSION (kjpindex), INTENT (in)            :: u          !! Lowest level wind speed 
    REAL,DIMENSION (kjpindex), INTENT (in)            :: v          !! Lowest level wind speed
    REAL,DIMENSION (kjpindex), INTENT (in)            :: q_cdrag    !! Surface drag coefficient  (-)
    REAL,DIMENSION (kjpindex), INTENT (in)            :: flood_res  !! water mass in flood reservoir
    REAL,DIMENSION (kjpindex), INTENT (in)            :: flood_frac !! fraction of floodplains
    REAL,DIMENSION (kjpindex), INTENT (in)            :: evapot     !! Potential evaporation
    REAL,DIMENSION (kjpindex), INTENT (in)            :: evapot_corr!! Potential evaporation2
    ! output fields
    REAL,DIMENSION (kjpindex), INTENT (out)           :: vbeta5     !! Beta for floodplains

    ! local declaration
    REAL                                              :: subtest, zrapp, speed
    INTEGER                                           :: ji, jv
...
!_ ================================================================================================================================
  ...
  !! 0 Variable and parameter declaration
    ...
    REAL,DIMENSION (kjpindex), INTENT (in)        :: v          !! Northward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: q_cdrag    !! Surface drag coefficient  (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: humrel     !! Soil moisture stress (within range 0 to 1)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: veget      !! vegetation fraction for each type (fraction)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: qsintveg   !! Water on vegetation due to interception (kg m^{-2})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: qsintmax   !! Maximum water on vegetation (kg m^{-2})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: rstruct    !! architectural resistance (s m^{-1})
    
    !! 0.2 Output variables
    
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)   :: vbeta2     !! Beta for interception loss (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)   :: vbeta23    !! Beta for fraction of wetted foliage that will 
                                                                       !! transpire (-)

    !! 0.4 Local variables

    INTEGER                                       :: ji, jv                               !! (-), (-)
    REAL                                          :: zqsvegrap, ziltest, zrapp, speed     !!
!_ ================================================================================================================================
...
!_ ================================================================================================================================
  ...
  SUBROUTINE diffuco_bare (kjpindex, u, v, q_cdrag, rsol, evap_bare_lim, humrel, &
    ...
    REAL,DIMENSION (kjpindex), INTENT (in)      :: v              !! Northward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: q_cdrag        !!  Surface drag coefficient  (-)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: rsol           !! resistance for bare soil evaporation  (s m^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: evap_bare_lim  !! limiting factor for bare soil evaporation when the 
                                                                         !! 11-layer hydrology is used (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: humrel         !! Soil moisture stress (within range 0 to 1)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: veget          !! Type of vegetation fraction (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: veget_max      !! Type of vegetation max fraction
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: vbeta2         !! Beta for Interception 
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: vbeta3         !! Beta for Transpiration 
    REAL, DIMENSION (kjpindex), INTENT(in)      :: tot_bare_soil  !! Total evaporating bare soil fraction 
    ...
    INTEGER                                     :: ji, jv
    REAL                                        :: speed          !! Surface wind speed (m s^{-1})
!_ ================================================================================================================================
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER, INTENT(in)                         :: kjpindex   !! Domain size (-)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: swnet      !! Short wave net flux at surface (W m^{-2})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: temp_air   !! Air temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: pb         !! Lowest level pressure (hPa)
    REAL,DIMENSION (kjpindex), INTENT (in)      :: qair       !! Lowest level specific air humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: rau        !! Air Density (kg m^{-3})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: u          !! Eastward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: v          !! Northward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)      :: q_cdrag    !! Surface drag coefficient  (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: humrel     !! Soil moisture stress (within range 0 to 1)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: veget      !! Type of vegetation (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: veget_max  !! Max. vegetation fraction (LAI->infty) (fraction)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: lai        !! Leaf area index (m^2 m^{-2})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: qsintveg   !! Water on vegetation due to interception (kg m^{-2})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: qsintmax   !! Maximum water on vegetation (kg m^{-2})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: rstruct    !! Structural resistance (s m^{-1})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)  :: vbeta23    !! Beta for wetted foliage fraction that will transpire (-)
    
    !! 0.2 Output variables
    
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: vbeta3     !! Beta for Transpiration (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: vbeta3pot  !! Beta for Potential Transpiration
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: rveget     !! Stomatal resistance of the whole canopy (s m^{-1})
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: cimean     !! STOMATE: mean intercellular ci (see enerbil) 
                                                                     !! (\mumol m^{-2} s^{-1}) 
    REAL,DIMENSION (kjpindex,nvm), INTENT (out) :: vbetaco2   !! STOMATE: Beta for CO2 (-)
    ...
    INTEGER                                     :: ji, jv
    REAL                                        :: speed
    REAL, DIMENSION(kjpindex)                   :: zdefconc, zqsvegrap
    REAL, DIMENSION(kjpindex)                   :: qsatt
    REAL,DIMENSION (kjpindex,nvm)               :: rveget_min           !! Minimal Surface resistance of vegetation
!_ ================================================================================================================================
...
SUBROUTINE diffuco_trans_co2 (kjpindex,swdown, pb, qsurf, q2m, t2m, temp_growth, rau, u, v, q_cdrag, humrel, &
                                assim_param, Ca, &
                                veget, veget_max, lai, qsintveg, qsintmax, vbeta3, vbeta3pot, rveget, rstruct, &
                                cimean, gsmean, gpp, vbeta23, VPD, hist_id, indexveg, indexlai, index, kjit, cim , GPP_unlimited)
    ...
    !! 0.1 Input variables
    !
    INTEGER, INTENT(in)                               :: kjpindex         !! Domain size (unitless)
    REAL,DIMENSION (kjpindex), INTENT (in)            :: swdown           !! Downwelling short wave flux 
                                                                                 !! @tex ($W m^{-2}$) @endtex 
    REAL,DIMENSION (kjpindex), INTENT (in)            :: pb               !! Lowest level pressure (hPa)
    REAL,DIMENSION (kjpindex), INTENT (in)            :: qsurf             !! Near surface specific humidity 
                                                                                 !! @tex ($kg kg^{-1}$) @endtex
! N. de Noblet - 2006/06 - addition of q2m and t2m
    REAL,DIMENSION (kjpindex), INTENT (in)            :: q2m              !! 2m specific humidity 
                                                                                 !! @tex ($kg kg^{-1}$) @endtex
...
! N. de Noblet - 2006/06 - addition of q2m and t2m
    REAL,DIMENSION (kjpindex), INTENT (in)            :: temp_growth      !! Growth temperature (°C) - Is equal to t2m_month
    REAL,DIMENSION (kjpindex), INTENT (in)            :: rau              !! air density @tex ($kg m^{-3}$) @endtex
    REAL,DIMENSION (kjpindex), INTENT (in)            :: u                !! Lowest level wind speed 
                                                                                 !! @tex ($m s^{-1}$) @endtex
    REAL,DIMENSION (kjpindex), INTENT (in)            :: v                !! Lowest level wind speed 
                                                                                 !! @tex ($m s^{-1}$) @endtex
    ...
    REAL,DIMENSION (kjpindex,nvm,npco2), INTENT (in)  :: assim_param      !! referenced vcmax, nue, leaf N
                                                                                 !! for photosynthesis 
    REAL,DIMENSION (kjpindex), INTENT (in)            :: Ca               !! CO2 concentration inside the canopy
                                                                                 !! @tex ($\mu mol mol^{-1}$) @endtex
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)        :: humrel           !! Soil moisture stress (0-1,unitless) 
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)        :: veget            !! Coverage fraction of vegetation for each PFT 
                                                                                 !! depending on LAI (0-1, unitless) 
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)        :: veget_max        !! Maximum vegetation fraction of each PFT inside 
                                                                                 !! the grid box (0-1, unitless) 
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)        :: lai              !! Leaf area index @tex ($m^2 m^{-2}$) @endtex
                                                                                 !! @tex ($m s^{-1}$) @endtex
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)        :: qsintveg         !! Water on vegetation due to interception 
                                                                                 !! @tex ($kg m^{-2}$) @endte
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)        :: qsintmax         !! Maximum water on vegetation
                                                                                 !! @tex ($kg m^{-2}$) @endtex
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)        :: vbeta23          !! Beta for fraction of wetted foliage that will 
                                                                                 !! transpire (unitless) 
    INTEGER,INTENT (in)                               :: hist_id          !! _History_ file identifier (-)    
    INTEGER,DIMENSION (kjpindex*nvm), INTENT (in) :: indexveg       !! Indeces of the points on the 3D map (-)   
    INTEGER,DIMENSION (kjpindex*(nlai+1)), INTENT (in) :: indexlai  !! Indeces of the points on the 3D map
INTEGER,DIMENSION (kjpindex), INTENT (in)   :: index            !! Indeces of the points on the map (-)
    ...
    !! 0.2 Output variables
    !
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: vbeta3           !! Beta for Transpiration (unitless)
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: vbeta3pot        !! Beta for Potential Transpiration
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: rveget           !! stomatal resistance of vegetation 
                                                                                 !! @tex ($s m^{-1}$) @endtex
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: rstruct          !! structural resistance @tex ($s m^{-1}$) @endtex
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: cimean           !! mean intercellular CO2 concentration 
                                                                                 !! @tex ($\mu mol mol^{-1}$) @endtex
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: gsmean           !! mean stomatal conductance to CO2 (umol m-2 s-1)
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: gpp
    REAL,DIMENSION (kjpindex)    , INTENT (out)       :: VPD              !! Vapor Pressure Deficit (kPa)
 !! Assimilation ((gC m^{-2} s^{-1}), total area)
    REAL,DIMENSION (kjpindex,nvm), INTENT (out)       :: cim              !! Intercellular CO2 over nlai 


    INTEGER,OPTIONAL, INTENT (in)       :: GPP_unlimited                  !! If this variable is passed and has value of 1, we compute the nutrient unlimited (=potential) GPP
    ...
    !! 0.4 Local variables
    !
    REAL,DIMENSION (kjpindex,nvm)  :: vcmax                               !! maximum rate of carboxylation 
                                                                                 !! @tex ($\mu mol CO2 m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION (kjpindex,nvm)  :: jmax                               !! maximum rate of electron transport
                                                                                 !! @tex ($\mu mol CO2 m^{-2} s^{-1}$) @endtex
!    REAL,DIMENSION (kjpindex)      :: vcmax_walker                        !! maximum rate of carboxylation derived from the Walker relationship 
!                                                                                 !! @tex ($\mu mol CO2 m^{-2} s^{-1}$) @endtex 
    REAL,DIMENSION (kjpindex,nvm)  :: nue                                 !! Nitrogen use Efficiency with impact of leaf age (umol CO2 (gN)-1 s-1) 
    REAL,DIMENSION (kjpindex,nvm)  :: ntot                                !! Total canopy (eg leaf) N (gN m-2[ground])  
    REAL                           :: N_profile                           !! N-content reduction factor for a specific canopy depth (unitless) 
    REAL,DIMENSION (kjpindex)      :: leafN_top                           !! Leaf N content - top of canopy (gN m-2[leaf]) 
    INTEGER                        :: ji, jv, jl, limit_photo             !! indices (unitless)
    REAL, DIMENSION(kjpindex,nlai+1) :: info_limitphoto
    REAL, DIMENSION(kjpindex,nvm,nlai)  :: leaf_ci                        !! intercellular CO2 concentration (ppm)
    REAL, DIMENSION(kjpindex)      :: leaf_ci_lowest                      !! intercellular CO2 concentration at the lowest 
                                                                                 !! LAI level
                                                                                 !! @tex ($\mu mol mol^{-1}$) @endtex
    INTEGER, DIMENSION(kjpindex)   :: ilai                                !! counter for loops on LAI levels (unitless)
    REAL, DIMENSION(kjpindex)      :: zqsvegrap                           !! relative water quantity in the water 
                                                                                 !! interception reservoir (0-1,unitless) 
    REAL                           :: speed                               !! wind speed @tex ($m s^{-1}$) @endtex
    ! Assimilation
    ...
    INTEGER                        :: nic,inic,icinic                     !! counter/indices (unitless)
    INTEGER, DIMENSION(kjpindex)   :: index_calc                          !! index (unitless)
    INTEGER                        :: nia,inia,nina,inina,iainia          !! counter/indices (unitless)
    INTEGER, DIMENSION(kjpindex)   :: index_assi,index_non_assi           !! indices (unitless)
    REAL, DIMENSION(kjpindex, nlai+1)      :: vc2                                 !! rate of carboxylation (at a specific LAI level) 
                                                                                 !! @tex ($\mu mol CO2 m^{-2} s^{-1}$) @endtex 
    REAL, DIMENSION(kjpindex, nlai+1)      :: vj2                                 !! rate of Rubisco regeneration (at a specific LAI 
                                                                                 !! level) @tex ($\mu mol e- m^{-2} s^{-1}$) @endtex 
    REAL, DIMENSION(kjpindex, nlai+1)      :: assimi                              !! assimilation (at a specific LAI level) 
                                                                                 !! @tex ($\mu mol m^{-2} s^{-1}$) @endtex
                                                                                 !! (temporary variables)
    REAL, DIMENSION(kjpindex)             :: gstop                        !! stomatal conductance to H2O at topmost level 
                                                                                 !! @tex ($m s^{-1}$) @endtex
    REAL, DIMENSION(kjpindex,nlai+1)      :: gs                                  !! stomatal conductance to CO2 
                                                                                 !! @tex ($\mol m^{-2} s^{-1}$) @endtex
    REAL, DIMENSION(kjpindex,nlai)      :: templeafci  


    REAL, DIMENSION(kjpindex)      :: gamma_star                          !! CO2 compensation point (ppm)
                                                                                 !! @tex ($\mu mol mol^{-1}$) @endtex

    REAL, DIMENSION(kjpindex)      :: air_relhum                          !! air relative humidity at 2m 
                                                                                 !! @tex ($kg kg^{-1}$) @endtex
    REAL, DIMENSION(kjpindex)      :: water_lim                           !! water limitation factor (0-1,unitless)

    REAL, DIMENSION(kjpindex)      :: gstot                               !! total stomatal conductance to H2O
                                                                                 !! Final unit is
                                                                                 !! @tex ($m s^{-1}$) @endtex
    REAL, DIMENSION(kjpindex)      :: assimtot                            !! total assimilation 
                                                                                 !! @tex ($\mu mol CO2 m^{-2} s^{-1}$) @endtex
    REAL, DIMENSION(kjpindex)      :: Rdtot                               !! Total Day respiration (respiratory CO2 release other than by photorespiration) (mumol CO2 m−2 s−1) 
    REAL, DIMENSION(kjpindex)      :: leaf_gs_top                         !! leaf stomatal conductance to H2O at topmost level 
                                                                                 !! @tex ($\mol H2O m^{-2} s^{-1}$) @endtex
    REAL, DIMENSION(nlai+1)        :: laitab                              !! tabulated LAI steps @tex ($m^2 m^{-2}$) @endtex
    REAL, DIMENSION(kjpindex)      :: qsatt                               !! surface saturated humidity at 2m (??) 
                                                                                 !! @tex ($g g^{-1}$) @endtex
    REAL, DIMENSION(nvm,nlai)      :: light                               !! fraction of light that gets through upper LAI    
                                                                                 !! levels (0-1,unitless)
    REAL, DIMENSION(kjpindex)      :: T_Vcmax                             !! Temperature dependance of Vcmax (unitless)
    REAL, DIMENSION(kjpindex)      :: S_Vcmax_acclim_temp                 !! Entropy term for Vcmax 
                                                                                 !! accounting for acclimation to temperature (J K-1 mol-1)
    REAL, DIMENSION(kjpindex)      :: T_Jmax                              !! Temperature dependance of Jmax
    REAL, DIMENSION(kjpindex)      :: S_Jmax_acclim_temp                  !! Entropy term for Jmax 
                                                                                 !! accounting for acclimation toxs temperature (J K-1 mol-1)
    REAL, DIMENSION(kjpindex)      :: T_gm                                !! Temperature dependance of gmw
    REAL, DIMENSION(kjpindex)      :: T_Rd                                !! Temperature dependance of Rd (unitless)
    REAL, DIMENSION(kjpindex)      :: T_Kmc                               !! Temperature dependance of KmC (unitless)
    REAL, DIMENSION(kjpindex)      :: T_KmO                               !! Temperature dependance of KmO (unitless)
    REAL, DIMENSION(kjpindex)      :: T_Sco                               !! Temperature dependance of Sco
    REAL, DIMENSION(kjpindex)      :: T_gamma_star                        !! Temperature dependance of gamma_star (unitless)    
    REAL, DIMENSION(kjpindex)      :: vc                                  !! Maximum rate of Rubisco activity-limited carboxylation (mumol CO2 m−2 s−1)
    REAL, DIMENSION(kjpindex)      :: vj                                  !! Maximum rate of e- transport under saturated light (mumol CO2 m−2 s−1)
    REAL, DIMENSION(kjpindex)      :: gm                                  !! Mesophyll diffusion conductance (molCO2 m−2 s−1 bar−1)
    REAL, DIMENSION(kjpindex)      :: g0var
    REAL, DIMENSION(kjpindex,nlai+1)      :: Rd                                  !! Day respiration (respiratory CO2 release other than by photorespiration) (mumol CO2 m−2 s−1)
    REAL, DIMENSION(kjpindex)      :: Kmc                                 !! Michaelis–Menten constant of Rubisco for CO2 (mubar)
    REAL, DIMENSION(kjpindex)      :: KmO                                 !! Michaelis–Menten constant of Rubisco for O2 (mubar)
    REAL, DIMENSION(kjpindex)      :: Sco                                 !! Relative CO2 /O2 specificity factor for Rubisco (bar bar-1)
    REAL, DIMENSION(kjpindex)      :: gb_co2                              !! Boundary-layer conductance (molCO2 m−2 s−1 bar−1)
    REAL, DIMENSION(kjpindex)      :: gb_h2o                              !! Boundary-layer conductance (molH2O m−2 s−1 bar−1)
    REAL, DIMENSION(kjpindex)      :: fvpd                                !! Factor for describing the effect of leaf-to-air vapour difference on gs (-)
    REAL, DIMENSION(kjpindex)      :: low_gamma_star                      !! Half of the reciprocal of Sc/o (bar bar-1)
    REAL                           :: fcyc                                !! Fraction of electrons at PSI that follow cyclic transport around PSI (-)
    REAL                           :: z                                   !! A lumped parameter (see Yin et al. 2009) ( mol mol-1)                          
    REAL                           :: Rm                                  !! Day respiration in the mesophyll (umol CO2 m−2 s−1)
    REAL                           :: Cs_star                             !! Cs -based CO2 compensation point in the absence of Rd (ubar)
    REAL, DIMENSION(kjpindex)      :: Iabs                                !! Photon flux density absorbed by leaf photosynthetic pigments (umol photon m−2 s−1)
!DSG: I need this one for use consisten with variable vcmax
!    REAL, DIMENSION(kjpindex)      :: Jmax                                !! Maximum value of J under saturated light (umol e− m−2 s−1)
    REAL, DIMENSION(kjpindex, nlai+1)      :: JJ                                  !! Rate of e− transport (umol e− m−2 s−1)
    REAL                           :: J2                                  !! Rate of all e− transport through PSII (umol e− m−2 s−1)
    REAL                           :: VpJ2                                !! e− transport-limited PEP carboxylation rate (umol CO2 m−2 s−1)
    REAL                           :: A_1, A_3                            !! Lowest First and third roots of the analytical solution for a general cubic equation (see Appendix A of Yin et al. 2009) (umol CO2 m−2 s−1)
    REAL                           :: A_1_tmp, A_3_tmp                            !! Temporary First and third roots of the analytical solution for a general cubic equation (see Appendix A of Yin et al. 2009) (umol CO2 m−2 s−1)
    REAL                           :: Obs                                 !! Bundle-sheath oxygen partial pressure (ubar)
    REAL, DIMENSION(kjpindex, nlai+1)      :: Cc                                  !! Chloroplast CO2 partial pressure (ubar)
    REAL                           :: ci_star                             !! Ci -based CO2 compensation point in the absence of Rd (ubar)        
    REAL                           :: a,b,c,d,m,f,j,g,h,i,l,p,q,r         !! Variables used for solving the cubic equation (see Yin et al. (2009))
    REAL                           :: QQ,UU,PSI,x1,x2,x3                      !! Variables used for solving the cubic equation (see Yin et al. (2009))
                                        
    REAL                           :: cresist                             !! coefficient for resistances (??)
    REAL, DIMENSION(kjpindex)      :: laisum                              !! when calculating cim over nlai
    REAL, DIMENSION(kjpindex)      :: PAR                                 !! photosynthetic active radiation


!DSG 
    LOGICAL                               :: kattge                              !! switch to go from NUE*leafN and Tacclim of Vcmax and Jmax (Kattge et al.,!2007,2009) 
                                                                                 !! to Vcmax (Jmax) as a function of leafN and leafP (ellsworht in prep)

    ! Kattge is from now on not supported anymore
...
!    ENDIF
    ...
    nue (:,:)  = assim_param(:,:,inue) 
    ntot(:,:)  = assim_param(:,:,ileafn) 

! DSG: these two will be overwritten in case we use NUE*leafN=Vcmax (plus Tacclimation)
    IF (PRESENT(GPP_unlimited).AND.GPP_unlimited.EQ.1) THEN
       vcmax(:,:) = assim_param(:,:,ivcmax_pot)
       jmax(:,:)  = assim_param(:,:,ijmax_pot)
    ELSE
       vcmax(:,:) = assim_param(:,:,ivcmax)
       jmax(:,:)  = assim_param(:,:,ijmax)
    ENDIF
! DSG
...
! N. de Noblet - 2006/06 
    ...
    DO jv = 2,nvm
      ...
      ! Two ways to derive Vcmax from leaf nutrients:
      ! 1. Kattge style: Vcmax is NUE * leaf N; Vcmax and the ratio of Vcmax to
      !                  Jmax acclimate to temperature in a second step
      ! 2. Ellsworth style: Vcmax and Jmax are a function of leaf N and leaf P
      !                     both are calculated in stomate_vmax.f90
      IF (Kattge) THEN
         !DSGdebug: avoid division by zero in case LAI=0
         WHERE((lai(:,jv).GT. 0.01) .AND. (ntot(:,jv).GT. zero)) 
            leafN_top(:) = ntot(:,jv) * ext_coeff_N(jv) / ( 1. - exp(-ext_coeff_N(jv) * lai(:,jv)) ) 
         ELSEWHERE
            ! no N no vcmax
            leafN_top(:) = zero
         ENDWHERE
         ...
         WHERE(leafN_top(:) .GT. zero)
            !Kattge et al. 2009:
            vcmax(:,jv)     = nue(:,jv)*leafN_top(:)
         ELSEWHERE
             vcmax(:,jv) = zero
         ENDWHERE

      ELSE ! Ellsworth: here we do only a canpoy scaling of Vcmax and Jmax

         WHERE((lai(:,jv).GT. 0.01) )
            vcmax(:,jv) = vcmax(:,jv) * ext_coeff_N(jv) / ( 1. - exp(-ext_coeff_N(jv) * lai(:,jv)) ) 
            jmax(:,jv)  = jmax(:,jv)  * ext_coeff_N(jv) / ( 1. - exp(-ext_coeff_N(jv) * lai(:,jv)) ) 
         ELSEWHERE
            ! no LAI no Vcmax
            vcmax(:,jv) = zero
            jmax(:,jv)  = zero
      ...
      DO ji=1,kjpindex
         ...
         IF ( ( lai(ji,jv) .GT. 0.01 ) .AND. &
             !DSG: no N no photosynthesis
              ( vcmax(ji,jv) .GT. zero) .AND. &
             !DSG: no N no photosynthesis
...
!DSG: kattge Tacclim END ? 
      ...
      DO jl = 1, nlai
         ...
         ! @}           
         !
         N_profile = exp( -ext_coeff_N(jv)*laitab(jl) )
         !

         vc2(:,jl) = vc(:) * N_profile * MAX(1-stress_vcmax(jv), water_lim(:))
         vj2(:,jl) = vj(:) * N_profile * MAX(1-stress_vcmax(jv), water_lim(:))

         ! see Comment in legend of Fig. 6 of Yin et al. (2009)
         ! Rd25 is assumed to equal 0.01 Vcmax25 
         Rd(:,jl) = vcmax(:,jv) * N_profile * 0.01 * T_Rd(:) * MAX(1-stress_vcmax(jv), water_lim(:))

         Iabs(:)=swdown(:)*W_to_mol*RG_to_PAR*ext_coeff(jv)*light(jv,jl)
!DSG: free the variable jmax to be used consistent with variable vcmax in
...
!              / ( 2 * theta(jv))
         ...
         JJ(:,jl) = ( alpha_LL(jv) * Iabs(:) + vj2(:,jl) - sqrt((alpha_LL(jv) * Iabs(:) + vj2(:,jl) )**2. &
              - 4 * theta(jv) * vj2(:,jl) * alpha_LL(jv) * Iabs(:)) ) &
              / ( 2 * theta(jv))
!DSG
      ...
      IF(jv==testpft.AND.(.NOT.PRESENT(GPP_unlimited)))THEN
         ...
         CALL histwrite_p(hist_id, 'leafci', kjit, templeafci, kjpindex*(nlai), indexlai)
      ...
      IF (nia .GT. 0) THEN
         ...
         DO inia=1,nia
            ...
            !     rveget(iainia,jv)
               ...
               ! rstruct corresponds to the bulk boundary layer resistance
               ! scaled at the canopy level
               ! This is taken from Ershadi et al. (2015, Appendix C)
               rstruct(iainia,jv) = (1/gb_ref)/(2.*lai(iainia,jv))
...
!_ ================================================================================================================================
  ...
  SUBROUTINE diffuco_comb (kjpindex, humrel, rau, u, v, q_cdrag, pb, qair, temp_sol, temp_air, &
       & snow, veget, lai, tot_bare_soil, vbeta1, vbeta2, vbeta3 , vbeta4, vbeta, qsintmax)    
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL,DIMENSION (kjpindex), INTENT (in)        :: rau        !! Air Density (kg m^{-3})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: u          !! Eastward Lowest level wind speed (m s^{-1}) 
    REAL,DIMENSION (kjpindex), INTENT (in)        :: v          !! Nortward Lowest level wind speed (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: q_cdrag    !! Surface drag coefficient  (-)
    REAL,DIMENSION (kjpindex), INTENT (in)        :: pb         !! Lowest level pressure (hPa)
    REAL,DIMENSION (kjpindex), INTENT (in)        :: qair       !! Lowest level specific air humidity (kg kg^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)        :: temp_sol   !! Skin temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)        :: temp_air   !! Lower air temperature (K)
    REAL,DIMENSION (kjpindex), INTENT (in)        :: snow       !! Snow mass (kg)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: veget      !! Fraction of vegetation type (fraction)
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: lai        !! Leaf area index (m^2 m^{-2})
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)    :: qsintmax   !! Maximum water on vegetation (kg m^{-2})
    REAL, DIMENSION (kjpindex), INTENT(in)        :: tot_bare_soil!! Total evaporating bare soil fraction 
    ...
    REAL,DIMENSION (kjpindex), INTENT (inout)     :: vbeta1     !! Beta for sublimation (-)
    REAL,DIMENSION (kjpindex), INTENT (inout)     :: vbeta4     !! Beta for Bare soil evaporation (-) 
    REAL,DIMENSION (kjpindex,nvm), INTENT (inout) :: humrel     !! Soil moisture stress (within range 0 to 1)
    REAL,DIMENSION (kjpindex,nvm), INTENT (inout) :: vbeta2     !! Beta for interception loss (-)
    REAL,DIMENSION (kjpindex,nvm), INTENT (inout) :: vbeta3     !! Beta for Transpiration (-)
    
    !! 0.4 Local variables
    
    INTEGER                                       :: ji, jv
    REAL                                          :: zevtest, zsoil_moist, zrapp
    REAL, DIMENSION(kjpindex)                     :: qsatt
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration
    ...
    INTEGER, INTENT(in)                     :: kjpindex     !! Domain size (-)
    REAL,DIMENSION (kjpindex), INTENT (in)  :: u            !! Eastward Lowest level wind velocity (m s^{-1}) 
    REAL,DIMENSION (kjpindex), INTENT (in)  :: v            !! Northward Lowest level wind velocity (m s^{-1})
    REAL,DIMENSION (kjpindex), INTENT (in)  :: q_cdrag      !! Surface drag coefficient  (-)
    
    !! 0.2 Output variables 
    
    REAL,DIMENSION (kjpindex), INTENT (out) :: raero        !! Aerodynamic resistance (s m^{-1})
...
!_ ================================================================================================================================
  ...
  FUNCTION Arrhenius (kjpindex,temp,ref_temp,energy_act) RESULT ( val_arrhenius )
    ...
    INTEGER,INTENT(in)                     :: kjpindex          !! Domain size (-)
    REAL,DIMENSION(kjpindex),INTENT(in)    :: temp              !! Temperature (K)
    REAL, INTENT(in)                       :: ref_temp          !! Temperature of reference (K)
    REAL,INTENT(in)                        :: energy_act        !! Activation Energy (J mol-1)
    
    !! 0.2 Result

    REAL, DIMENSION(kjpindex)              :: val_arrhenius     !! Temperature dependance based on
  ...
  FUNCTION Arrhenius_modified_1d (kjpindex,temp,ref_temp,energy_act,energy_deact,entropy) RESULT ( val_arrhenius )
    ...
    REAL, INTENT(in)                       :: ref_temp          !! Temperature of reference (K)
    REAL,INTENT(in)                        :: energy_act        !! Activation Energy (J mol-1)
    REAL,INTENT(in)                        :: energy_deact      !! Deactivation Energy (J mol-1)
    REAL,DIMENSION(kjpindex),INTENT(in)    :: entropy           !! Entropy term (J K-1 mol-1)
  ...
  FUNCTION Arrhenius_modified_0d (kjpindex,temp,ref_temp,energy_act,energy_deact,entropy) RESULT ( val_arrhenius )
    ...
    REAL,INTENT(in)                        :: energy_act        !! Activation Energy (J mol-1)
    REAL,INTENT(in)                        :: energy_deact      !! Deactivation Energy (J mol-1)
    REAL,INTENT(in)                        :: entropy           !! Entropy term (J K-1 mol-1)
