...
MODULE ioipslctrl
  ...
  !$OMP THREADPRIVATE(ok_histsync)
   REAL, SAVE               :: dw                      !! Frequency of history write (sec.)
!$OMP THREADPRIVATE(dw)
  INTEGER,PARAMETER         :: max_hist_level = 11     !! 
...
!_ ================================================================================================================================
  SUBROUTINE ioipslctrl_history(iim, jjm, lon, lat, kindex, kjpindex, istp_old, date0, dt, hist_id, hist2_id, &
    ...
    INTEGER, INTENT(in)                  :: iim, jjm  !! Size in x and y of the data to be handeled
    REAL,DIMENSION (iim,jjm), INTENT(in) :: lon, lat  !! Longitude and latitude of the data points
    INTEGER,INTENT (in)                            :: kjpindex
    INTEGER,DIMENSION (kjpindex), INTENT (in)      :: kindex
    
    INTEGER, INTENT(in)                  :: istp_old  !! Time step counter
    REAL, INTENT(in)                     :: date0     !! Julian day at which istp=0
    REAL, INTENT(in)                     :: dt        !! Time step of the counter in seconds

    INTEGER, INTENT(out)                 :: hist_id   !! History file identification for SECHIBA
    INTEGER, INTENT(out)                 :: hist2_id  !! History file 2 identification for SECHIBA
    !! History file identification for STOMATE and IPCC
    INTEGER, INTENT(out)                 :: hist_id_stom, hist_id_stom_IPCC 
    !
    !  LOCAL
    !
    ...
    REAL       :: dw2                          !! frequency of history write (sec.)
    ...
    INTEGER     :: hist_level, hist2_level     !! history output level (default is 10 => maximum output)
    ...
    INTEGER     :: i, jst
    ! SECHIBA AXIS
    INTEGER     :: hori_id                      !! ID of the default horizontal longitude and latitude map.
    INTEGER     :: vegax_id, laiax_id, solax_id, soltax_id, nobioax_id !! ID's for two vertical coordinates
    INTEGER     :: soildiagax_id                !! ID for diagnostic soil levels
    INTEGER     :: solayax_id                   !! ID for the vertical axis of the CWRR hydrology 
    INTEGER     :: hori_id2                      !! ID of the default horizontal longitude and latitude map.
    INTEGER     :: vegax_id2, laiax_id2, solax_id2, soltax_id2, nobioax_id2, albax_id2 !! ID's for two vertical coordinates
    INTEGER     :: solayax_id2                   !! ID for the vertical axis of the CWRR hydrology 
    INTEGER     :: snowax_id                     !! ID for snow level axis

    ! STOMATE AXIS
    INTEGER     :: hist_PFTaxis_id
! deforestation
    ...
    ! STOMATE IPCC AXIS
    INTEGER     :: hist_IPCC_PFTaxis_id
    !
    INTEGER                         :: ier
    REAL, ALLOCATABLE, DIMENSION(:) :: lon_rect, lat_rect
    !
    REAL,DIMENSION(nvm)   :: veg
    REAL,DIMENSION(nlai+1):: indlai
    REAL,DIMENSION(ngrnd) :: sol
    REAL,DIMENSION(nstm)  :: soltyp
    REAL,DIMENSION(nnobio):: nobiotyp
    REAL,DIMENSION(2)     :: albtyp
    REAL,DIMENSION(nslm)  :: solay
    REAL,DIMENSION(nsnow) :: snowlev           !! Layers for snow axis
    !
    CHARACTER(LEN=80)           :: var_name           !! To store variables names
    !
    ! STOMATE history file
    REAL                  :: hist_days_stom     !!- GK time step in days for this history file
    REAL                  :: hist_dt_stom       !!- GK time step in seconds for this history file
    REAL                  :: dt_stomate_loc     !!  for test : time step of slow processes and STOMATE
    REAL,DIMENSION(nvm)   :: hist_PFTaxis       !!- GK An axis we need for the history files
!
    REAL,DIMENSION(10)  :: hist_pool_10axis     !! Deforestation axis
    REAL,DIMENSION(100)  :: hist_pool_100axis     !! Deforestation axis
    REAL,DIMENSION(11)  :: hist_pool_11axis     !! Deforestation axis
    REAL,DIMENSION(101)  :: hist_pool_101axis     !! Deforestation axis
    !
    ! IPCC history file
    REAL                  :: hist_days_stom_ipcc     !!- GK time step in days for this history file
    REAL                  :: hist_dt_stom_ipcc       !!- GK time step in seconds for this history file
!
    !
    !=====================================================================
...
!end gmjc
          ...
          IF ( ok_co2 ) THEN
             ...
             CALL histdef(hist_id, 'gpp_pot', 'Nutrient unlimited net assimilation ', 'gC/m^2/s', &
                  & iim,jjm, hori_id, nvm, 1, nvm, vegax_id, 32, fluxop_scinsec(4), dt,dw)
          ENDIF
          IF ( ok_stomate ) THEN
       ...
       ELSE 
          ...
          IF ( ok_co2 ) THEN
            ...
            CALL histdef(hist_id, 'GPP', 'Net assimilation of carbon by the vegetation', 'gC/m^2/s', &
                  & iim,jjm, hori_id, nvm, 1, nvm, vegax_id, 32, fluxop_scinsec(1), dt,dw)
            CALL histdef(hist_id, 'GPP_POT', 'Nutrient unlimited net assimilation of carbon', 'gC/m^2/s', &
                  & iim,jjm, hori_id, nvm, 1, nvm, vegax_id, 32, fluxop_scinsec(1), dt,dw)
            CALL histdef(hist_id, 'gsmean', 'mean stomatal conductance', 'mol/m2/s', &
                  & iim,jjm, hori_id, nvm, 1, nvm, vegax_id, 32, avescatter(8), dt,dw)
             CALL histdef(hist_id, 'cim', 'cim', 'ppm', &
...
!      CALL histdef(hist_id, 'Wind', 'Wind speed', 'm/s',  &
!           & iim,jjm, hori_id, 1,1,1, -99, 32, avescatter(8), dt,dw)
!      CALL histdef(hist_id, 'PSurf', 'Surface Pressure', 'Pa',  &
!           & iim,jjm, hori_id, 1,1,1, -99, 32, avescatter(8), dt,dw)
!      CALL histdef(hist_id, 'CCO2', 'Canopy/Air CO2', 'ppm', &
!           & iim,jjm, hori_id, 1,1,1, -99, 32, avescatter(8), dt,dw)
!!DSGadded: AMAFACE

!DSGadded
      IF ( ok_co2 ) THEN
             CALL histdef(hist_id, 'VPD', 'vapor pressure deficit', 'kPa',  &
                  & iim,jjm, hori_id, 1,1,1, -99, 32, avescatter(10), dt, dw)
      ENDIF
!DSGadded
       !-
       ...
       IF (is_omp_root) THEN
          IF ( .NOT. almaoutput ) THEN
            ...
            CALL histdef(hist2_id, 'alb_vis', 'Albedo visible', '1', &
             ...
             IF ( ok_co2 ) THEN
                ...
                CALL histdef(hist2_id, 'gpp_pot', 'Nutrient unlimited net assimilation of carbon', 'gC/m^2/s', &
                     & iim,jjm, hori_id2, nvm, 1, nvm, vegax_id2, 32, fluxop_scinsec2(6), dt,dw2)
             ENDIF
             IF ( ok_stomate ) THEN
             ...
             CALL histdef(hist_id, 'Wind', 'Wind speed', 'm/s',  &
                  & iim,jjm, hori_id, 1,1,1, -99, 32, avescatter2(9), dt, dw2)
         ...
         ELSE 
             ...
             IF ( ok_co2 ) THEN
                ...
                CALL histdef(hist2_id, 'GPP_POT', 'Nutrient unlimited net assimilation of carbon ', 'gC/m^2/s', &
                     & iim,jjm, hori_id2, nvm, 1, nvm, vegax_id2, 32, fluxop_scinsec(1), dt,dw2)
             ENDIF
             IF ( ok_stomate ) THEN
                CALL histdef(hist2_id, 'NEE', 'Net Ecosystem Exchange', 'gC/m^2/s', &
                     & iim,jjm, hori_id2, nvm, 1, nvm, vegax_id2, 32, fluxop_scinsec2(1), dt,dw2)
...
!_ ================================================================================================================================
  SUBROUTINE ioipslctrl_histstom( &
    ...
    !-
    !- File id
    INTEGER,INTENT(in) :: hist_id_stom
    !- number of PFTs
    ...
    REAL,INTENT(in)    :: hist_dt
    !- id horizontal grid
    INTEGER,INTENT(in) :: hist_hori_id
    !- id of PFT axis
    INTEGER,INTENT(in) :: hist_PFTaxis_id
    !- id of Deforestation axis
    INTEGER,INTENT(in) :: hist_pool_10axis_id,hist_pool_100axis_id
    INTEGER,INTENT(in) :: hist_pool_11axis_id,hist_pool_101axis_id
    !-
    !- 1 local
    !-
    ...
    IF (is_omp_root) THEN
       DO l=1,nelements 
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("LITTER_STR_AB       ")//TRIM(element_str(l)), &
               &               TRIM("structural litter above ground                    "), &
               &               TRIM("gC(orN)/m^2/pft          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          
          ! metabolic litter above ground                     
          CALL histdef (hist_id_stom, &
               &               TRIM("LITTER_MET_AB       ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("LITTER_STR_BE       ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("LITTER_MET_BE       ")//TRIM(element_str(l)), &
               &               TRIM("metabolic litter below ground                     "), &
               &               TRIM("gC(orN)/m^2/pft          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          
          ! Woody litter above ground
          CALL histdef (hist_id_stom, &
               &               TRIM("LITTER_WOD_AB")//TRIM(element_str(l)), &
               &               TRIM("Woody litter above ground"), &
               &               TRIM("gC(orN)/m^2/pft"), iim, jjm, hist_hori_id, &
               &               nvm, 1, nvm, hist_PFTaxis_id, 32, ave(5), dt, hist_dt)
          
          ! Woody litter below ground
          CALL histdef (hist_id_stom, &
               &               TRIM("LITTER_WOD_BE ")//TRIM(element_str(l)), &
               &               TRIM("Woody litter below ground "), &
               &               TRIM("gC(orN)/m^2/pft"), iim,jjm, hist_hori_id, &
               &               nvm, 1, nvm, hist_PFTaxis_id, 32, ave(5), dt, hist_dt)
          ! active soil carbon in ground                 
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_ACTIVE       ")//TRIM(element_str(l)), &
               &               TRIM("active soil C or N in ground                      "), &
               &               TRIM("gC(orN)/m^2/pft          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          
          ! surface soil carbon in ground                 
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_SURF       ")//TRIM(element_str(l)), &
               &               TRIM("surface soil C or N in ground                      "), &
               &               TRIM("gC(orN)/m^2/pft          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          
          ! slow soil carbon in ground                   
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_SLOW         ")//TRIM(element_str(l)), &
               &               TRIM("slow soil C or N in ground                        "), &
               &               TRIM("gC(orN)/m^2/pft          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          
          ! passive soil carbon in ground                
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_PASSIVE      ")//TRIM(element_str(l)), &
               &               TRIM("passive soil C or N in ground                     "), &
               &               TRIM("gC(orN)/m^2/pft          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
   
          ! total living biomass
          CALL histdef (hist_id_stom, &
               &               TRIM("TOTAL_M             ")//TRIM(element_str(l)), &
          ...
          ! Leaf mass                                         
          CALL histdef (hist_id_stom, &
               &               TRIM("LEAF_M              ")//TRIM(element_str(l)), &
               &               TRIM("Leaf mass                                         "), &
               &               TRIM("gC(orN)/m^2              "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          
          ! Sap mass above ground                             
          CALL histdef (hist_id_stom, &
               &               TRIM("SAP_M_AB            ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("SAP_M_BE            ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("HEART_M_AB          ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("HEART_M_BE          ")//TRIM(element_str(l)), &
          ...
          ! Root mass                                         
          CALL histdef (hist_id_stom, &
               &               TRIM("ROOT_M              ")//TRIM(element_str(l)), &
               &               TRIM("Root mass                                         "), &
               &               TRIM("gC(orN)/m^2              "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          ...
          ! Fruit mass                                        
          CALL histdef (hist_id_stom, &
               &               TRIM("FRUIT_M             ")//TRIM(element_str(l)), &
               &               TRIM("Fruit mass                                        "), &
               &               TRIM("gC(orN)/m^2              "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("RESERVE_M           ")//TRIM(element_str(l)), &
               &               TRIM("Carbohydrate reserve mass                         "), &
               &               TRIM("gC(orN)/m^2              "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          ! Labile reserve mass                         
          CALL histdef (hist_id_stom, &
               &               TRIM("LABILE_M           ")//TRIM(element_str(l)), &
               &               TRIM("Labile reserve mass                         "), &
               &               TRIM("gC(orN)/m^2              "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          
          ! total turnover rate
          CALL histdef (hist_id_stom, &
               &               TRIM("TOTAL_TURN          ")//TRIM(element_str(l)), &
               &               TRIM("total turnover rate                               "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
   
          ! Leaf turnover                                     
          CALL histdef (hist_id_stom, &
               &               TRIM("LEAF_TURN           ")//TRIM(element_str(l)), &
               &               TRIM("Leaf turnover                                     "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          ...
          ! Sap turnover above                                
          CALL histdef (hist_id_stom, &
               &               TRIM("SAP_AB_TURN         ")//TRIM(element_str(l)), &
          ...
          ! Root turnover                                     
          CALL histdef (hist_id_stom, &
               &               TRIM("ROOT_TURN           ")//TRIM(element_str(l)), &
               &               TRIM("Root turnover                                     "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          ...
          ! Fruit turnover                                    
          CALL histdef (hist_id_stom, &
               &               TRIM("FRUIT_TURN          ")//TRIM(element_str(l)), &
               &               TRIM("Fruit turnover                                    "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
   
          ! Leaf recycling (nutrients)
          CALL histdef (hist_id_stom, &
               &               TRIM("LEAF_RECYCLE           ")//TRIM(element_str(l)), &
               &               TRIM("Leaf nutrient recycling                            "), &
               &               TRIM("gN(orP)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)

          ! Root recycling (nutrients)
          CALL histdef (hist_id_stom, &
               &               TRIM("ROOT_RECYCLE           ")//TRIM(element_str(l)), &
               &               TRIM("Root nutrient recycling                            "), &
               &               TRIM("gN(orP)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("TOTAL_BM_LITTER     ")//TRIM(element_str(l)), &
          ...
          ! Leaf death                                        
          CALL histdef (hist_id_stom, &
               &               TRIM("LEAF_BM_LITTER      ")//TRIM(element_str(l)), &
               &               TRIM("Leaf death                                        "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("SAP_AB_BM_LITTER    ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("SAP_BE_BM_LITTER    ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("HEART_AB_BM_LITTER  ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("HEART_BE_BM_LITTER  ")//TRIM(element_str(l)), &
          ...
          ! Root death                                        
          CALL histdef (hist_id_stom, &
               &               TRIM("ROOT_BM_LITTER      ")//TRIM(element_str(l)), &
               &               TRIM("Root death                                        "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          ...
          ! Fruit death                                       
          CALL histdef (hist_id_stom, &
               &               TRIM("FRUIT_BM_LITTER     ")//TRIM(element_str(l)), &
               &               TRIM("Fruit death                                       "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("RESERVE_BM_LITTER   ")//TRIM(element_str(l)), &
               &               TRIM("Carbohydrate reserve death                        "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
   
          ! Labile reserve death                        
          CALL histdef (hist_id_stom, &
               &               TRIM("LABILE_BM_LITTER   ")//TRIM(element_str(l)), &
               &               TRIM("Labile reserve death                        "), &
               &               TRIM("gC(orN)/m^2/day          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
   
          ! biomass allocated to leaves                       
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_LEAF       ")//TRIM(element_str(l)), &
               &               TRIM("biomass allocated to leaves                       "), &
               &               TRIM("gC(orN)/m**2/pft/dt      "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
   
          ! biomass allocated to sapwood above ground         
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_SAP_AB     ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_SAP_BE     ")//TRIM(element_str(l)), &
               &               TRIM("biomass allocated to sapwood below ground         "), &
               &               TRIM("gC(orN)/m**2/pft/dt      "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)

          ! biomass allocated to heartwood above ground         
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_HEART_AB     ")//TRIM(element_str(l)), &
               &               TRIM("biomass allocated to heartwood above ground         "), &
               &               TRIM("gC(orN)/m**2/pft/dt      "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
   
          ! biomass allocated to heartwood below ground         
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_HEART_BE     ")//TRIM(element_str(l)), &
               &               TRIM("biomass allocated to heartwood below ground         "), &
               &               TRIM("gC(orN)/m**2/pft/dt      "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_ROOT       ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_FRUIT      ")//TRIM(element_str(l)), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_RES        ")//TRIM(element_str(l)), &
               &               TRIM("biomass allocated to reserve pool                 "), &
               &               TRIM("gC(orN)/m**2/pft/dt          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)

          ! biomass allocated to labile 
          CALL histdef (hist_id_stom, &
               &               TRIM("BM_ALLOC_LAB        ")//TRIM(element_str(l)), &
               &               TRIM("biomass allocated to labile pool      "), &
               &               TRIM("gC(orN)/m**2/pft/dt          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
       ...
       ! Seasonal nitrogen stress                           
       CALL histdef (hist_id_stom, &
            &               TRIM("NSTRESS_SEASON      "), &
            &               TRIM("Seasonal nitrogen stress                          "), &
            &               TRIM("-                   "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(9), dt, hist_dt)
       ! Seasonal nitrogen stress                           
       CALL histdef (hist_id_stom, &
            &               TRIM("PSTRESS_SEASON      "), &
            &               TRIM("Seasonal phosphorus stress                          "), &
            &               TRIM("-                   "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(9), dt, hist_dt)
        
       ! Seasonal water stress                           
       CALL histdef (hist_id_stom, &
            &               TRIM("MOIAVAIL_SEASON    "), &
            &               TRIM("Seasonal water stress                             "), &
            &               TRIM("-                   "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(9), dt, hist_dt)
...
!       END IF
       ...
       CALL histdef (hist_id_stom, &
            &               TRIM("GPP_POT             "), &
            &               TRIM("Nutrient unlimted gross primary productivity      "), &
            &               TRIM("gC/day/m^2          "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(1), dt, hist_dt)
...
!JC add separate leaf/stem/root ages
       CALL histdef (hist_id_stom, &
            &               TRIM("SAP_AB_AGE            "), &
            &               TRIM("stem age  "), &
            &               TRIM("days                "), iim,jjm, hist_hori_id, &
...
!End JC add
       ...
       ! CO emission during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("CO_FIRE_EMISSION           "), &
            &               TRIM("CO emission during biomass burning"), &
            &               TRIM("gC/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! CH4 emission during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("CH4_FIRE_EMISSION           "), &
            &               TRIM("CH4 emission during biomass burning"), &
            &               TRIM("gC/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! NOx emission during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("NOX_FIRE_EMISSION           "), &
            &               TRIM("NOx emission during biomass burning                 "), &
            &               TRIM("gN/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! NH3 emission during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("NH3_FIRE_EMISSION           "), &
            &               TRIM("NH3 emission during biomass burning"), &
            &               TRIM("gN/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! N2O emission during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("N2O_FIRE_EMISSION           "), &
            &               TRIM("N2O emission during biomass burning"), &
            &               TRIM("gN/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! Other N emission during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("NOTHER_FIRE_EMISSION           "), &
            &               TRIM("Other N emission during biomass burning"), &
            &               TRIM("gN/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! P emission during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("P_FIRE_EMISSION             "), &
            &               TRIM("P emission during biomass burning"), &
            &               TRIM("gP/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! N into soil during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("SOIL_N_FIRE                 "), &
            &               TRIM("N into soil during biomass burning"), &
            &               TRIM("gN/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)

       ! P into soil during biomass burning
       CALL histdef (hist_id_stom, &
            &               TRIM("SOIL_P_FIRE                 "), &
            &               TRIM("P into soil during biomass burning"), &
            &               TRIM("gP/m2/day               "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(6), dt, hist_dt)
...
! JCADD 20170421 for CNP merge
    CALL histdef (hist_id_stom, &
         & TRIM("GRM_RANIMAL_C "), &
         & TRIM("Grazing animal respiration "), &
         & TRIM("gC(orNP)/m^2/pft "), iim,jjm, hist_hori_id, &
         & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt, hist_dt)
    CALL histdef (hist_id_stom, &
         & TRIM("GRM_EFCH4_C "), &
         & TRIM("Grazing animal enteric fermentation CH4 emission "), &
         & TRIM("gC(orNP)/m^2/pft "), iim,jjm, hist_hori_id, &
         & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt, hist_dt)
    CALL histdef (hist_id_stom, &
         & TRIM("GRM_NFERT_AMM "), &
         & TRIM("Ammonium fertilizer from grassland management "), &
         & TRIM("gC(orNP)/m^2/pft "), iim,jjm, hist_hori_id, &
         & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt, hist_dt)
    CALL histdef (hist_id_stom, &
         & TRIM("GRM_NFERT_NIT "), &
         & TRIM("Nitrate fertilizer from grassland management "), &
         & TRIM("gC(orNP)/m^2/pft "), iim,jjm, hist_hori_id, &
         & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt, hist_dt)
    CALL histdef (hist_id_stom, &
         & TRIM("GRM_NDEP "), &
         & TRIM("N deposition from grassland management "), &
         & TRIM("gC(orNP)/m^2/pft "), iim,jjm, hist_hori_id, &
         & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt, hist_dt)
    CALL histdef (hist_id_stom, &
         & TRIM("GRM_PFERT "), &
         & TRIM("Phosphorus fertilizer from grassland management "), &
         & TRIM("gC(orNP)/m^2/pft "), iim,jjm, hist_hori_id, &
         & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt, hist_dt)
       DO l=1,nelements
          IF     (l == icarbon) THEN
          ...
          ENDIF
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_LOSS")//TRIM(element_str(l)), &
               & TRIM("loss as bm_to_litter during grassland harvest"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_HARVEST")//TRIM(element_str(l)), &
               & TRIM("CNP in biomass that be harvest as forage"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_INTAKE")//TRIM(element_str(l)), &
               & TRIM("CNP in biomass that be grazed by domestic or wild animal"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_LIT_INTAKE")//TRIM(element_str(l)), &
               & TRIM("CNP in litter that be grazed by wild animal"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_TRAMPLING")//TRIM(element_str(l)), &
               & TRIM("CNP in biomass that trampled by grazing animal"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_MILK")//TRIM(element_str(l)), &
               & TRIM("CNP in grazing animal milk production"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_LIVEWEIGHT")//TRIM(element_str(l)), &
               & TRIM("CNP in grazing animal livewight gain"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_FAECES")//TRIM(element_str(l)), &
               & TRIM("CNP in grazing animal faeces"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_URINE")//TRIM(element_str(l)), &
               & TRIM("CNP in grazing animal urine"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("GRM_ORGFERT")//TRIM(element_str(l)), &
               & TRIM("CNP in grassland organic fertilizer"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("AGRI_ORGFERT")//TRIM(element_str(l)), &
               & TRIM("CNP in agricultural organic fertilizer"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
          CALL histdef (hist_id_stom, &
               & TRIM("CROP_HARVEST")//TRIM(element_str(l)), &
               & TRIM("CNP in crop havested biomass"), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm,hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(10), dt,hist_dt)
...
! END JCADD
       ...
       ! Nitrogen which is added to the ecosystem to support vegetation growth
       IF (ok_ncycle .AND. impose_cn) THEN
          CALL histdef (hist_id_stom, &
            &               TRIM("N_SUPPORT           "), &
            &               TRIM("N needed to support veget growth                  "), &
            &               TRIM("gN m-2 day-1        "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(3), dt, hist_dt)
       ENDIF
       ! Phosphorus which is added to the ecosystem to support vegetation growth
       IF (ok_pcycle .AND. impose_np) THEN
          CALL histdef (hist_id_stom, &
            &               TRIM("P_SUPPORT           "), &
            &               TRIM("P needed to support veget growth                  "), &
            &               TRIM("gP m-2 day-1        "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(3), dt, hist_dt)
       ENDIF


       ! Radiation use efficieny but defined as GPP/radiation
       CALL histdef (hist_id_stom, &
            &               TRIM("RUE_LONGTERM        "), &
            &               TRIM("Longterm radiation use efficieny                  "), &
            &               TRIM(" gC m-2 day-1       "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(3), dt, hist_dt)

       ! Sapwood to leaf allocation factor
       CALL histdef (hist_id_stom, &
            &               TRIM("KF                 "), &
            &               TRIM("Sapwood to leaf allocation factor                 "), &
            &               TRIM("-                  "), iim,jjm, hist_hori_id, &
            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(3), dt, hist_dt)
       ! Basal area
!DSG: this is never set       CALL histdef (hist_id_stom, &
!               &            TRIM("BA                 "), &
!               &            TRIM("Basal area of stand                               "), &
!               &            TRIM("m^2/ha             "), iim, jjm, hist_hori_id, &
!               &            nvm,1,nvm, hist_PFTaxis_id, 32, ave(1), dt, hist_dt)

       ! on-site wood volume or standing volume
 !DSG: this is never set      CALL histdef (hist_id_stom, &
 !              &            TRIM("WOOD_VOL           "), &
 !              &            TRIM("Standing wood volume                              "), &
 !              &            TRIM("m^3/m^2             "), iim, jjm, hist_hori_id, &
 !              &            nvm,1,nvm, hist_PFTaxis_id, 32, ave(1), dt, hist_dt)
          ! Basal area per diameter class
          CALL histdef (hist_id_stom, &
               TRIM("CCBA               "), &
               TRIM("Basal area of a trees in a circ class                       "), &
               TRIM("-                   "), iim,jjm, hist_hori_id, &
               nvm,1,nvm, hist_PFTaxis_id,32, ave(3), dt, hist_dt)

          ! Change in basal area per diameter class
          CALL histdef (hist_id_stom, &
               TRIM("CCDELTABA          "), &
               TRIM("Change in basal area of a trees in a circ class             "), &
               TRIM("-                   "), iim,jjm, hist_hori_id, &
               nvm,1,nvm, hist_PFTaxis_id,32, ave(3), dt, hist_dt)

       CALL histdef (hist_id_stom, &
               &            TRIM("K_LATOSA_ADAPT     "), &
               &            TRIM("Allocation factor adapted for wstress             "), &
               &            TRIM("m                  "), iim, jjm, hist_hori_id, &
               &            nvm,1,nvm, hist_PFTaxis_id, 32, ave(1), dt, hist_dt)
...
! JCADD 20170421 for CNP merge
          CALL histdef (hist_id_stom, &
               & TRIM("BNF_CLOVER "), &
               & TRIM("BNF directly into plant through Clover root nodules "), &
               & TRIM("gC(orNP)/m^2/pft "), iim,jjm, hist_hori_id, &
               & nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
! END JCADD
          CALL histdef (hist_id_stom, &
               &               TRIM("N_UPTAKE_NH4        "), &
               &               TRIM("plant nitrogen uptake                             "), &
               &               TRIM("gNH4-N/m**2/day         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          ! nitrogen uptake of the plant
          CALL histdef (hist_id_stom, &
               &               TRIM("N_UPTAKE_NO3        "), &
               &               TRIM("plant nitrogen uptake                             "), &
               &               TRIM("gNO3-N/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          ! nitrogen mineralisation rate in the field
          CALL histdef (hist_id_stom, &
               &               TRIM("N_MINERALISATION    "), &
               &               TRIM("net nitrogen mineralisation                      "), &
               &               TRIM("gNH4-N/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          ! inorganic nitrogen in the soil
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_NH4            "), &
               &               TRIM("soil ammonium concentration                       "), &
               &               TRIM("gNH4-N/m**2         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_NO3            "), &
               &               TRIM("soil nitrate concentration                        "), &
               &               TRIM("gNO3-N/m**2         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_NOX            "), &
               &               TRIM("soil NOx concentration                            "), &
               &               TRIM("gNOx-N/m**2         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_N2O            "), &
               &               TRIM("soil nitrous oxide concentration                  "), &
               &               TRIM("gN2O-N/m**2         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_N2             "), &
               &               TRIM("soil dinitrogen concentration                     "), &
               &               TRIM("gN2-N/m**2          "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(4), dt, hist_dt)
          ! oxigen concentration in the soil 
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_P_OX           "), &
               &               TRIM("soil oxigen concentration (hPa)                   "), &
               &               TRIM("hPaO2               "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(8), dt, hist_dt)
          ! bacterial population in the soil 
  !        CALL histdef (hist_id_stom, &
  !             &               TRIM("BACT                "), &
  !             &               TRIM("soil bacterial population                   "), &
  !             &               TRIM("gC/m**2               "), iim,jjm, hist_hori_id, &
  !             &               nvm,1,nvm, hist_PFTaxis_id,32, ave(8), dt, hist_dt)
          ! nitrogen emissions
          CALL histdef (hist_id_stom, &
               &               TRIM("NH3_EMISSION        "), &
               &               TRIM("ammonia emitted to atmosphere                     "), &
               &               TRIM("gNH3-N/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("NOX_EMISSION        "), &
               &               TRIM("nitric oxides emitted to the atmosphere           "), &
               &               TRIM("gNOx-N/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("N2O_EMISSION        "), &
               &               TRIM("nitrous oxide emitted to the atmosphere           "), &
               &               TRIM("gN2O-N/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("N2_EMISSION         "), &
               &               TRIM("nitrogen emitted to the atmosphere                "), &
               &               TRIM("gN2-N/m**2/day      "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          ! leaching
          CALL histdef (hist_id_stom, &
               &               TRIM("NH4_LEACHING        "), &
               &               TRIM("ammonium leached below root zone                  "), &
               &               TRIM("gNH4-N/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("NO3_LEACHING        "), &
               &               TRIM("nitrate leached below root zone                   "), &
               &               TRIM("gNO3-N/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
   !       CALL histdef (hist_id_stom, &
   !            &               TRIM("DOC_LEACHING        "), &
   !            &               TRIM("dissolved organic carbon leached                  "), &
   !            &               TRIM("gC/m**2/day         "), iim,jjm, hist_hori_id, &
   !            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
   !       CALL histdef (hist_id_stom, &
   !            &               TRIM("DON_LEACHING        "), &
   !            &               TRIM("dissolved organic nitrogen leached                "), &
   !            &               TRIM("gN/m**2/day         "), iim,jjm, hist_hori_id, &
   !            &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("NITRIFICATION      "), &
               &               TRIM("oxidation of NH4                                  "), &
               &               TRIM("gN/m**2/day         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("DENITRIFICATION    "), &
               &               TRIM("reduction of NO3                                  "), &
               &               TRIM("gN/m**2/day         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(5), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("NHX_DEPOSITION       "), &
               &               TRIM("nitrogen added from deposition of NHx      "), &
               &               TRIM("gN/m**2/day         "), iim,jjm, hist_hori_id, &
               &               1,1,1, -99,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("NOY_DEPOSITION       "), &
               &               TRIM("nitrogen added from deposition of NOy      "), &
               &               TRIM("gN/m**2/day         "), iim,jjm, hist_hori_id, &
               &               1,1,1, -99,32, ave(2), dt, hist_dt)
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("BNF                "), &
               &               TRIM("nitrogen added from biological N fixation         "), &
               &               TRIM("gN/m**2/day         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
       ...
       !    &               1,1,1, -99,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("N_FERTILISER       "), &
               &               TRIM("nitrogen added from nitrogen fertiliser appl.     "), &
               &               TRIM("gN/m**2/day         "), iim,jjm, hist_hori_id, &
               &               1,1,1, -99,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("AGRI_FERT_NH4       "), &
               &               TRIM("nitrogen added from agricultural nitrogen fertiliser appl.    "), &
               &               TRIM("gN/day/m^2/pft     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("AGRI_FERT_NO3       "), &
               &               TRIM("nitrogen added from agricultural nitrogen fertiliser appl.    "), &
          ...
          CALL histdef (hist_id_stom, &
               &               TRIM("N_TAKEN       "), &
               &               TRIM("nitrogen that comes virtually in the ecosystem    "), &
               &               TRIM("gN/day/m^2/pft     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
       ...
       IF(ok_pcycle) THEN
          CALL histdef (hist_id_stom, &
               &               TRIM("LABILE_TURN     "), &
               &               TRIM("turnover of diss. P consider. only sorption "), &
               &               TRIM("day**-1         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_P_DISSOLVED    "), &
               &               TRIM("soil P in solution concentration                    "), &
               &               TRIM("gP/m**2         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("SOIL_P_SORBED    "), &
               &               TRIM("soil P adsorbed concentration                    "), &
               &               TRIM("gP/m**2         "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)

          CALL histdef (hist_id_stom, &
               &               TRIM("f_PDISSOLVED     "), &
               &               TRIM("fraction of dissolved P at root                   "), &
               &               TRIM("                "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)


          CALL histdef (hist_id_stom, &
               &               TRIM("P_MINERALISATION    "), &
               &               TRIM("net phosphorus mineralisation                   "), &
               &               TRIM("gP/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("BC_MINERALISATION    "), &
               &               TRIM("phosphorus mineralised by phosphatases              "), &
               &               TRIM("gP/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("P_SSORPTION       "), &
               &               TRIM("P losses to strong sorption and occlusion           "), &
               &               TRIM("gP/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("AGRI_FERT_P       "), &
               &               TRIM("P input from agricultural mineral fertilization "), &
               &               TRIM("gP/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("P_UPTAKE          "), &
               &               TRIM("plant phosphorus uptake                             "), &
               &               TRIM("gP/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("P_LEACHING        "), &
               &               TRIM("dissolved P leached below root zone                 "), &
               &               TRIM("gP/m**2/day     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("P_DEPOSITION       "), &
               &               TRIM("phosphorus added from deposition                    "), &
               &               TRIM("gP/m**2/day         "), iim,jjm, hist_hori_id, &
               &               1,1,1, -99,32, ave(2), dt, hist_dt)

          CALL histdef (hist_id_stom, &
               &               TRIM("P_TAKEN       "), &
               &               TRIM("phosphorus that comes virtually in the ecosystem    "), &
               &               TRIM("gP/day/m^2/pft     "), iim,jjm, hist_hori_id, &
               &               nvm,1,nvm, hist_PFTaxis_id,32, ave(2), dt, hist_dt)
          CALL histdef (hist_id_stom, &
               &               TRIM("P_RELEASE     "), &
               &               TRIM("phosphorus released by chemical weathering          "), &
               &               TRIM("gP/day/m^2         "), iim,jjm, hist_hori_id, &
               &               1,1,1, -99,32, ave(2), dt, hist_dt)
...
!_ ================================================================================================================================
  SUBROUTINE ioipslctrl_histstomipcc( &
    ...
    !-
    !- File id
    INTEGER,INTENT(in) :: hist_id_stom_IPCC
    !- number of PFTs
    INTEGER,INTENT(in) :: nvm
    !- Domain size
    INTEGER,INTENT(in) :: iim, jjm
...
!_ ================================================================================================================================
  SUBROUTINE ioipslctrl_restini(istp, date0, dt, rest_id, rest_id_stom, itau_offset, date0_shifted)
    ...
    REAL                                 :: date0     !! The date at which itau = 0
    REAL                                 :: dt        !! Time step
    INTEGER, INTENT(out)                 :: rest_id, rest_id_stom   !! ID of the restart file
    INTEGER, INTENT(out)                 :: itau_offset    !! Note the result is always itau_offset=0 as overwrite_time=TRUE
    REAL, INTENT(out)                    :: date0_shifted  !! Note the result is always date0_shifted=date0 as overwrite_time=TRUE


    !  LOCAL
    !
    REAL                 :: dt_rest, date0_rest
    INTEGER              :: itau_dep
