...
!!
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_sechiba/chemistry.f90 $
...
MODULE chemistry
  ...
  USE pft_parameters
  USE grid
  USE time, ONLY : one_day, dt_sechiba, julian_diff, one_hour, one_year
  USE ioipsl_para 
  USE xios_orchidee
  ...
  PRIVATE
  PUBLIC :: chemistry_initialize, chemistry_bvoc, chemistry_flux_interface, chemistry_clear


  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_iso            !! Isoprene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_iso)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_mono           !! Monoterpene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_mono)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_ORVOC          !! Other Volatile Organic Compound emissions from vegetation (kgC.m^{-2}.s^{-1}) 
!$OMP THREADPRIVATE(flx_ORVOC)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_MBO            !! 2-methyl-3-buten-2-ol emissions from vegetation (mainly pines in America) (kgC.m^{-2}.s^{-1}) 
!$OMP THREADPRIVATE(flx_MBO)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_methanol       !! Methanol emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_methanol)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_acetone        !! Acetone emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_acetone)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_acetal         !! Acetaldehyde emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_acetal)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_formal         !! Formaldehyde emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_formal)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_acetic         !! Acetic acid emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_acetic)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_formic         !! Formic acid emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_formic)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_apinen         !! Alpha pinene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_apinen)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_bpinen         !! Beta pinene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_bpinen)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_limonen        !! Limonene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_limonen)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_myrcen         !! Myrcene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_myrcen)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_sabinen        !! Sabinene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_sabinen)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_camphen        !! Camphene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_camphen)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_3caren         !! 3-Carene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_3caren)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_tbocimen       !! T-beta Ocimene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_tbocimen)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_othermono      !! Emissions of other monoterpenes from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_othermono)
  REAL,ALLOCATABLE, SAVE, DIMENSION(:,:)  :: flx_sesquiter      !! Sesquiterpene emissions from vegetation (kgC.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_sesquiter)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_fertil_no      !! Biogenic nitrogen oxide soil emission due to N-fertilisation (ngN.m^{-2}.s^{-1}) 
!$OMP THREADPRIVATE(flx_fertil_no)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_no_soil        !! Nitrogen Oxide emissions from soil, before deposition on canopy 
                                                                       !! (ngN.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_no_soil)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_no             !! Net nitrogen Oxide emissions from soil, after deposition on canopy
!$OMP THREADPRIVATE(flx_no)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:) :: CRF                !! Canopy reduction factor for net NO flux calculation (kjpindex,nvm)    
...
!$OMP THREADPRIVATE(allow_pulse)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: pulse            !! Pulse fonction 
!$OMP THREADPRIVATE(pulse)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: pulseday         !! Counter for pulse period
!$OMP THREADPRIVATE(pulseday)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: siestaday        !! Counter for post-pulse period
!$OMP THREADPRIVATE(siestaday)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: pulselim         !! Pulse period length
!$OMP THREADPRIVATE(pulselim)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: siestalim        !! Post-pulse period length
!$OMP THREADPRIVATE(siestalim)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: area2            !! Grid cell area (m^2)
!$OMP THREADPRIVATE(area2)
  REAL, SAVE                            :: nbre_precip 
!$OMP THREADPRIVATE(nbre_precip)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: flx_co2_bbg_year !! CO2 emissions from biomass burning 
                                                                   !! Read in an input file (kgC.m^{-2}.year^{-1})
!$OMP THREADPRIVATE(flx_co2_bbg_year)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: N_qt_WRICE_year  !! N fertilizers applied on wetland rice
                                                                   !! Read in an input file (kgN.yr^{-1})
!$OMP THREADPRIVATE(N_qt_WRICE_year)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:) :: N_qt_OTHER_year  !! N fertilizers applied on other crops and grasses
                                                                   !! Read in an input file (kgN.yr^{-1}) 
!$OMP THREADPRIVATE(N_qt_OTHER_year)
  LOGICAL, SAVE   :: l_first_chemistry_inca=.TRUE.                 !! Initialisation for chemistry_flux_interface
...
!_ ================================================================================================================================
  SUBROUTINE chemistry_initialize(kjpindex, lalo, neighbours, resolution)
    ...
    INTEGER, DIMENSION(kjpindex,8), INTENT (in) :: neighbours       !! Vector of neighbours for each 
                                                                           !! grid point (1=N, 2=E, 3=S, 4=W)
    REAL,DIMENSION (kjpindex,2), INTENT(in)     :: resolution       !! The size in km of each grid-box in X and Y

    !! 0.2 Output variables
    REAL, DIMENSION(kjpindex)                   :: achem_wrice      !! Availability of data for the interpolation
    REAL, DIMENSION(kjpindex)                   :: achem_other      !! Availability of data for the interpolation
    REAL, DIMENSION(kjpindex)                   :: achem_co2        !! Availability of data for the interpolation

    !! 0.3 Modified variables

    !! 0.4 Local variables
    LOGICAL                                            :: allow_weathergen
    CHARACTER(LEN=80)                                  :: filename, fieldname
    INTEGER                                     :: iml, jml, lml, tml, force_id
    INTEGER                                     :: ier
    ...
    REAL, DIMENSION(:), ALLOCATABLE             :: variabletypevals !! Values for all the types of the variable
    ...
    REAL                                        :: chem_norefinf    !! No value
    REAL                                        :: chem_default     !! Default value
...
!_ ================================================================================================================================
    ...
    IF (ok_cropsfertil_NOx) THEN
       ...
       IF (printlev >= 1) WRITE(numout,*) "chemistry_initialize: Read and interpolate file " &
            // TRIM(filename) //" for variable N_qt_WRICE_year"
       CALL interpweight_2Dcont(kjpindex, 0, 0, lalo, resolution, neighbours,                        &
            ...
            maskvals, namemaskvar, -1, fractype, chem_default, chem_norefinf,                           &
            N_qt_WRICE_year, achem_wrice)
       
       fieldname= 'N_qt_OTHER_year'
       IF (printlev >= 1) WRITE(numout,*) "chemistry_initialize: Read and interpolate file " &
            // TRIM(filename) //" for variable N_qt_OTHER_year"
       CALL interpweight_2Dcont(kjpindex, 0, 0, lalo, resolution, neighbours,                        &
            contfrac, filename, fieldname, lonname, latname, vmin, vmax, nonegative, maskingtype,       &
            maskvals, namemaskvar, -1, fractype, chem_default, chem_norefinf,                           &
            N_qt_OTHER_year, achem_other)
    ...
    IF (ok_bbgfertil_NOx) THEN
       ...
       IF (printlev >= 1) WRITE(numout,*) "chemistry_initialize: Read and interpolate file " &
            // TRIM(filename) //" for variable flx_co2_bbg_year"
       CALL interpweight_2Dcont(kjpindex, 0, 0, lalo, resolution, neighbours,                         &
            contfrac, filename, fieldname, lonname, latname, vmin, vmax, nonegative, maskingtype,     &
            maskvals, namemaskvar, -1, fractype, chem_default, chem_norefinf,                         &
            flx_co2_bbg_year, achem_co2)
    END IF
    ...
    ! Write diagnostics
    IF (ok_cropsfertil_NOx) THEN
      CALL xios_orchidee_send_field("achem_wrice",achem_wrice)
      CALL xios_orchidee_send_field("achem_other",achem_other)
    END IF
    IF (ok_bbgfertil_NOx) THEN
      CALL xios_orchidee_send_field("achem_co2",achem_co2)
    END IF
...
!_ ================================================================================================================================
  ...
  SUBROUTINE chemistry_bvoc (kjpindex, swdown, coszang, temp_air, &
    ...
    INTEGER,INTENT (in)                                 :: hist_id          !! History file identifier (-)
    INTEGER,INTENT (in)                                 :: hist2_id         !! History file 2 identifier (-)
    ...
    REAL, DIMENSION(kjpindex), INTENT(in)               :: swdown           !! Down-welling surface short-wave flux 
                                                                                   !! (W.m^{-2})
    ...
    REAL, DIMENSION(kjpindex), INTENT(in)               :: ptnlev1          !! 1st level of soil temperature (K)
    REAL, DIMENSION(kjpindex), INTENT(in)               :: precip_rain      !! Rain precipitation !!?? init
    REAL, DIMENSION(kjpindex,nvm), INTENT(in)           :: humrel           !! Soil moisture stress (0-1, unitless)
    REAL, DIMENSION(kjpindex,nvm), INTENT(in)           :: veget_max        !! Max. vegetation fraction (0-1, unitless)
    ...
    REAL, DIMENSION(kjpindex,nvm,nleafages), INTENT(in) :: frac_age         !! Age efficacity from STOMATE for iso 
    REAL, DIMENSION(kjpindex,2), INTENT(in)             :: lalo             !! Geographical coordinates for pixels (degrees)
    REAL,DIMENSION (kjpindex), INTENT (in)              :: ccanopy          !! CO2 concentration inside the canopy
    REAL,DIMENSION (kjpindex,nvm), INTENT (in)          :: cim              !! Intercellular CO2 over nlai 
    REAL, DIMENSION (kjpindex), INTENT(in)              :: wind             !! Wind module (m s^{-1})
    ...
    INTEGER                             :: ji, jv, jf, jl    !! Indices (unitless)
    REAL, DIMENSION(kjpindex,nvm)       :: fol_dens          !! foliar density (gDM.m^{-2})
    REAL, DIMENSION(kjpindex)           :: tleaf             !! Foliar temperature (K)
    REAL, DIMENSION(kjpindex)           :: t_no              !! Temperature used for soil NO emissions (C)
    REAL, DIMENSION(kjpindex)           :: exp_1             !! First exponential used in the calculation of 
                                                                    !! isoprene dependancy to Temperature 
    REAL, DIMENSION(kjpindex)           :: exp_2             !! Second exponential used in the calculation of 
                                                                    !! Isoprene dependancy to Temperature
    REAL, DIMENSION(kjpindex)           :: Ct_iso            !! Isoprene dependancy to Temperature
    REAL, DIMENSION(kjpindex)           :: Cl_iso            !! Isoprene dependancy to Light 
    REAL, DIMENSION(kjpindex)           :: Ct_mono           !! Monoterpene dependancy to Temperature
    REAL, DIMENSION(kjpindex)           :: Ct_sesq           !! Sesquiterpenes dependancy to Temperature
    REAL, DIMENSION(kjpindex)           :: Ct_meth           !! Methanol dependancy to Temperature
    REAL, DIMENSION(kjpindex)           :: Ct_acet           !! Acetone dependancy to Temperature
    REAL, DIMENSION(kjpindex)           :: Ct_oxyVOC         !! Other oxygenated BVOC dependancy to Temperature
    REAL                                :: GAMMA_iso         !! Temperature and light dependancy for isoprene and fo each PFT
    REAL                                :: GAMMA_iso_m       !! Temperature and light dependancy for isoprene and fo each PFT for multilayer
    REAL, DIMENSION(kjpindex)           :: Ylt_mono          !! Total Temperature and light dependancy for monoterpenes
    REAL, DIMENSION(kjpindex)           :: Ylt_sesq          !! Total Temperature and light dependancy for sesquiterpens
    REAL, DIMENSION(kjpindex)           :: Ylt_meth          !! Total Temperature and light dependancy for methanol
    REAL, DIMENSION(kjpindex)           :: Ylt_acet          !! Total Temperature and light dependancy for acetone
    REAL, DIMENSION(kjpindex)           :: Ct_MBO            !! MBO dependance to Temperature
    REAL, DIMENSION(kjpindex)           :: Cl_MBO            !! MBO dependance to Light
    REAL, DIMENSION(kjpindex)           :: Xvar              !! Parameter used in the calculation 
                                                                    !! of MBO dependance to Temperature
    REAL, DIMENSION(kjpindex,nvm)       :: flx_OVOC          !! Biogenic OVOC emission - 
                                                                    !! Other Volatil Organic Components (kgC.m^{-2}.s^{-1})
    !!Canopy radiative transfer model
    REAL, DIMENSION(kjpindex)           :: So                !! Maximum radiation at the Earth surface (W.m^{-2})
    REAL, DIMENSION(kjpindex)           :: Rfrac             !! Parameter in the regression of diffuse 
                                                                    !! share on transmission
    REAL, DIMENSION(kjpindex)           :: Kfrac             !! Parameter in the regression of diffuse 
                                                                    !! share on transmission
    REAL, DIMENSION(kjpindex)           :: swdf              !! Sw diffuse radiation (W.m^{-2}) 
    REAL, DIMENSION(kjpindex)           :: swdr              !! Sw direct radiation (W.m^{-2}) 
    REAL, DIMENSION(kjpindex,nvm)       :: PARscat           !! Scatter PAR @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL, DIMENSION(kjpindex,nvm)       :: Clsun_iso         !! Isoprene dependance to light for sun leaves 
    REAL, DIMENSION(kjpindex,nvm)       :: Clsh_iso          !! Isoprene dependance to light for shaded leaves
    !! for multilayer canopy model for iso flux
    REAL, DIMENSION(kjpindex,nlai+1)    :: PARscattab        !! Scatter PAR @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL, DIMENSION(nlai+1)             :: laitab            !! LAI per layer (m^2.m^{-2})
    REAL, DIMENSION(kjpindex,nlai)      :: laisuntabdep      !! LAI of sun leaves in each layer (m^2.m^{-2})
    REAL, DIMENSION(kjpindex,nlai)      :: laishtabdep       !! LAI of shaded leaves in each layer 
                                                                    !! (m^2.m^{-2})
    REAL                                :: Clsun_iso_tab     !! Isoprene dependance to light 
                                                                    !! for sun leaves and per layer 
    REAL                                :: Clsh_iso_tab      !! Isoprene dependance to light 
                                                                    !! for shaded leaves and per layer
    !for multilayer canopy model Spitter et al. 1986
    REAL, DIMENSION(kjpindex,nlai+1)    :: PARnotscat        !! Not-Scattered PAR 
    REAL, DIMENSION(kjpindex,nlai+1)    :: PARabsdir         !! Absorbed light of the PAR direct flux  
    REAL, DIMENSION(kjpindex,nlai+1)    :: PARabsdiff        !! Absorbed light of the PAR diffuse flux  
    REAL, PARAMETER                     :: sigma = 0.20      !! scattering coefficient of single leaves and for visible radiation
    REAL, PARAMETER                     :: cluster = 0.85    !! clustering coefficient for leaves, the same that is setting for default in MEGAN V2.10
    REAL                                :: rho               !! reflection index of a green, closed vegetation
    REAL                                :: kbl               !! extinction coefficient of black leaves
    REAL                                :: kdf               !! extinction coefficient of diffuse flux
    !!Leaf age
    REAL, DIMENSION(kjpindex,nvm)       :: Eff_age_iso       !! Isoprene emission dependance to Leaf Age 
    REAL, DIMENSION(kjpindex,nvm)       :: Eff_age_meth      !! Methanol emission dependance to Leaf Age 
    REAL, DIMENSION(kjpindex,nvm)       :: Eff_age_VOC       !! Other VOC emission dependance to Leaf Age 
    !!BBG and Fertilizers for NOx soil emission
    REAL, DIMENSION(kjpindex)           :: veget_max_nowoody !! sum of veget_max for non-woody PFT
    REAL, DIMENSION(kjpindex,nvm)       :: N_qt_WRICE_pft    !! N fertiliser on RICE 
                                                                    !! (kgN per year per grid cell)
    REAL, DIMENSION(kjpindex,nvm)       :: N_qt_OTHER_pft    !! N fertiliser on other veg 
                                                                    !! (kgN per year per grid cell)
    !! CO2 inhibition effect on isoprene
    REAL,DIMENSION (kjpindex,nvm)       :: fco2_wshort       !! Wilkinson short term function for CO2 impact on BVOC (isoprene) 
    REAL,DIMENSION (kjpindex)           :: fco2_wlong        !! Wilkinson long term function for CO2 impact on BVOC (isoprene) 
    REAL                                :: fco2_ctrl
    REAL,DIMENSION (kjpindex,nvm)       :: fco2              !! Function for CO2 impact on BVOC (isoprene)
    REAL, DIMENSION(kjpindex)           :: Ismax_short
    REAL, DIMENSION(kjpindex)           :: h_short
    REAL, DIMENSION(kjpindex)           :: Cstar_short
    REAL                                :: Ismax_long
    REAL                                :: h_long
    REAL                                :: Cstar_long
    ...
    REAL, PARAMETER :: CT1 = 95000.0       !! Empirical coeffcient (see Guenther .et. al, 1995, eq(10)) (J.mol^{-1})
    REAL, PARAMETER :: CT2 = 230000.0      !! Empirical coefficient (see Guenther .et. al, 1995, eq(10)) (J.mol^{-1})
    REAL, PARAMETER :: TS = 303.0          !! Leaf temperature at standard condition
                                                  !! (see Guenther .et. al, 1995, eq(10)) (K)
    REAL, PARAMETER :: TM = 314.0          !! Leaf temperature (see Guenther .et. al, 1995, eq(10)) (K)

    REAL, PARAMETER :: alpha_ = 0.0027     !! Empirical coeffcient (see Guenther .et. al, 1995, eq(9)) (unitless)
    REAL, PARAMETER :: CL1 = 1.066         !! Empirical coeffcient (see Guenther .et. al, 1995, eq(9)) (unitless)
    REAL, PARAMETER :: beta = 0.09         !! Empirical coeffcient (see Guenther .et. al, 1995, eq(11)) (K^{-1})
    REAL, PARAMETER :: lai_threshold = 11. !! Lai threshold for the calculation of scattered radiation
                                                  !! based on Guenther .et. al (1995) (m^2.m^{-2})

                                              
    ! Biogenic emissions
    REAL,DIMENSION(kjpindex)          :: PAR                !! Photosynthetic active radiation, half of swdown
                                                                   !! @tex ($\mu mol photons. m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION(kjpindex,nvm)      :: PARsun             !! PAR received by sun leaves
                                                                   !! @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION(kjpindex,nvm)      :: PARsh              !! PAR received by shaded leaves 
                                                                   !! @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION(kjpindex,nvm)      :: laisun             !! Leaf area index of Sun leaves (m^2.m^{-2})
    REAL,DIMENSION(kjpindex,nvm)      :: laish              !! Leaf area index of Shaded leaves (m^2.m^{-2}) 
    ...
    REAL, DIMENSION(kjpindex,nlai+1)  :: Tleafsun_temp      !! temporary sunlit leaf temperature matrix for writing
    REAL, DIMENSION(kjpindex,nlai+1)  :: Tleafsh_temp       !! temporary shade leaf temperature matrix for writing
    REAL,DIMENSION(kjpindex)          :: Fdf                !! Fraction of the radiation which is diffused (0-1, unitless)
    REAL,DIMENSION(kjpindex,nlai+1)   :: PARsuntab          !! PAR received by sun leaves over LAI layers
                                                                   !! @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION(kjpindex,nlai+1)   :: PARshtab           !! PAR received by shaded leaves over LAI layers
                                                                   !! @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION(kjpindex)          :: PARdf              !! Diffuse photosynthetic active radiation
                                                                   !! @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION(kjpindex)          :: PARdr              !! Direct photosynthetic active radiation
                                                                   !! @tex ($\mu mol m^{-2} s^{-1}$) @endtex
    REAL,DIMENSION(kjpindex)          :: Trans              !! Atmospheric Transmissivity (unitless)
...
!_ ================================================================================================================================
    ...
    IF ( ok_radcanopy ) THEN
       ...
       DO ji = 1, kjpindex
          IF (coszang(ji) .GT. zero) THEN
             ...
             So(ji) = Sct*( un + 0.033*COS(360.*pi/180.*julian_diff/365.))*coszang(ji)
          ...
          PARdf(ji) = swdf(ji) * W_to_mol * RG_to_PAR
          PARdr(ji) = swdr(ji) * W_to_mol * RG_to_PAR 
       END DO
          ...
          DO jv = 1, nvm
             DO ji = 1, kjpindex
                ...
                ELSE
                   ...
                   PARsun(ji,jv) = zero 
                END IF
                IF (ABS(ACOS(coszang(ji))) .LT. pi/2. .AND. ABS(coszang(ji)) .GT. min_sechiba) THEN 
    ...
    DO ji = 1, kjpindex ! (loop over # pixels)
       ...
       PAR(ji)   = swdown(ji) * W_to_mol * RG_to_PAR        ! from W/m^2 to umol photons/m^2/s and half of sw for PAR
    ...
    DO jv = 1, nvm
       DO ji = 1, kjpindex
          ...
          IF (ok_bbgfertil_NOx) THEN
             IF ( natural(jv) ) THEN
                ...
                IF ((lalo(ji,1) .LE. 30. .AND. lalo(ji,1) .GE. zero).AND. &
                     (julian_diff .GE. 121. .AND. julian_diff .LE. 181).AND.(flx_co2_bbg_year(ji) .GT. 0.1)) THEN
                   flx_no_soil(ji,jv) = flx_no_soil(ji,jv)*3.
                   ! South Tropical zone from November to December
                ELSE IF ((lalo(ji,1) .GE. -30. .AND. lalo(ji,1) .LT. zero).AND.(julian_diff .GE. 305.).AND. & 
          ...
          IF (ok_cropsfertil_NOx) THEN
             ...
             IF (((lalo(ji,1) .GT. 30.) .AND. (julian_diff .GE. 121. .AND. julian_diff .LE. 243.).AND. &
                  (veget_max(ji,jv) .GT. min_sechiba)) .OR. &
                  ((lalo(ji,1) .LT. -30.) .AND. (julian_diff .GE. 305. .OR. julian_diff .LE. 59.) .AND.&
             ...
             ELSE IF ((lalo(ji,1) .GE. -30.).AND.(lalo(ji,1) .LE. 30.).AND.(veget_max(ji,jv) .GT. min_sechiba)) THEN
                flx_fertil_no(ji,jv) = (N_qt_WRICE_pft(ji,jv)*(1/30.)+N_qt_OTHER_pft(ji,jv))*(2.5/100.) &
...
!! ================================================================================================================================
!! SUBROUTINE   : chemistry_flux_interface
!!
!>\BRIEF         This subroutine will give to the interface between inca model and orchidee model in sechiba all the flux ask by inca
!!
!! DESCRIPTION  :  This subroutine allow the transfer of fluxes between surface and atmospheric chemistry. It is called from sechiba
!!
...
!_ ================================================================================================================================
 ...
 SUBROUTINE chemistry_flux_interface( field_out_names, fields_out, field_in_names, fields_in)
     ...
     REAL,DIMENSION(:,:,:), OPTIONAL, INTENT(OUT) :: fields_out
     !
...
!$OMP THREADPRIVATE(nb_fields_out, nb_fields_in)
     ...
     ENDIF
   END SUBROUTINE chemistry_flux_interface

!! ================================================================================================================================
!! SUBROUTINE   : chemistry_clear
!!
!>\BRIEF         Clear chemistry module
!!
...
!_ ================================================================================================================================
   SUBROUTINE chemistry_clear
     ...
     !! 1. Initialize as for first run
     l_first_chemistry_inca = .TRUE.
     
     !! 2. Deallocate dynamic variables
     IF (ALLOCATED(pulse)) DEALLOCATE (pulse)
     IF (ALLOCATED (ok_siesta)) DEALLOCATE (ok_siesta) 
     IF (ALLOCATED (allow_pulse)) DEALLOCATE (allow_pulse)
     IF (ALLOCATED (pulseday)) DEALLOCATE (pulseday)
     IF (ALLOCATED (siestaday)) DEALLOCATE (siestaday)
     IF (ALLOCATED (pulselim)) DEALLOCATE (pulselim)
     IF (ALLOCATED (siestalim)) DEALLOCATE (siestalim)
     IF (ALLOCATED (area2)) DEALLOCATE (area2)
     IF (ALLOCATED (N_qt_WRICE_year)) DEALLOCATE (N_qt_WRICE_year)
     IF (ALLOCATED (N_qt_OTHER_year)) DEALLOCATE (N_qt_OTHER_year)
     IF (ALLOCATED (flx_iso)) DEALLOCATE (flx_iso)
     IF (ALLOCATED (flx_mono)) DEALLOCATE (flx_mono)
     IF (ALLOCATED (flx_ORVOC)) DEALLOCATE (flx_ORVOC)
     IF (ALLOCATED (flx_MBO)) DEALLOCATE (flx_MBO)
     IF (ALLOCATED (flx_methanol)) DEALLOCATE (flx_methanol)
     IF (ALLOCATED (flx_acetone)) DEALLOCATE (flx_acetone)
     IF (ALLOCATED (flx_acetal)) DEALLOCATE (flx_acetal)
     IF (ALLOCATED (flx_formal)) DEALLOCATE (flx_formal)
     IF (ALLOCATED (flx_acetic)) DEALLOCATE (flx_acetic)
     IF (ALLOCATED (flx_formic)) DEALLOCATE (flx_formic)
     IF (ALLOCATED (flx_no_soil)) DEALLOCATE (flx_no_soil)
     IF (ALLOCATED (flx_no)) DEALLOCATE (flx_no)
     IF (ALLOCATED (flx_fertil_no)) DEALLOCATE (flx_fertil_no)
     IF (ALLOCATED (flx_apinen)) DEALLOCATE (flx_apinen)
     IF (ALLOCATED (flx_bpinen)) DEALLOCATE (flx_bpinen)
     IF (ALLOCATED (flx_limonen)) DEALLOCATE (flx_limonen)
     IF (ALLOCATED (flx_myrcen)) DEALLOCATE (flx_myrcen)
     IF (ALLOCATED (flx_sabinen)) DEALLOCATE (flx_sabinen)
     IF (ALLOCATED (flx_camphen)) DEALLOCATE (flx_camphen)
     IF (ALLOCATED (flx_3caren)) DEALLOCATE (flx_3caren)
     IF (ALLOCATED (flx_tbocimen)) DEALLOCATE (flx_tbocimen)
     IF (ALLOCATED (flx_othermono)) DEALLOCATE (flx_othermono)
     IF (ALLOCATED (flx_sesquiter)) DEALLOCATE (flx_sesquiter)
     IF (ALLOCATED (CRF)) DEALLOCATE (CRF)
     IF (ALLOCATED (flx_co2_bbg_year)) DEALLOCATE (flx_co2_bbg_year)

   END SUBROUTINE chemistry_clear
