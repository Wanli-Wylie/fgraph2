...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_sechiba/routing.f90 $
...
!! PARAMETERS
  INTEGER, PARAMETER                                  :: nbasmax=5                   !! The maximum number of basins we wish to have per grid box (truncation of the model) (unitless)
  INTEGER, SAVE                                       :: nbvmax                      !! The maximum number of basins we can handle at any time during the generation of the maps (unitless)
!$OMP THREADPRIVATE(nbvmax)
  REAL, PARAMETER                                     :: slow_tcst_cwrr = 25.0        !! Property of the slow reservoir, when CWRR hydrology is activated (day/m)
  REAL, PARAMETER                                     :: fast_tcst_cwrr = 3.0        !! Property of the fast reservoir, when CWRR hydrology is activated (day/m)
  REAL, PARAMETER                                     :: stream_tcst_cwrr = 0.24     !! Property of the stream reservoir, when CWRR hydrology is activated (day/m)
  REAL, PARAMETER                                     :: flood_tcst_cwrr = 4.0       !! Property of the floodplains reservoir, when CWRR hydrology is activated (day/m)
  REAL, PARAMETER                                     :: swamp_cst_cwrr = 0.2        !! Fraction of the river transport that flows to the swamps, when CWRR hydrology is activated (unitless;0-1)
  !
  REAL, PARAMETER                                     :: slow_tcst_chois = 25.0      !! Property of the slow reservoir, when Choisnel hydrology is activated (day/m)
  REAL, PARAMETER                                     :: fast_tcst_chois = 3.0       !! Property of the fast reservoir, when Choisnel hydrology is activated (day/m)
  REAL, PARAMETER                                     :: stream_tcst_chois = 0.24    !! Property of the stream reservoir, when Choisnel hydrology is activated (day/m)
  REAL, PARAMETER                                     :: flood_tcst_chois = 4.0      !! Property of the floodplains reservoir, when Choisnel hydrology is activated (day/m)
  REAL, PARAMETER                                     :: swamp_cst_chois = 0.2       !! Fraction of the river transport that flows to the swamps, when Choisnel hydrology is activated (unitless;0-1)
  !
  REAL, SAVE                                          :: fast_tcst                   !! Property of the fast reservoir, (day/m)
!$OMP THREADPRIVATE(fast_tcst)
  REAL, SAVE                                          :: slow_tcst                   !! Property of the slow reservoir, (day/m)
!$OMP THREADPRIVATE(slow_tcst)
  REAL, SAVE                                          :: stream_tcst                 !! Property of the stream reservoir, (day/m)
!$OMP THREADPRIVATE(stream_tcst)
  REAL, SAVE                                          :: flood_tcst                  !! Property of the floodplains reservoir, (day/m)
!$OMP THREADPRIVATE(flood_tcst)
  REAL, SAVE                                          :: swamp_cst                   !! Fraction of the river transport that flows to the swamps (unitless;0-1)
!$OMP THREADPRIVATE(swamp_cst)
  !
  !  Relation between volume and fraction of floodplains
  !
  REAL, SAVE                                          :: beta = 2.0                  !! Parameter to fix the shape of the floodplain (>1 for convex edges, <1 for concave edges) (unitless)
!$OMP THREADPRIVATE(beta)
  REAL, SAVE                                          :: betap = 0.5                 !! Ratio of the basin surface intercepted by ponds and the maximum surface of ponds (unitless;0-1)
!$OMP THREADPRIVATE(betap)
  REAL, SAVE                                          :: floodcri = 2000.0           !! Potential height for which all the basin is flooded (mm)
!$OMP THREADPRIVATE(floodcri)
  !
  !  Relation between maximum surface of ponds and basin surface, and drainage (mm/j) to the slow_res
  !
  REAL, PARAMETER                                     :: pond_bas = 50.0             !! [DISPENSABLE] - not used
  REAL, SAVE                                          :: pondcri = 2000.0            !! Potential height for which all the basin is a pond (mm)
!$OMP THREADPRIVATE(pondcri)
  !
  REAL, PARAMETER                                     :: maxevap_lake = 7.5/86400.   !! Maximum evaporation rate from lakes (kg/m^2/s)
  !
  REAL,SAVE                                           :: dt_routing                  !! Routing time step (s)
!$OMP THREADPRIVATE(dt_routing)
  !
  INTEGER, SAVE                                       :: diagunit = 87               !! Diagnostic file unit (unitless)
...
!$OMP THREADPRIVATE(doponds)
  ...
  !
  INTEGER, SAVE                                       :: num_largest                 !! Number of largest river basins which should be treated as independently as rivers
                                                                                            !! (not flow into ocean as diffusion coastal flow) (unitless)
!$OMP THREADPRIVATE(num_largest)
  REAL, SAVE                                          :: time_counter                !! Time counter (s)
!$OMP THREADPRIVATE(time_counter)
  REAL, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)     :: routing_area_loc            !! Surface of basin (m^2)
!$OMP THREADPRIVATE(routing_area_loc)
  REAL, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)     :: topo_resid_loc              !! Topographic index of the retention time (m)
!$OMP THREADPRIVATE(topo_resid_loc)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: route_togrid_loc            !! Grid into which the basin flows (unitless)
!$OMP THREADPRIVATE(route_togrid_loc)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: route_tobasin_loc           !! Basin in to which the water goes (unitless)
!$OMP THREADPRIVATE(route_tobasin_loc)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: route_nbintobas_loc         !! Number of basin into current one (unitless)
!$OMP THREADPRIVATE(route_nbintobas_loc)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: global_basinid_loc          !! ID of basin (unitless)
!$OMP THREADPRIVATE(global_basinid_loc)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: hydrodiag_loc               !! Variable to diagnose the hydrographs
!$OMP THREADPRIVATE(hydrodiag_loc)
  REAL, SAVE, ALLOCATABLE, TARGET, DIMENSION(:)       :: hydroupbasin_loc            !! The area upstream of the gauging station (m^2)
!$OMP THREADPRIVATE(hydroupbasin_loc)
  !
  ! parallelism
  REAL, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)     :: routing_area_glo            !! Surface of basin (m^2)
!$OMP THREADPRIVATE(routing_area_glo)
  REAL, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)     :: topo_resid_glo              !! Topographic index of the retention time (m)
!$OMP THREADPRIVATE(topo_resid_glo)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: route_togrid_glo            !! Grid into which the basin flows (unitless)
!$OMP THREADPRIVATE(route_togrid_glo)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: route_tobasin_glo           !! Basin in to which the water goes (unitless)
!$OMP THREADPRIVATE(route_tobasin_glo)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: route_nbintobas_glo         !! Number of basin into current one (unitless)
!$OMP THREADPRIVATE(route_nbintobas_glo)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: global_basinid_glo          !! ID of basin (unitless)
!$OMP THREADPRIVATE(global_basinid_glo)
  INTEGER, SAVE, ALLOCATABLE, TARGET, DIMENSION(:,:)  :: hydrodiag_glo               !! Variable to diagnose the hydrographs
!$OMP THREADPRIVATE(hydrodiag_glo)
  REAL, SAVE, ALLOCATABLE, TARGET, DIMENSION(:)       :: hydroupbasin_glo            !! The area upstream of the gauging station (m^2)
!$OMP THREADPRIVATE(hydroupbasin_glo)
  !
  REAL, SAVE, POINTER, DIMENSION(:,:)                 :: routing_area                !! Surface of basin (m^2)
!$OMP THREADPRIVATE(routing_area)
  REAL, SAVE, POINTER, DIMENSION(:,:)                 :: topo_resid                  !! Topographic index of the retention time (m)
!$OMP THREADPRIVATE(topo_resid)
  INTEGER, SAVE, POINTER, DIMENSION(:,:)              :: route_togrid                !! Grid into which the basin flows (unitless)
!$OMP THREADPRIVATE(route_togrid)
  INTEGER, SAVE, POINTER, DIMENSION(:,:)              :: route_tobasin               !! Basin in to which the water goes (unitless)
!$OMP THREADPRIVATE(route_tobasin)
  INTEGER, SAVE, POINTER, DIMENSION(:,:)              :: route_nbintobas             !! Number of basin into current one (unitless)
!$OMP THREADPRIVATE(route_nbintobas)
  INTEGER, SAVE, POINTER, DIMENSION(:,:)              :: global_basinid              !! ID of basin (unitless)
!$OMP THREADPRIVATE(global_basinid)
  INTEGER, SAVE, POINTER, DIMENSION(:,:)              :: hydrodiag                   !! Variable to diagnose the hydrographs
!$OMP THREADPRIVATE(hydrodiag)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: slowflow_diag               !! Diagnostic slow flow hydrographs (kg/dt)
!$OMP THREADPRIVATE(slowflow_diag)  
  REAL, SAVE, POINTER, DIMENSION(:)                   :: hydroupbasin                !! The area upstream of the gauging station (m^2)
!$OMP THREADPRIVATE(hydroupbasin)
  !
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: irrigated                   !! Area equipped for irrigation in each grid box (m^2)
!$OMP THREADPRIVATE(irrigated)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: floodplains                 !! Maximal surface which can be inundated in each grid box (m^2)
!$OMP THREADPRIVATE(floodplains)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: swamp                       !! Maximal surface of swamps in each grid box (m^2)
!$OMP THREADPRIVATE(swamp)
  !
  ! The reservoirs, also to be put into the restart file.
  !
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:)             :: fast_reservoir              !! Water amount in the fast reservoir (kg)
!$OMP THREADPRIVATE(fast_reservoir)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:)             :: slow_reservoir              !! Water amount in the slow reservoir (kg)
!$OMP THREADPRIVATE(slow_reservoir)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:)             :: stream_reservoir            !! Water amount in the stream reservoir (kg)
!$OMP THREADPRIVATE(stream_reservoir)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:)             :: flood_reservoir             !! Water amount in the floodplains reservoir (kg)
!$OMP THREADPRIVATE(flood_reservoir)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: lake_reservoir              !! Water amount in the lake reservoir (kg)
!$OMP THREADPRIVATE(lake_reservoir)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: pond_reservoir              !! Water amount in the pond reservoir (kg)
!$OMP THREADPRIVATE(pond_reservoir)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:)             :: flood_frac_bas              !! Flooded fraction per basin (unitless;0-1)
!$OMP THREADPRIVATE(flood_frac_bas)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: pond_frac                   !! Pond fraction per grid box (unitless;0-1)
!$OMP THREADPRIVATE(pond_frac)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: flood_height                !! Floodplain height (mm)
!$OMP THREADPRIVATE(flood_height)
  !
  ! The accumulated fluxes.
  !
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: floodout_mean               !! Accumulated flow out of floodplains (kg/m^2/dt)
!$OMP THREADPRIVATE(floodout_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: runoff_mean                 !! Accumulated runoff (kg/m^2/dt)
!$OMP THREADPRIVATE(runoff_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: drainage_mean               !! Accumulated drainage (kg/m^2/dt)
!$OMP THREADPRIVATE(drainage_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: transpot_mean               !! Mean potential transpiration from the plants (kg/m^2/dt)
!$OMP THREADPRIVATE(transpot_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: precip_mean                 !! Accumulated precipitation (kg/m^2/dt)
!$OMP THREADPRIVATE(precip_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: humrel_mean                 !! Mean soil moisture stress, mean root extraction potential (unitless)
!$OMP THREADPRIVATE(humrel_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: totnobio_mean               !! Mean last total fraction of no bio (unitless;0-1)
!$OMP THREADPRIVATE(totnobio_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: vegtot_mean                 !! Mean potentially vegetated fraction (unitless;0-1)
!$OMP THREADPRIVATE(vegtot_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: k_litt_mean                 !! Mean averaged conductivity for saturated infiltration in the 'litter' layer (kg/m^2/dt)
!$OMP THREADPRIVATE(k_litt_mean)
  !
  ! The averaged outflow fluxes.
  !
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: lakeinflow_mean              !! Mean lake inflow (kg/m^2/dt)
!$OMP THREADPRIVATE(lakeinflow_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: returnflow_mean              !! Mean water flow from lakes and swamps which returns to the grid box.
...
!$OMP THREADPRIVATE(returnflow_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: reinfiltration_mean          !! Mean water flow which returns to the grid box (kg/m^2/dt)
!$OMP THREADPRIVATE(reinfiltration_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: irrigation_mean              !! Mean irrigation flux.
...
!$OMP THREADPRIVATE(irrigation_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: riverflow_mean               !! Mean Outflow of the major rivers.
...
!$OMP THREADPRIVATE(riverflow_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: coastalflow_mean             !! Mean outflow on coastal points by small basins.
                                                                                             !! This is the water which flows in a disperse way into the ocean (kg/dt)
!$OMP THREADPRIVATE(coastalflow_mean)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: floodtemp                    !! Temperature to decide if floodplains work (K)
!$OMP THREADPRIVATE(floodtemp)
  INTEGER, SAVE                                       :: floodtemp_lev                !! Temperature level to decide if floodplains work (K)
!$OMP THREADPRIVATE(floodtemp_lev)
  !
  ! Diagnostic variables ... well sort of !
  !
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: irrig_netereq                !! Irrigation requirement (water requirements by the crop for its optimal growth (kg/m^2/dt)
!$OMP THREADPRIVATE(irrig_netereq)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: hydrographs                  !! Hydrographs at the outflow of the grid box for major basins (kg/dt)
!$OMP THREADPRIVATE(hydrographs)
  !
  ! Diagnostics for the various reservoirs we use (Kg/m^2)
  !
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: fast_diag                    !! Diagnostic for the fast reservoir (kg/m^2)
!$OMP THREADPRIVATE(fast_diag)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: slow_diag                    !! Diagnostic for the slow reservoir (kg/m^2)
!$OMP THREADPRIVATE(slow_diag)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: stream_diag                  !! Diagnostic for the stream reservoir (kg/m^2)
!$OMP THREADPRIVATE(stream_diag)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: flood_diag                   !! Diagnostic for the floodplain reservoir (kg/m^2)
!$OMP THREADPRIVATE(flood_diag)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: pond_diag                    !! Diagnostic for the pond reservoir (kg/m^2)
!$OMP THREADPRIVATE(pond_diag)
  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: lake_diag                    !! Diagnostic for the lake reservoir (kg/m^2)
!$OMP THREADPRIVATE(lake_diag)

  REAL, SAVE, ALLOCATABLE, DIMENSION(:)               :: mask_coast                   !! Mask with coastal gridcells on local grid(1/0)
!$OMP THREADPRIVATE(mask_coast)
  REAL, SAVE                                          :: max_lake_reservoir           !! Maximum limit of water in lake_reservoir [kg/m2]
  !$OMP THREADPRIVATE(max_lake_reservoir)
  INTEGER, SAVE                                       :: nb_coast_gridcells           !! Number of gridcells which can receive coastalflow
...
CONTAINS
  ...
  SUBROUTINE routing_initialize( kjit,       nbpt,           index,                 &
    ...
    INTEGER, INTENT(in)     :: index(nbpt)          !! Indices of the points on the map (unitless)
    INTEGER,INTENT(in)      :: rest_id              !! Restart file identifier (unitless)
    INTEGER,INTENT(in)      :: hist_id              !! Access to history file (unitless)
    INTEGER,INTENT(in)      :: hist2_id             !! Access to history file 2 (unitless)
    REAL, INTENT(in)        :: lalo(nbpt,2)         !! Vector of latitude and longitudes (beware of the order !)

    INTEGER, INTENT(in)     :: neighbours(nbpt,NbNeighb) !! Vector of neighbours for each grid point 
                                                           !! (1=N, 2=NE, 3=E, 4=SE, 5=S, 6=SW, 7=W, 8=NW) (unitless)
    REAL, INTENT(in)        :: resolution(nbpt,2)   !! The size of each grid box in X and Y (m)
    REAL, INTENT(in)        :: contfrac(nbpt)       !! Fraction of land in each grid box (unitless;0-1)
    REAL, INTENT(in)        :: stempdiag(nbpt,nslm) !! Diagnostic soil temperature profile

    !! 0.2 Output variables
    REAL, INTENT(out)       :: returnflow(nbpt)     !! The water flow from lakes and swamps which returns to the grid box.
                                                           !! This water will go back into the hydrol or hydrolc module to allow re-evaporation (kg/m^2/dt)
    REAL, INTENT(out)       :: reinfiltration(nbpt) !! Water flow from ponds and floodplains which returns to the grid box (kg/m^2/dt)
    REAL, INTENT(out)       :: irrigation(nbpt)     !! Irrigation flux. This is the water taken from the reservoirs and beeing put into the upper layers of the soil (kg/m^2/dt)
    REAL, INTENT(out)       :: riverflow(nbpt)      !! Outflow of the major rivers. The flux will be located on the continental grid but this should be a coastal point (kg/dt)

    REAL, INTENT(out)       :: coastalflow(nbpt)    !! Outflow on coastal points by small basins. This is the water which flows in a disperse way into the ocean (kg/dt)
    REAL, INTENT(out)       :: flood_frac(nbpt)     !! Flooded fraction of the grid box (unitless;0-1)
    REAL, INTENT(out)       :: flood_res(nbpt)      !! Diagnostic of water amount in the floodplains reservoir (kg)
    
    !! 0.3 Local variables
    REAL, DIMENSION(nbp_glo):: mask_coast_glo       !! Mask with coastal gridcells on global grid (1/0)
...
!_ ================================================================================================================================
    ...
    IF (ier /= 0) CALL ipslerr_p(3,'routing_inititalize','Pb in allocate for mask_coast','','')
    ...
    IF ( doswamps ) THEN
       ...
       IF (printlev>=3) WRITE(numout,*) 'Verification : range of swamp : ', MINVAL(swamp), MAXVAL(swamp) 
       IF ( .NOT. almaoutput ) THEN
...
SUBROUTINE routing_main(kjit, nbpt, index, &
    ...
    INTEGER, INTENT(in)     :: index(nbpt)          !! Indices of the points on the map (unitless)
    REAL, INTENT(in)        :: lalo(nbpt,2)         !! Vector of latitude and longitudes (beware of the order !)
    INTEGER, INTENT(in)     :: neighbours(nbpt,NbNeighb)   !! Vector of neighbours for each grid point (1=N, 2=NE, 3=E, 4=SE, 5=S, 6=SW, 7=W, 8=NW) (unitless)
    REAL, INTENT(in)        :: resolution(nbpt,2)   !! The size of each grid box in X and Y (m)
    REAL, INTENT(in)        :: contfrac(nbpt)       !! Fraction of land in each grid box (unitless;0-1)
    REAL, INTENT(in)        :: totfrac_nobio(nbpt)  !! Total fraction of no-vegetation (continental ice, lakes ...) (unitless;0-1)
    REAL, INTENT(in)        :: veget_max(nbpt,nvm)  !! Maximal fraction of vegetation (unitless;0-1)
    REAL, INTENT(in)        :: floodout(nbpt)       !! Grid-point flow out of floodplains (kg/m^2/dt)
    REAL, INTENT(in)        :: runoff(nbpt)         !! Grid-point runoff (kg/m^2/dt)
    REAL, INTENT(in)        :: drainage(nbpt)       !! Grid-point drainage (kg/m^2/dt)
    REAL, INTENT(in)        :: transpot(nbpt,nvm)   !! Potential transpiration of the vegetation (kg/m^2/dt)
    REAL, INTENT(in)        :: precip_rain(nbpt)    !! Rainfall (kg/m^2/dt)
    REAL, INTENT(in)        :: k_litt(nbpt)         !! Averaged conductivity for saturated infiltration in the 'litter' layer (kg/m^2/dt)
    REAL, INTENT(in)        :: humrel(nbpt,nvm)     !! Soil moisture stress, root extraction potential (unitless)
    REAL, INTENT(in)        :: stempdiag(nbpt,nslm) !! Diagnostic soil temperature profile
    REAL, INTENT(in)        :: reinf_slope(nbpt)    !! Coefficient which determines the reinfiltration ratio in the grid box due to flat areas (unitless;0-1)
    ...
    REAL, DIMENSION(1)      :: tmp_day              !! 
    REAL, DIMENSION(nbpt)   :: return_lakes         !! Water from lakes flowing back into soil moisture (kg/m^2/dt)

    INTEGER                 :: ig, jv               !! Indices (unitless)
    REAL, DIMENSION(nbpt)   :: tot_vegfrac_nowoody  !! Total fraction occupied by grass (0-1,unitless)

    REAL, DIMENSION(nbpt)   :: fast_diag_old        !! Reservoir in the beginning of the time step
    REAL, DIMENSION(nbpt)   :: slow_diag_old        !! Reservoir in the beginning of the time step
    REAL, DIMENSION(nbpt)   :: stream_diag_old      !! Reservoir in the beginning of the time step
    REAL, DIMENSION(nbpt)   :: lake_diag_old        !! Reservoir in the beginning of the time step
    REAL, DIMENSION(nbpt)   :: pond_diag_old        !! Reservoir in the beginning of the time step
    REAL, DIMENSION(nbpt)   :: flood_diag_old       !! Reservoir in the beginning of the time step
    ...
    REAL, DIMENSION(nbpt)   :: netflow_stream_diag  !! Input - Output flow to stream reservoir
    REAL, DIMENSION(nbpt)   :: netflow_fast_diag    !! Input - Output flow to fast reservoir
    REAL, DIMENSION(nbpt)   :: netflow_slow_diag    !! Input - Output flow to slow reservoir
...
!_ ================================================================================================================================
  ...
  SUBROUTINE routing_finalize( kjit, nbpt, rest_id, flood_frac, flood_res )
    ...
    INTEGER, INTENT(in)     :: nbpt                 !! Domain size (unitless)
    INTEGER,INTENT(in)      :: rest_id              !! Restart file identifier (unitless)
    REAL, INTENT(in)        :: flood_frac(nbpt)     !! Flooded fraction of the grid box (unitless;0-1)
    REAL, INTENT(in)        :: flood_res(nbpt)      !! Diagnostic of water amount in the floodplains reservoir (kg)
...
!_ ================================================================================================================================
    ...
    CALL restput_p (rest_id, 'routetogrid', nbp_glo, nbasmax, 1, kjit, REAL(route_togrid), 'scatter', nbp_glo, index_g)
    CALL restput_p (rest_id, 'routetobasin', nbp_glo, nbasmax, 1, kjit, REAL(route_tobasin), 'scatter', nbp_glo, index_g)
    CALL restput_p (rest_id, 'basinid', nbp_glo, nbasmax, 1, kjit, REAL(global_basinid), 'scatter', nbp_glo, index_g)
...
!! INPUT VARIABLES
    INTEGER, INTENT(in)                   :: kjit           !! Time step number (unitless)
    INTEGER, INTENT(in)                   :: nbpt           !! Domain size (unitless)
    INTEGER, DIMENSION (nbpt), INTENT(in) :: index          !! Indices of the points on the map (unitless)
    REAL, DIMENSION(nbpt,nslm),INTENT(in) :: stempdiag      !! Temperature profile in soil
    INTEGER, INTENT(in)                   :: rest_id        !! Restart file identifier (unitless)
    !
!! OUTPUT VARIABLES
    REAL, DIMENSION (nbpt),INTENT(out)    :: returnflow     !! The water flow from lakes and swamps which returns into the grid box.
                                                                   !! This water will go back into the hydrol or hydrolc module to allow re-evaporation (kg/m^2/dt)
    REAL, DIMENSION (nbpt),INTENT(out)    :: reinfiltration !! Water flow from ponds and floodplains which returns to the grid box (kg/m^2/dt)
    REAL, DIMENSION (nbpt),INTENT(out)    :: irrigation     !! Irrigation flux. This is the water taken from the reservoirs and beeing put into the upper layers of the soil.(kg/m^2/dt)
    REAL, DIMENSION (nbpt),INTENT(out)    :: riverflow      !! Outflow of the major rivers. The flux will be located on the continental grid but this should be a coastal point (kg/dt)
    REAL, DIMENSION (nbpt),INTENT(out)    :: coastalflow    !! Outflow on coastal points by small basins. This is the water which flows in a disperse way into the ocean (kg/dt)
    REAL, DIMENSION (nbpt),INTENT(out)    :: flood_frac     !! Flooded fraction of the grid box (unitless;0-1)
    REAL, DIMENSION (nbpt),INTENT(out)    :: flood_res      !! Diagnostic of water amount in the floodplains reservoir (kg)
    !
!! LOCAL VARIABLES
    CHARACTER(LEN=80)                            :: var_name       !! To store variables names for I/O (unitless)
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: tmp_real_g     !! A temporary real array for the integers
    REAL, DIMENSION(1)                    :: tmp_day        !!
    REAL                                  :: ratio          !! Diagnostic ratio to check that dt_routing is a multiple of dt_sechiba (unitless)
    REAL                                  :: totarea        !! Total area of basin (m^2)
    INTEGER                               :: ier, ig, ib, ipn(1) !! Indices (unitless)
...
!> This assumption should be re-discussed.
    ...
    IF (is_root_prc) THEN
       ...
       IF (tmp_day(1) == val_exp) THEN
          ! The variable was not found in restart file, initialize to zero
          time_counter = zero
       ELSE
...
!! INPUT VARIABLES
    INTEGER, INTENT(in)                   :: nbpt                      !! Domain size (unitless)
    REAL, INTENT (in)                     :: dt_routing                !! Routing time step (s)
    REAL, INTENT(in)                      :: lalo(nbpt,2)              !! Vector of latitude and longitudes
    REAL, INTENT(in)                      :: runoff(nbpt)              !! Grid-point runoff (kg/m^2/dt)
    REAL, INTENT(in)                      :: floodout(nbpt)            !! Grid-point flow out of floodplains (kg/m^2/dt)
    REAL, INTENT(in)                      :: drainage(nbpt)            !! Grid-point drainage (kg/m^2/dt)
    REAL, INTENT(in)                      :: vegtot(nbpt)              !! Potentially vegetated fraction (unitless;0-1)
    REAL, INTENT(in)                      :: totnobio(nbpt)            !! Other areas which can not have vegetation
    REAL, INTENT(in)                      :: transpot_mean(nbpt)       !! Mean potential transpiration of the vegetation (kg/m^2/dt)
    REAL, INTENT(in)                      :: precip(nbpt)              !! Rainfall (kg/m^2/dt)
    REAL, INTENT(in)                      :: humrel(nbpt)              !! Soil moisture stress, root extraction potential (unitless)
    REAL, INTENT(in)                      :: k_litt(nbpt)              !! Averaged conductivity for saturated infiltration in the 'litter' layer (kg/m^2/dt)
    REAL, INTENT(in)                      :: floodtemp(nbpt)           !! Temperature to decide if floodplains work (K)
    REAL, INTENT(in)                      :: reinf_slope(nbpt)         !! Coefficient which determines the reinfiltration ratio in the grid box due to flat areas (unitless;0-1)
    REAL, INTENT(out)                     :: lakeinflow(nbpt)          !! Water inflow to the lakes (kg/dt)
    !
!! OUTPUT VARIABLES
    REAL, INTENT(out)                     :: returnflow(nbpt)          !! The water flow from lakes and swamps which returns into the grid box.
                                                                              !! This water will go back into the hydrol or hydrolc module to allow re-evaporation (kg/m^2/dt_routing)
    REAL, INTENT(out)                     :: reinfiltration(nbpt)      !! Water flow from ponds and floodplains which returns to the grid box (kg/m^2/dt)
    REAL, INTENT(out)                     :: irrigation(nbpt)          !! Irrigation flux. This is the water taken from the reservoirs and beeing put into the upper layers of the soil (kg/m^2/dt_routing)
    REAL, INTENT(out)                     :: riverflow(nbpt)           !! Outflow of the major rivers. The flux will be located on the continental grid but this should be a coastal point (kg/dt_routing)
    REAL, INTENT(out)                     :: coastalflow(nbpt)         !! Outflow on coastal points by small basins. This is the water which flows in a disperse way into the ocean (kg/dt_routing)
    REAL, INTENT(out)                     :: hydrographs(nbpt)         !! Hydrographs at the outflow of the grid box for major basins (kg/dt)
    REAL, INTENT(out)                     :: slowflow_diag(nbpt)       !! Hydrographs of slow_flow = routed slow_flow for major basins (kg/dt)
    REAL, INTENT(out)                     :: flood_frac(nbpt)          !! Flooded fraction of the grid box (unitless;0-1)
    REAL, INTENT(out)                     :: flood_res(nbpt)           !! Diagnostic of water amount in the floodplains reservoir (kg)

    REAL, INTENT(out)                     :: netflow_stream_diag(nbpt) !! Input - Output flow to stream reservoir
    REAL, INTENT(out)                     :: netflow_fast_diag(nbpt)   !! Input - Output flow to fast reservoir
    REAL, INTENT(out)                     :: netflow_slow_diag(nbpt)   !! Input - Output flow to slow reservoir
    !
!! LOCAL VARIABLES
    REAL, DIMENSION(nbpt, nbasmax)        :: fast_flow                 !! Outflow from the fast reservoir (kg/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: slow_flow                 !! Outflow from the slow reservoir (kg/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: stream_flow               !! Outflow from the stream reservoir (kg/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: flood_flow                !! Outflow from the floodplain reservoir (kg/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: pond_inflow               !! Inflow to the pond reservoir (kg/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: pond_drainage             !! Drainage from pond (kg/m^2/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: flood_drainage            !! Drainage from floodplains (kg/m^2/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: return_swamp              !! Inflow to the swamp (kg/dt)
    !
    ! Irrigation per basin
    !
    REAL, DIMENSION(nbpt, nbasmax)        :: irrig_needs               !! Total irrigation requirement (water requirements by the crop for its optimal growth) (kg)
    REAL, DIMENSION(nbpt, nbasmax)        :: irrig_actual              !! Possible irrigation according to the water availability in the reservoirs (kg)
    REAL, DIMENSION(nbpt, nbasmax)        :: irrig_deficit             !! Amount of water missing for irrigation (kg)
    REAL, DIMENSION(nbpt, nbasmax)        :: irrig_adduct              !! Amount of water carried over from other basins for irrigation (kg)
    !
    REAL, DIMENSION(nbpt, 0:nbasmax+3)    :: transport                 !! Water transport between basins (kg/dt)
    REAL, DIMENSION(nbp_glo, 0:nbasmax+3) :: transport_glo             !! Water transport between basins (kg/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: floods                    !! Water flow in to the floodplains (kg/dt)
    REAL, DIMENSION(nbpt, nbasmax)        :: potflood                  !! Potential inflow to the swamps (kg/dt)
    REAL, DIMENSION(nbpt)                 :: tobeflooded               !! Maximal surface which can be inundated in each grid box (m^2)
    REAL, DIMENSION(nbpt)                 :: totarea                   !! Total area of basin (m^2)
    REAL, DIMENSION(nbpt)                 :: totflood                  !! Total amount of water in the floodplains reservoir (kg)
    REAL, DIMENSION(nbasmax)              :: pond_excessflow           !! 
    REAL                                  :: flow                      !! Outflow computation for the reservoirs (kg/dt)
    REAL                                  :: floodindex                !! Fraction of grid box area inundated (unitless;0-1)
    REAL                                  :: pondex                    !! 
    REAL                                  :: flood_frac_pot            !! Total fraction of the grid box which is flooded at optimum repartition (unitless;0-1)
    REAL                                  :: stream_tot                !! Total water amount in the stream reservoirs (kg)
    REAL                                  :: adduction                 !! Importation of water from a stream reservoir of a neighboring grid box (kg)
    REAL, DIMENSION(nbp_glo)              :: lake_overflow_g           !! Removed water from lake reservoir on global grid (kg/gridcell/dt_routing)
    REAL, DIMENSION(nbpt)                 :: lake_overflow             !! Removed water from lake reservoir on local grid (kg/gridcell/dt_routing)
    REAL, DIMENSION(nbpt)                 :: lake_overflow_coast       !! lake_overflow distributed on coast gridcells, only diag(kg/gridcell/dt_routing)
    REAL                                  :: total_lake_overflow       !! Sum of lake_overflow over full grid (kg)
    REAL, DIMENSION(8,nbasmax)            :: streams_around            !! Stream reservoirs of the neighboring grid boxes (kg)
    INTEGER, DIMENSION(8)                 :: igrd                      !! 
    INTEGER, DIMENSION(2)                 :: ff                        !! 
    INTEGER, DIMENSION(1)                 :: fi                        !! 
    INTEGER                               :: ig, ib, ib2, ig2          !! Indices (unitless)
    INTEGER                               :: rtg, rtb, in              !! Indices (unitless)
    INTEGER                               :: ier                       !! Error handling 
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: fast_flow_g               !! Outflow from the fast reservoir (kg/dt)
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: slow_flow_g               !! Outflow from the slow reservoir (kg/dt)
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: stream_flow_g             !! Outflow from the stream reservoir (kg/dt)
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: irrig_deficit_glo         !! Amount of water missing for irrigation (kg)
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: stream_reservoir_glo      !! Water amount in the stream reservoir (kg)
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: irrig_adduct_glo          !! Amount of water carried over from other basins for irrigation (kg)

    REAL, DIMENSION(nbpt, nbasmax)        :: netflow_stream            !! Input - Output flow to stream reservoir
    REAL, DIMENSION(nbpt, nbasmax)        :: netflow_fast              !! Input - Output flow to fast reservoir
    REAL, DIMENSION(nbpt, nbasmax)        :: netflow_slow              !! Input - Output flow to slow reservoir
...
!! INPUT VARIABLES
    ...
    !
!! OUTPUT VARIABLES
    REAL, INTENT(out)   :: return_lakes(nbpt) !! Water from lakes flowing back into soil moisture (kg/m^2/dt)
    !
!! LOCAL VARIABLES
    INTEGER             :: ig                 !! Indices (unitless)
    REAL                :: refill             !!
    REAL                :: total_area         !! Sum of all the surfaces of the basins (m^2)
...
!! INPUT VARIABLES
    ...
    !
!! LOCAL VARIABLES
    REAL, DIMENSION(nbpt)    :: nbrivers           !! Number of rivers in the grid (unitless)
    REAL, DIMENSION(nbpt)    :: basinmap           !! Map of basins (unitless)
    REAL, DIMENSION(nbp_glo) :: nbrivers_g         !! Number of rivers in the grid (unitless)
    REAL, DIMENSION(nbp_glo) :: basinmap_g         !! Map of basins (unitless)
...
!! INPUT VARIABLES
    INTEGER, INTENT(in)                   :: nbpt                !! Domain size  (unitless)
    INTEGER, INTENT(in)                   :: l_index(nbpt)       !! Indices of the points on the map (unitless)
    REAL, INTENT(in)                      :: lalo(nbpt,2)        !! Vector of latitude and longitudes (beware of the order !) 
    REAL, INTENT(in)                      :: resolution(nbpt,2)  !! The size of each grid box in X and Y (m)
    REAL, INTENT(in)                      :: contfrac(nbpt)      !! Fraction of land in each grid box (unitless;0-1)
    !
!! OUTPUT VARIABLES
    REAL, DIMENSION(nbpt), INTENT(out)    :: nbrivers            !! Number of rivers in the grid (unitless)
    REAL, DIMENSION(nbpt), INTENT(out)    :: basinmap            !! Map of basins (unitless)
    !
!! LOCAL VARIABLES
    INTEGER, DIMENSION(nbpt,nbasmax)      :: outids              !! IDs of river to which this basin contributes (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:,:)  :: pts                 !! List the points belonging to the basin (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:,:)  :: ptbas               !! List the basin number for this point (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:,:)  :: outpt               !! Outflow point for each basin (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:)    :: nb_pts              !! Number of points in the basin (unitless)
    REAL, ALLOCATABLE, DIMENSION(:)       :: totarea             !! Total area of basin (m^2)
    REAL, ALLOCATABLE, DIMENSION(:)       :: tmparea             !! 
    INTEGER, ALLOCATABLE, DIMENSION(:)    :: topids              !! The IDs of the first num_largest basins (unitless)
    ...
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: streams_nb         !! Number of streams in basin (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: streams_avehops    !! Average number of hops in streams (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: streams_minhops    !! Minimum number of hops in streams (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: streams_maxhops    !! Minimum number of hops in streams (unitless)
    REAL, ALLOCATABLE, DIMENSION(:)        :: streams_resid      !! Average residence time
    !
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: lbasin_area        !!
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: lbasin_uparea      !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: lrivercode         !!
    !
    INTEGER                                :: ig, ib, og, ob, ign, ibn, ff(1), ic, icc, nb_small, idbas, slen, ii !! Indices (unitless)
    INTEGER                                :: ier                !! Error handling
    CHARACTER(LEN=1)                              :: nn                 !!
    INTEGER                                :: name_found         !!
    !
    REAL                                   :: averesid           !!
    REAL, DIMENSION(nbasmax)               :: tmpbas             !!
    REAL, DIMENSION(nbpt,nbasmax)          :: areaupbasin        !!
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: sortedrivs         !!
    !
    ! Variables for the river coding
    !
    INTEGER                               :: longest_river       !!
    INTEGER                               :: nbmax               !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:)  :: allstreams          !!
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: upstreamchange      !!
    INTEGER, ALLOCATABLE, DIMENSION(:)    :: tstreams, tslen, tpts, tptbas, tcode !!
    REAL, ALLOCATABLE, DIMENSION(:)       :: tuparea             !!
    REAL, ALLOCATABLE, DIMENSION(:)       :: tupstreamchange     !!
    !
...
!_ ================================================================================================================================
    ...
    DO icc = 1, num_largest
       ...
       ELSE
          ...
          ELSE
             IF (printlev >=6) WRITE(numout,*) ">>> nb_small, ib, topids :", nb_small, ib, topids(ib)
...
!! INPUT VARIABLES
    ...
    INTEGER, DIMENSION(ip), INTENT(in)    :: tpts           !!
    INTEGER, DIMENSION(ip), INTENT(in)    :: tpbas          !!
    REAL, DIMENSION(ip), INTENT(in)       :: tuparea        !!
    INTEGER, DIMENSION(ip), INTENT(in)    :: tslen          !!
    INTEGER, DIMENSION(ip,ls), INTENT(in) :: allstreams     !!
    REAL, DIMENSION(ip,ls), INTENT(in)    :: upstreamchange !!
    !
!! OUTPUT VARIABLES
    INTEGER, DIMENSION(ip), INTENT(out)   :: streamcode     !!
    !
!! LOCAL VARIABLES
    INTEGER                               :: ilev, cntsubbas, ib, ic, i, it, ilevmax, imaxlen, nbzero !!
    INTEGER                               :: tstreamcode(ip)!!
    INTEGER                               :: indsubbas(ip)  !!
    INTEGER                               :: iw(ip)         !!
    INTEGER                               :: tdiff(ip)      !!
    INTEGER                               :: tmpjunc(4)     !!
    INTEGER                               :: junction(4)    !!
...
!! LOCAL VARIABLES
    ...
    INTEGER                              :: nbpt                     !! Domain size  (unitless)
    INTEGER, DIMENSION(num_largest)      :: nb_pts                   !! Number of points in the basin (unitless)
    INTEGER, DIMENSION(num_largest)      :: topids                   !! The IDs of the first num_largest basins (unitless)
    CHARACTER(LEN=25), DIMENSION(num_largest)   :: sorted_names             !! Names of the basins to be put into the file (unitless)
    INTEGER, DIMENSION(num_largest)      :: sortedrivs               !!
    INTEGER, DIMENSION(num_largest,nbpt) :: lbasin_index             !!
    REAL, DIMENSION(num_largest,nbpt)    :: lbasin_area              !!
    REAL, DIMENSION(num_largest,nbpt)    :: lbasin_uparea            !!
    INTEGER, DIMENSION(num_largest,nbpt) :: lrivercode               !!
    !
    INTEGER, DIMENSION(num_largest,2)    :: outpt                    !! Outflow point for each basin (unitless)
    INTEGER, DIMENSION(num_largest)      :: streams_nb               !! Number of streams in basin (unitless)
    INTEGER, DIMENSION(num_largest)      :: streams_avehops          !! Average number of hops in streams (unitless)
    INTEGER, DIMENSION(num_largest)      :: streams_minhops          !! Minimum number of hops in streams (unitless)
    INTEGER, DIMENSION(num_largest)      :: streams_maxhops          !! Minimum number of hops in streams (unitless)
    REAL, DIMENSION(num_largest)         :: streams_resid            !! Average residence time
    !
    INTEGER                              :: icc, fid, iret, ierr_tot, ib, ij, ik, i, j, lcc !! Indices (unitless)
    INTEGER                              :: nlonid, nlatid, varid, varid2, varid3
    INTEGER                              :: dims(2)                  !!
    ...
    !
    REAL                                 :: basinfrac(iim_g,jjm_g)   !!
    REAL                                 :: basinuparea(iim_g,jjm_g) !!
    INTEGER                              :: basincode(iim_g,jjm_g)   !!
    !
    LOGICAL                                     :: check=.FALSE.            !! (true/false)
    !
!! PARAMETERS
    INTEGER,PARAMETER                    :: kind_r_diag=NF90_REAL8   !!
    INTEGER,PARAMETER                    :: kind_i_diag=NF90_INT     !!
 ...
 &          TRIM(river_file_name),'(Solution ?)')
    ...
    DO icc = 1, num_largest
       ...
       IF ( nb_pts(ib) > 2 ) THEN
          ...
          !
          IF (check) WRITE(numout,*) "Define Fraction variable and add attributes"
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Units',  iret
             WRITE(numout,*) TRIM(NF90_STRERROR(iret))
             ierr_tot = ierr_tot + 1
          ENDIF
          ! Long name
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Long_Name', long_name, iret
             WRITE(numout,*) TRIM(NF90_STRERROR(iret))
             ierr_tot = ierr_tot + 1
          ENDIF
          ! Missing value
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Missing value', undef_sechiba, iret
             WRITE(numout,*) TRIM(NF90_STRERROR(iret))
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ib=sortedrivs(icc)
          IF ( check ) WRITE(numout,*) "Doing basin ", icc," corrsdponding to index = ", ib, "num_largest : ", num_largest
          !
          ! Nb of grid points in basin
          att_str='Nb_of_grid_points_in_basin'
          iret = NF90_PUT_ATT(fid, varid, att_str, nb_pts(ib))
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Nb of grid points in basin', nb_pts(ib), iret
             WRITE(numout,*) TRIM(NF90_STRERROR(iret))
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Longitude of outflow point', lalo(outpt(ib,1),2), iret
             WRITE(numout,*) TRIM(NF90_STRERROR(iret))
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Latitude of outflow point',  lalo(outpt(ib,1),1), iret
             WRITE(numout,*) TRIM(NF90_STRERROR(iret))
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Number of streams', streams_nb(ib), iret
             WRITE(numout,*) TRIM(NF90_STRERROR(iret))
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Total number of hops to go to the oceans ', streams_avehops(ib)*streams_nb(ib), iret
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Minimum number of hops to go tp the ocean for any stream', streams_minhops(ib), iret
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Average number of hops to go tp the ocean for any stream', streams_avehops(ib), iret
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Maximum number of hops to go tp the ocean for any stream', streams_maxhops(ib), iret
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             WRITE(numout,*) 'Average residence time in the basin', streams_resid(ib), iret
             ierr_tot = ierr_tot + 1
          ENDIF
          !
          IF (ierr_tot > 0 ) THEN
          ...
          !
          IF (check) WRITE(numout,*) "Define Upstream variable and add attributes"
          !
          ! Create a name more suitable for a variable in a netCDF file
          !
          ...
          !
          IF (check) WRITE(numout,*) "Define Pfafstetter codes variable and add attributes"
          !
          ...
          !
          IF (check) WRITE(numout,*) "END define"
          !
          iret = NF90_ENDDEF(fid)
          IF (iret /= NF90_NOERR) THEN
             CALL ipslerr_p (3,'routing_diagncfile', &
          ...
          !
          IF (check) WRITE(numout,*) "Put basinfrac"
          ...
          IF (check) WRITE(numout,*) "Put basinuparea"
          ...
          IF (check) WRITE(numout,*) "Put basincode"
    ...
    ENDDO
    !
    IF (check) WRITE(numout,*) "Close file"
    !
...
!! INPUT VARIABLES
    INTEGER, INTENT(in) :: nbpt               !! Domain size (unitless)
    REAL, INTENT(in)    :: lalo(nbpt,2)       !! Vector of latitude and longitudes (beware of the order !)
    INTEGER, INTENT(in) :: neighbours(nbpt,NbNeighb) !! Vector of neighbours for each grid point (1=North and then clockwise) (unitless)
    REAL, INTENT(in)    :: resolution(nbpt,2) !! The size of each grid box in X and Y (m)
    REAL, INTENT(in)    :: contfrac(nbpt)     !! Fraction of land in each grid box (unitless;0-1)

!_ ================================================================================================================================

!    INTEGER    :: neighbours_tmp(nbpt,8)
!    INTEGER :: i,j
...
!! LOCAL VARIABLES
    ...
    INTEGER                                :: iml, jml, lml, tml, fid, ib, ip, jp, fopt !! Indices (unitless)
    REAL                                   :: lev(1), date, dt, coslat
    INTEGER                                :: itau(1)               !!
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: trip                  !! The trip field (unitless)
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: basins                !! The basin field (unitless)
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: topoindex             !! Topographic index of the residence time (m)
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: hierarchy             !!
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: lat_rel               !!
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: lon_rel               !!
    !
    INTEGER                                :: nbi, nbj              !! Number of point in x and y within the grid (unitless)
    REAL                                   :: min_topoind           !! The current minimum of topographic index (m)
    REAL                                   :: max_basins            !!
    REAL                                   :: invented_basins       !!
    !
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: area_bx               !! Area of each small box in the grid box (m^2)
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: hierarchy_bx          !! Level in the basin of the point
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: lon_bx                !!
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: lat_bx                !!
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: topoind_bx            !! Topographic index of the residence time for each of the smaller boxes (m)
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: trip_bx               !! The trip field for each of the smaller boxes (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: basin_bx              !!
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: coast_pts             !! The coastal flow points (unitless)
    !
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: basin_count           !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: basin_id              !!
    REAL,  ALLOCATABLE, DIMENSION(:,:)     :: basin_area            !!
    REAL,  ALLOCATABLE, DIMENSION(:,:)     :: basin_hierarchy       !!
    REAL,  ALLOCATABLE, DIMENSION(:,:)     :: basin_topoind         !! Topographic index of the residence time for a basin (m)
    REAL,  ALLOCATABLE, DIMENSION(:,:)     :: fetch_basin           !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: basin_flowdir         !! Water flow directions in the basin (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: outflow_grid          !! Type of outflow on the grid box (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: outflow_basin         !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: inflow_number         !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: inflow_basin          !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: inflow_grid           !!
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: nbcoastal             !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: coastal_basin         !!
    !
    ! Interpolation help variables
    !
    INTEGER                                :: nix, njx              !!
    CHARACTER(LEN=30)                             :: callsign              !!
    REAL, ALLOCATABLE, DIMENSION(:,:,:)    :: resol_lu              !! Resolution
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: mask                  !! Mask to exclude some points (unitless)
    REAL, ALLOCATABLE, DIMENSION(:,:)      :: sub_area              !! Area on the fine grid (m^2)
    INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: sub_index             !! Indices of the points we need on the fine grid (unitless)
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: sub_pts               !! Number of high resolution points on this grid (unitless)
    INTEGER                                       :: ALLOC_ERR             !!
    LOGICAL                                       :: ok_interpol = .FALSE. !! Flag for interpolation (true/false)
    !
    INTEGER                                :: nb_basin              !! Number of sub-basins (unitless)
    INTEGER                                :: nwbas                 !!
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: basin_inbxid          !!
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: basin_sz              !!
    INTEGER, ALLOCATABLE, DIMENSION(:)     :: basin_bxout           !!
    INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: basin_pts             !!
    CHARACTER(LEN=7)                              :: fmt                   !!
    LOGICAL                                       :: debug = .FALSE.       !! (true/false)
    !
    INTEGER, DIMENSION(2)                  :: diagbox = (/ 1, 2 /)  !!
...
!_ ================================================================================================================================
    ...
    !
    nwbas = MAX(MAXVAL(sub_pts), NbNeighb+1)
    !
    ...
    DO ib =1, nbpt
       ...
       IF ( debug .AND. (COUNT(diagbox .EQ. ib) .GT. 0) ) THEN
          ...
          WRITE(numout,*) 'Neighbor options :',  neighbours(ib,1:NbNeighb)
    ...
    !
    CALL routing_linkup(nbpt, contfrac, neighbours, nwbas, basin_count, basin_area, basin_id, basin_flowdir, &
    ...
    !
    !
    IF (printlev >=3) WRITE(numout,*) "Start reducing the number of basins per grid to meet the required truncation."
    !
...
!!  INPUT VARIABLES
    INTEGER, INTENT(in)  :: nbpt                        !! Domain size (unitless)
    INTEGER, INTENT(in)  :: iml                         !! X resolution of the high resolution grid
    INTEGER, INTENT(in)  :: jml                         !! Y resolution of the high resolution grid
    INTEGER, INTENT(in)  :: ib                          !! Current basin (unitless)
    INTEGER, INTENT(in)  :: sub_pts(nbpt)               !! Number of high resolution points on this grid (unitless)
    INTEGER, INTENT(in)  :: sub_index(nbpt,nbvmax,2)    !! Indices of the points we need on the fine grid (unitless)
    ...
    REAL, INTENT(in)     :: sub_area(nbpt,nbvmax)       !! Area on the fine grid (m^2)
    REAL, INTENT(in)     :: lon_rel(iml,jml)            !!
    REAL, INTENT(in)     :: lat_rel(iml,jml)            !! coordinates of the fine grid
    REAL, INTENT(in)     :: lalo(nbpt,2)                !! Vector of latitude and longitudes (beware of the order !)
    REAL, INTENT(in)     :: resolution(nbpt,2)          !! The size of each grid box in X and Y (m)
    REAL, INTENT(in)     :: contfrac(nbpt)              !! Fraction of land in each grid box (unitless;0-1)
    REAL, INTENT(inout)  :: trip(iml,jml)               !! The trip field (unitless)
    REAL, INTENT(inout)  :: basins(iml,jml)             !! data on the fine grid
    REAL, INTENT(inout)  :: topoindex(iml,jml)          !! Topographic index of the residence time (m)
    REAL, INTENT(inout)  :: hierarchy(iml, jml)         !! data on the fine grid
    !
!!  OUTPUT VARIABLES
    INTEGER, INTENT(out) :: nbi, nbj                    !! Number of point in x and y within the grid (unitless)
    REAL, INTENT(out)    :: area_bx(nbvmax,nbvmax)      !! Area of each small box in the grid box (m^2)
    REAL, INTENT(out)    :: hierarchy_bx(nbvmax,nbvmax) !! Level in the basin of the point
    REAL, INTENT(out)    :: lon_bx(nbvmax,nbvmax)       !!
    REAL, INTENT(out)    :: lat_bx(nbvmax,nbvmax)       !!
    REAL, INTENT(out)    :: topoind_bx(nbvmax,nbvmax)   !! Topographic index of the residence time for each of the smaller boxes (m)
    INTEGER, INTENT(out) :: trip_bx(nbvmax,nbvmax)      !! The trip field for each of the smaller boxes (unitless)
    INTEGER, INTENT(out) :: basin_bx(nbvmax,nbvmax)     !!
    !
!! LOCAL VARIABLES
    INTEGER              :: ip, jp, ll(1), iloc, jloc   !! Indices (unitless)
    REAL                 :: lonstr(nbvmax*nbvmax)       !!
    REAL                 :: latstr(nbvmax*nbvmax)       !!
...
!! INPUT VARIABLES
    ...
    REAL, INTENT(inout)   :: coords(nb_in)     !!
    !
!! OUTPUT VARIABLES
    INTEGER, INTENT(out)  :: nb_out            !!
    !
!! LOCAL VARIABLES
    CHARACTER(LEN=2)             :: direction         !!
    INTEGER               :: ipos              !!
    REAL                  :: coords_tmp(nb_in) !!
    INTEGER, DIMENSION(1) :: ll                !!
    INTEGER               :: ind(nb_in)        !!
...
!_ ================================================================================================================================
  ...
  SUBROUTINE routing_findbasins(nbi, nbj, trip, basin, hierarchy, topoind, nb_basin, basin_inbxid, basin_sz,&
    ...
    !
!! INPUT VARIABLES
    INTEGER, INTENT(in)    :: nbi                          !! Number of point in x within the grid (unitless)
    INTEGER, INTENT(in)    :: nbj                          !! Number of point in y within the grid (unitless)
    REAL, INTENT(in)       :: hierarchy(:,:)               !!
    REAL, INTENT(in)       :: topoind(:,:)                 !! Topographic index of the residence time (m)
    !
    !  Modified
    INTEGER, INTENT(inout) :: trip(:,:)                    !! The trip field (unitless)
    INTEGER, INTENT(inout) :: basin(:,:)                   !!
    !
!! OUTPUT VARIABLES
    INTEGER, INTENT(out)   :: nb_basin                     !! Number of sub-basins (unitless)
    INTEGER, INTENT(out)   :: basin_inbxid(nbvmax)         !!
    INTEGER, INTENT(out)   :: basin_sz(nbvmax)             !!
    INTEGER, INTENT(out)   :: basin_bxout(nbvmax)          !!
    INTEGER, INTENT(out)   :: basin_pts(nbvmax, nbvmax, 2) !!
    INTEGER, INTENT(out)   :: coast_pts(nbvmax)            !! The coastal flow points (unitless)
    !
!! LOCAL VARIABLES
    INTEGER                :: ibas, ilf, nbb, nb_in        !!
    INTEGER                :: bname(nbvmax)                !!
    INTEGER                :: sz(nbvmax)                   !!
    INTEGER                :: pts(nbvmax,nbvmax,2)         !!
    INTEGER                :: nbout(nbvmax)                !!
    INTEGER                :: new_nb                       !!
    INTEGER                :: new_bname(nbvmax)            !!
    INTEGER                :: new_sz(nbvmax)               !!
    INTEGER                :: new_pts(nbvmax,nbvmax,2)     !!
    INTEGER                :: itrans                       !!
    INTEGER                :: trans(nbvmax)                !!
    INTEGER                :: outdir(nbvmax)               !!
    INTEGER                :: tmpsz(nbvmax)                !!
    INTEGER                :: ip, jp, jpp(1), ipb          !!
    INTEGER                :: sortind(nbvmax)              !!
    CHARACTER(LEN=7)              :: fmt                          !!
...
SUBROUTINE routing_simplify(nbi, nbj, trip, basin, hierarchy, basin_inbxid)
    ...
    !
!! LOCAL VARIABLES
    INTEGER                             :: nbi                        !! Number of point in x within the grid (unitless)
    INTEGER                             :: nbj                        !! Number of point in y within the grid (unitless)
    INTEGER                             :: trip(:,:)                  !! The trip field (unitless)
    INTEGER                             :: basin(:,:)                 !!
    REAL                                :: hierarchy(:,:)             !!
    INTEGER                             :: basin_inbxid               !!
    !
    INTEGER                             :: ip, jp, nbout, basin_sz, iborder !!
    INTEGER, DIMENSION(nbvmax,nbvmax)   :: trip_tmp                   !! Temporary trip field which only contains the values for the basin on which we currently work (1)
    INTEGER, DIMENSION(nbvmax,nbvmax,2) :: trip_flow                  !!
    INTEGER, DIMENSION(nbvmax,2)        :: outflow                    !!
    INTEGER, DIMENSION(nbvmax)          :: outsz                      !!
    CHARACTER(LEN=7)                           :: fmt                        !!
    !
    INTEGER, DIMENSION(8,2)             :: inc                        !!
    INTEGER                             :: itodo, ill(1), icc, ismall, ibas, iip, jjp, ib, id !! Indices (unitless)
    INTEGER, DIMENSION(nbvmax)          :: todopt                     !!
!!$, todosz
    REAL, DIMENSION(nbvmax)             :: todohi                     !!
...
SUBROUTINE routing_cutbasin (nbi, nbj, nbbasins, trip, basin, basin_inbxid, nb, bname, sz, pts)
    ...
    !
!! INPUT VARIABLES
    INTEGER, INTENT(in)                 :: nbi, nbj             !! Number of point in x and y within the grid (unitless)
    INTEGER, INTENT(in)                 :: nbbasins             !!
    INTEGER, INTENT(in)                 :: basin_inbxid         !!
    !
    !  Modified
    INTEGER, INTENT(inout)              :: trip(:,:)            !! The trip field (unitless)
    INTEGER, INTENT(inout)              :: basin(:,:)           !!
...
!! OUTPUT VARIABLES
    ...
    INTEGER, INTENT(out)                :: bname(nbvmax)        !!
    INTEGER, INTENT(out)                :: sz(nbvmax)           !!
    INTEGER, INTENT(out)                :: pts(nbvmax,nbvmax,2) !!
    !
!! LOCAL VARIABLES
    INTEGER                             :: ip, jp, iip, jjp, ib, ibb, id, nbout !! Indices (unitless)
    INTEGER                             :: basin_sz             !!
    INTEGER                             :: nb_in                !!
    INTEGER, DIMENSION(nbvmax,nbvmax)   :: trip_tmp             !! Temporary trip field which only contains the values for the basin on which we currently work (unitless)
    INTEGER, DIMENSION(nbvmax,nbvmax,2) :: trip_flow            !!
    INTEGER, DIMENSION(nbvmax,2)        :: outflow              !!
    INTEGER, DIMENSION(nbvmax)          :: outsz                !!
    CHARACTER(LEN=7)                           :: fmt                  !!
...
SUBROUTINE routing_hierarchy(iml, jml, trip, topoindex, hierarchy)
    ...
    !
!! LOCAL VARIABLES
    INTEGER                  :: iml          !! X resolution of the high resolution grid
    INTEGER                  :: jml          !! Y resolution of the high resolution grid
    REAL, DIMENSION(iml,jml) :: trip         !! The trip field (unitless)
    REAL, DIMENSION(iml,jml) :: hierarchy    !!
    REAL, DIMENSION(iml,jml) :: topoindex    !! Topographic index of the residence time (m)
    !
    INTEGER, DIMENSION(8,2)  :: inc          !!
    INTEGER                  :: ip, jp, ib, ntripi, ntripj, cnt, trp !!
    REAL                     :: topohier     !! The new value of topographically weighted hierarchy (m)
    REAL                     :: topohier_old !! The old value of topographically weighted hierarchy (m)
    CHARACTER(LEN=7)                :: fmt          !!
...
!! INPUT VARIABLES
    INTEGER                                          :: nbi       !! Number of point in x within the grid (unitless)
    INTEGER                                          :: nbj       !! Number of point in y within the grid (unitless)
    INTEGER, DIMENSION(nbvmax,nbvmax)                :: trip      !! The trip field (unitless)
    INTEGER                                          :: basin_sz  !!
    INTEGER                                          :: basinid   !!
    !
!! OUTPUT VARIABLES
    INTEGER, DIMENSION(nbvmax,2), INTENT(out)        :: outflow   !!
    INTEGER, DIMENSION(nbvmax,nbvmax,2), INTENT(out) :: trip_flow !!
    INTEGER, INTENT(out)                             :: nbout     !! 
    INTEGER, DIMENSION(nbvmax), INTENT(out)          :: outsz     !!
    !
!! LOCAL VARIABLES
    INTEGER, DIMENSION(8,2)                          :: inc       !!
    INTEGER                                          :: ip, jp, ib, cnt, trp, totsz !! Indices (unitless)
    CHARACTER(LEN=7)                                        :: fmt       !!
...
!! LOCAL VARIABLES 
    REAL, DIMENSION(nbvmax,nbvmax)      :: area_bx                !! Area of each small box in the grid box (m^2)
    INTEGER, DIMENSION(nbvmax,nbvmax)   :: trip_bx                !! The trip field for each of the smaller boxes (unitless)
    REAL, DIMENSION(nbvmax,nbvmax)      :: hierarchy_bx           !! Level in the basin of the point
    REAL, DIMENSION(nbvmax,nbvmax)      :: topoind_bx             !! Topographic index of the residence time for each of the smaller boxes (m)
    REAL                                :: min_topoind            !! The current minimum of topographic index (m)
    INTEGER                             :: nb_basin               !! Number of sub-basins (unitless)
    INTEGER, DIMENSION(nbvmax)          :: basin_inbxid, basin_sz !! ID of basin, number of points in the basin
    INTEGER, DIMENSION(nbvmax,nbvmax,2) :: basin_pts              !! Points in each basin
    INTEGER, DIMENSION(nbvmax)          :: basin_bxout            !! outflow direction
    INTEGER                             :: coast_pts(nbvmax)      !! The coastal flow points (unitless)
    ! global maps
    INTEGER                             :: nwbas                  !!
    INTEGER, DIMENSION(nbpt)            :: basin_count            !!
    INTEGER, DIMENSION(nbpt,nwbas)      :: basin_id               !!
    INTEGER, DIMENSION(nbpt,nwbas)      :: basin_flowdir          !! Water flow directions in the basin (unitless)
    REAL, DIMENSION(nbpt,nwbas)         :: basin_area             !!
    REAL, DIMENSION(nbpt,nwbas)         :: basin_hierarchy        !!
    REAL, DIMENSION(nbpt,nwbas)         :: basin_topoind          !! Topographic index of the residence time for a basin (m)
    INTEGER, DIMENSION(nbpt,nwbas)      :: outflow_grid           !! Type of outflow on the grid box (unitless)
    INTEGER, DIMENSION(nbpt)            :: nbcoastal              !!
    INTEGER, DIMENSION(nbpt,nwbas)      :: coastal_basin          !!
    !
    INTEGER                             :: ij, iz                 !! Indices (unitless)
...
!_ ================================================================================================================================
    ...
    DO ij=1, nb_basin
       ...
       ENDDO
       !
       basin_topoind(ib,ij) = basin_topoind(ib,ij)/basin_sz(ij)
       !
       SELECT CASE (hierar_method)
          !
          CASE("MEAN")
             basin_hierarchy(ib,ij) = basin_hierarchy(ib,ij)/basin_sz(ij)
          !
       END SELECT
       !
...
SUBROUTINE routing_linkup(nbpt, contfrac, neighbours, nwbas, basin_count, basin_area, basin_id, basin_flowdir, &
...
!! INPUT VARIABLES
    INTEGER, INTENT (in)                    :: nbpt                  !! Domain size  (unitless)
    REAL, DIMENSION(nbpt)                   :: contfrac
    INTEGER, DIMENSION(nbpt,NbNeighb), INTENT (in) :: neighbours            !!
    REAL, INTENT(in)                        :: invented_basins       !!
    !
    INTEGER                                 :: nwbas                 !!
    INTEGER, DIMENSION(nbpt)                :: basin_count           !!
    INTEGER, DIMENSION(nbpt,nwbas)          :: basin_id              !!
    ...
    REAL, DIMENSION(nbpt,nwbas)             :: basin_hierarchy       !!
    INTEGER, DIMENSION(nbpt,nwbas)          :: outflow_grid          !! Type of outflow on the grid box (unitless)
    INTEGER, DIMENSION(nbpt,nwbas)          :: outflow_basin         !!
    INTEGER, DIMENSION(nbpt,nwbas)          :: inflow_number         !!
    INTEGER, DIMENSION(nbpt,nwbas,nbvmax)   :: inflow_basin          !!
    INTEGER, DIMENSION(nbpt,nwbas,nbvmax)   :: inflow_grid           !!
    INTEGER, DIMENSION(nbpt)                :: nbcoastal             !!
    INTEGER, DIMENSION(nbpt,nwbas)          :: coastal_basin         !!
    !
!! LOCAL VARIABLES
    INTEGER                                 :: sp, sb, sbl, inp, bid, outdm1, outdp1 !! Indices (unitless)
    INTEGER                                 :: dp1, dm1, dm1i, dp1i, bp1, bm1 !! Indices (unitless)
    INTEGER                                 :: dop, bop              !!
    INTEGER                                 :: fbas(nwbas), nbfbas   !!
    REAL                                    :: fbas_hierarchy(nwbas) !!
    REAL                                    :: angle
    INTEGER                                 :: ff(1)                 !!
    !
    ! ERRORS 
    ...
    DO sp=1,nbpt
       DO sb=1,basin_count(sp)
          ...
          IF ( inp .GT. 0 ) THEN
             ...
             IF (nbfbas .GE. 1) THEN
                ...
                IF ( basin_hierarchy(inp,sbl) .LE. basin_hierarchy(sp,sb) ) THEN
                   IF ( basin_hierarchy(inp,sbl) .LT. basin_hierarchy(sp,sb) ) THEN
                      bop = sbl
                   ELSE
          ...
          IF ( outflow_basin(sp,sb) .EQ. undef_int &
             ...
             !
             dp1i = MOD(basin_flowdir(sp,sb)+1-1, NbNeighb)+1
             dp1 = neighbours(sp,dp1i)
             dm1i = MOD(basin_flowdir(sp,sb)+7-1, NbNeighb)+1
             IF ( dm1i .LT. 1 ) dm1i = 8
             ...
             ELSE
                ...
                IF ( dop .EQ. undef_int .AND. bop .EQ. undef_int ) THEN
                   IF (check) THEN
                      ...
                      WRITE(numout,*) 'Coordinates : (lon,lat) = ', lalo_g(sp,2), lalo_g(sp,1)
                      WRITE(numout,*) 'neighbours :', neighbours_g(sp,:)
                      ...
                      WRITE(numout,*) 'Coodinates outflow : (lon,lat) = ', lalo_g(inp,2), lalo_g(inp,1)
...
!! INPUT VARIABLES
    INTEGER, INTENT(in)                           :: nbpt          !! Domain size  (unitless)
    !
    REAL, DIMENSION(nbpt,2), INTENT(in)           :: resolution    !! The size of each grid box in X and Y (m)
    REAL, DIMENSION(nbpt), INTENT(in)             :: contfrac      !! Fraction of land in each grid box (unitless;0-1)
    !
    INTEGER                                       :: nwbas         !!
    INTEGER, DIMENSION(nbpt), INTENT(in)          :: basin_count   !!
    REAL, DIMENSION(nbpt,nwbas), INTENT(inout)    :: basin_area    !!
    INTEGER, DIMENSION(nbpt,nwbas), INTENT(in)    :: basin_id      !!
    INTEGER, DIMENSION(nbpt,nwbas), INTENT(inout) :: outflow_grid  !! Type of outflow on the grid box (unitless)
    INTEGER, DIMENSION(nbpt,nwbas), INTENT(in)    :: outflow_basin !!
!
!! OUTPUT VARIABLES
    REAL, DIMENSION(nbpt,nwbas), INTENT(out)      :: fetch_basin   !!
    !
!! LOCAL VARIABLES
    INTEGER                                        :: ib, ij, ff(1), it, itt, igrif, ibasf, nboutflow !! Indices (unitless)
    REAL                                           :: contarea     !!
    REAL                                           :: totbasins    !!
    REAL, DIMENSION(nbpt*nbvmax)                   :: tmp_area     !!
    INTEGER, DIMENSION(nbpt*nbvmax,2)              :: tmpindex     !!
...
SUBROUTINE routing_truncate(nbpt, resolution, contfrac, nwbas, basin_count, basin_area, basin_topoind,&
    ...
    !
!! PARAMETERS
    INTEGER, PARAMETER                       :: pickmax = 200  !!

!! INPUT VARIABLES
    INTEGER                                  :: nbpt           !! Domain size  (unitless)
    !
    REAL, DIMENSION(nbpt,2)                  :: resolution     !! The size of each grid box in X and Y (m)
    REAL, DIMENSION(nbpt), INTENT(in)        :: contfrac       !! Fraction of land in each grid box (unitless;0-1)
    !
    INTEGER                                  :: nwbas          !!
    INTEGER, DIMENSION(nbpt)                 :: basin_count    !!
    ...
    REAL, DIMENSION(nbpt,nwbas)              :: basin_area     !!
    REAL, DIMENSION(nbpt,nwbas)              :: basin_topoind  !! Topographic index of the residence time for a basin (m)
    REAL, DIMENSION(nbpt,nwbas)              :: fetch_basin    !!
    INTEGER, DIMENSION(nbpt,nwbas)           :: outflow_grid   !! Type of outflow on the grid box (unitless)
    INTEGER, DIMENSION(nbpt,nwbas)           :: outflow_basin  !!
    INTEGER, DIMENSION(nbpt,nwbas)           :: inflow_number  !!
    INTEGER, DIMENSION(nbpt,nwbas,nwbas)     :: inflow_basin   !!
    INTEGER, DIMENSION(nbpt,nwbas,nwbas)     :: inflow_grid    !!
    !
!! LOCAL VARIABLES
    INTEGER                                  :: ib, ij, ibf, ijf, igrif, ibasf, cnt, pold, bold, ff(2) !! Indices (unitless)
    INTEGER                                  :: ii, kbas, sbas, ik, iter, ibt, obj !! Indices (unitless)
    REAL, DIMENSION(nbpt,nbasmax)            :: floflo         !!
    REAL, DIMENSION(nbpt)                    :: gridarea       !!
    REAL, DIMENSION(nbpt)                    :: gridbasinarea  !!
    REAL                                     :: ratio          !!
    INTEGER, DIMENSION(pickmax,2)            :: largest_basins !!
    INTEGER, DIMENSION(pickmax)              :: tmp_ids        !!
    INTEGER                                  :: multbas        !!
    INTEGER                                  :: iml(1)         !! X resolution of the high resolution grid
    INTEGER, DIMENSION(pickmax)              :: multbas_sz     !!
    REAL, DIMENSION(pickmax)                 :: tmp_area       !!
    INTEGER, DIMENSION(pickmax,pickmax)      :: multbas_list   !!
    !
    INTEGER                                  :: nbtruncate     !!
    INTEGER, SAVE, ALLOCATABLE, DIMENSION(:) :: indextrunc     !!
       ...
       ELSE
          ...
          DO ib = 1,nbpt
             DO WHILE ( basin_count(ib) .GT. nbasmax )
                !
                IF (printlev>=3) WRITE(numout,*) 'HAMMER, ib, basin_count :', ib, basin_count(ib)
                !
...
SUBROUTINE routing_killbas(nbpt, ib, tokill, totakeover, nwbas, basin_count, basin_area, basin_topoind,&
    ...
    INTEGER                              :: totakeover    !!
    INTEGER                              :: nbpt          !! Domain size  (unitless)
    INTEGER                              :: ib            !! Current basin (unitless)
    !
    INTEGER                              :: nwbas         !!
    INTEGER, DIMENSION(nbpt)             :: basin_count   !!
    INTEGER, DIMENSION(nbpt,nwbas)       :: basin_id      !!
    ...
    !
!! LOCAL VARIABLES
    INTEGER                              :: inf, ibs, ing, inb, ibasf, igrif, it !! Indices (unitless)
...
SUBROUTINE routing_irrigmap (nbpt, index, lalo, neighbours, resolution, contfrac, &
    ...
    !
!! PARAMETERS
    INTEGER, PARAMETER                      :: ilake = 1             !! Number of type of lakes area (unitless)
    INTEGER, PARAMETER                      :: idam = 2              !! Number of type of dams area (unitless)
    INTEGER, PARAMETER                      :: iflood = 3            !! Number of type of floodplains area (unitless)
    INTEGER, PARAMETER                      :: iswamp = 4            !! Number of type of swamps area (unitless)
    INTEGER, PARAMETER                      :: isal = 5              !! Number of type of salines area (unitless)
    INTEGER, PARAMETER                      :: ipond = 6             !! Number of type of ponds area (unitless)
    INTEGER, PARAMETER                      :: ntype = 6             !! Number of types of flooded surfaces (unitless)

!! INPUT VARIABLES
    INTEGER, INTENT(in)                     :: nbpt                  !! Domain size  (unitless)
    INTEGER, INTENT(in)                     :: index(nbpt)           !! Index on the global map.
    REAL, INTENT(in)                        :: lalo(nbpt,2)          !! Vector of latitude and longitudes (beware of the order !)
    INTEGER, INTENT(in)                     :: neighbours(nbpt,NbNeighb)!! Vector of neighbours for each grid point
    REAL, INTENT(in)                        :: resolution(nbpt,2)    !! The size of each grid box in X and Y (m)
    REAL, INTENT(in)                        :: contfrac(nbpt)        !! Fraction of land in each grid box (unitless;0-1)
    ...
    !
!! OUTPUT VARIABLES
    REAL, INTENT(out)                       :: irrigated(:)          !! Irrigated surface in each grid box (m^2)
    REAL, INTENT(out)                       :: floodplains(:)        !! Surface which can be inundated in each grid box (m^2)
    REAL, INTENT(out)                       :: swamp(:)              !! Surface which can be swamp in each grid box (m^2)
    !
!! LOCAL VARIABLES
    ! Interpolation variables
    ! 
    INTEGER                                 :: nbpmax, nix, njx, fopt !!
    CHARACTER(LEN=30)                              :: callsign              !!
    REAL, ALLOCATABLE, DIMENSION(:,:,:)     :: resol_lu              !! Resolution read on the map
    INTEGER, ALLOCATABLE, DIMENSION(:,:)    :: mask                  !! Mask to exclude some points (unitless)
    REAL, ALLOCATABLE, DIMENSION(:,:)       :: irrsub_area           !! Area on the fine grid (m^2)
    INTEGER, ALLOCATABLE, DIMENSION(:,:,:)  :: irrsub_index          !! Indices of the points we need on the fine grid (unitless)
    INTEGER                                        :: ALLOC_ERR             !!
    LOGICAL                                        :: ok_interpol = .FALSE. !! Flag for interpolation (true/false)
    !
    CHARACTER(LEN=80)                              :: filename              !! Name of the netcdf file (unitless)
    INTEGER                                 :: iml, jml, lml, tml, fid, ib, ip, jp, itype !! Indices (unitless)
    REAL                                    :: lev(1), date, dt, coslat !!
    INTEGER                                 :: itau(1)               !!
    REAL, ALLOCATABLE, DIMENSION(:,:)       :: latrel                !! Latitude
    REAL, ALLOCATABLE, DIMENSION(:,:)       :: lonrel                !! Longitude
    REAL, ALLOCATABLE, DIMENSION(:,:)       :: irrigated_frac        !! Irrigated fraction of the grid box (unitless;0-1)
    REAL, ALLOCATABLE, DIMENSION(:,:,:)     :: flood_fracmax         !! Maximal flooded fraction of the grid box (unitless;0-1)
    REAL                                    :: area_irrig            !! Irrigated surface in the grid box (m^2)
    REAL                                    :: area_flood(ntype)     !! Flooded surface in the grid box (m^2)
!!$    REAL                                :: irrigmap(nbpt)
!!$    REAL                                :: floodmap(nbpt)
!!$    REAL                                :: swampmap(nbpt)
...
!! INPUT VARIABLES
    ...
    REAL, INTENT(in)    :: runoff(nbpt)         !! Grid-point runoff (kg/m^2/dt)
    REAL, INTENT(in)    :: drainage(nbpt)       !! Grid-point drainage (kg/m^2/dt)
    REAL, INTENT(in)    :: returnflow(nbpt)     !! The water flow from lakes and swamps which returns to the grid box.
                                                       !! This water will go back into the hydrol or hydrolc module to allow re-evaporation (kg/m^2/dt)
    REAL, INTENT(in)    :: reinfiltration(nbpt) !! Water flow from ponds and floodplains which returns to the grid box (kg/m^2/dt)
    REAL, INTENT(in)    :: irrigation(nbpt)     !! Irrigation flux. This is the water taken from the reservoirs and beeing put into the upper layers of the soil (kg/m^2/dt)
    REAL, INTENT(in)    :: riverflow(nbpt)      !! Outflow of the major rivers. The flux will be located on the continental grid but this should be a coastal point (kg/dt)
    REAL, INTENT(in)    :: coastalflow(nbpt)    !! Outflow on coastal points by small basins. This is the water which flows in a disperse way into the ocean (kg/dt)
    !
    ! We sum-up all the water we have in the warious reservoirs
    !
    REAL, SAVE          :: totw_flood           !! Sum of all the water amount in the floodplains reservoirs (kg)
!$OMP THREADPRIVATE(totw_flood)
    REAL, SAVE          :: totw_stream          !! Sum of all the water amount in the stream reservoirs (kg)
!$OMP THREADPRIVATE(totw_stream)
    REAL, SAVE          :: totw_fast            !! Sum of all the water amount in the fast reservoirs (kg)
!$OMP THREADPRIVATE(totw_fast)
    REAL, SAVE          :: totw_slow            !! Sum of all the water amount in the slow reservoirs (kg)
!$OMP THREADPRIVATE(totw_slow)
    REAL, SAVE          :: totw_lake            !! Sum of all the water amount in the lake reservoirs (kg)
!$OMP THREADPRIVATE(totw_lake)
    REAL, SAVE          :: totw_pond            !! Sum of all the water amount in the pond reservoirs (kg)
!$OMP THREADPRIVATE(totw_pond)
    REAL, SAVE          :: totw_in              !! Sum of the water flow in to the routing scheme
!$OMP THREADPRIVATE(totw_in)
    REAL, SAVE          :: totw_out             !! Sum of the water flow out to the routing scheme
!$OMP THREADPRIVATE(totw_out)
    REAL, SAVE          :: totw_return          !! 
!$OMP THREADPRIVATE(totw_return)
    REAL, SAVE          :: totw_irrig           !! 
!$OMP THREADPRIVATE(totw_irrig)
    REAL, SAVE          :: totw_river           !! 
!$OMP THREADPRIVATE(totw_river)
    REAL, SAVE          :: totw_coastal         !! 
!$OMP THREADPRIVATE(totw_coastal)
    REAL                :: totarea              !! Total area of basin (m^2)
    REAL                :: area                 !! Total area of routing (m^2)
    INTEGER             :: ig                   !! 
    !
    ! Just to make sure we do not get too large numbers !
    !
!! PARAMETERS
    REAL, PARAMETER     :: scaling = 1.0E+6     !!
    REAL, PARAMETER     :: allowed_err = 50.    !!
