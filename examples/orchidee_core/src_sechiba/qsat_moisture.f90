...
MODULE qsat_moisture
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC qsatcalc, dev_qsatcalc
  PUBLIC snow3lhold_2d,  snow3lhold_1d,  snow3lhold_0d
  PUBLIC snow3lheat_2d,  snow3lheat_1d
  PUBLIC snow3lscap_2d,  snow3lscap_1d
  PUBLIC snow3ltemp_2d,  snow3ltemp_1d
  PUBLIC snow3lgrain_2d, snow3lgrain_1d, snow3lgrain_0d
  PUBLIC snow3lliq_2d,   snow3lliq_1d
...
!$OMP THREADPRIVATE(l_qsat_first)
  ...
  INTEGER,PARAMETER :: max_temp=370           !! Maximum temperature for saturated humidity (K) and also used as 
                                                     !! the size of local array to keep saturated humidity (unitless)

  INTEGER,PARAMETER :: min_temp=100           !! Minimum temperature for saturated humidity (K)

  REAL,DIMENSION(max_temp),SAVE :: qsfrict    !! Array to keep water vapor pressure at saturation for each temperature level 
                                                     !! (hPa)
...
!_ ================================================================================================================================
    ...
    SUBROUTINE qsatcalc (kjpindex,temp_in,pres_in,qsat_out)
      ...
      REAL,DIMENSION(kjpindex),INTENT(in)  :: temp_in  !! Temperature in degre Kelvin (K)
      REAL,DIMENSION(kjpindex),INTENT(in)  :: pres_in  !! Pressure (hPa)
      
      !! 0.2 Output variables 
      
      REAL,DIMENSION(kjpindex),INTENT(out) :: qsat_out !! Saturated humidity at the surface (kg of water/kg of air)
      
      !! 0.4 Local variables
      
      INTEGER, DIMENSION(kjpindex) :: jt               !! Temporary array stocking the truncated temperatures in Kelvin 
                                                              !!(converted into integers)
      INTEGER                      :: ji               !! indices (unitless)
      REAL,DIMENSION(kjpindex)     :: zz_a, zz_b, zz_f !! Temporary variables
      INTEGER                      :: nbad             !! Number of points where the temperature is too high or too low
      INTEGER,DIMENSION(1)         :: lo               !! Temporary vector to mark the position of the highest temperature
...
!_ ================================================================================================================================    
    ...
    FUNCTION qsat_old (temp_in,pres_in) RESULT (qsat_result)
      ...
      REAL,INTENT(in) :: pres_in   !! Pressure    (hPa)

      !! 0.2 Result
      
      REAL :: qsat_result          !! Saturated humidity calculated at the surface (kg/kg)
      
      !! 0.4 Local variables
      
      INTEGER   :: jt              !! Temporary scalar stocking the truncated temperature in Kelvin 
                                          !! (converted into integer)
      REAL      :: zz_a,zz_b,zz_f  !! Temporary scalar variables      
...
!_ ================================================================================================================================ 
    ...
    SUBROUTINE dev_qsatcalc (kjpindex,temp_in,pres_in,dev_qsat_out)
      ...
      INTEGER,INTENT(in)                   :: kjpindex       !! Domain size (unitless)
      REAL,DIMENSION(kjpindex),INTENT(in)  :: temp_in        !! Temperature (K)
      REAL,DIMENSION(kjpindex),INTENT(in)  :: pres_in        !! Pressure (hPa)


      !! 0.2 Output variables 
      
      REAL,DIMENSION(kjpindex),INTENT(out) :: dev_qsat_out   !! Result (??units??) 

      
      !! 0.4 Local variables
      
      INTEGER,DIMENSION(kjpindex) :: jt                      !! Temporary array stocking the truncated temperatures
                                                                    !! in Kelvin (converted into integers) 
      INTEGER                     :: ji                      !! Indice (unitless)
      REAL,DIMENSION(kjpindex)    :: zz_a, zz_b, zz_c, zz_f  !! Temporary vector variables
      INTEGER                     :: nbad                    !! Number of points where the temperature is too high or too low
...
!! ================================================================================================================================
!! FUNCTION 	: [DISPENSABLE] dev_qsat_old
!!
...
!_ ================================================================================================================================
    ...
    FUNCTION dev_qsat_old (temp_in,pres_in) RESULT (dev_qsat_result)
      ...
      REAL,INTENT(in)  :: pres_in          !! Pressure (hPa)
      REAL,INTENT(in)  :: temp_in          !! Temperture (K)
      
      !! 0.2 Result

      REAL :: dev_qsat_result              !! (??units??) !!
      
      !! 0.4 Local variables

      INTEGER  :: jt                       !! Index (unitless)
      REAL     :: zz_a, zz_b, zz_c, zz_f   !! Temporary scalars   
...
!_ ================================================================================================================================
    ...
    SUBROUTINE qsfrict_init
      ...
      INTEGER :: ji                             !! Indice(unitless)
      REAL    :: zrapp,zcorr,ztemperature,zqsat !! Temporary vector variables      
...
!================================================================================================================================
  ...
  FUNCTION snow3lhold_2d(PSNOWRHO,PSNOWDZ) RESULT(PWHOLDMAX)
    ...
    !! 0.2 Output variables
    REAL, DIMENSION(SIZE(PSNOWRHO,1),SIZE(PSNOWRHO,2)) :: PWHOLDMAX  !! Maximum Water holding capacity
...
!================================================================================================================================
  FUNCTION snow3lgrain_2d(PSNOWRHO) RESULT(PDSGRAIN)
    ...
    !! 0.2 Output variables
    REAL, DIMENSION(SIZE(PSNOWRHO,1),SIZE(PSNOWRHO,2)) :: PDSGRAIN                   !! Snow grain size
...
!================================================================================================================================
  FUNCTION snow3lgrain_0d(PSNOWRHO) RESULT(PDSGRAIN)
    ...
    !! 0.1 Input variables
    REAL, INTENT(IN)               :: PSNOWRHO                  !! Snow density

    !! 0.2 Output variables
    REAL                           :: PDSGRAIN                  !! Snow grain size
