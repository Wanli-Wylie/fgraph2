...
!!                                                before the first call to intersurf_main_gathered.
...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_sechiba/intersurf.f90 $
...
!$OMP THREADPRIVATE(lstep_init_intersurf)
  INTEGER, SAVE                               :: printlev_loc            !! Write level to this module
!$OMP THREADPRIVATE(printlev_loc)
  INTEGER, SAVE                               :: hist_id, rest_id        !! IDs for history and restart files
!$OMP THREADPRIVATE(hist_id, rest_id)
  INTEGER, SAVE                               :: hist2_id                !! ID for the second history files (Hi-frequency ?)
!$OMP THREADPRIVATE(hist2_id)
  INTEGER, SAVE                               :: hist_id_stom, hist_id_stom_IPCC, rest_id_stom !! Dito for STOMATE
!$OMP THREADPRIVATE(hist_id_stom, hist_id_stom_IPCC, rest_id_stom)
  INTEGER, SAVE                               :: itau_offset  !! This offset is used to phase the 
                                                                     !! calendar of the GCM or the driver.
!$OMP THREADPRIVATE(itau_offset)
  REAL, SAVE                                  :: date0_shifted
...
!$OMP THREADPRIVATE(fatmco2)
  REAL, SAVE                                  :: atmco2            !! atmospheric CO2 
!$OMP THREADPRIVATE(atmco2)
  REAL, SAVE                                  :: coeff_rel         !! Coefficient for time filter on riverflow and coastalflow
!$OMP THREADPRIVATE(coeff_rel)
  REAL, ALLOCATABLE, DIMENSION(:), SAVE       :: riverflow_cpl0    !! Value from previous time step for riverflow
!$OMP THREADPRIVATE(riverflow_cpl0)
  REAL, ALLOCATABLE, DIMENSION(:), SAVE       :: coastalflow_cpl0  !! Value from previous time step for coastalflow
!$OMP THREADPRIVATE(coastalflow_cpl0)
  INTEGER, SAVE                               :: nb_fields_in=-1   !!  Number of fields to give to the GCM
!$OMP THREADPRIVATE(nb_fields_in)  
  INTEGER, SAVE                               :: nb_fields_out=-1  !!  Number of fields to get from the GCM
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE intersurf_initialize_2d (kjit, iim, jjm, kjpindex, kindex, xrdt, &
    ...
    INTEGER,INTENT (in)                            :: iim, jjm        !! Dimension of input fields
    INTEGER,INTENT (in)                            :: kjpindex        !! Number of continental points
    REAL,INTENT (in)                               :: xrdt            !! Time step in seconds
    ...
    REAL, INTENT (in)                              :: date0           !! Date at which kjit = 0
    INTEGER,DIMENSION (kjpindex), INTENT (in)      :: kindex          !! Index for continental points
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: u             !! Lowest level wind speed
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: v             !! Lowest level wind speed 
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: zlev          !! Height of first layer
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: qair          !! Lowest level specific humidity
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: precip_rain   !! Rain precipitation
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: precip_snow   !! Snow precipitation
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: lwdown        !! Down-welling long-wave flux 
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: swnet         !! Net surface short-wave flux
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: swdown        !! Downwelling surface short-wave flux
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: temp_air      !! Air temperature in Kelvin
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: epot_air      !! Air potential energy
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: ccanopy       !! CO2 concentration in the canopy
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: petAcoef      !! Coeficients A from the PBL resolution
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: peqAcoef      !! One for T and another for q
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: petBcoef      !! Coeficients B from the PBL resolution
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: peqBcoef      !! One for T and another for q
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: pb            !! Surface pressure
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: lon, lat      !! Geographical coordinates
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: zcontfrac     !! Fraction of continent in the grid
    REAL,DIMENSION (iim,jjm,2), INTENT(in)           :: zresolution   !! resolution in x and y dimensions

    !! 0.2 Output variables
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: z0m            !! Surface roughness
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: coastalflow   !! Diffuse flow of water into the ocean (m^3/s)
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: riverflow     !! Largest rivers flowing into the ocean (m^3/s)
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: tsol_rad      !! Radiative surface temperature
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: vevapp        !! Total of evaporation
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: temp_sol_new  !! New soil temperature
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: qsurf         !! Surface specific humidity
    REAL,DIMENSION (iim,jjm,2), INTENT(out)          :: albedo        !! Albedo
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: fluxsens      !! Sensible chaleur flux
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: fluxlat       !! Latent chaleur flux
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: emis          !! Emissivity

    !! 0.3 Modified variables
    REAL,DIMENSION (iim,jjm), INTENT(inout)          :: cdrag         !! Cdrag

    !! 0.4 Local variables
    REAL,DIMENSION (kjpindex)                      :: zu            !! Work array to keep u
    REAL,DIMENSION (kjpindex)                      :: zv            !! Work array to keep v
    REAL,DIMENSION (kjpindex)                      :: zzlev         !! Work array to keep zlev
    REAL,DIMENSION (kjpindex)                      :: zqair         !! Work array to keep qair
    REAL,DIMENSION (kjpindex)                      :: zprecip_rain  !! Work array to keep precip_rain
    REAL,DIMENSION (kjpindex)                      :: zprecip_snow  !! Work array to keep precip_snow
    REAL,DIMENSION (kjpindex)                      :: zlwdown       !! Work array to keep lwdown
    REAL,DIMENSION (kjpindex)                      :: zswnet        !! Work array to keep swnet
    REAL,DIMENSION (kjpindex)                      :: zswdown       !! Work array to keep swdown
    REAL,DIMENSION (kjpindex)                      :: ztemp_air     !! Work array to keep temp_air
    REAL,DIMENSION (kjpindex)                      :: zepot_air     !! Work array to keep epot_air
    REAL,DIMENSION (kjpindex)                      :: zccanopy      !! Work array to keep ccanopy
    REAL,DIMENSION (kjpindex)                      :: zpetAcoef     !! Work array to keep petAcoef
    REAL,DIMENSION (kjpindex)                      :: zpeqAcoef     !! Work array to keep peqAcoef
    REAL,DIMENSION (kjpindex)                      :: zpetBcoef     !! Work array to keep petBcoef
    REAL,DIMENSION (kjpindex)                      :: zpeqBcoef     !! Work array to keep peqVcoef
    REAL,DIMENSION (kjpindex)                      :: zcdrag        !! Work array to keep cdrag
    REAL,DIMENSION (kjpindex)                      :: zpb           !! Work array to keep pb
    REAL,DIMENSION (kjpindex)                      :: zz0m, zz0h    !! Work array to keep zz0m, zz0h
    REAL,DIMENSION (kjpindex)                      :: zcoastal      !! Work array to keep coastalflow (m^3/dt)
    REAL,DIMENSION (kjpindex)                      :: zriver        !! Work array to keep riverflow (m^3/dt)
    REAL,DIMENSION (kjpindex)                      :: znetco2       !! Work array to keep netco2flux
    REAL,DIMENSION (kjpindex)                      :: zcarblu       !! Work array to keep fco2_land_use
    REAL,DIMENSION (kjpindex)                      :: ztsol_rad     !! Work array to keep tsol_rad
    REAL,DIMENSION (kjpindex)                      :: zvevapp       !! Work array to keep vevapp
    REAL,DIMENSION (kjpindex)                      :: ztemp_sol_new !! Work array to keep temp_sol_new
    REAL,DIMENSION (kjpindex)                      :: zqsurf        !! Work array to keep qsurf
    REAL,DIMENSION (kjpindex,2)                    :: zalbedo       !! Work array to keep albedo
    REAL,DIMENSION (kjpindex)                      :: zfluxsens     !! Work array to keep fluxsens
    REAL,DIMENSION (kjpindex)                      :: zfluxlat      !! Work array to keep fluxlat
    REAL,DIMENSION (kjpindex)                      :: zemis         !! Work array to keep emis
    REAL,ALLOCATABLE, DIMENSION (:)                :: soilth_lev    !! Vertical soil axis for thermal scheme (m)
    INTEGER                                       :: i, j, ik
    INTEGER                                       :: ier
    INTEGER                                       :: itau_sechiba
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE intersurf_main_2d (kjit, iim, jjm, kjpindex, kindex, xrdt, &
    ...
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: swnet         !! Net surface short-wave flux
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: swdown        !! Downwelling surface short-wave flux
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: coszang       !! Cosine of the solar zenith angle (unitless)
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: temp_air      !! Air temperature in Kelvin
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: epot_air      !! Air potential energy
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: ccanopy       !! CO2 concentration in the canopy
    REAL,DIMENSION (iim,jjm), INTENT(in)             :: petAcoef      !! Coeficients A from the PBL resolution
    ...
    !! 0.2 Output variables
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: z0m            !! Surface roughness for momemtum
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: coastalflow   !! Diffuse flow of water into the ocean (m^3/s)
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: riverflow     !! Largest rivers flowing into the ocean (m^3/s)
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: tsol_rad      !! Radiative surface temperature
    REAL,DIMENSION (iim,jjm), INTENT(out)            :: vevapp        !! Total of evaporation
    ...
    REAL,DIMENSION (kjpindex)                      :: zswnet        !! Work array to keep swnet
    REAL,DIMENSION (kjpindex)                      :: zswdown       !! Work array to keep swdown
    REAL,DIMENSION (kjpindex)                      :: zcoszang      !! Work array to keep coszang
    REAL,DIMENSION (kjpindex)                      :: ztemp_air     !! Work array to keep temp_air
    REAL,DIMENSION (kjpindex)                      :: zepot_air     !! Work array to keep epot_air
    REAL,DIMENSION (kjpindex)                      :: zccanopy      !! Work array to keep ccanopy
    REAL,DIMENSION (kjpindex)                      :: zpetAcoef     !! Work array to keep petAcoef
    ...
    REAL,DIMENSION (kjpindex)                      :: zpb           !! Work array to keep pb
    REAL,DIMENSION (kjpindex)                      :: zz0m, zz0h    !! Work array to keep zz0m, zz0h
    REAL,DIMENSION (kjpindex,nvm)                  :: zveget        !! Work array to keep veget
    REAL,DIMENSION (kjpindex,nvm)                  :: zlai          !! Work array to keep lai
    REAL,DIMENSION (kjpindex,nvm)                  :: zheight       !! Work array to keep height
    REAL,DIMENSION (kjpindex)                      :: zcoastal      !! Work array to keep coastalflow (m^3/dt)
    REAL,DIMENSION (kjpindex)                      :: zriver        !! Work array to keep riverflow (m^3/dt)
    REAL,DIMENSION (kjpindex)                      :: znetco2       !! Work array to keep netco2flux
    REAL,DIMENSION (kjpindex)                      :: zcarblu       !! Work array to keep fco2_land_use
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE intersurf_initialize_gathered (kjit, iim_glo, jjm_glo, kjpindex, kindex, xrdt, &
    ...
    !! 0.1 Input 
    INTEGER,INTENT (in)                             :: kjit           !! Time step number
    INTEGER,INTENT (in)                             :: iim_glo        !! Dimension of global fields
    INTEGER,INTENT (in)                             :: jjm_glo        !! Dimension of global fields
    INTEGER,INTENT (in)                             :: kjpindex       !! Number of continental points
    REAL,INTENT (in)                                :: xrdt           !! Time step in seconds
    LOGICAL, INTENT (in)                                   :: lrestart_read  !! Logical for _restart_ file to read
    LOGICAL, INTENT (in)                                   :: lrestart_write !! Logical for _restart_ file to write'
    ...
    REAL,DIMENSION (kjpindex), INTENT(in)           :: swdown         !! Downwelling surface short-wave flux
    REAL,DIMENSION (kjpindex), INTENT(in)           :: temp_air       !! Air temperature in Kelvin
    REAL,DIMENSION (kjpindex), INTENT(in)           :: epot_air       !! Air potential energy
    REAL,DIMENSION (kjpindex), INTENT(in)           :: petAcoef       !! Coeficients A from the PBL resolution
    REAL,DIMENSION (kjpindex), INTENT(in)           :: peqAcoef       !! One for T and another for q
    REAL,DIMENSION (kjpindex), INTENT(in)           :: petBcoef       !! Coeficients B from the PBL resolution
    REAL,DIMENSION (kjpindex), INTENT(in)           :: peqBcoef       !! One for T and another for q
    REAL,DIMENSION (kjpindex), INTENT(in)           :: pb             !! Surface pressure
    REAL,DIMENSION (kjpindex,2), INTENT(in)         :: latlon         !! Geographical coordinates
    REAL,DIMENSION (kjpindex), INTENT(in)           :: zcontfrac      !! Fraction of continent
    INTEGER,DIMENSION (kjpindex,NbNeighb), INTENT(in):: zneighbours    !! neighbours
    REAL,DIMENSION (kjpindex,2), INTENT(in)         :: zresolution    !! size of the grid box
    REAL,DIMENSION (iim_glo,jjm_glo), INTENT(IN)    :: lon_scat_g     !! The scattered values for longitude 
    REAL,DIMENSION (iim_glo,jjm_glo), INTENT(IN)    :: lat_scat_g     !! The scattered values for latitudes

    !! 0.2 Output variables
    REAL,DIMENSION (kjpindex), INTENT(out)          :: z0m            !! Surface roughness (momentum)
    REAL,DIMENSION (kjpindex), INTENT(out)          :: coastalflow_cpl!! Diffuse flow of water into the ocean (m^3/s)
    REAL,DIMENSION (kjpindex), INTENT(out)          :: riverflow_cpl  !! Largest rivers flowing into the ocean (m^3/s)
    ...
    REAL,DIMENSION (kjpindex), INTENT(out)          :: temp_sol_new   !! New soil temperature
    REAL,DIMENSION (kjpindex), INTENT(out)          :: qsurf          !! Surface specific humidity
    REAL,DIMENSION (kjpindex,2), INTENT(out)        :: albedo         !! Albedo
    REAL,DIMENSION (kjpindex), INTENT(out)          :: fluxsens       !! Sensible chaleur flux
    REAL,DIMENSION (kjpindex), INTENT(out)          :: fluxlat        !! Latent chaleur flux
    REAL,DIMENSION (kjpindex), INTENT(out)          :: emis           !! Emissivity

    !! 0.3 Modified variables
    REAL,DIMENSION (kjpindex), INTENT(inout)        :: cdrag          !! Cdrag

    !! 0.4 Optional input and output variables
    REAL,DIMENSION (kjpindex), INTENT(in), OPTIONAL :: q2m            !! Surface specific humidity
    REAL,DIMENSION (kjpindex), INTENT(in), OPTIONAL :: t2m            !! Surface air temperature
    REAL,DIMENSION (kjpindex), INTENT(out),OPTIONAL :: z0h            !! Surface roughness (heat)
    INTEGER, INTENT(out), OPTIONAL                  :: nvm_out        !! Number of vegetation types, PFTs
    ...
    REAL,DIMENSION(:,:), OPTIONAL, INTENT(IN)       :: fields_in      !! Fields for deposit variables to be transported 
                                                                             !! from chemistry model by GCM to ORCHIDEE
    ...
    REAL,DIMENSION(:,:), OPTIONAL, INTENT(OUT)      :: fields_out     !! Fields for emission variables to be transported
                                                                             !! to chemistry model by GCM from ORCHIDEE

    !! 0.5 Local variables
    REAL,DIMENSION (kjpindex)                      :: zprecip_rain  !! Work array to keep precip_rain
    REAL,DIMENSION (kjpindex)                      :: zprecip_snow  !! Work array to keep precip_snow
    REAL,DIMENSION (kjpindex)                      :: zz0m          !! Work array to keep zz0m
    REAL,DIMENSION (kjpindex)                      :: zz0h          !! Work array to keep zz0h
    REAL,DIMENSION (kjpindex)                      :: zcdrag        !! Work array for surface drag
    REAL,DIMENSION (kjpindex)                      :: zcoastal      !! Work array to keep coastal flow
    REAL,DIMENSION (kjpindex)                      :: zriver        !! Work array to keep river out flow
    REAL,DIMENSION (kjpindex)                      :: znetco2       !! Work array to keep netco2flux
    REAL,DIMENSION (kjpindex)                      :: zcarblu       !! Work array to keep fco2_land_use
    REAL,DIMENSION (kjpindex)                      :: ztsol_rad     !! Work array to keep tsol_rad
    REAL,DIMENSION (kjpindex)                      :: zvevapp       !! Work array to keep vevapp
    ...
    REAL,DIMENSION (kjpindex)                      :: zemis         !! Work array to keep emis
    REAL,ALLOCATABLE, DIMENSION (:)                :: soilth_lev    !! Vertical soil axis for thermal scheme (m)
    REAL,DIMENSION (kjpindex)                      :: q2m_loc       !! Work array for q2m or qair
    REAL,DIMENSION (kjpindex)                      :: t2m_loc       !! Work array for t2m or temp_air
    REAL,DIMENSION (:,:),ALLOCATABLE               :: lon_scat      !! The scattered values for longitude 
    REAL,DIMENSION (:,:),ALLOCATABLE               :: lat_scat      !! The scattered values for latitude
    INTEGER                                        :: i, j, ik
    INTEGER                                        :: ier
    INTEGER                                        :: itau_sechiba
    REAL, ALLOCATABLE, DIMENSION(:,:)              :: tmp_lon, tmp_lat, tmp_lev
    INTEGER                                               :: old_fileout   !! old Logical Int for std IO output
    REAL,ALLOCATABLE,DIMENSION(:,:)                       :: lalo_mpi
    REAL,DIMENSION (kjpindex)                      :: landpoints    !! Local landpoints vector
    REAL                                           :: tau_outflow   !! Number of days for temoral filter on river- and coastalflow
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE intersurf_main_gathered (kjit, iim_glo, jjm_glo, kjpindex, kindex, xrdt, &
    ...
    REAL,DIMENSION (kjpindex), INTENT(in)          :: temp_air        !! Air temperature in Kelvin
    REAL,DIMENSION (kjpindex), INTENT(in)          :: epot_air        !! Air potential energy
    REAL,DIMENSION (kjpindex), INTENT(in)          :: ccanopy         !! CO2 concentration in the canopy
    REAL,DIMENSION (kjpindex), INTENT(in)          :: petAcoef        !! Coeficients A from the PBL resolution
    REAL,DIMENSION (kjpindex), INTENT(in)          :: peqAcoef        !! One for T and another for q
    REAL,DIMENSION (kjpindex), INTENT(in)          :: petBcoef        !! Coeficients B from the PBL resolution
    REAL,DIMENSION (kjpindex), INTENT(in)          :: peqBcoef        !! One for T and another for q
    ...
    REAL,DIMENSION (kjpindex,2), INTENT(in)        :: zresolution     !! size of the grid box
    REAL,DIMENSION (iim_glo,jjm_glo), INTENT(IN)   :: lon_scat_g      !! The scattered values for longitude 
    REAL,DIMENSION (iim_glo,jjm_glo), INTENT(IN)   :: lat_scat_g      !! The scattered values for latitude

    !! 0.2 Output variables
    REAL,DIMENSION (kjpindex), INTENT(out)         :: z0m             !! Surface roughness for momentum
    REAL,DIMENSION (kjpindex), INTENT(out)         :: coastalflow_cpl !! Diffuse flow of water into the ocean, time filtered (m^3/s)
    REAL,DIMENSION (kjpindex), INTENT(out)         :: riverflow_cpl   !! Largest rivers flowing into the ocean, time filtered (m^3/s)
    REAL,DIMENSION (kjpindex), INTENT(out)         :: tsol_rad        !! Radiative surface temperature
    REAL,DIMENSION (kjpindex), INTENT(out)         :: vevapp          !! Total of evaporation
    REAL,DIMENSION (kjpindex), INTENT(out)         :: temp_sol_new    !! New soil temperature
    REAL,DIMENSION (kjpindex), INTENT(out)         :: qsurf           !! Surface specific humidity
    ...
    REAL,DIMENSION (kjpindex), INTENT(in), OPTIONAL:: q2m             !! Surface specific humidity
    REAL,DIMENSION (kjpindex), INTENT(in), OPTIONAL:: t2m             !! Surface air temperature
    REAL,DIMENSION (kjpindex), INTENT(out),OPTIONAL:: z0h             !! Surface roughness for heat
    REAL,DIMENSION (kjpindex,nvm), INTENT(out), OPTIONAL :: veget     !! Fraction of vegetation type (unitless, 0-1) 
    REAL,DIMENSION (kjpindex,nvm), INTENT(out), OPTIONAL :: lai       !! Leaf area index (m^2 m^{-2}
    REAL,DIMENSION (kjpindex,nvm), INTENT(out), OPTIONAL :: height    !! Vegetation Height (m)
    REAL, DIMENSION(kjpindex), OPTIONAL, INTENT(in):: coszang         !! Cosine of the solar zenith angle (unitless)
    CHARACTER(LEN=*),DIMENSION(:), OPTIONAL, INTENT(IN)   :: field_in_names  !! Names for deposit variables to be transported
                                                                             !! from chemistry model by GCM to ORCHIDEE
    REAL,DIMENSION(:,:), OPTIONAL, INTENT(IN)      :: fields_in       !! Fields for deposit variables to be transported 
                                                                             !! from chemistry model by GCM to ORCHIDEE
