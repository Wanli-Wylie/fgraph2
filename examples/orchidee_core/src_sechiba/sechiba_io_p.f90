...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_sechiba/sechiba_io_p.f90 $
...
SUBROUTINE i0setvar_p (var, val_exp, key_wd, val_put)
  ...
  INTEGER, INTENT(inout)                   :: var                  !! Integer scalar to modify
  INTEGER, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                    :: key_wd               !! The Key word we will look for
  INTEGER, INTENT(in)                      :: val_put              !! Initial value to stored
...
SUBROUTINE i10setvar_p (var, val_exp, key_wd, val_put)
  ...
  INTEGER, DIMENSION(:), INTENT(inout)     :: var                  !! 1D integer array to modify
  INTEGER, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                :: key_wd               !! The Key word we will look for
  INTEGER, INTENT(in)                      :: val_put              !! Scalar value to stored
...
SUBROUTINE i11setvar_p (var, val_exp, key_wd, val_put, is_grid)
  ...
  INTEGER, INTENT(in)                      :: val_exp             !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                    :: key_wd              !! The Key word we will look for
  INTEGER, DIMENSION(:), INTENT(in)        :: val_put             !! 1D integer array to stored
  LOGICAL,        OPTIONAL                        :: is_grid             !! Parameter present indicates a setvar for a grid variable 

  INTEGER, ALLOCATABLE,DIMENSION(:)        :: val_tmp
  INTEGER, ALLOCATABLE,DIMENSION(:)        :: val_tmp_g
  INTEGER                                  :: is_key
...
SUBROUTINE i20setvar_p (var, val_exp, key_wd, val_put)
  ...
  INTEGER, DIMENSION(:,:), INTENT(inout)   :: var                  !! 2D integer array to modify
  INTEGER, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                    :: key_wd               !! The Key word we will look for
  INTEGER, INTENT(in)                      :: val_put              !! Scalar value to be used as default
...
SUBROUTINE r0setvar_p (var, val_exp, key_wd, val_put)
  ...
  REAL, INTENT(inout)                   :: var                  !! Real scalar to modify
  REAL, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                   :: key_wd               !! The Key word we will look for
  REAL, INTENT(in)                      :: val_put              !! Initial value to stored
...
SUBROUTINE r10setvar_p (var, val_exp, key_wd, val_put)
  ...
  REAL, DIMENSION(:), INTENT(inout)     :: var                  !! 1D real array to modify
  REAL, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                 :: key_wd               !! The Key word we will look for
  REAL, INTENT(in)                      :: val_put              !! Scalar value to stored
...
SUBROUTINE r11setvar_p (var, val_exp, key_wd, val_put, is_grid)
  ...
  REAL, INTENT(in)                      :: val_exp             !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                 :: key_wd              !! The Key word we will look for
  REAL, DIMENSION(:), INTENT(in)        :: val_put             !! 1D integer array to stored
  LOGICAL,        OPTIONAL                     :: is_grid             !! Parameter present indicates a setvar for a grid variable 

  REAL, ALLOCATABLE,DIMENSION(:)        :: val_tmp
  INTEGER                               :: is_key
...
SUBROUTINE r20setvar_p (var, val_exp, key_wd, val_put)
  ...
  REAL, DIMENSION(:,:), INTENT(inout)   :: var                  !! 2D integer array to modify
  REAL, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                 :: key_wd               !! The Key word we will look for
  REAL, INTENT(in)                      :: val_put              !! Scalar value to stored
...
SUBROUTINE r21setvar_p (var, val_exp, key_wd, val_put, is_grid)
  ...
  REAL, DIMENSION(:,:), INTENT(inout)   :: var                  !! 2D real array to modify
  REAL, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                 :: key_wd               !! The Key word we will look for
  REAL, DIMENSION(:), INTENT(in)        :: val_put              !! 1D real array to stored
  LOGICAL,        OPTIONAL                     :: is_grid              !! Parameter present indicates a setvar for a grid variable 
...
SUBROUTINE r22setvar_p (var, val_exp, key_wd, val_put)
  ...
  REAL, INTENT(in)                      :: val_exp             !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                 :: key_wd              !! The Key word we will look for
  REAL, DIMENSION(:,:), INTENT(in)      :: val_put             !! 2D integer array to stored
  REAL, ALLOCATABLE, DIMENSION(:,:)     :: val_tmp
  REAL                                  :: val_scal            !! Temporary variable to read a scalar value from run.def
  INTEGER                               :: is_key
...
SUBROUTINE r30setvar_p (var, val_exp, key_wd, val_put)
  ...
  REAL, DIMENSION(:,:,:), INTENT(inout) :: var                  !! 3D integer array to modify
  REAL, INTENT(in)                      :: val_exp              !! Exceptional value
  CHARACTER(LEN=*), INTENT(in)                 :: key_wd               !! The Key word we will look for
  REAL, INTENT(in)                      :: val_put              !! Scalar value to stored
