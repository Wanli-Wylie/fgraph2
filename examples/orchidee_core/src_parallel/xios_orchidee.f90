...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_parallel/xios_orchidee.f90 $
!! $Date: 2019-02-07 16:49:01 +0100 (四, 2019-02-07) $
!! $Revision: 5715 $
!! \n
!_ ================================================================================================================================
...
MODULE xios_orchidee
  ...
  USE constantes_soil_var, ONLY : nstm, check_waterbal, diaglev, check_cwrr2, ok_freeze_cwrr
...
CONTAINS
  ...
  SUBROUTINE xios_orchidee_init(MPI_COMM_ORCH,                   &
    ...
    !! 0.1 Input variables
    !
    INTEGER, INTENT(in)                            :: MPI_COMM_ORCH    !! Orchidee MPI communicator (from module mod_orchidee_mpi_data)
    REAL, INTENT(in)                               :: date0            !! Julian day at first time step
    INTEGER, INTENT(in)                            :: year, month, day !! Current date information
    REAL, INTENT(in)                               :: julian_diff      !! Current day in the year [1,365(366)]
    REAL,DIMENSION (iim_g,jj_nb), INTENT(in)       :: lon_mpi, lat_mpi !! Longitudes and latitudes on MPI local domain 2D domain
    REAL,DIMENSION (ngrnd), INTENT(in)             :: soilth_lev       !! Vertical soil levels for thermal scheme (m)
    !
    !! 0.2 Local variables
    !
#ifdef XIOS
...
#endif
    INTEGER                 :: i
    INTEGER                 :: year0, month0, day0 !! Time origin date information
    REAL                    :: sec0                !! Time origin date information
    ...
    IF (xios_orchidee_ok .AND. is_omp_root) THEN
       ...
       ELSE
          ...
          ! tws is defined in field_def.xml as a sum of several variables cacluated only for cwrr
          CALL xios_set_field_attr("tws",enabled=.FALSE.)
          CALL xios_set_field_attr("mrsow",enabled=.FALSE.)
          CALL xios_set_field_attr("ksat",enabled=.FALSE.)
       END IF
       ...
       IF (.NOT. ok_freeze_cwrr) THEN
          ...
          CALL xios_set_field_attr("kk_moy",enabled=.FALSE.)
          CALL xios_set_field_attr("profil_froz_hydro_ns",enabled=.FALSE.)
       END IF
       ...
       IF ( .NOT. ok_stomate ) THEN
          ...
          CALL xios_set_field_attr("hetero_resp_lit",enabled=.FALSE.)
       ...
       IF ( .NOT. ok_co2)THEN
          ...
          CALL xios_set_field_attr("gpp_ipcc2",enabled=.FALSE.)
       END IF
       ...
       IF (.NOT. check_waterbal) THEN
          CALL xios_set_field_attr("tot_flux",enabled=.FALSE.)
       END IF
      ...
      IF(.NOT. ok_ncycle) THEN
          CALL xios_set_field_attr("N_UPTAKE_NH4",enabled=.FALSE.)
          CALL xios_set_field_attr("N_UPTAKE_NO3",enabled=.FALSE.)
          CALL xios_set_field_attr("N_MINERALISATION",enabled=.FALSE.)
          CALL xios_set_field_attr("N_MINERALISATION_LIT",enabled=.FALSE.)
          CALL xios_set_field_attr("SOIL_NH4",enabled=.FALSE.)
          CALL xios_set_field_attr("SOIL_NO3",enabled=.FALSE.)
          CALL xios_set_field_attr("SOIL_NOX",enabled=.FALSE.)
          CALL xios_set_field_attr("SOIL_N2O",enabled=.FALSE.)
          CALL xios_set_field_attr("SOIL_N2",enabled=.FALSE.)
          CALL xios_set_field_attr("SOIL_P_OX",enabled=.FALSE.)
          CALL xios_set_field_attr("BACT",enabled=.FALSE.)
          CALL xios_set_field_attr("NH3_EMISSION",enabled=.FALSE.)
          CALL xios_set_field_attr("NOX_EMISSION",enabled=.FALSE.)
          CALL xios_set_field_attr("N2O_EMISSION",enabled=.FALSE.)
          CALL xios_set_field_attr("N2_EMISSION",enabled=.FALSE.)
          CALL xios_set_field_attr("NH4_LEACHING",enabled=.FALSE.)
          CALL xios_set_field_attr("NO3_LEACHING",enabled=.FALSE.)
          CALL xios_set_field_attr("NITRIFICATION",enabled=.FALSE.)
          CALL xios_set_field_attr("DENITRIFICATION",enabled=.FALSE.)
          CALL xios_set_field_attr("NHX_DEPOSITION",enabled=.FALSE.)
          CALL xios_set_field_attr("NOY_DEPOSITION",enabled=.FALSE.)
          CALL xios_set_field_attr("BNF",enabled=.FALSE.)
          CALL xios_set_field_attr("N_FERTILISER",enabled=.FALSE.) 
          CALL xios_set_field_attr("N_TAKEN",enabled=.FALSE.)
       ENDIF

       IF(.NOT. ok_ncycle .OR. .NOT. impose_cn) THEN
          CALL xios_set_field_attr("N_SUPPORT",enabled=.FALSE.)
       ENDIF

       IF(.NOT. ok_pcycle) THEN
          CALL xios_set_field_attr("P_DEPOSITION",enabled=.FALSE.) 
          CALL xios_set_field_attr("P_RELEASE",enabled=.FALSE.)
          CALL xios_set_field_attr("P_TAKEN",enabled=.FALSE.)
          CALL xios_set_field_attr('P_LEACHING',enabled=.FALSE.)
          CALL xios_set_field_attr('P_UPTAKE',enabled=.FALSE.)
          CALL xios_set_field_attr('P_MINERALISATION',enabled=.FALSE.)
          CALL xios_set_field_attr('P_MINERALISATION_LIT',enabled=.FALSE.)
          CALL xios_set_field_attr('BC_MINERALISATION',enabled=.FALSE.)
          CALL xios_set_field_attr('P_SSORPTION',enabled=.FALSE.)
          CALL xios_set_field_attr('SOIL_P_DISSOLVED',enabled=.FALSE.)
          CALL xios_set_field_attr('SOIL_P_SORBED',enabled=.FALSE.)
          CALL xios_set_field_attr('f_PDISSOLVED',enabled=.FALSE.)
          CALL xios_set_field_attr('LABILE_TURN',enabled=.FALSE.) 
       ENDIF

       IF(.NOT. enhanced_weat) THEN
          CALL xios_set_field_attr('EW_CO2C'      ,enabled=.FALSE.) 
          CALL xios_set_field_attr('EW_PR'        ,enabled=.FALSE.) 
          CALL xios_set_field_attr('EW_BASALT'    ,enabled=.FALSE.) 
       ENDIF

       IF(.NOT. ok_pcycle .OR. .NOT. impose_np) THEN
          CALL xios_set_field_attr("P_SUPPORT",enabled=.FALSE.)
       ENDIF
  ...
  SUBROUTINE xios_orchidee_update_calendar(itau_sechiba)
    ...
    !! 0.1 Input variables
    !
    INTEGER, INTENT(IN) :: itau_sechiba    !! Current time step of the model
    !
    !! 0.2 Local variables
    !
    LOGICAL, SAVE         :: first=.TRUE.         !! Flag for first entering in subroutine
    INTEGER, SAVE  :: offset               !! Offset to substract from itau_sechiba
    INTEGER        :: itau_xios            !! Current time step for XIOS
  ...
  SUBROUTINE xios_orchidee_send_field_r3d(field_id,field)
    ...
    !! 0.2 Local variables
    REAL, DIMENSION(nbp_mpi,size(field,2),size(field,3)) :: field_mpi
  ...
  SUBROUTINE xios_orchidee_send_field_r4d(field_id,field)
    ...
    !! 0.2 Local variables
    INTEGER :: jv
    REAL, DIMENSION(nbp_mpi,size(field,2),size(field,3),size(field,4)) :: field_mpi
  ...
  SUBROUTINE xios_orchidee_send_field_r5d(field_id,field)
    ...
    !! 0.2 Local variables
    INTEGER :: jv
    REAL, DIMENSION(nbp_mpi,size(field,2),size(field,3),size(field,4),size(field,5)) :: field_mpi
