...
!!                 The variables in here can be used by USE mod_orchidee_para. Only at some specific cases it it needed to 
!!                 use this module directly to avoid depencendy problems.
!!
...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_parallel/mod_orchidee_para_var.F90 $
...
MODULE mod_orchidee_para_var
  ...
  INTEGER,SAVE              :: nbp_mpi                         !! number of local continental points in each mpi group
  ...
  INTEGER,SAVE,DIMENSION(:),ALLOCATABLE :: nbp_omp_para_nb
  INTEGER,SAVE,DIMENSION(:),ALLOCATABLE :: nbp_omp_para_begin
  INTEGER,SAVE,DIMENSION(:),ALLOCATABLE :: nbp_omp_para_end    

  INTEGER,SAVE :: nbp_omp
  INTEGER,SAVE :: nbp_omp_begin
  INTEGER,SAVE :: nbp_omp_end
  INTEGER,SAVE :: offset_omp
  ...
  INTEGER,SAVE,ALLOCATABLE,DIMENSION(:) :: omp_function
  ! It is not SHARED.
  ! List of values :
  ...
  !
  !! Global grid arrays used by stomate and sechiba and available on each processor
  !! They need to be broadcasted after initilisation of the parallelisation
  !-
  ! Dimensions
  INTEGER,SAVE              :: iim_g                           !! Dimension of global fields for longitude
  INTEGER,SAVE              :: jjm_g                           !! Dimension of global fields for latitude
  INTEGER,SAVE              :: nbp_glo                         !! number of global continental points
  ! Fields
  !! index of land points on the 2D map
  INTEGER,ALLOCATABLE,DIMENSION(:),SAVE   :: index_g
  !-
  !! indices of the 4 neighbours of each grid point (1=N, 2=E, 3=S, 4=W)
  INTEGER,ALLOCATABLE,DIMENSION(:,:),SAVE :: neighbours_g
  !-
  ! Heading of the direction out of the grid box either through the vertex 
  ! of the mid-segment of the polygon.
  !
  REAL, ALLOCATABLE, DIMENSION(:,:), SAVE      :: headings_g
  !
  ! Length of segments of the polygon.
  !
  REAL, ALLOCATABLE, DIMENSION(:,:), SAVE      :: seglength_g
  !
  ! Area of the grid box
  !
  REAL, ALLOCATABLE, DIMENSION(:), SAVE        :: area_g
  !
  ! Coordinats of the vertices
  !
  REAL, ALLOCATABLE, DIMENSION(:,:,:), SAVE    :: corners_g
  !
  ! TEMPORARY !!!!!
  !
  !! resolution at each grid point in m (1=E-W, 2=N-S)
  REAL,ALLOCATABLE,DIMENSION(:,:),SAVE    :: resolution_g 
  !-
  !! Geographical coordinates
  REAL,ALLOCATABLE,DIMENSION(:,:),SAVE    :: lalo_g
  ! Global grid, for all process
  REAL, ALLOCATABLE, DIMENSION(:,:), SAVE     :: lon_g, lat_g
  !-
  !! Fraction of continents
  REAL,ALLOCATABLE,DIMENSION(:),SAVE      :: contfrac_g  
  !
