...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_stomate/stomate_season.f90 $ 
!! $Date: 2018-04-09 15:18:25 +0200 (一, 2018-04-09) $
!! $Revision: 5182 $
!! \n
!_ =================================================================================================================================
...
!_ =================================================================================================================================
  ...
  SUBROUTINE season (npts, dt, veget_cov, veget_cov_max, &
       moiavail_daily, t2m_daily, tsoil_daily, soilhum_daily, lalo,  &
    ...
    REAL, INTENT(in)                                :: dt                !! time step in days (dt_days)
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: veget_cov         !! coverage fraction of a PFT. Here: fraction of 
                                                                                !! total ground. (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: veget_cov_max     !! "maximal" coverage fraction of a PFT (for LAI -> 
    ...
    REAL, DIMENSION(npts), INTENT(in)               :: t2m_daily         !! Daily 2 meter temperature (K)
    REAL, DIMENSION(npts,nslm), INTENT(in)          :: tsoil_daily       !! Daily soil temperature (K)
    REAL, DIMENSION(npts,nslm), INTENT(in)          :: soilhum_daily     !! Daily soil humidity (0-1, unitless)
    REAL, DIMENSION(npts,2), INTENT(in)             :: lalo              !!  array of lat/lon
    REAL, DIMENSION(npts), INTENT(in)               :: precip_daily      !! Daily mean precipitation @tex ($mm day^{-1}$) 
                                                                                !! @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: npp_daily         !! daily net primary productivity @tex ($gC m^{-2} 
                                                                                !! day^{-1}$) @endtex
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(in) :: biomass     !! biomass @tex ($gC m^{-2} of ground$) @endtex
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(in) :: turnover_daily  !! Turnover rates @tex ($gC m^{-2} day^{-1}$) 
                                                                                !! @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: gpp_daily         !! daily gross primary productivity  
                                                                                !! (Here: @tex $gC m^{-2} of total ground 
                                                                                !! day^{-1}$) @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: when_growthinit   !! how many days ago was the beginning of the 
                                                                                !! growing season (days) 
    LOGICAL, DIMENSION(npts,nvm), INTENT(in)               :: begin_leaves      !! signal to start putting leaves on (true/false)
    REAL, DIMENSION(npts), INTENT(in)               :: t2m_min_daily     !! Daily minimum 2-meter temperature (K)
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: lai_target        !! target lai
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: sla_calc          !! leaf age-related SLA
    !
    !! 0.2 Output variables 
    ! (diagnostic)
    !
    REAL, DIMENSION(npts,nvm), INTENT(out)          :: herbivores        !! time constant of probability of a leaf to be 
                                                                                !! eaten by a herbivore (days) 

    !
    !! 0.3 Modified variables
    !
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: maxmoiavail_lastyear      !! last year's maximum moisture 
                                                                                        !! availability (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: maxmoiavail_thisyear      !! this year's maximum moisture 
                                                                                        !! availability (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: minmoiavail_lastyear      !! last year's minimum moisture 
                                                                                        !! availability (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: minmoiavail_thisyear      !! this year's minimum moisture 
                                                                                        !! availability (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: maxgppweek_lastyear       !! last year's maximum weekly GPP 
                                                                                        !! @tex ($gC m^{-2} week^{-1}$) @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: maxgppweek_thisyear       !! this year's maximum weekly GPP 
                                                                                        !! @tex ($gC m^{-2} week^{-1}$) @endtex 
    REAL, DIMENSION(npts), INTENT(inout)            :: gdd0_lastyear             !! last year's annual GDD0 (C)
    REAL, DIMENSION(npts), INTENT(inout)            :: gdd0_thisyear             !! this year's annual GDD0 (C)
    REAL, DIMENSION(npts), INTENT(inout)            :: precip_lastyear           !! last year's annual precipitation 
                                                                                        !! @tex ($mm year^{-1}$) @endtex 
    REAL, DIMENSION(npts), INTENT(inout)            :: precip_thisyear           !! this year's annual precipitation 
                                                                                        !! @tex ($mm year^{-1}$) @endtex
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: lm_lastyearmax            !! last year's maximum leaf mass, for each 
                                                                                        !! PFT @tex ($gC m^{-2}$) @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: lm_thisyearmax            !! this year's maximum leaf mass, for each 
                                                                                        !! PFT @tex ($gC m^{-2}$) @endtex
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: maxfpc_lastyear           !! last year's maximum fpc for each PFT, on 
                                                                                        !! ground (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: maxfpc_thisyear           !! this year's maximum fpc for each PFT, on 
                                                                                        !! ground (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: moiavail_month            !! "monthly" moisture availability 
                                                                                        !! (0-1, unitless)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: moiavail_week             !! "weekly" moisture availability 
                                                                                        !! (0-1, unitless)
    REAL, DIMENSION(npts), INTENT(inout)            :: t2m_longterm              !! "long term" 2-meter temperatures (K)
    REAL, INTENT(inout)                             :: tau_longterm     
    REAL, DIMENSION(npts), INTENT(inout)            :: t2m_month                 !! "monthly" 2-meter temperatures (K)
    REAL, DIMENSION(npts), INTENT(inout)            :: t2m_week                  !! "weekly" 2-meter temperatures (K)
    REAL, DIMENSION(npts), INTENT(inout)            :: t2m_14                    !! "14days" 2-meter temperatures (K)
    REAL, DIMENSION(npts), INTENT(inout)            :: Tseason                   !! "seasonal" 2-meter temperatures (K), 
                                                                                        !! used to constrain boreal treeline
    REAL, DIMENSION(npts), INTENT(inout)            :: Tseason_tmp               !! temporary variable to calculate Tseason 
    REAL, DIMENSION(npts), INTENT(inout)            :: Tseason_length            !! temporary variable to calculate Tseason: 
                                                                                        !! number of days when t2m_week is higher than 0 degree 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: Tmin_spring_time          !! Number of days after begin_leaves (leaf onset)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: onset_date                !! Date in the year at when the leaves started to grow(begin_leaves)
    REAL, DIMENSION(npts,nslm), INTENT(inout)       :: tsoil_month               !! "monthly" soil temperatures (K)
    REAL, DIMENSION(npts,nslm), INTENT(inout)       :: soilhum_month             !! "monthly" soil humidity (0-1, unitless)

    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: drainage_daily            !! "daily"  sum of drainage 
                                                                                        !! @tex ($g H2O m^{-2} day^{-1}$) @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: runoff_daily              !! "long term" sum of drainage 
                                                                                        !! @tex ($g H2O m^{-2} day^{-1}$) @endtex 
    REAL, DIMENSION(npts), INTENT(inout)            :: drainage_longterm         !! "daily" sum of drainage 
                                                                                        !! @tex ($kg H2O m^{-2} year^{-1}$) @endtex 
    REAL, DIMENSION(npts), INTENT(inout)            :: runoff_longterm           !! "long term" annual sum of drainage 
                                                                                        !! @tex ($kg H2O m^{-2} year^{-1}$) @endtex 
    ...
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout) :: turnover_longterm !! "long term" turnover rate 
                                                                                        !! @tex ($gC m^{-2} year^{-1}$) @endtex
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: gpp_week                  !! "weekly" GPP @tex ($gC m^{-2} day^{-1}$) 
                                                                                        !! @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: npp_week                  !! "weekly" NPP @tex ($gC m^{-2} day^{-1}$) 
                                                                                        !! @endtex 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: gdd_m5_dormance           !! growing degree days above threshold -5 
                                                                                        !! deg. C (C) 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: gdd_midwinter             !! growing degree days since midwinter (C)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: ncd_dormance              !! number of chilling days since leaves 
                                                                                        !! were lost (days) 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: ngd_minus5                !! number of growing days (days)
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: time_hum_min              !! time elapsed since strongest moisture 
                                                                                        !! availability (days) 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: hum_min_dormance          !! minimum moisture during dormance 
                                                                                        !! (0-1, unitless)
    REAL, DIMENSION(npts,2), INTENT(inout)          :: gdd_init_date             !! inital date for gdd count
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: gdd_from_growthinit       !! growing degree days, threshold 0 deg. C 
                                                                                        !! since beginning of season
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: cn_leaf_avg_season        !! seasonal average leaf CN ratio  
	                                                                                    !! ((gC) (gN)-1) 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: np_leaf_avg_season        !! seasonal average leaf NP ratio  
	                                                                                    !! ((gN) (gP)-1) 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: nstress_season            !! Seasonal nitrogen stress 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: pstress_season            !! Seasonal phosphorus stress 
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: lai_target_longterm       !! Seasonal lai target
 	   
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: moiavail_growingseason    !! soil moisture during growing season
                                                                                        !! (used for allometry&allocation)

    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: rue_longterm              !! Longterm radiation use efficiency 
                                                                                        !! (units?)
    !
    !! 0.4 Local variables
    !
    INTEGER                                          :: j                        !! indices (unitless)
    REAL                                             :: ncd_max                  !! maximum ncd (to avoid floating point 
                                                                                        !! underflows) (days) 
    REAL, DIMENSION(npts)                            :: sumfpc_nat               !! sum of natural fpcs (0-1, unitless)
                                                                                        !! [DISPENSABLE] 
    REAL, DIMENSION(npts,nvm)                        :: weighttot                !! weight of biomass @tex ($gC m^{-2}$) 
                                                                                        !! @endtex 
    REAL, DIMENSION(npts,nvm)                        :: nlflong_nat              !! natural long-term leaf NPP 
                                                                                        !! @tex ($gC m^{-2} year^{-1}$) @endtex
    REAL, DIMENSION(npts,nvm)                        :: green_age                !! residence time of green tissue (years)
    REAL, DIMENSION(npts)                            :: consumption              !! herbivore consumption 
                                                                                        !! @tex ($gC m^{-2} day^{-1}$) @endtex
    REAL, DIMENSION(npts)                            :: fracnat                  !! fraction of each gridcell occupied by 
                                                                                        !! natural vegetation (0-1, unitless)
    REAL, DIMENSION(npts)                            :: solad                    !! maximal radation during current day 
                                                                                        !! (clear sky condition)
    REAL, DIMENSION(npts)                            :: solai                    !! maximal radation during current day
                                                                                        !! (clear sky condition)
    REAL, DIMENSION(npts)                            :: cloud                    !! cloud fraction
    REAL, DIMENSION(npts)                            :: nn                       !! Temporary variable for storing biomass(initrogen) for the sum of labile+root+leaf pools (gN m-2) 
    REAL, DIMENSION(npts)                            :: pp                       !! Temporary variable for storing biomass(iphosphorus) for the sum of labile+root+leaf pools (gN m-2) 
    REAL, DIMENSION(npts)                            :: cn                       !! CN ration for  labile+root+leaf pools (gC (gN)-1) 
    REAL, DIMENSION(npts)                            :: cp                       !! CP ration for  labile+root+leaf pools (gC (gP)-1) 
...
!_ =================================================================================================================================
    ...
    IF ( firstcall_season ) THEN
      ...
      IF ( ABS( SUM( moiavail_growingseason(:,:) ) ) .LT. min_stomate ) THEN

        ! in this case, set them it today's moisture availability
        WRITE(numout,*) 'Warning! We have to initialize the ''weekly'' moisture availabilities.'
        moiavail_growingseason(:,:) = 1.0
      ...
      ENDIF
       ...
       !! 1.3.XX "weekly" NPP (::npp_week):
       IF ( ABS( SUM( npp_week(:,2:nvm) ) ) .LT. min_stomate ) THEN
          WRITE(numout,*) 'Warning! Weekly NPP is initialized to 0.'
       ENDIF
       ...
       !! 1.3.13 "long term" drainage (::drainage_longterm):
       IF ( ABS( SUM( drainage_longterm(:) ) ) .LT. min_stomate ) THEN
          WRITE(numout,*) 'Warning! Long term drainage is initialized to 0.'
       ENDIF
       
       !! 1.3.14 "long term" runoff (::runoff_longterm):
       IF ( ABS( SUM( runoff_longterm(:) ) ) .LT. min_stomate ) THEN
          WRITE(numout,*) 'Warning! Long term runoff is initialized to 0.'
       ENDIF
...
!!$          ENDWHERE
    ...
    !
    !! XX. Update the "long term" annuals sum of drainge. (::drainge_daily in gH2O/m^2/day, ::drainage_longterm in gH2O/m^2/year.) 
    !!     The time constant is given by the parameter ::tau_longterm, 
    !!     which is set in ::stomate_data to be 3 * one year (in seconds, as 
    !!     described above). If the ::drainge_longterm is less than zero then set it to zero. \n
    !!     ::drainge_longterm is used in ::stomate_phosphorus in the calculation of 
    !!     P release from minerals by weathering                                      
    !

    drainage_longterm = ( drainage_longterm * ( tau_longterm - dt ) + &
    !     (drainage_daily*one_year) * dt                          ) / &
         (SUM(drainage_daily*veget_cov,DIM=2)*one_year) * dt                          ) / &
         tau_longterm

    WHERE ( ABS(drainage_longterm(:)) .LT. EPSILON(zero) )
       drainage_longterm(:) = zero
    ENDWHERE


    !
    !! XX. Update the "long term" annuals sum of runoff. (::runoff_daily in gH2O/m^2/day, ::runoff_longterm in gH2O/m^2/year.)
    !!     The time constant is given by the parameter ::tau_longterm, 
    !!     which is set in ::stomate_data to be 3 * one year (in seconds, as 
    !!     described above). If the ::drainge_longterm is less than zero then set it to zero. \n
    !!     ::runoff_longterm is used in ::stomate_phosphorus in the calculation of 
    !!     P release from minerals by weathering                                      
    !

    runoff_longterm = ( runoff_longterm * ( tau_longterm - dt ) + &
         !(runoff_daily*one_year) * dt                          ) / &
         (SUM(runoff_daily*veget_cov,DIM=2)*one_year) * dt                          ) / &
         tau_longterm

    WHERE ( ABS(runoff_longterm(:)) .LT. EPSILON(zero) )
       runoff_longterm(:) = zero
    ENDWHERE
    ...
    WHERE ( veget_cov_max .GT. zero )
       ...
       npp_week = ( npp_week * ( tau_gpp_week - dt ) + &
            npp_daily * dt ) / tau_gpp_week
    ...
    DO j = 2,nvm ! Loop over # PFTs
       WHERE ( ABS(gpp_week(:,j)) .LT. EPSILON(zero) )
          gpp_week(:,j) = zero
       ENDWHERE
       WHERE ( ABS(npp_week(:,j)) .LT. EPSILON(zero) )
          npp_week(:,j) = zero
       ENDWHERE
    ENDDO
    ...
    DO j=2,nvm
       ...
       WHERE(biomass(:,j,ileaf,icarbon).GT. min_stomate)
          ...
          rue_longterm(:,j) =  ( rue_longterm(:,j) * ( one_year - dt ) + &
               gpp_daily(:,j) / &
               (1. - exp(-0.5 * biomass(:,j,ileaf,icarbon)*sla_calc(:,j))) * dt ) / &
               (one_year)
       ...
       ! Make sure rue_longterm is never less than min_stomate
       ! else we may get error messages in allocation and other
       ! routines
       WHERE (rue_longterm(:,j) .LE. min_stomate)

          rue_longterm(:,j) = min_stomate
...
!DSGliedetector: leaflife_tab likely is not  leaf lifetime in yr
    ...
    ! as the nstress_season calculation takes into account imbalance between
    ! labile C and labile N, nstress_season can be <1 althoug
    ! leaf_cn=leaf_cn_min; thus we need to include these if statements; same
    ! goes for P
    IF (.NOT.impose_cn) THEN 

       ! 20.X leaf carbon to nitrogen ratio
       DO j=2,nvm 
!DSGseason: the growing season cannot be defined by leaf biomass as leaf biomass
! and gpp does not drop below min_stomate during dormant season
!thus I add a gpp_week threshold using 0.1 g m-2 day-1
          WHERE ( (biomass(:,j,ileaf,initrogen) .GT. min_stomate).AND.(gpp_week(:,j).GT. 0.1) )
             cn_leaf_avg_season(:,j)=( cn_leaf_avg_season(:,j) * ( one_year/leaflife_tab(j) - dt ) + &  
                      biomass(:,j,ileaf,icarbon)/biomass(:,j,ileaf,initrogen) *dt )/ (one_year/leaflife_tab(j) )  
          ENDWHERE 
       ENDDO 

       ! 20.X nitrogen stress for carbon allocation 
       ! DSG: if the P cycle is active P stress first increase the C content of
       ! leaf, subsequent the N to P ratio. Thus P stress is hidden in nstress.
       ! The nstress is rather the stress on the C to nutrient ratio.

       IF ( Dmax.GT.0.0 ) THEN 
          DO j=2,nvm 
             nn(:) = biomass(:,j,ileaf,initrogen)  + biomass(:,j,iroot,initrogen) + & 
                     biomass(:,j,ilabile,initrogen)  
             WHERE (nn(:) .GT. min_stomate)  
                cn(:) = ( biomass(:,j,ileaf,icarbon) + biomass(:,j,iroot,icarbon) + & 
                          biomass(:,j,ilabile,icarbon) ) / nn(:)   
             ELSEWHERE 
                cn(:) = zero 
             ENDWHERE 

             WHERE ((cn(:) .GT. min_stomate).AND.(gpp_week(:,j).GT. 0.1))  

                nstress_season(:,j)= ( nstress_season(:,j) * ( one_year - dt ) + & 
                                       MAX(MIN( (cn_leaf_min(j)/cn(:)),1.),0.3 ) & 
                     * dt ) / one_year  
             ENDWHERE 
          ENDDO 
       ...
       ELSE 
          nstress_season(:,:) = un
       ENDIF 
    ELSE ! we impose_cn ratios; no stress 
       nstress_season(:,:) = un
    ENDIF
    ...
    IF (.NOT.impose_np) THEN 
       ...
       ! 20.X leaf nitrogen to phosphors ratio 
       DO j=2,nvm 
          WHERE ( (biomass(:,j,ileaf,iphosphorus) .GT. min_stomate).AND.(gpp_week(:,j).GT. 0.1) )
             np_leaf_avg_season(:,j)=( np_leaf_avg_season(:,j) * ( one_year/leaflife_tab(j) - dt ) + &  
                      biomass(:,j,ileaf,initrogen)/biomass(:,j,ileaf,iphosphorus) *dt )/ (one_year/leaflife_tab(j) )  
          ENDWHERE 
       ENDDO 


 
       ! 20.X phosphorus stress for carbon allocation 
       IF ( Dmax.GT.0.0 ) THEN 
          DO j=2,nvm 
             pp(:) = biomass(:,j,ileaf,iphosphorus)  + biomass(:,j,iroot,iphosphorus) + & 
                     biomass(:,j,ilabile,iphosphorus)  
             WHERE (pp(:) .GT. min_stomate)  
                cp(:) = ( biomass(:,j,ileaf,icarbon) + biomass(:,j,iroot,icarbon) + & 
                          biomass(:,j,ilabile,icarbon) ) / pp(:)   
             ELSEWHERE 
                cp(:) = zero 
             ENDWHERE 
 
             WHERE ((cp(:) .GT. min_stomate).AND.(gpp_week(:,j).GT. 0.1))  
                pstress_season(:,j)= ( pstress_season(:,j) * ( one_year - dt ) + & 
 !DSG: Here we use a threshold ratio of 16 to decided that P limitation is starting to be dominant constraint
 !     this threshold is in line with the analysis of PS dependence on leaf
 !     nitrogen and phosphorus (Elssworht et al. in prep)
 !                                      MAX(MIN( ((cn_leaf_min(j)*np_leaf_min(j))/cp(:)),1.),0.3 ) & 
                                        MAX(MIN( ((cn_leaf_min(j)*16.5          )/cp(:)),1.),0.3 ) & 
 !DSG
                     * dt ) / one_year  
             ENDWHERE 
          ENDDO 
       ...
       ELSE 
          pstress_season(:,:)=1.0 
       ENDIF 
    ELSE ! we impose_np ratios; no stress 
       pstress_season(:,:) = un
    ENDIF


    lai_target_longterm = ( lai_target_longterm * ( tau_longterm - dt ) + &
         lai_target * dt                          ) / &
         tau_longterm

    ! bare soil -> zero lai
    lai_target_longterm(:,1) = zero

    ! DSG: negatve lai_target can happen in the allometric calculation
    !      they shouldn't though:
    DO j = 2,nvm ! Loop over # PFTs
       WHERE ( lai_target_longterm(:,j) .LT. EPSILON(zero) )
          lai_target_longterm(:,j) = min_stomate
       ENDWHERE
    ENDDO
     
    CALL histwrite_p (hist_id_stomate, 'NSTRESS_SEASON', itime, &
         nstress_season, npts*nvm, horipft_index)

    CALL histwrite_p (hist_id_stomate, 'PSTRESS_SEASON', itime, &
         pstress_season, npts*nvm, horipft_index)

    CALL xios_orchidee_send_field('NSTRESS_SEASON',nstress_season)

    CALL xios_orchidee_send_field('PSTRESS_SEASON',pstress_season)
    ...
    DO j=2,nvm
       IF(is_tree(j))THEN
         WHERE((biomass(:,j,ileaf,icarbon).GT.min_stomate).AND.(gpp_week(:,j).GT.min_stomate))

            moiavail_growingseason(:,j) = ( moiavail_growingseason(:,j) * ( tau_sap(j) - dt ) + &
                        MIN(1.,2.*moiavail_daily(:,j)) * dt ) / tau_sap(j)
         ENDWHERE
       ELSE
         WHERE((biomass(:,j,ileaf,icarbon).GT.min_stomate).AND.(gpp_week(:,j).GT. 0.1))
            moiavail_growingseason(:,j) = ( moiavail_growingseason(:,j) * ( 30.  - dt ) + &
                        MIN(1.,2.*moiavail_daily(:,j)) * dt ) / 30.
         ENDWHERE
       ENDIF
    ENDDO

    CALL histwrite_p (hist_id_stomate, 'MOIAVAIL_SEASON', itime, &
         moiavail_growingseason, npts*nvm, horipft_index)

    CALL xios_orchidee_send_field('MOIAVAIL_SEASON',moiavail_growingseason)
