...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_stomate/lpj_crown.f90 $
...
!_ ================================================================================================================================
  ...
  SUBROUTINE crown &
       &  (npts, PFTpresent, ind, biomass, woodmass_ind, veget_cov_max, cn_ind, height)
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL,DIMENSION(npts,nvm),INTENT(in)        :: ind               !! [DISPENSABLE] Density of individuals 
                                                                           !! @tex $(m^{-2})$ @endtex
    REAL,DIMENSION(npts,nvm,nparts,nelements),INTENT(in) :: biomass !! [DISPENSABLE] Biomass @tex $(gC.m^{-2})$ @endtex
    REAL,DIMENSION(npts,nvm),INTENT(in)        :: woodmass_ind      !! Woodmass of the individual, needed to calculate 
                                                                           !! crownarea in lpj_crown (gC)

    !! 0.2 Output variables
  
    REAL,DIMENSION(npts,nvm),INTENT(out)       :: cn_ind            !! Crown area per individual @tex $(m^{2})$ @endtex    

    !! 0.3 Modified variables

    REAL,DIMENSION(npts,nvm),INTENT(inout)     :: veget_cov_max    !! [DISPENSABLE] "Maximal" coverage fraction of a PFT 
                                                                          !! infinity) on ground (unitless)
    REAL,DIMENSION(npts,nvm),INTENT(inout)     :: height           !! Height of vegetation (m)           

    !! 0.4 Local variables
	
!   REAL,DIMENSION(npts)                       :: woodmass        !! Wood mass of an individual (gC)
    INTEGER                                    :: j               !! Index
    REAL,DIMENSION(npts)                       :: dia             !! Stem diameter (m)
    REAL,DIMENSION(nvm)                        :: height_presc_12 !! [DISPENSABLE] Prescribed height of each pfts (m)
...
!_ ================================================================================================================================
  ...
  !! 2. Calculate (or prescribe) crown area
    ...
    DO j = 2,nvm ! loop over PFTs
       IF (is_tree(j)) THEN
          ...
          IF (natural(j)) THEN
             ...
             WHERE (PFTpresent(:,j) .AND.woodmass_ind(:,j).GT.min_stomate)
                ...
                !! biomass*veget_cov_max/ind, not as stated here, correction MERGE 
                !!         woodmass(:) = &
                ...
                cn_ind(:,j) = pipe_tune1(j)*MIN(dia(:),maxdia(j))**pipe_tune_exp_coeff(j)
       ...
       !!!$S. Zaehle : since now all state variables are defined on veget_cov_max it is very
       !!!$ dangerous to change this several times in stomate_lpj, as then GPP, turnover and allocated 
       !!!$ biomass are not defined on the same space! Hence, veget_cov_max is now kept constant
