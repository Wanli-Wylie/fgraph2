...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_stomate/lpj_kill.f90 $
...
!_ ================================================================================================================================
 ...
 !! 0. Variable and parameter description
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: lai             !! [DISPENSABLE] leaf area index OF AN INDIVIDUAL PLANT 
                                                                                 !! @tex $(m^2 m^{-2})$ @endtex
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: npp_longterm    !! "Long term" (default = 3-year) net primary 
                                                                                 !! productivity 
    ...
    INTEGER                                            :: j,m,ipar             !! Indices       (unitless)
    ...
    DO j = 2,nvm ! loop plant functional types
       ...
       IF ( natural(j) ) THEN
          ...
          ELSE
             ...
             !! 1.2.1 Kill PFTs
             WHERE ( PFTpresent(:,j) .AND. & 
                  (((biomass(:,j,icarbres,icarbon) .LE.zero).AND.(biomass(:,j,ilabile,icarbon) .LE.zero)) .OR. & 
          ...
          IF ( ANY( was_killed(:) ) ) THEN
             ...
             DO m = 1,nelements
                ...
                WHERE ( was_killed(:) )
                   ...
                   bm_to_litter(:,j,ilabile,m) = bm_to_litter(:,j,ilabile,m) + biomass(:,j,ilabile,m)
