...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declaration 
    ...
    REAL, INTENT(in)                        :: dt_days          !! Time step of vegetation dynamics for stomate (days)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: veget_max_old    !! Current "maximal" coverage fraction of a PFT (LAI
                                                                       !! -> infinity) on ground
    REAL, DIMENSION(npts), INTENT(in)       :: harvestwood      !! Harvested wood biomass (gC m-2 yr-1)
    ...
    REAL, DIMENSION(npts,0:10), INTENT(inout)  :: prod10_harvest  !! products remaining in the 10/100 year-turnover pool 
                                                                         !! after the annual release for each compartment
                                                                         !! (10 or 100 + 1 : input from year of land cover change)
    REAL, DIMENSION(npts,0:100), INTENT(inout) :: prod100_harvest !! see prod10_havest
    REAL, DIMENSION(npts,10), INTENT(inout)    :: flux10_harvest  !! annual release from the 10 year-turnover pool compartments

    REAL, DIMENSION(npts,100), INTENT(inout)   :: flux100_harvest !! annual release from the 100 year-turnover pool compartments
                                                   
    !! 0.4 Local variables

    INTEGER            :: i, j, l, m, n    !! indices (unitless)
    REAL               :: harvest       !! wood harvest renomalized by forest fraction
    REAL,DIMENSION(nelements)               :: relharvest    !! relative harvest fraction of each pft 
    REAL,DIMENSION(nelements)               :: relbiomass    !! relative biomass of each PFT 
    REAL               :: vegettree     !! vegfrac of trees
    REAL, DIMENSION(nelements)      :: abovemean     !! mean aboveground biomass @tex ($gC m^{-2}$) @endtex
    REAL, DIMENSION(nelements)      :: remove_biomass_heartabove !! Removed biomass from heartabove pool
    REAL, DIMENSION(nelements)      :: remove_biomass_sapabove   !! Removed biomass from sapabove pool
    REAL, DIMENSION(nelements)      :: r_h2s                     !! ratio of heart to sapwood biomass
    REAL, DIMENSION(nelements)      :: wood_mass                 !! sum of heart and sapwood
...
!_ ================================================================================================================================
  ...
  !! 1. initialization
    ...
    DO i = 1, npts  
       ...
       DO j=1, nvm
          if (is_tree(j)) THEN
             abovemean(:)=abovemean(:)+((biomass(i,j,iheartabove,:)+biomass(i,j,isapabove,:)))*SPREAD(veget_max_old(i,j),NCOPIES=nelements,DIM=1)
       ...
       IF (vegettree > min_stomate) THEN
          ...
          abovemean(:)=abovemean(:) /SPREAD(vegettree,NCOPIES=nelements,DIM=1)

          !! 2.4 wood harvest for each PFT is estimated proportionally to the relative biomass.
          DO j=1,nvm
             relharvest=0
             IF ((is_tree(j)) .AND. (abovemean(icarbon) > min_stomate)  .AND. &
                  (biomass(i,j,iheartabove,icarbon)+biomass(i,j,isapabove,icarbon)>min_stomate))  THEN

                relbiomass(:)=((biomass(i,j,iheartabove,:)+biomass(i,j,isapabove,:))/abovemean(:)) 
                relharvest(:)=SPREAD(harvest,NCOPIES=nelements,DIM=1)*relbiomass(:)

!DSGdbg                IF (( j == 4 )) THEN 
!DSGdbg                   WRITE(numout,*)'pixel i',i
!DSGdbg                   WRITE(numout,*)'relharvest',relharvest
!DSGdbg                   WRITE(numout,*)'above_mean',abovemean
!DSGdbg                ENDIF

                !DSG_debug: you must compute relharvest for each nelement to avoid negative pools
                !! 2.5 the harvest could not be higher than 20% of the biomasse to avoid rapid killing of vegetation
                WHERE ((biomass(i,j,iheartabove,:)+biomass(i,j,isapabove,:))*0.2<relharvest(:))
                     relharvest(:)=(biomass(i,j,iheartabove,:)+biomass(i,j,isapabove,:))*0.2
                ENDWHERE
                !DSG_debug: 
                !DSG might not needed anymore to compute element specific
                wood_mass(:) = biomass(i,j,iheartabove,:) + biomass(i,j,isapabove,:)
                r_h2s(:)     = biomass(i,j,iheartabove,:) / wood_mass(:)

                remove_biomass_heartabove(:) = relharvest(:) * r_h2s(:)       
                remove_biomass_sapabove(:)   = relharvest(:) * (un - r_h2s(:))
                !DSG
!DSGdbg
!DSGdbg                IF (( j == 4 )) THEN 
!DSGdbg                   WRITE(numout,*)'biomass(i,4,iheartabove,initrogen), i',biomass(i,4,iheartabove,initrogen), i
!DSGdbg                   WRITE(numout,*)'remove_biomass_heartabove(initrogen)',remove_biomass_heartabove(initrogen)
!DSGdbg                   WRITE(numout,*)'r_h2s(:)=', r_h2s(:)
!DSGdbg                   WRITE(numout,*)'wood_mass(:)',wood_mass(:)
!DSGdbg                   WRITE(numout,*)'relharvest',relharvest
!DSGdbg                ENDIF
                ! update biomass: 
                 biomass(i,j,iheartabove,:)   = biomass(i,j,iheartabove,:) - remove_biomass_heartabove(:)
                 biomass(i,j,isapabove  ,:)   = biomass(i,j,isapabove  ,:) - remove_biomass_sapabove(:)

                ! for testing: return nutrients to plants:
                ! biomass(i,j,icarbres,initrogen)  = biomass(i,j,icarbres,initrogen)   + remove_biomass_heartabove(initrogen) +remove_biomass_sapabove(initrogen)
                ! biomass(i,j,icarbres,iphosphorus)= biomass(i,j,icarbres,iphosphorus) + remove_biomass_heartabove(iphosphorus) + remove_biomass_sapabove(iphosphorus)

                ! 2.6 calculation of different flux like for LU
                convflux_harvest(i)  = convflux_harvest(i) +coeff_lcchange_1(j)*relharvest(icarbon)*veget_max_old(i,j)
                harvestpft(i,j)=relharvest(icarbon)*dt_days/one_year
                prod10_harvest(i,0)  = prod10_harvest(i,0)  +coeff_lcchange_10(j)*relharvest(icarbon)*veget_max_old(i,j)
                prod100_harvest(i,0) = prod100_harvest(i,0) +coeff_lcchange_100(j)*relharvest(icarbon)*veget_max_old(i,j)
             ENDIF
          ENDDO
       ENDIF
