...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_stomate/lpj_pftinout.f90 $
...
!end gmjc
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL, INTENT(in)                                   :: dt              !! Time step of vegetation dynamics for stomate
                                                                                 !! (days)               
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: adapted         !! Winter not too cold (unitless)   
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: regenerate      !! Winter sufficiently cold (unitless)  
    INTEGER, DIMENSION(npts,NbNeighb), INTENT(in)      :: neighbours      !! Indices of the 8 neighbours of each grid point
                                                                                 !! (unitless); 1=North and then clockwise.
    ...
    REAL, DIMENSION(npts), INTENT(out)                 :: avail_tree      !! Space availability for trees (unitless)   
    REAL, DIMENSION(npts), INTENT(out)                 :: avail_grass     !! Space availability for grasses (unitless)      


    !! 0.3 Modified variables   

    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout) :: biomass  !! Biomass (gC m^{-2})      
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: ind             !! Density of individuals (m^{-2})
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: RIP_time        !! How much time ago was the PFT eliminated for 
                                                                                 !! the last time (years)    
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: co2_to_bm       !! C biomass uptaken (gC m^{-2} day^{-1})   
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: n_to_bm         !! N biomass uptaken (gN m^{-2} day^{-1})   
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: p_to_bm         !! P biomass uptaken (gN m^{-2} day^{-1})   
 
    !! 0.4 Local variables

    REAL, DIMENSION(npts)                              :: avail           !! availability           
    INTEGER                                            :: i,j,m,ipar           !! indices      
    REAL, DIMENSION(npts)                              :: sumfrac_wood    !! total woody vegetation cover     
    INTEGER, DIMENSION(npts)                           :: n_present       !! number of adjacent grid cells where PFT is 
                                                                                 !! ubiquitous        
    LOGICAL, DIMENSION(npts)                                  :: can_introduce   !! we can introduce this PFT    
    REAL, DIMENSION(npts)                              :: fracnat         !! no real need for dimension(ntps) except for 
                                                                                 !! vectorisation          
!_ ================================================================================================================================
       ...
       IF ( .NOT. natural(j) ) THEN
          ...
          ELSE
             ...
             DO i = 1, npts ! Loop over # pixels - domain size
                ...
                IF ( ( veget_cov_max(i,j) .GT. min_stomate ) .AND. ( .NOT. PFTpresent(i,j) ) ) THEN

                   ! prescribed, but not yet there.
                   ind(i,j) = veget_cov_max(i,j)
                   biomass(i,j,:,:) = bm_sapl(j,:,:) * ind(i,j) /veget_cov_max(i,j) 
                   co2_to_bm(i,j) =  co2_to_bm(i,j) +SUM( biomass(i,j,:,icarbon) ) / dt
                   n_to_bm(i,j) =  n_to_bm(i,j) +SUM( biomass(i,j,:,initrogen) ) / dt
                   p_to_bm(i,j) =  p_to_bm(i,j) +SUM( biomass(i,j,:,iphosphorus) ) / dt
                   PFTpresent(i,j) = .TRUE.
                   everywhere(i,j) = un
  ...
  !! 6. Introduce PFTs
    ...
    DO j = 2,nvm ! Loop over # PFTs
       ...
       IF ( natural(j) ) THEN
          ...
          IF ( NbNeighb /= 8 ) THEN
             CALL ipslerr(3, "pftinout", "This routine needs to be adapted to non rectengular grids", &
                  &           "Talk to Jan Polcher", " ")
          ENDIF
          ...
          WHERE ( can_introduce(:) )
             ...
             WHERE(veget_cov_max(:,j) .GT. min_stomate)
                biomass(:,j,ileaf,icarbon) = bm_sapl(j,ileaf,icarbon) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapabove,icarbon) = bm_sapl(j,isapabove,icarbon) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapbelow,icarbon) = bm_sapl(j,isapbelow,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartabove,icarbon) = bm_sapl(j,iheartabove,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartbelow,icarbon) = bm_sapl(j,iheartbelow,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iroot,icarbon) = bm_sapl(j,iroot,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ifruit,icarbon) = bm_sapl(j,ifruit,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,icarbres,icarbon) = bm_sapl(j,icarbres,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ilabile,icarbon) = bm_sapl(j,ilabile,icarbon) * ind(:,j)/veget_cov_max(:,j)

                biomass(:,j,ileaf,initrogen) = bm_sapl(j,ileaf,initrogen) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapabove,initrogen) = bm_sapl(j,isapabove,initrogen) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapbelow,initrogen) = bm_sapl(j,isapbelow,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartabove,initrogen) = bm_sapl(j,iheartabove,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartbelow,initrogen) = bm_sapl(j,iheartbelow,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iroot,initrogen) = bm_sapl(j,iroot,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ifruit,initrogen) = bm_sapl(j,ifruit,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,icarbres,initrogen) = bm_sapl(j,icarbres,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ilabile,initrogen) = bm_sapl(j,ilabile,initrogen) * ind(:,j)/veget_cov_max(:,j)

                biomass(:,j,ileaf,iphosphorus) = bm_sapl(j,ileaf,iphosphorus) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapabove,iphosphorus) = bm_sapl(j,isapabove,iphosphorus) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapbelow,iphosphorus) = bm_sapl(j,isapbelow,iphosphorus) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartabove,iphosphorus) = bm_sapl(j,iheartabove,iphosphorus) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartbelow,iphosphorus) = bm_sapl(j,iheartbelow,iphosphorus) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iroot,iphosphorus) = bm_sapl(j,iroot,iphosphorus) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ifruit,iphosphorus) = bm_sapl(j,ifruit,iphosphorus) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,icarbres,iphosphorus) = bm_sapl(j,icarbres,iphosphorus) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ilabile,iphosphorus) = bm_sapl(j,ilabile,iphosphorus) * ind(:,j)/veget_cov_max(:,j)
             ...
             ELSEWHERE              
                ...
                biomass(:,j,ilabile,icarbon) = bm_sapl(j,ilabile,icarbon) * ind(:,j) 

                biomass(:,j,ileaf,initrogen) = bm_sapl(j,ileaf,initrogen) * ind(:,j)
                biomass(:,j,isapabove,initrogen) = bm_sapl(j,isapabove,initrogen) * ind(:,j)
                biomass(:,j,isapbelow,initrogen) = bm_sapl(j,isapbelow,initrogen) * ind(:,j)
                biomass(:,j,iheartabove,initrogen) = bm_sapl(j,iheartabove,initrogen) * ind(:,j)
                biomass(:,j,iheartbelow,initrogen) = bm_sapl(j,iheartbelow,initrogen) * ind(:,j)
                biomass(:,j,iroot,initrogen) = bm_sapl(j,iroot,initrogen) * ind(:,j)
                biomass(:,j,ifruit,initrogen) = bm_sapl(j,ifruit,initrogen) * ind(:,j)
                biomass(:,j,icarbres,initrogen) = bm_sapl(j,icarbres,initrogen) * ind(:,j)
                biomass(:,j,ilabile,initrogen) = bm_sapl(j,ilabile,initrogen) * ind(:,j) 

                biomass(:,j,ileaf,iphosphorus) = bm_sapl(j,ileaf,iphosphorus) * ind(:,j)
                biomass(:,j,isapabove,iphosphorus) = bm_sapl(j,isapabove,iphosphorus) * ind(:,j)
                biomass(:,j,isapbelow,iphosphorus) = bm_sapl(j,isapbelow,iphosphorus) * ind(:,j)
                biomass(:,j,iheartabove,iphosphorus) = bm_sapl(j,iheartabove,iphosphorus) * ind(:,j)
                biomass(:,j,iheartbelow,iphosphorus) = bm_sapl(j,iheartbelow,iphosphorus) * ind(:,j)
                biomass(:,j,iroot,iphosphorus) = bm_sapl(j,iroot,iphosphorus) * ind(:,j)
                biomass(:,j,ifruit,iphosphorus) = bm_sapl(j,ifruit,iphosphorus) * ind(:,j)
                biomass(:,j,icarbres,iphosphorus) = bm_sapl(j,icarbres,iphosphorus) * ind(:,j)
                biomass(:,j,ilabile,iphosphorus) = bm_sapl(j,ilabile,iphosphorus) * ind(:,j) 
             ...
             co2_to_bm(:,j) = &
                  ...
                  biomass(:,j,ilabile,icarbon) )/dt

             n_to_bm(:,j) = &
                  n_to_bm(:,j) +  &
                  ( biomass(:,j,ileaf,initrogen) + biomass(:,j,isapabove,initrogen) + &
                  biomass(:,j,isapbelow,initrogen) + biomass(:,j,iheartabove,initrogen) + &
                  biomass(:,j,iheartbelow,initrogen) + biomass(:,j,iroot,initrogen) + &
                  biomass(:,j,ifruit,initrogen) + biomass(:,j,icarbres,initrogen)+ &
                  biomass(:,j,ilabile,initrogen) )/dt

             p_to_bm(:,j) = &
                  p_to_bm(:,j) +  &
                  ( biomass(:,j,ileaf,iphosphorus) + biomass(:,j,isapabove,iphosphorus) + &
                  biomass(:,j,isapbelow,iphosphorus) + biomass(:,j,iheartabove,iphosphorus) + &
                  biomass(:,j,iheartbelow,iphosphorus) + biomass(:,j,iroot,iphosphorus) + &
                  biomass(:,j,ifruit,iphosphorus) + biomass(:,j,icarbres,iphosphorus)+ &
                  biomass(:,j,ilabile,iphosphorus) )/dt
...
! need to cycle for nparts
         DO ipar = 1,nparts
          WHERE ( can_introduce(:) )
           leaf_frac(:,j,ipar,1) = un
           !JC is this a bug? with only leaf_frac age class 1 is defined to
           !un
           leaf_frac(:,j,ipar,2) = zero
           leaf_frac(:,j,ipar,3) = zero
           leaf_frac(:,j,ipar,4) = zero
          ENDWHERE
         ENDDO
