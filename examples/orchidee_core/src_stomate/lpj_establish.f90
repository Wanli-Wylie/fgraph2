...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_stomate/lpj_establish.f90 $
!! $Date: 2018-09-13 17:44:45 +0200 (四, 2018-09-13) $
!! $Revision: 5407 $
!! \n
!_ ================================================================================================================================
...
!_ ================================================================================================================================
  ...
  SUBROUTINE establish (npts, dt, PFTpresent, regenerate, &
       ...
       soil_n_min, n_uptake, soil_p_min, p_uptake, &
    ...
    INTEGER, INTENT(in)                                :: npts            !! Domain size - number of pixels (dimensionless)    
    REAL, INTENT(in)                                   :: dt              !! Time step of vegetation dynamics for stomate 
                                                                                 !! (days)            
    LOGICAL, DIMENSION(npts,nvm), INTENT(in)                  :: PFTpresent      !! Is pft there (unitless)    
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: regenerate      !! Winter sufficiently cold (unitless)   
    INTEGER, DIMENSION(npts,NbNeighb), INTENT(in)      :: neighbours      !! indices of the neighbours of each grid point 
                                                                                 !! (unitless);  
                                                                                 !! [1=North and then clockwise]  
    REAL, DIMENSION(npts,2), INTENT(in)                :: resolution      !! resolution at each grid point (m); 1=E-W, 2=N-S     
    ...
    REAL, DIMENSION(npts), INTENT(in)                  :: precip_annual   !! annual precipitation (mm year^{-1}) 
    REAL, DIMENSION(npts), INTENT(in)                  :: gdd0            !! growing degree days (degree C)
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: lm_lastyearmax  !! last year's maximum leaf mass for each PFT 
                                                                                 !! (gC m^{-2 })
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: cn_ind          !! crown area of individuals (m^2)        
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: lai             !! leaf area index OF an individual plant 
                                                                                 !! (m^2 m^{-2})           
    REAL, DIMENSION(npts), INTENT(in)                  :: avail_tree      !! space availability for trees (unitless)     
    REAL, DIMENSION(npts), INTENT(in)                  :: avail_grass     !! space availability for grasses (unitless)  
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: npp_longterm    !! longterm NPP, for each PFT (gC m^{-2})   
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: veget_cov_max   !! "maximal" coverage fraction of a PFT 
                                                                                 !! (LAI -> infinity) on ground (unitless)
    REAL, DIMENSION(npts,nvm),INTENT(in)               :: mortality       !! Fraction of individual dying this time 
    ...
    REAL, DIMENSION(npts,nvm,nparts,nleafages), INTENT(inout) :: leaf_frac       !! fraction of leaves in leaf age class (unitless)    
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: ind             !! Number of individuals (individuals m^{-2})          
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout):: biomass   !! biomass (gC m^{-2 })     
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: everywhere      !! is the PFT everywhere in the grid box or very 
                                                                                 !! localized (unitless)    
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: co2_to_bm       !! biomass up take for establishment i.e. 
                                                                                 !! pseudo-photosynthesis (gC m^{-2} day^{-1}) 
    REAL, DIMENSION(npts,nvm,nnspec), INTENT(inout)    :: soil_n_min      !! Nitrogen in mineral soil,    
 !! (gN/(m**2 of PFT)) 
 !! to correct for nitrogen creation associated with establishmnet   
    REAL, DIMENSION(npts,nvm,nionspec), INTENT(inout)  :: n_uptake        !! Plant uptake of ammonium and nitrate  
 !! (gN/m**2/day) 
    REAL, DIMENSION(npts,nvm,npspec), INTENT(inout)           :: soil_p_min !! Phosphorus in mineral soil
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: p_uptake   !! Uptake of soil P by plants


    REAL, DIMENSION(npts,nvm), INTENT(in)              :: nstress_season  !! seasonal nitrogen stress  
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: pstress_season  !! seasonal phosphorus stress  
 !! (unitless) 
 !! (get's initialised at first establishment) 
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: woodmass_ind    !! woodmass of the individual, needed to calculate
                                                                                 !! crownarea in lpj_crownarea (gC m^{-2 })  
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout) :: bm_to_litter !!Biomass transfer to litter

    !! 0.4 Local variables

    REAL                                               :: tau_eatup       !! time during which a sapling can be entirely 
                                                                                 !! eaten by herbivores (days) 
    REAL, DIMENSION(npts,nvm)                          :: fpc_nat         !! new fpc, foliage projective cover: fractional
                                                                                 !! coverage (unitless)       
    REAL, DIMENSION(npts)                              :: estab_rate_max_climate_tree  !! maximum tree establishment rate, 
                                                                                              !! based on climate only (unitless)  
    REAL, DIMENSION(npts)                              :: estab_rate_max_climate_grass !! maximum grass establishment rate,
                                                                                              !! based on climate only (unitless)
    REAL, DIMENSION(npts)                              :: estab_rate_max_tree          !! maximum tree establishment rate, 
                                                                                              !! based on climate and fpc 
                                                                                              !! (unitless) 
    REAL, DIMENSION(npts)                              :: estab_rate_max_grass         !! maximum grass establishment rate,
                                                                                              !! based on climate and fpc 
                                                                                              !! (unitless) 
    REAL, DIMENSION(npts)                              :: sumfpc          !! total natural fpc (unitless)
    REAL, DIMENSION(npts)                              :: fracnat         !! total fraction occupied by natural 
                                                                                 !! vegetation (unitless)  
    REAL, DIMENSION(npts)                              :: sumfpc_wood     !! total woody fpc (unitless)    
    REAL, DIMENSION(npts)                              :: spacefight_grass!! for grasses, measures the total concurrence 
                                                                                 !! for available space (unitless)
    REAL, DIMENSION(npts,nvm)                          :: d_ind           !! change in number of individuals per time step 
                                                                                 !! (individuals m^{-2} day{-1})         
    REAL, DIMENSION(npts,nelements)                    :: bm_new          !! biomass increase (gC m^{-2 })        
    REAL, DIMENSION(npts,nvm,nparts,nelements)         :: biomass_old     !! Save the original biomass passed into the subroutine 
    REAL, DIMENSION(npts,nelements)                    :: bm_non          !! Non-effective establishment: the "virtual" saplings that die instantly
    REAL, DIMENSION(npts,nelements)                    :: bm_eff          !! Effective (or real) establishment
    REAL, DIMENSION(npts)                              :: dia             !! stem diameter (m)    
    REAL, DIMENSION(npts)                              :: b1              !! temporary variable           
    REAL, DIMENSION(npts)                              :: woodmass        !! woodmass of an individual (gC m^{-2})  
    REAL, DIMENSION(npts,nparts)                              :: leaf_mass_young !! carbon mass in youngest leaf age class 
                                                                                 !! (gC m^{-2})
    REAL, DIMENSION(npts)                              :: factor          !! reduction factor for establishment if many 
                                                                                 !! trees or grasses are present (unitless)  
    REAL, DIMENSION(npts,nelements)                    :: total_bm        !! Total carbon (or nitrogen) mass for all pools (gC m^{-2})    
    REAL, DIMENSION(npts,nelements)                    :: total_bm_sapl   !! Total sappling biomass for all pools 
                                                                                 !! (gC(orN) m^{-2})  
    REAL, DIMENSION(npts,nelements)                    :: total_bm_sapl_non !! total non-effective sapling biomass 

    INTEGER                                            :: nfrontx         !! from how many sides is the grid box invaded
                                                                                 !! (unitless?)   
    INTEGER                                            :: nfronty         !! from how many sides is the grid box invaded
                                                                                 !! (unitless?)   
    REAL, DIMENSION(npts)                              :: vn              !! flow due to new individuals veget_cov_max after 
                                                                                 !! establishment, to get a proper estimate of 
                                                                                 !! carbon and nitrogen 
    REAL, DIMENSION(npts)                              :: lai_ind         !! lai on each PFT surface (m^2 m^{-2})   
    REAL, DIMENSION(npts)                              :: veget_cov_max_tree  !! Sum of veget_cov_max for the pft's which are trees
    INTEGER                                            :: nbtree          !! Number of PFT's which are trees
    INTEGER                                            :: i,j,k,m         !! indices (unitless)       
    REAL, DIMENSION(npts)                              :: soil_n_constraint !! Soil inorganic N pool (gN m-2) 
    REAL, DIMENSION(npts)                              :: ntake           !! total N associated to establishment (gN) 
    REAL, DIMENSION(npts)                              :: ntake_nitrate   !! nitrate associated to establishment (gN) 
    REAL, DIMENSION(npts)                              :: ptake           !! total P associated to establishment (gP)
    REAL, DIMENSION(npts,nvm)                          :: ptake_avail !! total P associated to establishment (gP)
    REAL, DIMENSION(npts)                              :: soil_p_constraint !! soil inorganic P pool (gP m-2)
!DSG_added
    REAL, DIMENSION(npts,nvm)                          :: junk1           !! temporary array
    REAL, DIMENSION(npts,nvm)                          :: junk2           !! temporary array
    REAL, DIMENSION(npts,nvm)                          :: junk3           !! temporary array
!DSG_added
!_ ================================================================================================================================
  ...
  !! 1. messages and initialization
    ...
    ! time during which young plants can be completely eaten by herbivores after germination
    ! (and then individual die) assume to be half year   
    ! No reference
    tau_eatup = one_year/2.
    
    ! Calculate the sum of the vegetation over the tree pft's and the number of pft's which are trees
    veget_cov_max_tree(:) = 0.0
    nbtree=0
    DO j = 1, nvm 
       IF (is_tree(j)) THEN
          veget_cov_max_tree(:) = veget_cov_max_tree(:) + veget_cov_max(:,j)
          nbtree = nbtree + 1
       END IF
    END DO
    ! Set nbtree=1 to avoid zero division later if there are no tree PFT's.
    ! For that case veget_cov_max_tree=0 so there will not be any impact.
  ...
  !! 2. recalculate fpc
    ...
    IF (ok_dgvm) THEN
       ...
       !      to avoid equal veget_cov_max, the idea is that more reestablishment
       ...
       IF ( NbNeighb /= 8 ) THEN
          CALL ipslerr(3, "establish", "This routine needs to be adapted to non rectengular grids", "Talk to Jan Polcher", " ")
       ENDIF
    ...
    ELSE 
       ...
       DO j = 2,nvm ! Loop over # PFTs
          ...
          IF ( natural(j) .AND. is_tree(j)) THEN 
             ...
             WHERE (veget_cov_max(:,j).GT.min_stomate.AND.ind(:,j).LE.2.)
                ...
                !DSG: Effect of P-stress handled like N stress
                estab_rate_max_tree(:) = estab_max_tree * factor(:) * MAX(0.11 , MIN(nstress_season(:,j),pstress_season(:,j)) )
             ...
             WHERE (veget_cov_max(:,j).GT.min_stomate .AND. ind(:,j).EQ.zero)
                d_ind(:,j) = ind_0_estab
             ENDWHERE
          ...
          ELSEIF ( natural(j) .AND. .NOT.is_tree(j)) THEN 
             ...
             WHERE (veget_cov_max(:,j).GT.min_stomate)
  ...
  !! 4. Biomass calculation
    ...
    DO j = 2,nvm ! Loop over # PFTs
       ...
       IF ( natural(j) ) THEN ! only for natural PFTs
          ...
          leaf_mass_young(:,:) = leaf_frac(:,j,:,1) * biomass(:,j,:,icarbon)

          ! total biomass of existing PFT to limit biomass added from establishment
          total_bm(:,:) = zero

          DO k = 1, nparts
             total_bm(:,:) = total_bm(:,:) + biomass(:,j,k,:)
          ENDDO
          ! soil nitrogen available to feed plant establishment.  
          ! This needs to be taken from soil N. Reason: plants must not produce nitrogen by establishment 
          ! (otherwise this creates N) 
          soil_n_constraint(:)=soil_n_min(:,j,iammonium)+soil_n_min(:,j,initrate) 

          ! soil phosphorus available to feed plant establishment
          ! total dissolved is assumed as constraint rather than f_Pdissolved in
          ! around root system
          soil_p_constraint(:)=soil_p_min(:,j,ipdissolved)
          ...
          IF(ok_dgvm) THEN
             ...
             ! S. Zaehle calculate new woodmass_ind and veget_cov_max after establishment (needed for correct scaling!)
             ! essential correction for MERGE!
             IF(is_tree(j))THEN
                DO i=1,npts ! Loop over # pixels - domain size
                   IF((d_ind(i,j)+ind(i,j)).GT.min_stomate) THEN

                      IF((total_bm(i,icarbon).LE.min_stomate) .OR. (veget_cov_max(i,j) .LE. min_stomate)) THEN
                      ...
                      ! new diameter of PFT
                      dia(i) = (woodmass_ind(i,j)/(pipe_density(j)*pi/4.*pipe_tune2(j))) &
                           &                **(1./(2.+pipe_tune3(j)))
                      vn(i) = (ind(i,j) + d_ind(i,j))*pipe_tune1(j)*MIN(dia(i),maxdia(j))**pipe_tune_exp_coeff(j)
          ...
          ELSE 
             DO i=1,npts ! Loop over # pixels - domain size
                IF(is_tree(j).AND.(d_ind(i,j)+ind(i,j)).GT.min_stomate) THEN
                   IF(total_bm(i,icarbon).LE.min_stomate) THEN
             ...
             vn(:) = un ! cannot change in static!, and veget_cov_max implicit in d_ind

          ENDIF

          !! 4.4 total biomass of PFT added by establishment defined over veget_cov_max ...

          total_bm_sapl(:,:) = zero
          total_bm_sapl_non(:,:) = zero
          biomass_old(:,j,:,:)=biomass(:,j,:,:)
          DO k = 1, nparts ! Loop over # litter tissues (nparts=8); see 'stomate_constants.f90'
             WHERE(d_ind(:,j).GT.min_stomate.AND.total_bm(:,icarbon).GT.min_stomate.AND.veget_cov_max(:,j).GT.min_stomate)

                total_bm_sapl(:,icarbon) = total_bm_sapl(:,icarbon) + bm_sapl(j,k,icarbon) * d_ind(:,j) / veget_cov_max(:,j)
                total_bm_sapl(:,initrogen) = total_bm_sapl(:,initrogen) + bm_sapl(j,k,initrogen) * d_ind(:,j)  / veget_cov_max(:,j) 
                total_bm_sapl(:,iphosphorus) = total_bm_sapl(:,iphosphorus) + bm_sapl(j,k,iphosphorus) * d_ind(:,j)  / veget_cov_max(:,j)

                ! non-effective establishment
                total_bm_sapl_non(:,icarbon) = total_bm_sapl_non(:,icarbon) + &
                     bm_sapl(j,k,icarbon) * (ind(:,j)+d_ind(:,j))*mortality(:,j) / veget_cov_max(:,j)

                total_bm_sapl_non(:,initrogen) = total_bm_sapl_non(:,initrogen) + &
                     bm_sapl(j,k,initrogen) * (ind(:,j)+d_ind(:,j))*mortality(:,j) / veget_cov_max(:,j)
                
                total_bm_sapl_non(:,iphosphorus) = total_bm_sapl_non(:,iphosphorus) + &
                     bm_sapl(j,k,iphosphorus) * (ind(:,j)+d_ind(:,j))*mortality(:,j) / veget_cov_max(:,j)

             ENDWHERE
          ENDDO ! Loop over # litter tissues


          total_bm_sapl(:,initrogen)=MIN(total_bm_sapl(:,initrogen),soil_n_constraint(:)) 
          total_bm_sapl(:,iphosphorus)=MIN(total_bm_sapl(:,iphosphorus),soil_p_constraint(:)) 
          ntake(:)=zero 
          ptake(:)=zero
...
!corresponding biomass has been lost in lpj_gap).
          ...
          DO k = 1, nparts ! Loop over # litter tissues
             ...
             WHERE( d_ind(:,j).GT.min_stomate .AND. &
                  total_bm(:,icarbon).LE.min_stomate .AND. &
                  veget_cov_max(:,j).GT.min_stomate)

                bm_new(:,icarbon) = d_ind(:,j) * bm_sapl(j,k,icarbon) / veget_cov_max(:,j)
                biomass(:,j,k,icarbon) = biomass(:,j,k,icarbon) + bm_new(:,icarbon)

                bm_new(:,initrogen) = d_ind(:,j) * bm_sapl(j,k,initrogen) / veget_cov_max(:,j)
                biomass(:,j,k,initrogen) = biomass(:,j,k,initrogen) + bm_new(:,initrogen)

                bm_new(:,iphosphorus) = d_ind(:,j) * bm_sapl(j,k,iphosphorus) / veget_cov_max(:,j)
                biomass(:,j,k,iphosphorus) = biomass(:,j,k,iphosphorus) + bm_new(:,iphosphorus)

                ! bm_to_litter minus the 'non-effective' establishment (mortality), but cannot be less than 0
                WHERE((veget_cov_max_tree(:) .GT. 0.1) .AND. (veget_cov_max(:,j) .LT. veget_cov_max_tree(:)/nbtree) )

                   bm_non(:,icarbon) = MIN( biomass(:,j,k,icarbon)+bm_to_litter(:,j,k,icarbon), &
                        (ind(:,j)+d_ind(:,j))*mortality(:,j) * bm_sapl(j,k,icarbon)/veget_cov_max(:,j) )
                   bm_eff(:,icarbon) = MIN( npp_longterm(:,j)/365, bm_new(:,icarbon)-bm_non(:,icarbon) )
                   bm_non(:,icarbon) = MIN( biomass(:,j,k,icarbon)+bm_to_litter(:,j,k,icarbon), bm_new(:,icarbon)-bm_eff(:,icarbon) )

                   bm_non(:,initrogen) = MIN( biomass(:,j,k,initrogen)+bm_to_litter(:,j,k,initrogen), &
                        (ind(:,j)+d_ind(:,j))*mortality(:,j) * bm_sapl(j,k,initrogen)/veget_cov_max(:,j) )
                   bm_eff(:,initrogen) = bm_new(:,initrogen)-bm_non(:,initrogen) 
                   bm_non(:,initrogen) = MIN( biomass(:,j,k,initrogen)+bm_to_litter(:,j,k,initrogen), bm_new(:,initrogen)-bm_eff(:,initrogen) )

                   bm_non(:,iphosphorus) = MIN( biomass(:,j,k,iphosphorus)+bm_to_litter(:,j,k,iphosphorus), &
                        (ind(:,j)+d_ind(:,j))*mortality(:,j) * bm_sapl(j,k,iphosphorus)/veget_cov_max(:,j) )
                   bm_eff(:,iphosphorus) = bm_new(:,iphosphorus)-bm_non(:,iphosphorus)
                   bm_non(:,iphosphorus) = MIN( biomass(:,j,k,iphosphorus)+bm_to_litter(:,j,k,iphosphorus), bm_new(:,iphosphorus)-bm_eff(:,iphosphorus) )

                   ! I think there was an error 
                   ! co2_to_bm(:,j)=co2_to_bm(:,j) + bm_new(:) - bm_non(:)
                   co2_to_bm(:,j)=co2_to_bm(:,j) + bm_new(:,icarbon)/dt - bm_non(:,icarbon)/dt
                   ntake(:) = ntake(:) + bm_new(:,initrogen) - bm_non(:,initrogen)
                   ptake(:) = ptake(:) + bm_new(:,iphosphorus) - bm_non(:,iphosphorus)

                   WHERE( bm_to_litter(:,j,k,icarbon) .LT. bm_non(:,icarbon) )
                      biomass(:,j,k,icarbon) = biomass(:,j,k,icarbon) - ( bm_non(:,icarbon) - bm_to_litter(:,j,k,icarbon) )
                   ENDWHERE
                   bm_to_litter(:,j,k,icarbon) = bm_to_litter(:,j,k,icarbon) - MIN(bm_to_litter(:,j,k,icarbon), bm_non(:,icarbon) )

                   WHERE( bm_to_litter(:,j,k,initrogen) .LT. bm_non(:,initrogen) )
                      biomass(:,j,k,initrogen) = biomass(:,j,k,initrogen) - ( bm_non(:,initrogen) - bm_to_litter(:,j,k,initrogen) )
                   ENDWHERE
                   bm_to_litter(:,j,k,initrogen) = bm_to_litter(:,j,k,initrogen) - MIN(bm_to_litter(:,j,k,initrogen), bm_non(:,initrogen) )

                   WHERE( bm_to_litter(:,j,k,iphosphorus) .LT. bm_non(:,iphosphorus) )
                      biomass(:,j,k,iphosphorus) = biomass(:,j,k,iphosphorus) - ( bm_non(:,iphosphorus) - bm_to_litter(:,j,k,iphosphorus) )
                   ENDWHERE
                   bm_to_litter(:,j,k,iphosphorus) = bm_to_litter(:,j,k,iphosphorus) - MIN(bm_to_litter(:,j,k,iphosphorus), bm_non(:,iphosphorus) )

                ELSEWHERE

                   bm_non(:,icarbon) = MIN( bm_to_litter(:,j,k,icarbon), (ind(:,j)+d_ind(:,j))*mortality(:,j) * bm_sapl(j,k,icarbon)/veget_cov_max(:,j) )
                   co2_to_bm(:,j)=co2_to_bm(:,j) + bm_new(:,icarbon)/dt - bm_non(:,icarbon)/dt
                   bm_to_litter(:,j,k,icarbon)=bm_to_litter(:,j,k,icarbon)- bm_non(:,icarbon)

                   bm_non(:,initrogen) = MIN( bm_to_litter(:,j,k,initrogen), (ind(:,j)+d_ind(:,j))*mortality(:,j) * bm_sapl(j,k,initrogen)/veget_cov_max(:,j) )
                   ntake(:)= ntake(:) + bm_new(:,initrogen) - bm_non(:,initrogen)
                   bm_to_litter(:,j,k,initrogen)=bm_to_litter(:,j,k,initrogen)- bm_non(:,initrogen)


                   bm_non(:,iphosphorus) = MIN( bm_to_litter(:,j,k,iphosphorus), (ind(:,j)+d_ind(:,j))*mortality(:,j) * bm_sapl(j,k,iphosphorus)/veget_cov_max(:,j) )
                   ptake(:)= ptake(:) + bm_new(:,iphosphorus) - bm_non(:,iphosphorus)
                   bm_to_litter(:,j,k,iphosphorus)=bm_to_litter(:,j,k,iphosphorus)- bm_non(:,iphosphorus)
                ENDWHERE

             ENDWHERE

             ! establishment into existing population, C flows
             WHERE(d_ind(:,j).GT.min_stomate.AND.total_bm(:,icarbon).GT.min_stomate)

                bm_new(:,icarbon) = total_bm_sapl(:,icarbon) * biomass_old(:,j,k,icarbon) / total_bm(:,icarbon)
                biomass(:,j,k,icarbon) = biomass(:,j,k,icarbon) + bm_new(:,icarbon)

                bm_new(:,initrogen) = total_bm_sapl(:,initrogen) * biomass_old(:,j,k,initrogen) / total_bm(:,initrogen)
                biomass(:,j,k,initrogen) = biomass(:,j,k,initrogen) + bm_new(:,initrogen)

                bm_new(:,iphosphorus) = total_bm_sapl(:,iphosphorus) * biomass_old(:,j,k,iphosphorus) / total_bm(:,iphosphorus)
                biomass(:,j,k,iphosphorus) = biomass(:,j,k,iphosphorus) + bm_new(:,iphosphorus)

                WHERE((veget_cov_max_tree(:) .GT. 0.1) .AND. (veget_cov_max(:,j) .LT. veget_cov_max_tree(:)/nbtree) )
                   bm_non(:,icarbon) = MIN( biomass(:,j,k,icarbon)+bm_to_litter(:,j,k,icarbon), &
                        total_bm_sapl_non(:,icarbon) *biomass_old(:,j,k,icarbon)/total_bm(:,icarbon) )
                   bm_eff(:,icarbon) = MIN( npp_longterm(:,j)/one_year, bm_new(:,icarbon)-bm_non(:,icarbon) )
                   bm_non(:,icarbon) = MAX( zero, MIN( biomass(:,j,k,icarbon)+bm_to_litter(:,j,k,icarbon)-min_stomate, &
                        bm_new(:,icarbon)-bm_eff(:,icarbon) ) )

                   bm_non(:,initrogen) = MIN( biomass(:,j,k,initrogen)+bm_to_litter(:,j,k,initrogen), &
                        total_bm_sapl_non(:,initrogen) *biomass_old(:,j,k,initrogen)/total_bm(:,initrogen) )
                   bm_eff(:,initrogen) = bm_new(:,initrogen)-bm_non(:,initrogen)
                   bm_non(:,initrogen) = MAX( zero, MIN( biomass(:,j,k,initrogen)+bm_to_litter(:,j,k,initrogen)-min_stomate, &
                        bm_new(:,initrogen)-bm_eff(:,initrogen) ) )

                   bm_non(:,iphosphorus) = MIN( biomass(:,j,k,iphosphorus)+bm_to_litter(:,j,k,iphosphorus), &
                        total_bm_sapl_non(:,iphosphorus) *biomass_old(:,j,k,iphosphorus)/total_bm(:,iphosphorus) )
                   bm_eff(:,iphosphorus) = bm_new(:,iphosphorus)-bm_non(:,iphosphorus)
                   bm_non(:,iphosphorus) = MAX( zero, MIN( biomass(:,j,k,iphosphorus)+bm_to_litter(:,j,k,iphosphorus)-min_stomate, &
                        bm_new(:,iphosphorus)-bm_eff(:,iphosphorus) ) )

                   co2_to_bm(:,j)=co2_to_bm(:,j) + bm_new(:,icarbon) - bm_non(:,icarbon)
                   ntake(:) = ntake(:) + bm_new(:,initrogen) - bm_non(:,initrogen)
                   ptake(:) = ptake(:) + bm_new(:,iphosphorus) - bm_non(:,iphosphorus)
                ...
                ELSEWHERE
                   ...
                   bm_non(:,icarbon) = MIN( bm_to_litter(:,j,k,icarbon), &
                        total_bm_sapl_non(:,icarbon) *biomass_old(:,j,k,icarbon)/total_bm(:,icarbon) )
                   co2_to_bm(:,j) = co2_to_bm(:,j) + bm_new(:,icarbon)/dt - bm_non(:,icarbon)/dt
                   bm_to_litter(:,j,k,icarbon)=bm_to_litter(:,j,k,icarbon)- bm_non(:,icarbon)

                   bm_non(:,initrogen) = MIN( bm_to_litter(:,j,k,initrogen), &
                        total_bm_sapl_non(:,initrogen) *biomass_old(:,j,k,initrogen)/total_bm(:,initrogen) )
                   ntake(:) = ntake(:) + bm_new(:,initrogen) - bm_non(:,initrogen)
                   bm_to_litter(:,j,k,initrogen)=bm_to_litter(:,j,k,initrogen)- bm_non(:,initrogen)

                   bm_non(:,iphosphorus) = MIN( bm_to_litter(:,j,k,iphosphorus), &
                        total_bm_sapl_non(:,iphosphorus) *biomass_old(:,j,k,iphosphorus)/total_bm(:,iphosphorus) )
                   ptake(:) = ptake(:) + bm_new(:,iphosphorus) - bm_non(:,iphosphorus)
                   bm_to_litter(:,j,k,iphosphorus)=bm_to_litter(:,j,k,iphosphorus)- bm_non(:,iphosphorus)
                ENDWHERE
          ...
          ! JC comment not very sure the required ntake and ptake can be always
          ! fulfilled by soil nutrient
          ntake_nitrate(:)=MIN(ntake(:),MAX(soil_n_min(:,j,iammonium)-min_stomate,0.0)) 
          soil_n_min(:,j,iammonium)=soil_n_min(:,j,iammonium)-ntake_nitrate(:) 
          n_uptake(:,j,iammonium)=n_uptake(:,j,iammonium) + ntake_nitrate(:)/dt 
  
          ntake(:)=MIN(ntake(:)-ntake_nitrate(:),MAX(soil_n_min(:,j,initrate)-min_stomate,0.0)) 
          soil_n_min(:,j,initrate)=soil_n_min(:,j,initrate)-ntake(:) 
          n_uptake(:,j,initrate)=n_uptake(:,j,initrate) + ntake(:)/dt 

          ptake_avail(:,j) = MIN(ptake(:),MAX(soil_p_min(:,j,ipdissolved)-min_stomate,0.0))
          IF (ANY(ptake_avail(:,j) .LT. ptake(:)) ) THEN
             CALL ipslerr_p(3,'establish','there is not enough soil disolved P for establishment','','')
          ENDIF
          ! DSG: no need to remove it now from soil_p_min; p_uptake was already
          ! removed from soil_p_min thus we can add ptake_avail here:
          p_uptake(:,j)=p_uptake(:,j) + ptake_avail(:,j)/dt
          ! we update soil_p_min with ptake_avail at the end of this routine
          ! (outside the PFT loop)

          IF (ANY( bm_to_litter(:,j,:,icarbon) .LT. 0.0 ) .OR. ANY( biomass(:,j,:,icarbon) .LT. 0.0 ) ) THEN
...
! here, I consider all tissues
          DO k = 1, nparts ! Loop over # litter tissues

          WHERE ( d_ind(:,j) * bm_sapl(j,k,icarbon) .GT. min_stomate )
             ...
             leaf_age(:,j,k,1) = leaf_age(:,j,k,1) * leaf_mass_young(:,k) / &
                  ( leaf_mass_young(:,k) + d_ind(:,j) * bm_sapl(j,k,icarbon) )

          ENDWHERE

          leaf_mass_young(:,k) = leaf_mass_young(:,k) + d_ind(:,j) * bm_sapl(j,k,icarbon)   

          !! 4.7 Youngest class: new mass in youngest class divided by total new mass
          WHERE ( biomass(:,j,k,icarbon) .GT. min_stomate )
             ! new age class fractions (fraction in youngest class increases)
             leaf_frac(:,j,k,1) = leaf_mass_young(:,k) / biomass(:,j,k,icarbon)
          ...
          DO m = 2, nleafages
             ...
             WHERE ( biomass(:,j,k,icarbon) .GT. min_stomate )
                ...
                leaf_frac(:,j,k,m) = leaf_frac(:,j,k,m) * & 
                     ( biomass(:,j,k,icarbon) + d_ind(:,j) * bm_sapl(j,k,icarbon) ) /  biomass(:,j,k,icarbon)
...
!End JC separate tissue age
    ...
    ! We still have to remove the ptake_avail from the soil_p_min pool
    junk1=zero
    junk2=zero
    junk3=zero
    CALL phosphorus_freundlich(npts,dt,                             &
                               ...
                               junk1,                               & ! losses to strong sorption
                               junk2,                               & ! mass added magically
                               junk3,                               & ! gains
                               ptake_avail )                          ! losses
