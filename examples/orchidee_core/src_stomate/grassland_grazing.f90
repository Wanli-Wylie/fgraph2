...
!  USE parallel
  ...
  REAL, PARAMETER :: fnurine        = 0.6
  ! repartition de n dans l'urine et les fèces (-)
  REAL, PARAMETER :: kintake        = 1.0
  ! parameter zu intake (m**2/m**2)
  REAL, PARAMETER :: fmethane       = 0.03
  ! c-pertes en méthane (-)
  REAL, PARAMETER :: AnimalqintakeM = 3.0
  REAL, PARAMETER :: franimal       = 0.5
  ! c-pertes en respiration (-)

  ! parameter subroutine :: grazing_fonction
  REAL, PARAMETER :: rf1 = 0.17
  REAL, PARAMETER :: rf3 = 0.22
  REAL, PARAMETER :: rf7 = 0.36
  REAL, PARAMETER :: t_seuil_OMD = 288.15
  ! threshold temperature for calculation of temperature effect on OMD (K)
!gmjc 05Feb2016 avoid wet grazing
  REAL, PARAMETER :: ct_threshold = 10.0
  REAL, PARAMETER :: ct_max = 12
  REAL, PARAMETER :: moi_threshold = 0.99
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!!!!! Variables locales au module
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milk
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkn
  ! n dans le lait (kg n /(m**2*d))
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkc
  ! c dans le lait (kg c /(m**2*d))     
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: ranimal
  ! c perte en respiration (kg c /(m**2*d))
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Methane
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: faecesnsumprev
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkndaily  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: faecesndaily
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: urinendaily  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milksum
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: nelgrazingsum  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkcsum                       
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: ranimalsum                     
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Methanesum            
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: urinecsum 
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: faecescsum   
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: faecesnsum
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: urinensum 
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milknsum                       
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milknsumprev  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: urinensumprev 
  INTEGER   , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: stockingstart
  INTEGER   , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: stockingend
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: wshtotstart   
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: grazingc
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: grazingn
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementc
  ! C flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementn
  ! N flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementcsum
  ! C flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: forage_complementnsum
  ! N flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: grazingsum
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: grazingcsum
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: grazingnsum       
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: grazingnsumprev       
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: grazingndaily     
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: methane_ani
  ! Enteric methane emission per animal(kg C animal-1 d-1)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: methane_aniSum
  ! Annual enteric methane emission per animal(kg C animal-1 )
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkanimalSum
  ! Annual milk production per animal(kg C animal-1 )
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: milkanimal
  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: ugb
  ! equals 0 (no animals) or 1 (animals)
  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: ok_ugb
  ! 1 if autogestion is optimal; 0 else
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: extra_feed
  ...
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowN
  ! N in daily milk production per m2 for primiparous or multiparous cows (kgN/m2/d)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowC
  ! C in daily milk production per m2 for primiparous or multiparous cows (kgC/m2/d)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowCsum
  ! Cumulated C in milk production per m2 for primiparous or multiparous cows (kgC/m2)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  MPcowNsum
  ! Cumulated N in milk production per m2 for primiparous or multiparous cows (kgN/m2)

  !Intake cow
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowsum
  ! Cumulated intake per m2 for primiparous or multiparous cows(kg/m2)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowNsum
  ! N in Cumulated intake per m2 for primiparous or multiparous cows(kgN/m2)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowCsum
  ! C in Cumulated intake per m2 for primiparous or multiparous cows(kgC/m2)    
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:)   ::  DMIcowanimalsum
  ! Cumulated animal intake for primiparous or multiparous cows(kg/animal)
  !local module variable for calves
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Wanimalcalf
  ! Calf liveweigth (kg/animal)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  DMIcalfsum
  ! Cumulated calf intake per m2(kg/m2)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  DMIcalfnsum
  ! N in cumulated calf intake per m2(kgN/m2)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  DMIcalfanimalsum
  ! Cumulated calf intake per animal kg/animal)  

  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Tcalving
  ! Calving date (d)  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Tsevrage
  ! Suckling period of calves (d)  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Age_sortie_calf
  ! Calf age at sale (d)  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Pyoung
  ! Fraction of young or primiparous in the cattle (-)  
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Wcalfborn
  ! Calf liveweigth at birth (kg/animal)
  INTEGER,      ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  calfinit
  ! Boolean to calf weight computation
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  Wanimalcalfinit
  ! Initial calf liveweigth (kg/animal) (birth liveweight or liveweight at the beginning of the grazing period)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   ::  nanimaltot_prec
  ! nanimaltot at previous time step (animal/m2)

  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Gestation
  ! equals 0 (outside of the gestation period) or 1 (during gestation)
  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Calf
  ! equals 0 (when calves are sale or at barn) or 1 (when calves are at pasture)

  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: compte_pature
  ! Number of the pasture periode when stocking rate automanagement
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: autogestion_weightcow
  ! Initial cow liveweight when stocking rate automanagement (kg/animal)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: autogestion_BCScow
  ! Initial BCS when stocking rate automanagement (-)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: autogestion_AGEcow
  ! Initial age when stocking rate automanagement (months)
  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: autogestion_init
  ! to intialize cow liveweight and BCS the first time step when f_autogestion=2
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: QIc
  ! to intialize concentrate amount per kg of milk per day or per kg of Liveweight per day (Kg)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: EVf
  ! to intialize forage energy content  (UF/kg)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: FVf
  ! to intialize forage fill value  (UE/kg)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: EVc
  ! to intialize concentrate energy content(UF/kg)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: fN_forage
  ! Nitrogen fraction in the forage (kgN/kg)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)   :: fN_concentrate
  ! Nitrogen fraction in the concentrate (kgN/kg)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: NEBcow_prec
  ! Net energy Balance at previous time step (young:1, mature:2)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: MPwmax
  ! Maximum of theoretical milk production (kg/animal/d)
  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: Fday_pasture
  ! the first julian day of the actual pasture periode
  INTEGER     , ALLOCATABLE, SAVE, DIMENSION(:,:)   :: delai_ugb
  ! time before start grazing is possible
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: Local_autogestion_out
  ! Fraction F (npts,1), ratio F (npts,2), and lenght of the grazing period when autgestion
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: PEmax
  ! Perte d'etat maximale des vaches laitières sur la periode de paturage 
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: PEpos
  ! Perte d'etat possible des vaches laitières au jour j
  REAL,              SAVE                 :: BM_threshold
  ! Biomass threshold above which animals are moved out the paddock (kg/m2)
  REAL,              SAVE                 :: BM_threshold_turnout
  ! [autogestion] Biomass threshold above which animals are moved in the paddock (kg/m2)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: DMIc
  ! concentrate ingested with auto-complementation (dairy cow only)
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: DMIf
  ! forage ingested with auto-complementation (suckler cow only)

  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:,:) :: NER
  ! Net energy requirement (MJ)

  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: Substrate_grazingwc
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: Substrate_grazingwn
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: grazingcstruct
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: grazingnstruct

  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: DNDFlam
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: DNDF
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: NDF
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: DNDFI
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: DNDFstem
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: DNDFear
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: NDFmean
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: plam
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: pstem
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: pear
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: MassePondTot
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: grazingstruct
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: grazinglam
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: grazingstem
  REAL, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: grazingear

!  REAL,ALLOCATABLE,    SAVE , DIMENSION(:,:) :: nb_grazingdays
  REAL,ALLOCATABLE,    SAVE , DIMENSION(:,:) :: amount_yield
  REAL,ALLOCATABLE,    SAVE , DIMENSION(:,:) :: consump
  REAL,ALLOCATABLE,    SAVE , DIMENSION(:,:) :: outside_food
  REAL,ALLOCATABLE,    SAVE , DIMENSION(:,:) :: add_nb_ani
!gmjc
  REAL,ALLOCATABLE,    SAVE , DIMENSION(:,:) :: ct_dry
! counter determine the days of wet/dry soil
  REAL, SAVE    :: buffer_snow = 3
  REAL, SAVE    :: buffer_wet = 0.05
  ...
  REAL,ALLOCATABLE,    SAVE , DIMENSION(:) :: t2m_below_zero
!end gmjc
  REAL, SAVE    ::   DNDFlam1             = 0.92
  REAL, SAVE    ::   DNDFlam2             = 0.82
  REAL, SAVE    ::   DNDFlam3             = 0.76
  REAL, SAVE    ::   DNDFlam4             = 0.74

  REAL, ALLOCATABLE,    SAVE , DIMENSION(:,:)    ::   NDFlam    !0.6
  REAL, ALLOCATABLE,    SAVE , DIMENSION(:,:)    ::   NDFstem   !0.7
  REAL, ALLOCATABLE,    SAVE , DIMENSION(:,:)    ::   NDFear    !0.8

  REAL, SAVE    ::   DNDFstem1             = 0.84
  REAL, SAVE    ::   DNDFstem2             = 0.65
  REAL, SAVE    ::   DNDFstem3             = 0.53
  REAL, SAVE    ::   DNDFstem4             = 0.50

  REAL, SAVE    ::   DNDFear1             = 0.76
  REAL, SAVE    ::   DNDFear2             = 0.48
  REAL, SAVE    ::   DNDFear3             = 0.30
  REAL, SAVE    ::   DNDFear4             = 0.26

  REAL, SAVE    ::   LimDiscremine        = 0.10
  
  INTEGER                  , SAVE                 :: mgraze_C3
  INTEGER                  , SAVE                 :: mgraze_C4
  INTEGER                  , SAVE                 :: mnatural_C3
  INTEGER                  , SAVE                 :: mnatural_C4
  REAL, SAVE :: read_min_grazing
  REAL, ALLOCATABLE,    SAVE , DIMENSION(:,:)      :: able_grazing
...
! END JCADD
    ...
    REAL                               , INTENT(in)    :: dt
    REAL, DIMENSION(npts,nvm)          , INTENT(in)    :: devstage
    ...
    REAL, DIMENSION(npts,nvm)          , INTENT(in)    :: intakemax
    ...
    REAL, DIMENSION(npts,nvm)          , INTENT(in)    :: Animalwgrazingmin
    !  ????----> LiLH
    REAL, DIMENSION(npts,nvm)          , INTENT(in)    :: AnimalkintakeM
    !  ????----> LiLH
    REAL, DIMENSION(npts,nvm)          , INTENT(out)   :: nel
    ! énergie nette de lactation (mj/kg)
    REAL, DIMENSION(npts,nvm)          , INTENT(in)    :: wanimal
    ! weight of lactating cows (kg)
    REAL, DIMENSION(npts,nvm)          , INTENT(inout) :: nanimaltot
    ! densité de paturage (gve/m**2)
    REAL, DIMENSION(npts,nvm)          , INTENT(in)    :: ntot
    ! concentration totale en n 
    REAL, DIMENSION(npts,nvm)          , INTENT(out)   :: intake
    ! intake
    REAL, DIMENSION(npts,nvm)          , INTENT(inout)   :: urinen
    ! n dans l'urine (kg n /(m**2 d))     
    REAL, DIMENSION(npts,nvm)          , INTENT(inout)   :: faecesn
    ! n dans les fèces (kg n /(m**2*d)) 
    REAL, DIMENSION(npts,nvm)          , INTENT(inout)   :: urinec
    ! c dans les urines
    REAL, DIMENSION(npts,nvm)          , INTENT(inout)   :: faecesc
    ! c dans les fèces (kg c /(m**2*d))
    REAL, DIMENSION(npts,nvm)          , INTENT(in)    :: tgrowth
    ...
    ! day julian
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(in)    :: nanimal
    ! densité du paturage  h (1,..,nstocking) (gve/m**2)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(in)    :: tanimal
    ! début du paturage    h (1,..,nstocking) (d)       
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(in)    :: danimal
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: intakesum
    ! Yearly intake (kg animal-1 y-1)
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: intakensum
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout)    :: intake_animal
    ! Daily intake per animal(kg animal-1 d-1)
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: intake_animalsum
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout):: trampling
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  ::  AnimalDiscremineQualite
    REAL, DIMENSION(npts,nvm), INTENT(inout)  ::  YIELD_RETURN
    REAL, DIMENSION(npts), INTENT(in)  ::  sr_ugb_init
    INTEGER                              , INTENT(in)    :: year_count1
    INTEGER                              , INTENT(in)    :: year_count2
    !gmjc for autogestion 5 grazing AGB and litter
    ! flag determine grazing litter (1) or AGB (0)
    INTEGER, DIMENSION(npts,nvm), INTENT(inout)  ::  grazing_litter
    ...
    REAL, DIMENSION(npts,nvm), INTENT(out)  ::  intake_animal_litter 
    ! animal intake kgDM/m^2/day
    REAL, DIMENSION(npts,nvm), INTENT(out)  ::  intake_litter
...
!end gmjc
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: Fert_PRP
! JCADD 20170421 for CNP merge
    REAL, DIMENSION(npts,nvm,nelements), INTENT(inout)  :: intakecnp
    REAL, DIMENSION(npts,nlitt,nvm,nlevs,nelements), INTENT(inout)  :: litter_intakecnp
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout)  :: tramplingcnp
    REAL, DIMENSION(npts,nvm,nelements), INTENT(inout)  :: faecescnp
    REAL, DIMENSION(npts,nvm,nelements), INTENT(inout)  :: urinecnp
    REAL, DIMENSION(npts,nvm,nelements), INTENT(inout)  :: milkcnp
    REAL, DIMENSION(npts,nvm,nelements), INTENT(inout)  :: liveweightcnp
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: ranimal_c
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: EFch4_c
...
! END JCADD
    ...
    REAL, DIMENSION(npts,nvm)      :: wshtotgrazing
    REAL, DIMENSION(npts,nvm)      :: deltaanimal

    INTEGER                          :: type_animal    
    ! local Variables:

    REAL     , DIMENSION(npts,nvm)   :: nb_ani_old
    ! Actual stocking rate per ha of total pasture "D" at previous iteration (animal (ha of total grassland)-1)
    REAL     , DIMENSION(npts,2) :: tampon
    REAL, DIMENSION(npts,nvm)            :: wshtotinit
    ...
    init_animal : IF (l_first_animaux) THEN
      ...
      read_min_grazing = 0.06
      CALL getin_p("READ_MIN_GRAZING",read_min_grazing)
      WRITE (numout,*) 'READ_MIN_GRAZING',read_min_grazing
      ...
      CALL variablesPlantes(&
           ...
           AnimalDiscremineQualite,&
    ! JCADD 20170421 for CNP merge
    ! just add some inout variables to make it clearer
       plam, pstem, pear, MassePondTot)
    ! END JCADD
    END IF init_animal
...
!            nb_ani(:,mgraze_C4) = sr_ugb(:,mgraze_C4) *grazed_frac(:,mgraze_C4)
    ...
    n_day : IF (new_day .EQV. .TRUE. ) THEN
      ...
      calc_nanimaltot  : IF ((tcutmodel .EQ. 0) .AND.  (f_autogestion .EQ. 0) &
        ...
        WHERE (wshtot(:,:) .GE. (read_min_grazing+0.02) .AND. &
               when_growthinit .GE. reserve_time_grass)
          delai_ugb(:,:) = delai_ugb(:,:) +1
          WHERE ((delai_ugb(:,:) .GE. 0) .AND. &
                 (nanimaltot(:,:) .GT. 0.0) .AND. &
                 devstage(:,:) .GT. un)
            ugb(:,:) = 1
          ELSEWHERE
            ugb(:,:) = 0
          ENDWHERE
        ELSEWHERE ((wshtot(:,:) .LT. (read_min_grazing+0.02)) .AND. &
            (wshtot(:,:) .GE. read_min_grazing) .AND. &
            when_growthinit .GE. reserve_time_grass)
          ...
          ELSEWHERE
            ugb(:,:) = 0
          ENDWHERE
        ELSEWHERE (wshtot(:,:) .LT. read_min_grazing)
...
!        ELSEWHERE (wshtot(:,mgraze_C4) .LT. min_grazing)
      ...
      ! with < 1 LSU of 2*2^(1-sr_ugb*1e4)*sr_ugb*1e4*125
...
!      IF (f_autogestion .EQ. 3) THEN
        able_grazing(:,mgraze_C3) = sr_ugb(:,mgraze_C3) * 1e4 * 130.0 * & 
               2.0**(1.0-(sr_ugb(:,mgraze_C3)*1e4))/1e3
        able_grazing(:,mgraze_C4) = sr_ugb(:,mgraze_C4) * 1e4 * 130.0 * &
               2.0**(1.0-(sr_ugb(:,mgraze_C4)*1e4))/1e3
...
!                  .OR. after_wet(:) .GT. 0.5 .OR. after_snow(:) .GT. 0.5)
          ...
          WHERE (ugb(:,mgraze_C3) .EQ. 1)
              compt_ugb(:,mgraze_C3)  = compt_ugb(:,mgraze_C3) + 1
!JCMODIF 20180731 modification for Mongolia, because the actuall grazing intensity is
!very low in Mongolia (NOTE Mongolia only)
...
!                  .OR. after_wet(:) .GT. 0.5 .OR. after_snow(:) .GT. 0.5)
          ...
          WHERE (ugb(:,mgraze_C4) .EQ. 1)
              compt_ugb(:,mgraze_C4)  = compt_ugb(:,mgraze_C4) + 1
!JCMODIF 20180731 modification for Mongolia, because the actuall grazing
!intensity is
!very low in Mongolia (NOTE Mongolia only)
...
!            ENDWHERE
          END WHERE
!!!!!! gmjc for global simulation with wild animal grazing natural grassland
        able_grazing(:,mnatural_C3) = sr_wild(:,mnatural_C3) * 1e4 * 130.0 * &
               2.0**(1.0-(sr_wild(:,mnatural_C3)*1e4))/1e3
        able_grazing(:,mnatural_C4) = sr_wild(:,mnatural_C4) * 1e4 * 130.0 * &
               2.0**(1.0-(sr_wild(:,mnatural_C4)*1e4))/1e3
      ...
      IF (f_autogestion .EQ. 5) THEN
        ...
        able_grazing(:,mgraze_C3) = sr_ugb(:,mgraze_C3) * 1e4 * 250.0 * & 
               2.0**(1.0-(sr_ugb(:,mgraze_C3)*1e4))/1e3
        able_grazing(:,mgraze_C4) = sr_ugb(:,mgraze_C4) * 1e4 * 250.0 * &
               2.0**(1.0-(sr_ugb(:,mgraze_C4)*1e4))/1e3
...
!        WRITE(numout,*) 'zd ','mgraze',mgraze_C3,grazing_litter(:,mgraze_C3)
    ...
    ! 1. calculate nel of grazing animal
    CALL nel_grazing_calcul(&
       npts, dt             , &
       nanimaltot         , &
       devstage, tgrowth, nel, &
       ntot)
!    WRITE (numout,*) 'before grazing_intake',biomass(:,10,:,icarbon)
    ! 2. calculate intake and intake_litter by animals
    ! intake intake_litter kg DM m-2
    CALL Grazing_intake(&
       npts, dt, wsh     , &
       intakemax         , &
       Animalwgrazingmin , &
    ...
    ! 3. calculate plant nutrient status and intake preference
    CALL variablesPlantes(&
       npts,biomass,&
       c,n,intake_animal,intakemax,&
       AnimalDiscremineQualite, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!! Modified subroutines for CNP merge
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! JCADD 20170421 for CNP merge
    ! I did not change the major subroutines
    ! but arrange them to fit the CNP calculation requirement


    ! 4. update biomass
    ! calculate intakecnp, litter_intakecnp, and tramplingcnp
    CALL chg_plante_cnp(&
     npts, dt, biomass, litter, &
     leaf_frac, litter_avail_totDM, &
     plam, pstem, pear, MassePondTot, &
     nanimaltot, intake_animal, intake, intake_litter, &
     grazing_litter, DNDF, NDF, DNDFI, &
     intakecnp,litter_intakecnp, &
     tramplingcnp)
!    WRITE (numout,*) 'after chg_plante_cnp',biomass(:,10,:,icarbon)
    ! 5. calculate milkcnp
    CALL Milk_Animal_cnp(&
     npts      , &
     dt        , &
     nel       , &
     intake_animal , &
    ...
    ! 6. calculate cnp in respiration and CH4 emission
    CALL Respiration_Methane_cnp(&
     npts       , &
     dt         , &
     intakecnp   , &
     litter_intakecnp , &
    ...
    ! 7. calculate cnp in urine and faeces
    CALL Urine_Faeces_cnp(&
     npts      , &
     dt        , &
     intakecnp , &
     litter_intakecnp , &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE Animal_Init(&
    ...
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(in) :: nanimal             ! Stocking density  h (1,..,nstocking) (animal m-2)
    INTEGER (i_std)                   ,  INTENT(in) :: type_animal         ! 1: Dairy cows, 2: Suckler cows, 3: Old module, 4: Dairy heifers, 5 : Suckler heifers
    REAL,                            INTENT(in) :: intake_tolerance    ! Intake tolerance threshold (-)
...
!gmjc
    ...
    IF(type_animal.EQ.1) THEN
          BM_threshold=LOG10((1.-intake_tolerance)/16.95)/(-0.00275*1e4)
          BM_threshold_turnout = LOG10((1- (intake_tolerance +0.1))/16.95)/(-0.00275*1e4)
    ELSE
          BM_threshold=LOG10(1.-intake_tolerance)/(-0.0012*1e4)
          BM_threshold_turnout=LOG10(1-(intake_tolerance +0.1))/(-0.0012*1e4)
    ENDIF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Grazing_intake(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: Animalwgrazingmin ! 0.03
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: AnimalkintakeM
    REAL, DIMENSION(npts,nvm), INTENT(out) :: intake
    REAL, DIMENSION(npts,nvm), INTENT(inout) :: intakesum
    ! Yearly intake per m2 (kg m-2 y-1)  
    REAL, DIMENSION(npts,nvm), INTENT(out)   :: intake_animal
    ! Daily intake per animal(kg animal-1 d-1)
    REAL, DIMENSION(npts,nvm), INTENT(inout) :: intake_animalsum
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)    :: n
    ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL, DIMENSION(npts,nvm), INTENT(in)    :: nanimaltot
    ! Stocking rate (animal m-2)
    REAL, DIMENSION(npts,nvm), INTENT(out) :: intake_litter
    ! Daily intake per animal(kg animal-1 d-1)
    REAL, DIMENSION(npts,nvm), INTENT(out)   :: intake_animal_litter    
    INTEGER, DIMENSION(npts,nvm), INTENT(in)    :: grazing_litter

    INTEGER           :: i,h,j
    REAL, DIMENSION(npts,nvm)  ::temp
    ...
    WRITE(numout,*) 'intake',intake(1,10),intake_animal(1,10),nanimaltot(1,10),wsh(1,10)
    WRITE(numout,*) 'threshold',Animalwgrazingmin-0.025,intakemax
    ! cumulated value
    ...
    ELSEIF (f_autogestion .EQ. 5 .OR. f_postauto .EQ. 5) THEN
      ! intake fresh biomass when grazing_litter == 0 or 2 
      WHERE (ugb(:,:) .EQ. 1 .AND. grazing_litter(:,:) .NE. 1 &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Milk_Animal(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: intake_animal
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: wanimal
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Respiration_Methane(&
    ...
    INTEGER                    , INTENT(in)  :: npts
    REAL                 , INTENT(in)  :: dt
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: grazingc
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot 
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: DNDFI
    ! Amount of digestible neutral detergent fiber in the intake (kg d-1)
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: Wanimal
    ! Animal life weight (kg)

    ! variables locales
    REAL, DIMENSION(npts,nvm) :: methane_ani !c im methan (kg c /(m**2*d))
    INTEGER           :: j
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Urine_Faeces(&
    ...
    INTEGER                    , INTENT(in)  :: npts
    REAL                 , INTENT(in)  :: dt
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: grazingn
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: grazingc       
    REAL, DIMENSION(npts,nvm), INTENT(out) :: urinen   
    REAL, DIMENSION(npts,nvm), INTENT(out) :: faecesn  
    REAL, DIMENSION(npts,nvm), INTENT(out) :: urinec
    REAL, DIMENSION(npts,nvm), INTENT(out) :: faecesc

    ! variables locales
    REAL, DIMENSION(npts,nvm) :: excretan 
    INTEGER           :: j
    !urine and faeces
    !(thornley 1998)
...
! JCADD 20170421 for CNP merge
  SUBROUTINE Milk_Animal_cnp(& 
    ...
    IF (f_autogestion .EQ. 0 .AND. f_postauto .EQ. 0 ) THEN
      ...
      WHERE (nanimaltot  .GT. 0)
        ...
        milkc      = 0.0588*milk
        milkn      = 0.00517*milk
        milkcnp(:,:,icarbon) = 0.0588*milk*1e3
        milkcnp(:,:,initrogen) = 0.00517*milk*1e3
        milkcnp(:,:,iphosphorus) = 0.0 
      ELSEWHERE
        milkanimal = 0.0
        milk = 0.0
        milkc = 0.0
  ...
  END SUBROUTINE Milk_Animal_cnp

  SUBROUTINE Respiration_Methane_cnp(&
     npts       , &
     dt         , &
     intakecnp   , &
     litter_intakecnp , &
    ...
    INTEGER                    , INTENT(in)  :: npts
    REAL                 , INTENT(in)  :: dt
    REAL, DIMENSION(npts,nvm,nelements), INTENT(in)  :: intakecnp
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: wanimal
    ! Animal life weight (kg)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: ranimal_c
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: EFch4_c
    ...
    ranimal_c = franimal * (intakecnp(:,:,icarbon) + &
              SUM(SUM(litter_intakecnp(:,:,:,:,icarbon),DIM=4),DIM=2))

!    methane = fmethane * intakecnp(:,:,icarbon) + &
!              SUM(SUM(litter_intakecnp(:,:,:,:,icarbon),DIM=4),DIM=2)
    ...
    WHERE (nanimaltot  .GT. 0.0)
        ...
        ! Inversion de ach4 & bch4
            ...
            methane_ani = (ach4 + bch4 * DNDFI)*wanimal*ch4toc
            !kg ch4_c / LU * LU/m2 * 1000 -> g ch4_c m-2
            EFch4_c  = methane_ani*nanimaltot*1e3
  ...
  END SUBROUTINE Respiration_Methane_cnp

  SUBROUTINE Urine_Faeces_cnp(&
     npts      , &
     dt        , &
     intakecnp , &
     litter_intakecnp , &
    ...
    REAL, DIMENSION(npts,nvm,nelements), INTENT(in)  :: intakecnp
    REAL, DIMENSION(npts,nlitt,nvm,nlevs,nelements), INTENT(in)  :: litter_intakecnp
    REAL, DIMENSION(npts,nvm,nelements), INTENT(in)  :: milkcnp
    REAL, DIMENSION(npts,nvm,nelements), INTENT(in)  :: liveweightcnp
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: ranimal_c
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: EFch4_c
    REAL, DIMENSION(npts,nvm,nelements), INTENT(inout)  :: urinecnp
    REAL, DIMENSION(npts,nvm,nelements), INTENT(inout)  :: faecescnp
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: wshtot
    INTEGER, DIMENSION(npts,nvm), INTENT(in)  :: ugb
    INTEGER, DIMENSION(npts,nvm), INTENT(in)  :: delai_ugb
    INTEGER, DIMENSION(npts,nvm), INTENT(in)  :: grazing_litter
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: sr_ugb
    ! variables locales
    REAL, DIMENSION(npts,nvm) :: excretan
    INTEGER           :: i,j
    !urine and faeces
    ...
    !n in excreta
    !all n intake - milkcnp - liveweightcnp
    excretan = intakecnp(:,:,initrogen) + &
             SUM(SUM(litter_intakecnp(:,:,:,:,initrogen),DIM=4),DIM=2) - &
             milkcnp(:,:,initrogen) - liveweightcnp(:,:,initrogen)
    IF (ANY(excretan .LT. 0.0)) THEN
      STOP "ERROR: negative excretan"
    END IF

    urinecnp(:,:,initrogen) = fnurine*excretan
    faecescnp(:,:,initrogen) = (1.0 - fnurine)*excretan
    !c respired and in excreta
    ! ï¿½quation (4.4e) de "grassland dynamics" thornley
    ! = urinen 12.0/28.0
    ! 12 => un atome de C
    ! 28 => deux atomes de N
    urinecnp(:,:,icarbon) = urinecnp(:,:,initrogen) * 12.0 / 28.0

    faecescnp(:,:,icarbon) = intakecnp(:,:,icarbon) + &
             SUM(SUM(litter_intakecnp(:,:,:,:,icarbon),DIM=4),DIM=2) - &
             milkcnp(:,:,icarbon) - &
             liveweightcnp(:,:,icarbon) - &
             urinecnp(:,:,icarbon) - &
             ranimal_c(:,:) - &
             EFch4_c(:,:)

    ! currently no phosphorus fluxes into liveweight and milk
    ! assuming all goes into faecescnp)
    faecescnp(:,:,iphosphorus) = intakecnp(:,:,iphosphorus) + &
             SUM(SUM(litter_intakecnp(:,:,:,:,iphosphorus),DIM=4),DIM=2) - &
             milkcnp(:,:,iphosphorus) - &
             liveweightcnp(:,:,iphosphorus) 


    IF (ANY(milkcnp .LT. 0.0)) THEN
      STOP "ERROR: negative milkcnp"
    END IF
    IF (ANY(liveweightcnp .LT. 0.0)) THEN
      STOP "ERROR: negative liveweightcnp"
    END IF
    IF (ANY(urinecnp .LT. 0.0)) THEN
      STOP "ERROR: negative urinecnp"
    END IF
    IF (ANY(ranimal_c .LT. 0.0)) THEN
      STOP "ERROR: negative ranimal_c"
    END IF
    IF (ANY(EFch4_c .LT. 0.0)) THEN
      STOP "ERROR: negative EFch4_c"
    END IF
    IF (ANY(faecescnp .LT. 0.0)) THEN
    DO j=2,nvm
      DO i=1,npts
        IF (ANY(faecescnp(i,j,:) .LT. 0.0)) THEN
          WRITE(numout,*) 'negative intake',i,j,intakecnp(i,j,:),milkcnp(i,j,:),liveweightcnp(i,j,:)
          WRITE(numout,*) 'negative litter_intake',SUM(SUM(litter_intakecnp(i,:,j,:,:),DIM=2),DIM=1)
          WRITE(numout,*) 'wshtot nanimaltot',wshtot(i,j),nanimaltot(i,j),ugb(i,j),delai_ugb(i,j)
          WRITE(numout,*) 'able grazing',able_grazing(i,j),grazing_litter(i,j),sr_ugb(i,j)
          WRITE(numout,*) 'negative urine',urinecnp(i,j,:),ranimal_c(i,j),EFch4_c(i,j)
          WRITE(numout,*) 'negative faeces',faecescnp(i,j,:)
        END IF
      END DO
    END DO
      STOP "ERROR: negative faecescnp"
    END IF

  END SUBROUTINE Urine_Faeces_cnp
! END JCADD
...
!!!!!!!!!!!!   they have been moved before Respiration
  ...
  SUBROUTINE nel_grazing_calcul(&
    ...
    ! variables locales :
    REAL, DIMENSION(npts,nvm)     :: os
    ! organische substanz (kg/kg)
    REAL, DIMENSION(npts,nvm)     :: rp
    ! rohproteingehalt (kg/kg)
    REAL, DIMENSION(npts,nvm)     :: be
    ! bruttoenergie (mj/kg)
    REAL, DIMENSION(npts,nvm)     :: vos
    ! verdauliche organische substanz (kg/kg) 
    REAL, DIMENSION(npts,nvm)     :: fvos
    REAL, DIMENSION(npts,nvm)     :: vp
    REAL, DIMENSION(npts,nvm)     :: ue
    ! energie métabolisable (mj/kg)
    REAL, DIMENSION(npts,nvm)     :: knel
    REAL, DIMENSION(npts,nvm)     :: rf
    ! rohfasergehalt (concentration en cellulose) (kg/kg) 
    REAL, DIMENSION(npts,nvm)     :: temp_ratio
...
!  END SUBROUTINE deallocation_animaux
  ...
  SUBROUTINE cal_grazing(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: n
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: fn
    REAL, DIMENSION(npts,nvm), INTENT(out) :: Substrate_grazingwc
    REAL, DIMENSION(npts,nvm), INTENT(out) :: Substrate_grazingwn
    REAL, DIMENSION(npts,nvm), INTENT(out) :: grazingcstruct
    REAL, DIMENSION(npts,nvm), INTENT(out) :: grazingnstruct
    REAL, DIMENSION(npts,nvm), INTENT(in) :: intake
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE chg_plante(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: intake_animal
    REAL, DIMENSION(npts,nvm), INTENT(out)       :: trampling
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: intake
    REAL, DIMENSION(npts,nvm), INTENT(inout)       :: DNDF
    REAL, DIMENSION(npts,nvm), INTENT(inout)       :: NDF
    REAL, DIMENSION(npts,nvm), INTENT(out)       :: DNDFI
    INTEGER, DIMENSION(npts,nvm), INTENT(in)    :: grazing_litter

    REAL, DIMENSION(npts,nvm) :: wlam
    ! masse sèche (structurelle) de la limbe (kg/m2) ----> dry mass (structural) of the lamina  
    REAL, DIMENSION(npts,nvm) :: wst
    ! masse sèche (structurelle) de la tige  (kg/m2) ----> dry mass (structural) of the stem
    REAL, DIMENSION(npts,nvm) :: wear
    ! masse sèche (structurelle) de la tige  (kg/m2) ----> dry mass (structural) of the ear
    REAL, DIMENSION(npts,nvm) :: lm_old_ani

    REAL, DIMENSION(npts,nvm) :: tmp_fracsum
    REAL, DIMENSION(npts,nvm,nleafages) :: tmp_frac
    INTEGER :: m

    REAL, DIMENSION(npts,nvm)     :: fGrazinglam
    REAL, DIMENSION(npts,nvm)     :: PlantLaminazlamgrazing
    REAL, DIMENSION(npts,nvm)     :: fGrazingstem
    REAL, DIMENSION(npts,nvm)     :: PlantEarzeargrazing
    REAL, DIMENSION(npts,nvm)     :: PlantStemzstemgrazing
...
! Initialisations    
    ...
    wlam(:,:) = (biomass(:,:,ileaf,icarbon)/(1e3*CtoDM)) / &
         (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )      ! leaf dry mass 
    wst(:,:)  = (biomass(:,:,isapabove,icarbon)/(1e3*CtoDM)) / &
         (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )      ! stem dry mass
    wear(:,:) = (biomass(:,:,ifruit,icarbon)/(1e3*CtoDM)) / &
    ...
    WHERE ((wlam .GT. 0.0) .AND. (MassePondTot .GT. 0.0) &
        ...
        DNDFlam = &
           DNDFlam1*leaf_frac(:,:,ileaf,1) + &
           DNDFlam2*leaf_frac(:,:,ileaf,2) + &
           DNDFlam3*leaf_frac(:,:,ileaf,3) + &
           DNDFlam4*leaf_frac(:,:,ileaf,4)
...
!print *, 'PlantLam'
    WHERE ((wst .GT. 0.0) .AND. (MassePondTot .GT. 0.0) .AND. &
        ...
        DNDFstem = &
           DNDFstem1*leaf_frac(:,:,ileaf,1) + &
           DNDFstem2*leaf_frac(:,:,ileaf,2) + &
           DNDFstem3*leaf_frac(:,:,ileaf,3) + &
           DNDFstem4*leaf_frac(:,:,ileaf,4)
...
! # structural dry matter flux from EARS into the animal per unit ground aera
    ...
    WHERE (wear .GT. 0.0)
        ...
        DNDFear = &
           DNDFear1*leaf_frac(:,:,ileaf,1) + &
           DNDFear2*leaf_frac(:,:,ileaf,2) + &
           DNDFear3*leaf_frac(:,:,ileaf,3) + &
           DNDFear4*leaf_frac(:,:,ileaf,4)
...
!print *, 'PlantEar',PlantEarzeargrazing(:,6)
    ...
    WHERE (leaf_frac(:,:,ileaf,1)*lm_old_ani .GT.  0.90 * grazinglam)
      !!if there is enough biomass of leaf age 1 for eating (0.7 of total intake), animal prefer to eat more
      !young leaf
      leaf_frac(:,:,ileaf,1) = (leaf_frac(:,:,ileaf,1)*lm_old_ani - 0.9 * grazinglam)/wlam

    ELSEWHERE
      !!if not enough biomass of leaf age 1 can be eat, only 10% of it left
      leaf_frac(:,:,ileaf,1) = (leaf_frac(:,:,ileaf,1)*lm_old_ani * 0.10)/wlam
    END WHERE
    ENDWHERE
    tmp_fracsum(:,:)=0.0
    tmp_frac(:,:,:)= 0.0
    DO m = 2, nleafages
      tmp_frac(:,:,m)= leaf_frac(:,:,ileaf,m)
      tmp_fracsum(:,:)= tmp_fracsum(:,:)+ tmp_frac(:,:,m)
    ENDDO
    DO m = 2, nleafages
      WHERE (tmp_fracsum(:,:) .GT. 0.0)
      leaf_frac(:,:,ileaf,m)=tmp_frac(:,:,m)/tmp_fracsum(:,:)*(1.0-leaf_frac(:,:,ileaf,1))
      ENDWHERE
    ENDDO
!print *,'after frac'
   ...
   ! when grazing litter, now assumed to be without trampling
    WHERE (nanimaltot(:,:) .GT. 0.0 .AND. grazing_litter(:,:) .NE. 1 )
       trampling(:,:) = nanimaltot(:,:) * 1e4 * 0.008 * &
               (wlam(:,:)+wst(:,:)+wear(:,:))* 1e3*CtoDM * &
               (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )       
       wlam(:,:) = wlam(:,:) * (1 - nanimaltot(:,:) * 1e4 * 0.008 )
       wst(:,:) = wst(:,:) * (1 - nanimaltot(:,:) * 1e4 * 0.008 )
       wear(:,:) =  wear(:,:) * (1 - nanimaltot(:,:) * 1e4 * 0.008 )
!!JCMODIF for gaps in NBP calculation
!       trampling(:,:) = nanimaltot * 1e4 * 0.008 *(biomass(:,:,ileaf)+biomass(:,:,isapabove)+biomass(:,:,ifruit))

    ELSEWHERE
       trampling(:,:) = 0.0
    ENDWHERE

    biomass(:,:,ileaf,icarbon)     = (wlam(:,:) * 1e3*CtoDM) * &
         (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )
    biomass(:,:,isapabove,icarbon) = (wst(:,:)  * 1e3*CtoDM) * &
         (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )
    biomass(:,:,ifruit,icarbon)    = (wear(:,:)  * 1e3*CtoDM) * &
         (1.0 + (mc /12.0)*c(:,:) + (mn /14.0)*n(:,:) )
...
! JCADD 20170421 for CNP merge
  SUBROUTINE chg_plante_cnp(&
    ...
    REAL, DIMENSION(npts,nvm,nparts,nleafages), INTENT(inout)       :: leaf_frac
    REAL, DIMENSION(npts,nvm), INTENT(inout)       :: litter_avail_totDM
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: plam
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: pstem
    REAL, DIMENSION(npts,nvm), INTENT(inout)  :: pear
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: MassePondTot
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: nanimaltot
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: intake_animal
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: intake
    ...
    REAL, DIMENSION(npts,nvm) :: wlam
    ! masse sï¿½che (structurelle) de la limbe (kg/m2) ----> dry mass
    ! (structural) of the lamina
    REAL, DIMENSION(npts,nvm) :: wst
    ! masse sï¿½che (structurelle) de la tige  (kg/m2) ----> dry mass
    ! (structural) of the stem
    REAL, DIMENSION(npts,nvm) :: wear
    ! masse sï¿½che (structurelle) de la tige  (kg/m2) ----> dry mass
    ! (structural) of the ear
    REAL, DIMENSION(npts,nvm) :: lm_old_ani
    ...
    REAL, DIMENSION(npts,nvm)     :: fGrazingstem
    REAL, DIMENSION(npts,nvm)     :: PlantEarzeargrazing
    REAL, DIMENSION(npts,nvm)     :: fGrazingear
    REAL, DIMENSION(npts,nvm)     :: PlantStemzstemgrazing
    REAL, DIMENSION(npts,nvm)     :: frac_litter_intake
    REAL, DIMENSION(npts,nvm)     :: wtissue
    REAL, DIMENSION(npts,nvm)     :: wroot
    REAL, DIMENSION(npts,nvm)     :: wsub
    REAL, DIMENSION(npts,nvm)     :: sublam
    REAL, DIMENSION(npts,nvm)     :: subst
    REAL, DIMENSION(npts,nvm)     :: subear
    REAL, DIMENSION(npts,nvm)     :: subroot
    ! Initialisations
    DNDF           (:,:) = 0.0
    NDF            (:,:) = 0.0
    DNDFI          (:,:) = 0.0
    ...
    lm_old_ani(:,:) = 0.0
    frac_litter_intake(:,:) = 0.0
    !transform biomass to dry matter kgDM/m2

    ! here the calculation of wlam wst wear should be exactly
    ! the same as in variablesPlantes
    ! otherwise the MassePondTot = plam * wlam + pstem * wst + pear * wear
    ! will not correct    
    wlam(:,:) = ( biomass(:,:,ileaf,icarbon)/(1e3*CtoDM) )
    wst(:,:)  = ( biomass(:,:,isapabove,icarbon)/(1e3*CtoDM) )
    wear(:,:) = ( biomass(:,:,ifruit,icarbon)/(1e3*CtoDM) )
    wroot(:,:) = ( biomass(:,:,iroot,icarbon)/(1e3*CtoDM) )
    wsub(:,:) = ( biomass(:,:,ilabile,icarbon)/(1e3*CtoDM) )
    wtissue(:,:) = wlam+wst+wear+wroot
    WHERE(wtissue .GT. min_stomate)
      sublam = wsub*wlam/wtissue
      subst = wsub*wst/wtissue
      subear = wsub*wear/wtissue
      subroot = wsub*wroot/wtissue
    ELSEWHERE
      sublam = zero
      subst = zero
      subear = zero
      subroot = zero     
    ENDWHERE
!    WRITE (numout,*) 'before Plantgrazing'
!    WRITE (numout,*) 'w',wlam(:,10),wst(:,10),wear(:,10)
!    WRITE (numout,*) 'p',plam(:,10),pstem(:,10),pear(:,10)
!    WRITE (numout,*) 'MassePondTot',MassePondTot,plam * wlam + pstem * wst + pear * wear
    ! 1. animal intake selection (with preference)
    !!! NOTE MassePondTot = plam * wlam + pstem * wst + pear * wear
    WHERE ((wlam .GT. 0.0) .AND. (MassePondTot .GT. 0.0) &
              .AND. (intake .GT. 0.0))
        ! # factor of lam structural dry mass preference
        fgrazinglam = plam*wlam/MassePondTot
        ! # fraction of the intake in the available lam strutural dry mass
        PlantLaminazlamgrazing = fgrazinglam*intake/(wlam+sublam)
        DNDFlam = &
           DNDFlam1*leaf_frac(:,:,ileaf,1) + &
           DNDFlam2*leaf_frac(:,:,ileaf,2) + &
           DNDFlam3*leaf_frac(:,:,ileaf,3) + &
    ...
    ELSEWHERE
        ...
        DNDFlam = 0.0
    END WHERE
    IF (ANY(PlantLaminazlamgrazing .GT. 1.0))  print *, 'warning: Component LAM not enough for optimal grazing'
    IF (ANY(PlantLaminazlamgrazing .LT. 0.0))  print *, 'warning: Component LAM over grazing'
    WHERE ((wst .GT. 0.0) .AND. (MassePondTot .GT. 0.0) .AND. &
         (intake .GT. 0.0))
        ! # factor of stem structural dry mass preference
        fgrazingstem = pstem*wst/MassePondTot
        ! # fraction of the intake in the available stem strutural dry mass
        PlantStemzstemgrazing = fgrazingstem*intake/(wst+subst)
        DNDFstem = &
           DNDFstem1*leaf_frac(:,:,ileaf,1) + &
           DNDFstem2*leaf_frac(:,:,ileaf,2) + &
           DNDFstem3*leaf_frac(:,:,ileaf,3) + &
   ...
   ELSEWHERE
    ...
    END WHERE
!gmjc 20141121 for avoid over grazing stem and leaf simutaneously
! it should not happen because in variablesPlantes
! it has been prescribed not over 1.0
!!!!    WHERE ((fgrazingstem + fgrazinglam) .GT. 1.0 .AND. &
!!!!           (intake .GT. 0.0) .AND. &
!!!!           ( wst .GT. 0.0))
!!!!      fgrazingstem = 1.0 - fgrazinglam
!!!!      PlantStemzstemgrazing = fgrazingstem*intake/wst
!!!!        DNDFstem = &
...
!!!!    ENDWHERE
!end gmjc
    IF (ANY(PlantStemzstemgrazing .GT. 1.0))  print *, 'warning: Component STEM not enough for optimal grazing'
    IF (ANY(PlantStemzstemgrazing .LT. 0.0))  print *, 'warning: Component STEM over grazing'
    WHERE (wear .GT. 0.0 .AND. (MassePondTot .GT. 0.0) .AND. &
         (intake .GT. 0.0))
        ! # factor of lam structural dry mass preference
        fgrazingear = pear*wear/MassePondTot
        ! # fraction of the intake in the available ear dry mass
        PlantEarzeargrazing =  fgrazingear*intake/(wear+subear)
        DNDFear = &
           DNDFear1*leaf_frac(:,:,ileaf,1) + &
           DNDFear2*leaf_frac(:,:,ileaf,2) + &
           DNDFear3*leaf_frac(:,:,ileaf,3) + &
    ...
    ELSEWHERE
        ...
        DNDFear = 0.0
    END WHERE
    IF (ANY(PlantEarzeargrazing .GT. 1.0))  print *, 'warning: Component EAR not enough for optimal grazing'
    IF (ANY(PlantEarzeargrazing .LT. 0.0))  print *, 'warning: Component STEM LAM over grazing'
!      WRITE (numout,*) 'before > 1 check'
!      WRITE (numout,*) 'PlantLaminazlamgrazing',PlantLaminazlamgrazing(:,10)
!      WRITE (numout,*) 'PlantStemzstemgrazing',PlantStemzstemgrazing(:,10)
!      WRITE (numout,*) 'PlantEarzeargrazing',PlantEarzeargrazing(:,10)
    ! JCADD 20170422 for merge CNP
    ! now update biomass and calculate intakecnp, litter_intakecnp
    
    ! 2. there is possibility that the animal's intake preference 
    ! can not be satisfied i.e., PlantLaminazlamgrazing PlantStemzstemgrazing
    ! PlantEarzeargrazing > 1, then intake should be modified
    ! by assuming animal prefer leaf then stem and ear 
    DO j=2,nvm
      DO i=1,npts
        IF (intake(i,j) .GT. wlam(i,j)+wst(i,j)+wear(i,j)+sublam(i,j)+subst(i,j)+subear(i,j) ) THEN
          WRITE(numout,*) 'intake',i,j,intake(i,j),wlam(i,j)+wst(i,j)+wear(i,j)+sublam(i,j)+subst(i,j)+subear(i,j)
          STOP "ERROR : not enough biomass for grazing"
        ELSE 
        ! if intake preference can not be satisfied
        IF (intake(i,j) .GT. 0.0 .AND. PlantLaminazlamgrazing(i,j) .GT. 1.0 .OR. &
           PlantStemzstemgrazing(i,j) .GT. 1.0 .OR. &
           PlantEarzeargrazing(i,j) .GT. 1.0) THEN
          ! if leaf is enough for intake
          IF (0.75*(wlam(i,j)+sublam(i,j)) .GE. intake(i,j) .AND. wlam(i,j) .GT. 0.0) THEN
            PlantEarzeargrazing(i,j) = 0.0
            PlantStemzstemgrazing(i,j) = 0.0
            PlantLaminazlamgrazing(i,j) = intake(i,j)/(0.75*(wlam(i,j)+sublam(i,j)))
          ELSE IF (wlam(i,j) .GT. 0.0 .AND. 0.75*(wlam(i,j)+sublam(i,j)) .LT. intake(i,j) .AND. &
                   wst(i,j)+wear(i,j)+subst(i,j)+subear(i,j) .GT. &
                   (intake(i,j)-0.75*(wlam(i,j)+sublam(i,j)))  ) THEN
            PlantLaminazlamgrazing(i,j) = 0.75
            ! the rest part from stem and ear with same fraction
            PlantStemzstemgrazing(i,j) = (intake(i,j)-0.75*(wlam(i,j)+sublam(i,j)))/&
                   (wst(i,j)+wear(i,j)+subst(i,j)+subear(i,j))
            PlantEarzeargrazing(i,j) = PlantStemzstemgrazing(i,j)
          ELSE   
            ! from all components according to their relative fraction
            PlantLaminazlamgrazing(i,j) = intake(i,j)/(wlam(i,j)+wst(i,j)+wear(i,j)+sublam(i,j)+subst(i,j)+subear(i,j))      
            PlantStemzstemgrazing(i,j) = PlantLaminazlamgrazing(i,j)
            PlantEarzeargrazing(i,j) = PlantLaminazlamgrazing(i,j)
          ENDIF
        ENDIF ! preference not satisfied         
        ENDIF ! enough biomass
      END DO ! npts
    END DO ! nvm    
!      WRITE (numout,*) 'after > 1 check'
!      WRITE (numout,*) 'intakecnp(:,:,icarbon)',intakecnp(:,10,icarbon)
!      WRITE (numout,*) 'PlantLaminazlamgrazing',PlantLaminazlamgrazing(:,10)
!      WRITE (numout,*) 'PlantStemzstemgrazing',PlantStemzstemgrazing(:,10)
!      WRITE (numout,*) 'PlantEarzeargrazing',PlantEarzeargrazing(:,10)
    ! 3. update leaf_frac
! JC Debug temporary delete the young leaf preference
! because for grassland, only little part is grazed per day
!!    !!!!!!!!
!!    !gmjc 120409 new update leaf_frac for each class
!!    ! we assumed a grazing preference with 90% age class 1, 10% age clas 2 3 4
!!    ! here wlam is the original wlam
!!    WHERE (PlantLaminazlamgrazing .GT. 0.0 .AND. wlam .GT. 0)
!!      WHERE (leaf_frac(:,:,1)*wlam .GT.  0.90*intake*PlantLaminazlamgrazing)
!!        !!if there is enough biomass of leaf age 1 for eating (0.9 of total
!!        !intake), animal prefer to eat more young leaf
!!        leaf_frac(:,:,1) = (leaf_frac(:,:,1)*wlam - &
!!                           0.90*intake*PlantLaminazlamgrazing)/ &
!!                            (wlam-intake*PlantLaminazlamgrazing)
!!      ELSEWHERE
!!        !!if not enough biomass of leaf age 1 can be eat, only 10% of it left
!!        leaf_frac(:,:,1) = (leaf_frac(:,:,1)*wlam*0.10)/ &
!!                            (wlam-intake*PlantLaminazlamgrazing)
!!      END WHERE
!!    ENDWHERE
!!    ! for age class 2-4 they share the residual biomass according to their 
!!    ! original share (i.e. age class 2-4 is evenly eaten
!!    tmp_fracsum(:,:)=0.0
!!    tmp_frac(:,:,:)= 0.0
!!    DO m = 2, nleafages
...
!!    ENDDO
    ...
    ELSEWHERE
        DNDFI = 0.0
    ENDWHERE
!      WRITE (numout,*) 'before update biomass',biomass(:,10,:,icarbon)
!      WRITE (numout,*) 'frac biomass intake',PlantLaminazlamgrazing(:,10),PlantStemzstemgrazing(:,10),PlantEarzeargrazing(:,10)
    ! 5. update biomass and calculate intake_cnp for other animal CNP processes
    DO m=1,nelements
      WHERE (wsub .GT. min_stomate/1e2)
        intakecnp(:,:,m) = biomass(:,:,ileaf,m) * PlantLaminazlamgrazing + &
                         biomass(:,:,isapabove,m) * PlantStemzstemgrazing + &
                         biomass(:,:,ifruit,m) * PlantEarzeargrazing + &
                         biomass(:,:,ilabile,m) * &
                         (( PlantLaminazlamgrazing*sublam + PlantStemzstemgrazing*subst + &
                         PlantEarzeargrazing*subear)/wsub)
        biomass(:,:,ileaf,m) = biomass(:,:,ileaf,m) * (1.0-PlantLaminazlamgrazing)
        biomass(:,:,isapabove,m) = biomass(:,:,isapabove,m) * (1.0-PlantStemzstemgrazing)
        biomass(:,:,ifruit,m) = biomass(:,:,ifruit,m) * (1.0-PlantEarzeargrazing)
        biomass(:,:,ilabile,m) = biomass(:,:,ilabile,m) * (1.0 - &
                         (( PlantLaminazlamgrazing*sublam + &
                         PlantStemzstemgrazing*subst + &
                         PlantEarzeargrazing*subear)/wsub) )
      ELSEWHERE
        intakecnp(:,:,m) = biomass(:,:,ileaf,m) * PlantLaminazlamgrazing + &
                         biomass(:,:,isapabove,m) * PlantStemzstemgrazing + &
      ...
      ENDWHERE
    END DO
    IF (ANY(intakecnp(:,:,icarbon) - intake(:,:) * CtoDM*1e3 .GT. min_stomate)) THEN
      WRITE (numout,*) 'intakecnp(:,:,icarbon)',intakecnp(:,10,icarbon)
      WRITE (numout,*) 'intake(:,:)',intake(:,10),intake(:,10) * CtoDM*1e3
      WRITE (numout,*) 'PlantLaminazlamgrazing',PlantLaminazlamgrazing(:,10)
      WRITE (numout,*) 'PlantStemzstemgrazing',PlantStemzstemgrazing(:,10)
      WRITE (numout,*) 'PlantEarzeargrazing',PlantEarzeargrazing(:,10)
      WRITE (numout,*) 'wsub',wsub(:,10)
      STOP "ERROR: too much intakecnp"
    ENDIF
    IF (ANY(intakecnp(:,:,icarbon) - intake(:,:) * CtoDM*1e3 .LT. -min_stomate)) THEN
      WRITE (numout,*) 'intakecnp(:,:,icarbon)',intakecnp(:,10,icarbon)
      WRITE (numout,*) 'intake(:,:)',intake(:,10),intake(:,10) * CtoDM*1e3
      WRITE (numout,*) 'PlantLaminazlamgrazing',PlantLaminazlamgrazing(:,10)
      WRITE (numout,*) 'PlantStemzstemgrazing',PlantStemzstemgrazing(:,10)
      WRITE (numout,*) 'PlantEarzeargrazing',PlantEarzeargrazing(:,10)
      WRITE (numout,*) 'wsub',wsub(:,10)
      STOP "ERROR: too few intakecnp"
    ENDIF

    ! 3. calculate tramplingcnp and update biomass
    !!!!!!!!!!!!!!!!!!!!!!!!!!! Trampingling and excretal returns effects
    !! according to Vuichard,2007 an additional 0.8% of the aboveground herbage
    !biomass is returned each day
    !! to litter for an instantaneous stocking rate of 1 LSU/ha
   ! when grazing AGB trampling exist
   ! when grazing litter, now assumed to be without trampling
!      WRITE (numout,*) 'before update trampling',biomass(:,10,:,icarbon)
    DO m=1,nelements
    WHERE (nanimaltot(:,:) .GT. 0.0 .AND. grazing_litter(:,:) .NE. 1 )
      tramplingcnp(:,:,ileaf,m) = nanimaltot(:,:) * 1e4 * 0.008 * biomass(:,:,ileaf,m)
      tramplingcnp(:,:,isapabove,m) = nanimaltot(:,:) * 1e4 * 0.008 * biomass(:,:,isapabove,m)
      tramplingcnp(:,:,ifruit,m) = nanimaltot(:,:) * 1e4 * 0.008 * biomass(:,:,ifruit,m)

      biomass(:,:,ileaf,m) = biomass(:,:,ileaf,m) * (1.0 - nanimaltot(:,:) * 1e4 * 0.008)
      biomass(:,:,isapabove,m) = biomass(:,:,isapabove,m) * (1.0 - nanimaltot(:,:) * 1e4 * 0.008)
      biomass(:,:,ifruit,m) = biomass(:,:,ifruit,m) * (1.0 - nanimaltot(:,:) * 1e4 * 0.008)
      
    ELSEWHERE
      tramplingcnp(:,:,ileaf,m) = 0.0
      tramplingcnp(:,:,isapabove,m) = 0.0
      tramplingcnp(:,:,ifruit,m) = 0.0
    ENDWHERE
    END DO ! nelements
!      WRITE (numout,*) 'after update trampling',biomass(:,10,:,icarbon)
    ! 4. calculate litter_intakecnp when litter is enough
    ! for other animal CNP processes
    ! intake_litter kg DM m-2
    ! litter_avail_totDM
    litter_avail_totDM(:,:) = (litter(:,imetabolic,:,iabove,icarbon) + &
                         litter(:,istructural,:,iabove,icarbon) ) / (1e3* CtoDM)
    WHERE (intake_litter .GT. 0.0 .AND. litter_avail_totDM .GT. intake_litter)
      frac_litter_intake = intake_litter/litter_avail_totDM
    ENDWHERE

    DO m=1,nelements
      WHERE (frac_litter_intake .GT. 0.0 .AND. frac_litter_intake .LE. 1.0)
        litter_intakecnp(:,imetabolic,:,iabove,m) = &
              litter(:,imetabolic,:,iabove,m) * frac_litter_intake
        litter_intakecnp(:,istructural,:,iabove,m) = &    
              litter(:,istructural,:,iabove,m) * frac_litter_intake
      ENDWHERE
    END DO ! nelements

  END SUBROUTINE chg_plante_cnp
! END JCADD
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE variablesPlantes(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: intakemax
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: AnimalDiscremineQualite
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: plam
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: pstem
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: pear
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: MassePondTot
    ...
    REAL, DIMENSION(npts,nvm) :: test_lam
    REAL, DIMENSION(npts,nvm) :: test_stem
    REAL, DIMENSION(npts,nvm) :: test_ear
    REAL, DIMENSION(npts,nvm) :: ncomp
    REAL, DIMENSION(npts,nvm) :: betaGrazing

    REAL, DIMENSION(npts,nvm) :: DNDF_total
    REAL, DIMENSION(npts,nvm) :: NDF_total

    REAL, DIMENSION(npts,nvm) :: exposant_lam
    REAL, DIMENSION(npts,nvm) :: exposant_stem
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE Animaux_main_dynamic(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: devstage
    ! Developmental stage (-)
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: intakemax
    ...
    ! Julian day (-)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: nanimal
    ! Stocking density  h (1,..,nstocking) (animal m-2)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: tanimal
    ! Beginning of the grazing period    h (1,..,nstocking) (d)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: danimal
    ! Lenght of the grazing period    h (1,..,nstocking) (d)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: PIYcow
    ! Initial weight of Young cow (Kg)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: PIMcow
    ! Initial weight of Mature cow (Kg)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: BCSYcow
    ! Initial body score condition of Young cow(Kg)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: BCSMcow
    ! Initial body score condition of mature cow(Kg)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: PICcow
    ! Initial weight of cow's calves (Kg)
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: AGE_cow_P
    ! Average age of dairy primiparous cows for autogestion
    REAL, DIMENSION(npts,nvm,nstocking), INTENT(inout) :: AGE_cow_M
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)              :: n
    ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL, DIMENSION(npts,nvm,nleafages), INTENT(inout)              :: leaf_frac
    REAL, DIMENSION(npts,nvm), INTENT(out)             :: intake_animal
    ! Daily intake per animal(kg animal-1 d-1)
    REAL, DIMENSION(npts,nvm), INTENT(inout)           :: intake_animalsum
    ! Yearly intake per animal(kg animal-1 d-1)
    REAL, DIMENSION(npts), INTENT(in)              :: tadmin
    ! Daily minimum temperature
    REAL, DIMENSION(npts), INTENT(in)              :: tadmoy
    ! Daily average temperature (K)
    REAL, DIMENSION(npts,nvm), INTENT(out)             :: IC_tot
    ! Daily average ingested capacity of cows (kg)
    REAL, DIMENSION(npts,nvm,n_out),INTENT(out)        :: Autogestion_out
    ! Fraction F (npts,1), ratio F (npts,2), and lenght of the grazing period when autgestion
    ...
    REAL  :: FVf_t
    REAL  :: fN_forage_t
    REAL  :: fN_concentrate_t

    REAL, DIMENSION(2)        :: QIc_t
    REAL, DIMENSION(4)        :: autogestion_weightcow_t
    REAL, DIMENSION(4)        :: autogestion_BCScow_t
    REAL, DIMENSION(4)        :: autogestion_AGEcow_t
    REAL, DIMENSION(2)        :: MPwmax_t
    INTEGER :: ier
    REAL,DIMENSION(npts)  :: toto 
    ...
    ! END JCADD
      ...
      fN_forage_t= fN_forage_t/(6.25*1e3)

      fN_concentrate_t= fN_concentrate_t/(6.25*1e3)
...
!
  ...
  !*suckler Cow
   SUBROUTINE intake_capacity_cow( &
     ...
     INTEGER, INTENT(in)                               :: npts
     ! Number of spatial points (-)
     REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: wanimalcow
     ! Animal liveweight (kg/animal) (young:1, adult:2)
     REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: MPwcow2
     ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
     REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: BCScow
     ! Body score condition cow (young in first, and adult in second) (/5)
     REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: nanimaltot
     ! Stocking rate (animal m-2)
     REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: ICcow
     ! Cow intake capacity of primiparous or multiparous cows(kg/animal/d)
  ...
  ! Suckler Calf
   ...
   SUBROUTINE intake_capacity_calves(&
     ...
     INTEGER, INTENT(in)                               :: npts
     ! Number of spatial points (-)
     REAL, DIMENSION(npts,nvm), INTENT(in)         :: wanimalcalf
     ! Calf liveweigth (kg/animal)
     REAL, DIMENSION(npts,nvm), INTENT(in)         :: nanimaltot
     ! Stocking rate (animal m-2)
     INTEGER(i_std ), INTENT(in)                          :: tjulian
     ! Julian day (-) 
     REAL, DIMENSION(npts,nvm), INTENT(out)        :: ICcalf
     ! Calf intake capacity  (kg/animal/d)
  ...
  SUBROUTINE intake_capacity_cow_d(&
    ...
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: MPwcow2
    ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: BCS
    ! Body Condition Score (for cow only /5)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: wanimalcow
    ! Animal liveweight (kg/animal) (young:1, adult:2)
    REAL, DIMENSION(npts,nvm)     , INTENT(in)    :: nanimaltot
    ! Stocking rate (animal m-2)
    REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: IC_animal
    ! intake Capacity (Kg)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: AGE_animal
    ! Animal age in case of simulation of dairy cows (months)
    REAL, DIMENSION(npts,nvm)     , INTENT(in)    :: nWeekLact
    ! Lactation week (in weeks from calving) 
    REAL, DIMENSION(npts,nvm)     , INTENT(in)    :: nWeekGest
    ! Gestation week (in weeks from mating)
         
    REAL,DIMENSION(npts,nvm,npta)                 :: IL
    ! Lactation Index
    REAL,DIMENSION(npts,nvm)                      :: IG
    ! Gestation Index
    REAL,DIMENSION(npts,nvm,npta)                 :: IM
    ! Maturity Index
  ...
  SUBROUTINE intake_capacity_heifer(&
    ...
    INTEGER, INTENT(in)                              :: type_animal
    ! 1: Dairy cows, 2: Suckler cows, 3: Old module, 4: Dairy heifers, 5 : Suckler heifers
    REAL, DIMENSION(npts,nvm), INTENT(in)        :: winit
    ! Initial live weigth of heifer
    REAL, DIMENSION(npts,nvm), INTENT(in)        :: wanimalcow
    ! Animal liveweight (kg/animal) (young:1, adult:2) 
    REAL, DIMENSION(npts,nvm), INTENT(out)       :: IC_animal
    ! Heifer intake capacity
                
    ! variable local            
    REAL, DIMENSION(npts,nvm)                    :: P1
    ! Parameter for IC calculation 
    REAL, DIMENSION(npts,nvm)                    :: itype
    ! Parameter for IC calculation  
  ...
  SUBROUTINE Grazing_intake_cow(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout) :: nanimaltot
    ! Stocking rate (animal m-2)
    REAL, DIMENSION(npts,nvm), INTENT(in)    :: DNDF
    ! fraction of digestible fibres in total fibres (-)
    REAL, DIMENSION(npts,nvm), INTENT(in)    :: NDF
    ! fraction of fibres in the intake(-)
    REAL, DIMENSION(npts,nvm), INTENT(in)    :: IC
    ! intake capacity (Kg)    
    REAL, DIMENSION(npts,nvm), INTENT(out)   :: DMIanimal
    ! Dry Matter intake of a cow/calf (Kg)
    REAL, DIMENSION(npts,nvm), INTENT(out)   :: OMD
    ! Digestible organic matter in the intake(kg/kg)
    REAL, DIMENSION(npts), INTENT(in)    :: tadmoy
    ! Daily average temperature (K)
    REAL, DIMENSION(npts,nvm), INTENT(out)   :: FVh
    ! Herbage Fill Value (UE)
    REAL, DIMENSION(npts,nvm), INTENT(in)    :: ntot
    ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL, DIMENSION(npts), INTENT(in)    :: tmoy_14
    ! 14 day running average of daily air temperature (K)
    REAL,                  INTENT(in)    :: BM_threshold
    ! Biomass threshold above which animals are moved out the paddock (kg/m2)
...
!     ENDDO
  ...
  SUBROUTINE Grazing_intake_cow_d(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: NDF
    ! fraction of fibres in the intake(-)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)      :: IC
    ! intake capacity (Kg)    
    REAL, DIMENSION(npts), INTENT(in)           :: tadmin
    ! Daily minimum temperature
    REAL, DIMENSION(npts), INTENT(in)           :: tadmoy
    ! Daily average temperature
    REAL, DIMENSION(npts,nvm,npta), INTENT(out)     :: DMIanimal
    ! Dry Matter intake of a cow/calf (Kg)
    ...
    REAL,DIMENSION(npts)                        :: temperature_effect
...
!    ENDDO
  ...
  SUBROUTINE grazing_intake_complementation(npts,dt                              ,&
    ...
    REAL, INTENT(in)                          :: dt
    ! Time step (d)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: DMIcowanimal
    ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: FVh
    ! Herbage Fill Value (UE)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: ICcow
    ! Cow intake capacity of primiparous or multiparous cows(kg/animal/d)
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: FVf
    ! forage fill value (Kg)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: MPcow2
    ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: MPwcow2
    ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm)  , INTENT(inout)    :: forage_quantity_period
    ! Daily forage quantity provided to herbivors during the current stocking period (Kg/Animal/d)
    REAL, DIMENSION(npts,nvm,2)  , INTENT(inout)    :: QIc
    ! Daily concentrate quantity per kg of milk or per kg of lw
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: NELherbage
    ! Energetic content of the herbage (MJ/kg) 
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: EVf
    ! Energetic content of the forage (MJ/Kg)
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: nanimaltot
    ! Stocking rate (animal/m²)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: DMIcowsum
    ! Cumulated intake per m2 for primiparous or multiparous cows(kg/m2)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: DMIcowanimalsum
    ! Cumulated animal intake for primiparous or multiparous cows(kg/animal)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: DMIcow
    ! Daily intake per m2 for primiparous or multiparous cows(kg/m2/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: DMIcowNsum
    ! N in daily intake per m2 for primiparous or multiparous cows(kgN/m2)
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: n
    ! nitrogen substrate concentration in plant,(kg n/kg)
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: fn
    ! nitrogen in structural dry matter
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: pyoung
    ! Fraction of young or primiparous in the cattle (-)
    INTEGER                        , INTENT(in)       :: type_animal
    ...
    REAL                   , INTENT(in)       :: Q_max_complement
    ! Maximum quantity of forage or concentrate to supplement animals when auto-supplementation (kg)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: NER
    ! Net energy requirement (MJ)
    REAL, DIMENSION(npts,nvm)  , INTENT(out)      :: forage_complementc
    ! fraction of carbon in Forage + concentrate (kgC/m²/d)
    REAL, DIMENSION(npts,nvm)  , INTENT(out)      :: forage_complementn
    ! fraction of nitrogen in Forage + concentrate (kgC/m²/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: NEI
    ! Net energy intake(MJ)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: NEM
    ! Net energy requirements for maintenance(MJ)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: NEIh
    ! Net Energy intake from ingested herbage(MJ)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: NEIf
    ! Net Energy intake from ingested forage(MJ)
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)    :: NEIc
    ! Net Energy intake from ingested concentrate(MJ)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: NEG
    ! Net energy required for gestation (MJ)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: DMIc
    ! Concentrate intake (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: DMIf
    ! forage intake (kg/animal/d)
  
  !local variables
    REAL, DIMENSION(npts,nvm,2)                   :: Shf
    ! substitution rate of herbage by forage in the cow diet (-)
    REAL, DIMENSION(npts,nvm,2)                   :: Shc1
    ! substitution rate of herbage by concentrate in the cow diet (-)
    REAL, DIMENSION(npts,nvm,2)                   :: Shc2
    ! substitution rate of herbage by concentrate in the cow diet (-)
    REAL, DIMENSION(npts,nvm,2)                   :: Shfc
    ! substitution rate of herbage by concentrate in the cow diet (-)
    REAL, DIMENSION(npts,nvm,2)                   :: EDhf
    ! substitution rate of herbage by concentrate in the cow diet (-)
    REAL, DIMENSION(npts,nvm,2)                   :: A
    ! intermediary variable
  
    REAL, DIMENSION(npts,nvm)                     :: ICmoy
    ! Average intake capacity of the cattle [kg MS/animal/d]
    REAL, DIMENSION(npts,nvm)                     :: DMImoy
    ! Average dry matter intake of the cattle [kg MS/animal/d] 
  
    REAL, DIMENSION(npts,nvm,2)                   :: temp
    ! temporary variable
  ...
  SUBROUTINE auto_complementation_dairy(npts,DMIcowanimal,FVh,ICcow,NER,NELherbage, EVc,&
    ...
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: FVh
    ! Herbage Fill Value (UE)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: ICcow
    ! Cow intake capacity of primiparous or multiparous cows(kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: NER
    ! Net energy requirement (MJ)  
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: NELherbage
    ! Energetic content of the herbage (MJ/kg) 
    REAL, DIMENSION(npts,nvm)  , INTENT(in)       :: EVc
    ...
    REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: MPwcow2
    ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: NEI
    ! Net energy intake(MJ)
     REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: NEM
     ! Net energy requirements for maintenance (MJ)
     REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: NEIh
     ! Net Energy intake from ingested herbage(MJ)
     REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: NEIf
     ! Net Energy intake from ingested forage(MJ)
     REAL, DIMENSION(npts,nvm,2), INTENT(out)      :: NEIc
     ! Net Energy intake from ingested concentrate(MJ)
     REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: NEG
     ...
     !local variables
     REAL, DIMENSION(npts,nvm,2)                   :: Shc1,shc2,shc
     ! Substitution rate of herbage by concentrate in the cow diet (-)
     REAL, DIMENSION(npts,nvm,2)                   :: A
     ! Intermediary variable
     REAL, DIMENSION(npts,nvm,2)                   :: MPpos_loc
     ! Possible milk production (local) (Kg/UGB)
     REAL, DIMENSION(npts,nvm,2)                   :: Qic
     ! Quantité de concentré ingéré par Kg de lait
     REAL, DIMENSION(npts,nvm)                     :: EDh
     ! Substitution rate of herbage by concentrate in the cow diet (-)
     REAL, DIMENSION(npts,nvm)                     :: temp
  ...
  SUBROUTINE Milk_Animal_cow(         &
    ...
    INTEGER(i_std ),                    INTENT(in)    :: tjulian
    ! Julian day (d)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)    :: NEBcow
    ! Net energy Balance (young :1 , adult:2) (MJ) 
    REAL, DIMENSION(npts,nvm,2), INTENT(out)   :: MPcow2
    ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)   :: MPcow
    ! Daily milk production per m2 for primiparous or multiparous cows (kg/m2/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)   :: MPwcow2
    ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowC
    ! C in daily milk production per m2 for primiparous or multiparous cows (kgC/m2/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowN
    ! N in daily milk production per m2 for primiparous or multiparous cows (kgN/m2/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowCsum
    ! Cumulated C in milk production per m2 for primiparous or multiparous cows (kgC/m2)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)   :: MPcowNsum
    ! Cumulated N in milk production per m2 for primiparous or multiparous cows (kgN/m2)
    REAL, DIMENSION(npts,nvm)  , INTENT(inout) :: milkanimalsum
  ...
  SUBROUTINE balance_energy_cow(npts,dt,&
    ...
    REAL, INTENT(in)                    :: dt
    ! Time step (d)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: DMIcowanimal
    ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: MPcow2
    ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    INTEGER,                       INTENT(in)   :: Agecow
    ! 0:young, 1:adult
    REAL, DIMENSION(npts,nvm), INTENT(inout):: BCS
    ! Body Condition Score (for cow only /5)
    INTEGER(i_std ), INTENT(in)                    :: tjulian
    ! Julian day (-) 
    REAL, DIMENSION(npts,nvm), INTENT(inout):: wanimalcow
    ! Animal liveweight (kg/animal) (young:1, adult:2)  
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: nanimaltot
    ! Stocking rate (animal m-2)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEB
    ! Net energy balance(MJ)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: NELherbage
    ! Energetic content of the herbage (MJ/kg) 
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: EVf
    ! Energy of the forage based (MJ/Kg)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: Forage_quantity_period
    ! Forage quantity  (MJ/Kg)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: EVc
    ! Energy of the concentrate (MJ/Kg)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: Qic
    ! Concentrate quantity per kg of milk or per kg of LW (MJ/Kg)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEI
    ! Net energy intake from ingested herbage(MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEIh
    ! Net energy intake from ingested herbage(MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEIf
    ! Net energy intake from ingested forage(MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEIc
    ! Net energy intake from ingested concentrate(MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEPgest
    ! Net energy for gestation (suckler cows)(MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEPlact
    ! Net energy for milk production(MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEP
    ! Net energy for production (MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEM
    ! Net energy for maintenance (MJ)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NER
    ! Total net energy requirements (maintenance and production)(MJ)


  
  !Local variable
    REAL, DIMENSION(npts,nvm)               :: NEBcow_calc
  ...
  SUBROUTINE balance_energy_calf(npts,dt ,&
    ...
    REAL, INTENT(in)                    :: dt
    ! Time step (d)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: DMIcowcalf
    ! Calf dry matter intake (Kg/animal/d)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: MPcow2
    ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: nanimaltot
    ! Stocking density (animal m-2)
    REAL, DIMENSION(npts,nvm), INTENT(inout):: wanimalcalf
    ! Calf liveweigth (kg/animal)
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: NELherbage
    ! Energetic content of the herbage (MJ/kg) 
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEIherbage
    ! Net energy intake from ingested herbage (MJ/Kg)  
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEImilk
    ! Net Erengy of ngested milk(MJ/Kg)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEI
    ! Net energy of global intake(MJ/Kg)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEM
    ! Net energy  metabolic(MJ/Kg)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NEG
    ! Net energy growth(MJ/Kg)
  
  !Local variable
    REAL                                :: beta=0.2
    ! Parameter for NEM computation
    REAL, DIMENSION(npts,nvm)               :: NEG_calc
  ...
  SUBROUTINE balance_energy_cow_d(npts,npta,dt,&
    ...
    REAL, INTENT(in)                          :: dt
    ! Time step (d)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: MPcow2
    ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: MPpos
    ! Possible milk production of dairy cows according to the diet (kg/animal/d)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: MPwcow2
    ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,npta), INTENT(inout) :: BCS
    ! Body Condition Score (for cow only /5)
    REAL, DIMENSION(npts,nvm,npta), INTENT(inout) :: BCScow_prev
    ! Body Condition Score at previsou time step (for cow only /5)
    REAL, DIMENSION(npts,nvm,npta),INTENT(in)     :: AGE_animal
    ! Animal age in case of simulation of dairy cows (months)
    REAL, DIMENSION(npts,nvm,npta), INTENT(inout) :: wanimalcow
    ! Animal liveweight (kg/animal) (young:1, adult:2)  
    REAL, DIMENSION(npts,nvm)     , INTENT(in)    :: nanimaltot
  ...
  !Local variable
    REAL, DIMENSION(npts,nvm,npta)               :: NEBcow_W
    ! Daily variation of cow liveweight (kg/d)
    REAL, DIMENSION(npts,nvm,npta)               :: NEBcow_BCS
  ...
  SUBROUTINE balance_energy_heifer(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)         :: nanimaltot
    ! StockRate of cattle
    REAL, DIMENSION(npts,nvm), INTENT(in)         :: DMIheifer
    ! Dry Matter intake of a cow/calf (Kg)
    REAL, DIMENSION(npts,nvm), INTENT(in)         :: NELherbage
    ! Energetic content of the herbage (MJ/kg) 
    REAL, DIMENSION(npts,nvm), INTENT(in)         :: EVf
  ...
  SUBROUTINE Respiration_Methane_cow(&
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)  :: Wanimal
    ! Animal life weight (kg)
    REAL, DIMENSION(npts,nvm), INTENT(out) :: R_cow
    ! Animal respiration (kg C / m²)
    REAL, DIMENSION(npts,nvm), INTENT(out) :: CH4_cow
 ...
 SUBROUTINE Respiration_Methane_cow_2(npts, npta, type_animal, OMD,NEIh,NEIf,NEIc,grazingc,nanimaltot,&
   ...
   INTEGER, INTENT(in)                              :: type_animal
   ! 1: Dairy cows, 2: Suckler cows, 3: Old module, 4: Dairy heifers, 5 : Suckler heifers
   REAL, DIMENSION(npts,nvm), INTENT(in)        :: OMD
   ! Digestible organic matter in the intake(kg/kg)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)   :: NEIh        
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)   :: NEIf        
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)   :: NEIc        
    REAL, DIMENSION(npts,nvm), INTENT(in)        :: grazingc
    ! C flux associated to grazing (kg C m-2 d-1)
    REAL, DIMENSION(npts,nvm), INTENT(in)        :: nanimaltot

    ! Stocking rate (animal m-2)
    REAL, DIMENSION(npts,nvm), INTENT(in)        :: panimaltot
    ! proportion of primipare
    REAL, DIMENSION(npts,nvm), INTENT(out)       :: R_cow
    ! Daily animal respiration (kg C m-2 d-1)
    REAL, DIMENSION(npts,nvm), INTENT(out)       :: CH4


    ! Daily enteric methane production (kg C/m2/d); 
    REAL, DIMENSION(npts,nvm,npta), INTENT(out)  :: CH4animal
    ! Daily enteric methane production for young or mature cows (kg C/m2/d); 
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)   :: MPcow2
    ! Daily actual milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm)     , INTENT(inout):: forage_complementc
    ! C flux associated to complemtation with forage and concentrate (kg C m-2 d-1)
    INTEGER, INTENT(in)                              :: f_complementation
 ...
 SUBROUTINE Urine_Faeces_cow(&
   ...
   REAL, DIMENSION(npts,nvm), INTENT(in)  :: grazingc
   ! C flux associated to grazing (kg C m-2 d-1)
   REAL, DIMENSION(npts,nvm), INTENT(in)  :: forage_complementc
   ! C flux associated to forage anc complementation (kg C m-2 d-1)
   REAL, DIMENSION(npts,nvm), INTENT(in)  :: forage_complementn
  ...
  SUBROUTINE Calcul_NEL_herbage(npts,OMD, NELherbage)
    ...
    REAL, DIMENSION(npts,nvm), INTENT(in)   :: OMD           ! Digestible organic matter in the intake(kg/kg)
    REAL, DIMENSION(npts,nvm), INTENT(out)  :: NELherbage    ! Energetic content of the herbage (MJ/kg) 
  ...
  SUBROUTINE histwrite_cow_Part1(npts,DMIyoung,DMImature,DMicalf,pyoung_in,OMD,MPcow2,NEBcow, NEIcow, nanimaltot,type_animal,&
    ...
    INTEGER, INTENT(in)                             :: npts
    ! Number of spatial points (-)
    REAL, DIMENSION(npts,nvm), INTENT(in)       :: DMIyoung
    ! Ingested dry matter for calf (Kg/d)         
    REAL, DIMENSION(npts,nvm), INTENT(in)       :: DMImature
    ! Ingested dry matter for calf (Kg/d)         
    REAL, DIMENSION(npts,nvm), INTENT(in)       :: DMIcalf
    ! Daily calf intake per m2 (Kg/d)          
    REAL, DIMENSION(npts,nvm), INTENT(in)       :: pyoung_in
    ! Ingested dry matter for calf (Kg/d)         
    REAL, DIMENSION(npts,nvm), INTENT(in)       :: OMD
    ! Digestible organic matter in the intake(kg/kg)
    
    REAL, DIMENSION(npts,nvm)                   :: BCScows
    ! Average BCS of cattle
    REAL, DIMENSION(npts,nvm)                   :: Weightcows
    ...
    REAL, DIMENSION(npts,nvm,2), INTENT(in)     :: NEBcow
    ! Net energy Balance (young :1 , adult:2) (MJ) 
    REAL, DIMENSION(npts,nvm,2), INTENT(in)     :: NEIcow
    ! Net energy intake (MJ)
    REAL, DIMENSION(npts,nvm)                   :: nanimaltot
    ! Stocking density (animal/m2)
    INTEGER, INTENT(in)                             :: type_animal
    ! 1 or 2 or 4 or 5= > new module animal
    REAL, DIMENSION(npts,nvm,2), INTENT(in)     :: MPwcow2
    ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)     :: MPpos
    ! Possible milk production of dairy cows according to the diet (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)     :: DMIc
    ! Concentrate intake (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)     :: DMIf
    ! forage intake (kg/animal/d)

    !Local variable
    
    REAL, DIMENSION(npts,nvm)                   :: Milk_animal
  ...
  SUBROUTINE histwrite_cow_Part2(npts,CH4young, CH4mature)
    INTEGER, INTENT(in)                             :: npts                 ! Number of spatial points (-)
    REAL, DIMENSION(npts,nvm)                   :: CH4young             ! 
    REAL, DIMENSION(npts,nvm)                   :: CH4mature            ! 
  ...
  SUBROUTINE estime_weightcalf(age_calf, weight_init, liveweight_calf)
     REAL, INTENT(in)  :: age_calf     ! Age of calf
     REAL, INTENT(in)  :: weight_init  ! Initial weight of calf
     REAL, INTENT(out) :: liveweight_calf  ! weight of calf
...
SUBROUTINE calcul_perte_etat(npts,tjulian,BCScow,MPwmax,tcalving,PEmax) 
  ...
  REAL, DIMENSION(npts,nvm,2), INTENT(in)            :: BCScow
  ! Body Condition Score (for cow only /5)
  REAL, DIMENSION(npts,nvm,2), INTENT(in)            :: MPwmax
  ! Maximum of theoretical milk production (kg/animal/d)
  REAL, DIMENSION(npts,nvm), INTENT(in)              :: tcalving
  ! Calving date (d) 
  REAL, DIMENSION(npts,nvm,2), INTENT(out)           :: PEmax
  ! Perte d'etat maximale des vaches laitières sur la periode de paturage 
...
!----------------------------------------
  ...
  SUBROUTINE Calcul_NER_cow(npts,npta,wanimalcow,wcalfborn, Age_animal, nweekgest, MPwcow2,NER,NEGcow,NEMcow)
    ...
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: wanimalcow
    ! Animal liveweight (kg/animal) (young:1, adult:2) 
    REAL, DIMENSION(npts,nvm)     , INTENT(in)    :: Wcalfborn
    ! Calf liveweigth at birth (kg/animal)
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: AGE_animal
    ! Animal age in case of simulation of dairy cows (months)
    REAL, DIMENSION(npts,nvm),      INTENT(in)    :: Nweekgest
    ...
    REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: MPwcow2
    ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
    REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NER
    ! Total net energy required (MJ)
    REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NEGcow
    ! Net energy required for gestation (MJ)
    REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NEMcow
    ! Net energy required for gestation (MJ)
    REAL, DIMENSION(npts,nvm,npta)                :: NEPlact                 ! Net energy required for milk prduction (MJ)
  ...
  SUBROUTINE calcul_NEI_cow_d(npts,npta,MPcow2,DMIcowanimal,NELherbage,&
    ...
    ! 
      REAL, DIMENSION(npts,nvm,2), INTENT(in)       :: MPcow2
      ! Daily actual milk production per animal for primiparous or multiparous cows at previous time step (kg/animal/d)
      REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: DMIcowanimal
      ! Daily animal intake for primiparous or multiparous cows(kg/animal/d)
      REAL, DIMENSION(npts,nvm)     , INTENT(in)    :: NELherbage
      ! Energetic content of the herbage (MJ/kg) 
      REAL, DIMENSION(npts,nvm), INTENT(in)         :: EVf
      ...
      REAL, DIMENSION(npts,nvm), INTENT(in)         :: EVc
      ! Energy of the concentrate (MJ/Kg)
      REAL, DIMENSION(npts,nvm,npta), INTENT(in)    :: Qic
      ! Concentrate quantity per kg of milk or per kg of LW (MJ/Kg)
      REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NEI
      ! Net energy intake(MJ/Kg)
      REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NEM
      ! Net energy intake(MJ/Kg)
      REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NEIh
      ! Net Energy intake from ingested herbage(MJ)
      REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NEIf
      ! Net Energy intake from ingested forage(MJ)
      REAL, DIMENSION(npts,nvm,npta), INTENT(out)   :: NEIc
      ! Net Energy intake from ingested concentrate(MJ)
  ...
  SUBROUTINE Potentiel_dairy_d(npts,tjulian,nweekLact,nweekGest,MPwcow2max,MPwcow2)
    ...
    INTEGER(i_std ),                    INTENT(in)     :: tjulian
    ! Julian day (d)
    REAL, DIMENSION(npts,nvm)  , INTENT(out)    :: nWeeklact
    ! Lactation week (in weeks from calving)
    REAL, DIMENSION(npts,nvm)  , INTENT(out)    :: nWeekGest
    ! Gestation week (in weeks from mating)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)     :: MPwcow2max
    ! Daily potential milk production per animal for primiparous or multiparous cows at peak of lactation(kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)    :: MPwcow2
    ! Daily potential milk production per animal for primiparous or multiparous cows (kg/animal/d)
  ...
  SUBROUTINE Milk_Animal_cow_d(                &
    ...
    REAL, DIMENSION(npts,nvm)  , INTENT(out)     :: nWeekGest
    ! Gestation week (in weeks from mating)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)      :: PEmax
    ! Perte d'etat maximale des vaches laitières sur la periode de paturage 
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)   :: PEpos
    ! Perte d'etat possible des vaches laitières au jour j
    REAL, DIMENSION(npts,nvm,2), INTENT(in)      :: deltaBCS
    ! Body condition score variation between two consecutive time steps (-)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)     :: MPpos
    ! Possible milk production of dairy cows according to the diet (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)      :: NEIcow
    ! Total net energy intake (1:young, 2:adult) (MJ) 
    REAL, DIMENSION(npts,nvm,2), INTENT(in)      :: NEMcow
    ! Net energy for maintenance (young :1 , adult:2) (MJ)
    REAL, DIMENSION(npts,nvm,2), INTENT(in)      :: NEGcow
    ! Net energy for gestation (dairy cows)(young :1 , adult:2) (MJ) 
    REAL, DIMENSION(npts,nvm,2), INTENT(inout)   :: MPcow2_prec
    ! Daily actual milk production per animal for primiparous or multiparous cows at previous time step (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2), INTENT(out)     :: MPpot
    ! Potential milk production (kg/d)

    REAL, DIMENSION(npts,nvm)                   :: milkKG
    ! Daily actual milk production per animal for the whole cattle (kg/animal/d)
    REAL, DIMENSION(npts,nvm,2)                 :: MR
    ! Milk response (-)
    REAL, DIMENSION(npts,nvm,2)                 :: RF
    ! Remobilisation fraction (-)
    REAL, DIMENSION(npts,nvm)                   :: Fremob
    ! facteur de remobilisation (fonction de la lactation)
    REAL, DIMENSION(npts,nvm,2)                 :: MPwcow2max
    ! Daily potential milk production per animal for primiparous or multiparous cows at peak of lactation(kg/animal/d)
    REAL, DIMENSION(npts,nvm)                   :: milkanimal_write
    ! Milk production per animal and per day (kg animal-1 d-1)
    REAL, DIMENSION(npts,nvm,2)                 :: min_NEB
