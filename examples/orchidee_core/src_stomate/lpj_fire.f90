...
!!
!! SVN           :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_stomate/lpj_fire.f90 $ 
!! $Date: 2018-08-03 21:49:20 +0200 (五, 2018-08-03) $
!! $Revision: 5367 $
!! \n
!_ ================================================================================================================================
...
!_ ================================================================================================================================
  ...
  !! 0. Variable and parameter declarations
    ...
    INTEGER, INTENT(in)                                   :: npts                !! Domain size - number of pixels 
                                                                                        !! (unitless)   
    REAL, INTENT(in)                                      :: dt                  !! Time step of the simulations for stomate 
                                                                                        !! (days)   
    REAL, DIMENSION(npts), INTENT(in)                     :: litterhum_daily     !! Daily litter moisture (unitless, 0-1)   
    REAL, DIMENSION(npts), INTENT(in)                     :: t2m_daily           !! Daily 2 meter temperature (K)   
    REAL, DIMENSION(npts,nvm,nlevs), INTENT(in)           :: lignin_struc        !! Ratio Lignin/Carbon in structural above 
                                                                                        !! and below ground litter 
                                                                                        !! @tex $(gC gC^{-1})$ @endtex 
    REAL, DIMENSION(npts,nvm,nlevs), INTENT(in)           :: lignin_wood         !! Ratio Lignine/Carbon in woody litter for above and	 
                                                                                        !! and below ground litter 
    REAL, DIMENSION(npts,nvm), INTENT(in)                 :: veget_cov_max       !! Maximum fraction of vegetation type including 
    ...
    REAL, DIMENSION(npts,nvm), INTENT(out)                :: co2_fire            !! Carbon emitted into the atmosphere by 
                                                                                        ...
                                                                                        !! biomass 
    REAL, DIMENSION(npts,nvm,nelements), INTENT(out)      :: fire_masschangecnp  !! mass change during fire
                                                                                        !! (in gCNP/m**2/day) 
                                                                                        !! @tex $(gC m^{-2} dtslow^{-1})$ @endtex
    !! 0.3 Modified variables
    
    REAL, DIMENSION(npts,nvm), INTENT(inout)              :: fireindex           !! Probability of fire; in the code means 
                                                                                        !! long term fire index (unitless, 0-1)   
    REAL, DIMENSION(npts,nvm), INTENT(inout)              :: firelitter          !! Total natural litter 
    ...
    REAL, DIMENSION(npts,nlitt,nvm,nlevs,nelements), INTENT(inout) :: litter     !! Metabolic and structural litter, above
                                                                                        !! and below ground 
                                                                                        !! @tex $(gC m^{-2})$ @endtex
    REAL, DIMENSION(npts,nvm,nlitt), INTENT(inout)        :: dead_leaves         !! Dead leaves on ground, per PFT, 
                                                                                        !! metabolic and structural 
                                                                                        !! @tex $(gC m^{-2})$ @endtex
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout):: bm_to_litter     !! Biomass entering the litter pool 
                                                                                        !! @tex $(gC m^{-2} dtslow^{-1})$ @endtex
    ...
    REAL, DIMENSION(npts)                                  :: fire_disturb       !! Actual fire disturbance fraction 
                                                                                        !! after consideration of inherent fire 
                                                                                        !! resistance of different PFTs 
                                                                                        !! (unitless, 0-1)  
    REAL, DIMENSION(npts,nvm)                              :: firedeath          !! In case of activation of dynamic 
                                                                                        !! vegetation, the daily fraction of 
                                                                                        !! burned individuals (unitless, 0-1)  
    REAL, DIMENSION(npts)                                  :: moistlimit         !! Moisture for fire inhibition per PFT                                                                                         !! 
    ...
    REAL, DIMENSION(npts)                                  :: litter_above       !! Total aboveground litter per PFT                                                                                         !! 
                                                                                        !! @tex $(gC m^{-2})$ @endtex
    REAL, DIMENSION(npts,nvm)                              :: fireindex_daily    !! Daily fire index (unitless, 0-1) 
    REAL, DIMENSION(npts, nvm)                             :: firefrac           !! Daily fire burning fraction on ground 
                                                                                        !! (unitless, 0-1) 
    REAL, DIMENSION(npts)                                  :: struc_residual     !! Fraction of structural litter not burned
                                                                                        !! (thus residual), depending on strutural 
                                                                                        !! litter lignin content  (unitless, 0-1) 
    REAL, DIMENSION(npts)                                  :: wood_residual      !! Fraction of woody litter not burned
                                                                                        !! (thus residual), depending on strutural 
                                                                                        !! litter lignin content  (unitless, 0-1) 
    REAL, DIMENSION(npts)                                  :: residue_frac       !! Fraction of Fuel (either biomass or litter) not 
                                                                                        !! burned  (unitless, 0-1)
    REAL, DIMENSION(npts)                                  :: x                  !! Intermediate variable
    REAL, DIMENSION(npts)                                  :: aff                !! Annual fire fraction (unitless, 0-1) 
    INTEGER                                                :: j,k,m              !! Indeces

    ! fire emissions and mineralized nutrients
    REAL, DIMENSION(npts, nvm)                             :: co_fire       !! CO emission during fire
    REAL, DIMENSION(npts, nvm)                             :: ch4_fire      !! CH4 emission during fire
    REAL, DIMENSION(npts, nvm)                             :: ntotal_fire   !! burned total N
    REAL, DIMENSION(npts, nvm)                             :: ptotal_fire   !! burned total P
    REAL, DIMENSION(npts, nvm)                             :: nh3_fire      !! NH3 emission during fire
    REAL, DIMENSION(npts, nvm)                             :: nox_fire      !! NOx emission during fire
    REAL, DIMENSION(npts, nvm)                             :: n2o_fire      !! N2O emission during fire
    REAL, DIMENSION(npts, nvm)                             :: nother_fire   !! all Other form of N-containing aerosols during fire
    REAL, DIMENSION(npts, nvm)                             :: nemit_fire    !! total N emitted to atmosphere during fire
    REAL, DIMENSION(npts, nvm)                             :: pemit_fire    !! total P emitted to atmosphere during fire
    REAL, DIMENSION(npts, nvm)                             :: soil_n_fire   !! N left into soil during fire
    REAL, DIMENSION(npts, nvm)                             :: soil_p_fire   !! P left into soil during fire
!_ ================================================================================================================================

    IF (printlev>=3) WRITE(numout,*) 'Entering fire'
    !DSG WRITE(6,*)'SUM(biomass(test_grid,test_pft,:,icarbon)',SUM(biomass(test_grid,test_pft,:,icarbon))
    !DSG WRITE(6,*)'SUM(litter(test_grid,:,test_pft,:,icarbon)',SUM(SUM(litter(test_grid,:,test_pft,:,icarbon),DIM=1),DIM=1)
 ...
 !! 2. Determine fire probability
  ...
  !! 4. Determine fire impact
       ...
       DO k = 1, nparts
          ...
          IF ( .NOT. ( ( .NOT. is_tree(j) ) .AND. ( ( k.EQ.iroot ) .OR. ( k.EQ.icarbres ) ) ) ) THEN
             ...
             ntotal_fire(:,j) = ntotal_fire(:,j) + biomass(:,j,k,initrogen) &  
                  * fire_disturb(:) * co2frac(k) / dt 

             ptotal_fire(:,j) = ptotal_fire(:,j) + biomass(:,j,k,iphosphorus) &
                  * fire_disturb(:) * co2frac(k) / dt

             !! 4.2.2 Determine the residue
             !        Residual is expressed as a fraction
             residue_frac(:) = fire_disturb(:) * ( un - co2frac(k) )

             !! 4.2.2.3 The rest (largest part) of the residue becomes litter.
             bm_to_litter(:,j,k,icarbon) = bm_to_litter(:,j,k,icarbon) + residue_frac(:) &
                  * biomass(:,j,k,icarbon)

             bm_to_litter(:,j,k,initrogen) = bm_to_litter(:,j,k,initrogen) + residue_frac(:) & 
                  * biomass(:,j,k,initrogen) 

             !! 4.2.2.3.1 In case of P we assume all P goes to litter; there is no gasous P in atmosphere
             !!bm_to_litter(:,j,k,iphosphorus) = bm_to_litter(:,j,k,iphosphorus) +  fire_disturb(:)*biomass(:,j,k,iphosphorus) 
             bm_to_litter(:,j,k,iphosphorus) = bm_to_litter(:,j,k,iphosphorus) + residue_frac(:) &
                  * biomass(:,j,k,iphosphorus)
          ENDIF
  ...
  !! 5. Burn litter
    ...
    DO j = 2,nvm  !loop over #PFT
       ...
       ntotal_fire(:,j) = ntotal_fire(:,j) + litter(:,imetabolic,j,iabove,initrogen) * & 
            firefrac(:,j) / dt 

       ptotal_fire(:,j) = ptotal_fire(:,j) + litter(:,imetabolic,j,iabove,iphosphorus) * &
            firefrac(:,j) / dt

       !! 5.1.2 Decrease metabolic litter
       DO m = 1,iphosphorus ! include gaseous/aerosol losses of P
       ...
       struc_residual(:) = fire_resist_lignin * lignin_struc(:,j,iabove)
       ...
       ntotal_fire(:,j) = ntotal_fire(:,j) + & 
            litter(:,istructural,j,iabove,initrogen) * firefrac(:,j) * & 
            ( 1. - struc_residual(:) )/ dt

       ptotal_fire(:,j) = ptotal_fire(:,j) + &
            litter(:,istructural,j,iabove,iphosphorus) * firefrac(:,j) * &
            ( 1. - struc_residual(:) )/ dt
 
       !! 5.2.3 Determine residue 
       !        The residue is litter that undergoes fire, but is not transformed into CO2
       residue_frac(:) = firefrac(:,j) * struc_residual(:)
       ...
       litter(:,istructural,j,iabove,icarbon) = &
            ...
            residue_frac(:) * litter(:,istructural,j,iabove,icarbon)

       litter(:,istructural,j,iabove,initrogen) = & 
            litter(:,istructural,j,iabove,initrogen) * ( un - firefrac(:,j) ) + & 
            residue_frac(:) * litter(:,istructural,j,iabove,initrogen) 

       litter(:,istructural,j,iabove,iphosphorus) = &
            litter(:,istructural,j,iabove,iphosphorus) * ( un - firefrac(:,j) ) + &
            residue_frac(:) * litter(:,istructural,j,iabove,iphosphorus)

       !! 5.3 Burning exposed woody litter is not totally transformed into CO2.
       
       !! 5.3.1 Fraction of exposed woody litter that does not
       !       burn totally should depend on lignin content (lignin_wood). 
       
       wood_residual(:) =  fire_resist_lignin * lignin_wood(:,j,iabove)
       
       !! 5.3.2 CO2 flux, in gC/(m**2 of total ground)/day.
       
       co2_fire(:,j) = co2_fire(:,j) + &
            litter(:,iwoody,j,iabove,icarbon) * firefrac(:,j) * &
            ( 1. - wood_residual(:) )/ dt

       ntotal_fire(:,j) = ntotal_fire(:,j) + &
            litter(:,iwoody,j,iabove,initrogen) * firefrac(:,j) * &
            ( 1. - wood_residual(:) )/ dt

       ptotal_fire(:,j) = ptotal_fire(:,j) + &
            litter(:,iwoody,j,iabove,iphosphorus) * firefrac(:,j) * &
            ( 1. - wood_residual(:) )/ dt

       !! 5.3.3 determine residue (litter that undergoes fire, but is not transformed
       !       into CO2)

       residue_frac(:) = firefrac(:,j) * wood_residual(:)

       !! 5.3.5 The rest (largest part) of the residue remains litter. Remaining litter
       !       is the sum of this and of the litter which has not undergone a fire.

       litter(:,iwoody,j,iabove,icarbon) = &
            litter(:,iwoody,j,iabove,icarbon) * ( 1. - firefrac(:,j) ) + &
            residue_frac(:) * litter(:,iwoody,j,iabove,icarbon)

       litter(:,iwoody,j,iabove,initrogen) = &
            litter(:,iwoody,j,iabove,initrogen) * ( 1. - firefrac(:,j) ) + &
            residue_frac(:) * litter(:,iwoody,j,iabove,initrogen)

       litter(:,iwoody,j,iabove,iphosphorus) = &
            litter(:,iwoody,j,iabove,iphosphorus) * ( 1. - firefrac(:,j) ) + &
            residue_frac(:) * litter(:,iwoody,j,iabove,iphosphorus)
       ...
       IF (spinup_analytic) THEN
          ...
          ! litter woody above
          MatrixA(:,j,iwoody_above,iwoody_above) =  MatrixA(:,j,iwoody_above,iwoody_above) - firefrac(:,j) &
                                                              +  firefrac(:,j)* wood_residual(:) 
  ...
  !! 6. calculate gaseous/Aerosol N P emissions following Akagi et al., 2011
    DO j = 2,nvm
      ! note here co and ch4 from fire is a fraction of co2_fire
      ! co2_fire is still the total C lost during fire
      ! including CO2, CO, and CH4
      co_fire(:,j) = co2_fire(:,j) * frac_co_fire(j)
      ch4_fire(:,j) = co2_fire(:,j) * frac_ch4_fire(j)
      ! frac_Xemit_fire is the N:C and P:C ratio in fire emission, thus we use
      ! co2_fire 
      nemit_fire(:,j) = co2_fire(:,j) * frac_Nemit_fire(j)
      pemit_fire(:,j) = co2_fire(:,j) * frac_Pemit_fire(j) 
    ENDDO
    ! incase emitted CN ratio < burned CN ratio (not enough N)
    WHERE (nemit_fire .GT. ntotal_fire) 
      nemit_fire = ntotal_fire
    ENDWHERE
    DO j = 2,nvm
      nh3_fire(:,j) = nemit_fire(:,j)*frac_nh3_fire(j)
      nox_fire(:,j) = nemit_fire(:,j)*frac_nox_fire(j)
      n2o_fire(:,j) = nemit_fire(:,j)*frac_n2o_fire(j)
      nother_fire(:,j) = nemit_fire(:,j)*(un-frac_nh3_fire(j)-frac_nox_fire(j)-frac_n2o_fire(j))
      ! here assuming all non-emitted N were as aboveground metabolic litter
      soil_n_fire(:,j) = ntotal_fire(:,j)-nemit_fire(:,j)
      litter(:,imetabolic,j,iabove,initrogen) = litter(:,imetabolic,j,iabove,initrogen) + &
               soil_n_fire(:,j)
    ENDDO

    ! for now P can be emitted
    WHERE (pemit_fire .GT. ptotal_fire)
      pemit_fire = ptotal_fire
    ENDWHERE
    DO j = 2,nvm
      ! here assuming all non-emitted P were as aboveground metabolic litter
      soil_p_fire(:,j) = ptotal_fire(:,j)-pemit_fire(:,j)
      litter(:,imetabolic,j,iabove,iphosphorus) = litter(:,imetabolic,j,iabove,iphosphorus) + &
               soil_p_fire(:,j)
    ENDDO    

    ! C loss as emission
    fire_masschangecnp(:,:,icarbon) = co2_fire
    ! N loss as emission or mineralization
    fire_masschangecnp(:,:,initrogen) = nemit_fire(:,:)
    ! now no P loss is assumed
    fire_masschangecnp(:,:,iphosphorus) = pemit_fire(:,:)

  !! 7. write out history variables
    ...
    CALL xios_orchidee_send_field("CO_FIRE_EMISSION",co_fire(:,:))
    CALL xios_orchidee_send_field("CH4_FIRE_EMISSION",ch4_fire(:,:))
    CALL xios_orchidee_send_field("NOX_FIRE_EMISSION",nox_fire(:,:))
    CALL xios_orchidee_send_field("NH3_FIRE_EMISSION",nh3_fire(:,:))
    CALL xios_orchidee_send_field("N2O_FIRE_EMISSION",n2o_fire(:,:))
    CALL xios_orchidee_send_field("NOTHER_FIRE_EMISSION",nother_fire(:,:))
    CALL xios_orchidee_send_field("P_FIRE_EMISSION",pemit_fire(:,:))
    CALL xios_orchidee_send_field("SOIL_N_FIRE",soil_n_fire(:,:))
    CALL xios_orchidee_send_field("SOIL_P_FIRE",soil_p_fire(:,:))
    ...
    CALL histwrite_p (hist_id_stomate, 'CO_FIRE_EMISSION', itime, &
         co_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'CH4_FIRE_EMISSION', itime, &
         ch4_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'NOX_FIRE_EMISSION', itime, & 
         nox_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'NH3_FIRE_EMISSION', itime, &
         nh3_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'N2O_FIRE_EMISSION', itime, &
         n2o_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'NOTHER_FIRE_EMISSION', itime, &
         nother_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'P_FIRE_EMISSION', itime, &
         pemit_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'SOIL_N_FIRE', itime, &
         soil_n_fire(:,:), npts*nvm, horipft_index)
    CALL histwrite_p (hist_id_stomate, 'SOIL_P_FIRE', itime, &
         soil_p_fire(:,:), npts*nvm, horipft_index)
  END SUBROUTINE fire
...
!! 0. Variable and parameter declaration
    ...
    REAL, DIMENSION(npts), INTENT(in)   :: x                !! fire index (unitless, 0-1)

    !! 0.2 Output variables

    REAL, DIMENSION(npts)               :: firefrac_result  !! fire fraction (unitless, 0-1)

    !! 0.3 Modified variables

    !! 0.4 Local variables
    REAL, DIMENSION(npts)               :: xm1              !! intermediate variable
!_ ================================================================================================================================
