...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_stomate/lpj_light.f90 $
...
!!
!! If agriculture is present, fpc can never reach 1.0 since natural veget_cov_max < 1.0. To
...
!end gmjc
 ...
 !! 0. Variable and parameter declaration
    ...
    REAL, INTENT(in)                                :: dt                       !! Time step (days)      
    LOGICAL, DIMENSION(npts,nvm), INTENT(in)               :: PFTpresent               !! TRUE if pft is present (true/false)
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: cn_ind                   !! Crown area of individuals 
                                                                                       !! @tex $(m^2)$ @endtex  
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: lai                      !! Leaf area index OF AN INDIVIDUAL PLANT 
                                                                                       !! @tex $(m^2 m^{-2})$ @endtex   
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: maxfpc_lastyear          !! Last year's maximum fpc for each natural 
                                                                                       !! PFT(unitless)  
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: lm_lastyearmax           !! Last year's maximum leafmass for each 
                                                                                       !! natural PFT 
                                                                                       !! @tex $(gC m^2 s^{-1})$ @endtex   
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: veget_cov_max            !! Current year's maximum fpc for each natural 
                                                                                       !! PFT (unitless;0-1)   
    REAL, DIMENSION(npts,nvm), INTENT(in)           :: fpc_max                  !! Last year's maximum fpc for each natural 
                                                                                       !! PFT (unitless)    
    !gmjc
    ...
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: ind                      !! Number of individuals 
                                                                                       !! @tex $(m^{-2})$ @endtex    
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout) :: biomass        !! Biomass @tex $(gCm^{-2})$ @endtex    
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: veget_lastlight          !! Vegetation cover after last light 
                                                                                       !! competition (unitless;0-1)     
    REAL, DIMENSION(npts,nvm,nparts,nelements), INTENT(inout) :: bm_to_litter   !! Biomass transfer to litter per timestep
                                                                                       !! @tex $(gCm^{-2})$ @endtex    
    REAL, DIMENSION(npts,nvm), INTENT(inout)        :: mortality                !! Fraction of individuals that died this 
    ...
    REAL, DIMENSION(npts)                           :: sumfpc                   !! Total natural fpc, sum of all the PFTs
                                                                                       !! (unitless)   
    REAL, DIMENSION(npts)                           :: fracnat                  !! Fraction of natural vegetation within a 
                                                                                       !! grid cell (unitless;0-1)   
    REAL                                            :: sumfpc_wood              !! Total natural woody fpc (unitless)   
    REAL                                            :: sumdelta_fpc_wood        !! Change in total woody fpc (unitless)   
    REAL                                            :: maxfpc_wood              !! Maximum wood fpc (unitless)   
    INTEGER                                         :: optpft_wood              !! Which woody pft is maximum (unitless)   
    REAL                                            :: sumfpc_grass             !! Total natural grass fpc (unitless)   
    REAL, DIMENSION(npts,nvm)                       :: fpc_nat                  !! This year's foliage projected cover on 
                                                                                       !! natural part of the grid cell 
                                                                                       !! @tex $(m^2)$ @endtex
    REAL, DIMENSION(nvm)                            :: deltafpc                 !! fpc change within last year (unitless)    
    REAL                                            :: reduct                   !! Relative change of number of individuals
                                                                                       !! for trees (ind)    
    REAL, DIMENSION(nvm)                            :: survive                  !! Fraction of plants that survive 
                                                                                       !! (unitless;0-1)     
    REAL, DIMENSION(npts)                           :: fpc_real                 !! FPC for static mode (unitless)     
    REAL, DIMENSION(npts)                           :: lai_ind                  !! FPC mortality for static mode     
    REAL                                            :: sumfpc_grass2            !! New total grass fpc     
    REAL, DIMENSION(npts,nvm)                       :: light_death              !! Fraction of plants that dies each day 
                                                                                       !! @tex $(day^{-1})$ @endtex     
    REAL                                            :: fpc_dec                  !! Relative change of number of individuals 
                                                                                       !! for trees
!_ ================================================================================================================================
       ...
       DO j = 2, nvm ! loop over #PFTs
	     ...
	     !!?? CODE SHOULD BE CLEANED UP BELOW
             ...
             IF ( is_tree(j) ) THEN
                ...
                !             ENDIF
                !          ENDDO
                !NV : modif from S. Zaehle version : fpc is based on veget_cov_max, not veget.
