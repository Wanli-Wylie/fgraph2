...
MODULE time
  ...
  PUBLIC :: time_initialize, time_nextstep

  REAL, PUBLIC                 :: dt_sechiba         !! Lenght of time step in sechiba (s)
!$OMP THREADPRIVATE(dt_sechiba)
  REAL, PUBLIC                 :: dt_stomate         !! Length of time step in slow processes in stomate (s)
!$OMP THREADPRIVATE(dt_stomate)
  CHARACTER(LEN=20), PUBLIC           :: calendar_str       !! The calendar type
!$OMP THREADPRIVATE(calendar_str)
  INTEGER, SAVE, PUBLIC        :: year_start         !! Year at beginng of current time step
!$OMP THREADPRIVATE(year_start)
  INTEGER, SAVE, PUBLIC        :: month_start        !! Month at beginng of current time step
!$OMP THREADPRIVATE(month_start)
  INTEGER, SAVE, PUBLIC        :: day_start          !! Day in month at beginng of current time step
!$OMP THREADPRIVATE(day_start)
  REAL, SAVE, PUBLIC           :: sec_start          !! Seconds in the day at beginng of current time step
!$OMP THREADPRIVATE(sec_start)
  INTEGER, SAVE, PUBLIC        :: year_end           !! Year at end of current time step
!$OMP THREADPRIVATE(year_end)
  INTEGER, SAVE, PUBLIC        :: month_end          !! Month at end of current time step
!$OMP THREADPRIVATE(month_end)
  INTEGER, SAVE, PUBLIC        :: day_end            !! Day in month at end of current time step
!$OMP THREADPRIVATE(day_end)
  REAL, SAVE, PUBLIC           :: sec_end            !! Seconds in the day at end of current time step
...
!$OMP THREADPRIVATE(tstepint_type)
  INTEGER, PUBLIC              :: month_len          !! Lenght of current month (d)
!$OMP THREADPRIVATE(month_len)
  INTEGER, PRIVATE             :: year_length        !! Lenght of current year in time step (tstp)
!$OMP THREADPRIVATE(year_length)
  REAL, PUBLIC                 :: one_day            !! Lenght of one day in seconds (s)
!$OMP THREADPRIVATE(one_day)
  REAL, PUBLIC                 :: one_year           !! Length of current year in days (d)
!$OMP THREADPRIVATE(one_year)
  REAL, PARAMETER, PUBLIC      :: one_hour = 3600.0  !! Lenght of hour in seconds (s)  
...
!$OMP THREADPRIVATE(LastTsDay)
  REAL, SAVE, PUBLIC           :: date0_save         !! Start date of simulation, in juilan calendar
!$OMP THREADPRIVATE(date0_save)
  REAL, PUBLIC                 :: julian_diff        !! Days since the beginning of current year
!$OMP THREADPRIVATE(julian_diff)
  REAL, PUBLIC                 :: julian_start       !! Beginning of the interval for current time step, in juilan calendar
!$OMP THREADPRIVATE(julian_start)
  REAL, PUBLIC                 :: julian_end         !! End of the interval for current time step, in juilan calendar
!$OMP THREADPRIVATE(julian_end)
  REAL, SAVE, PUBLIC           :: julian0            !! First day of this year in julian caledrier
!$OMP THREADPRIVATE(julian0)
  INTEGER, SAVE, PRIVATE       :: printlev_loc       !! Local level of text output for current module
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE time_initialize(kjit, date0_loc, dt_sechiba_loc, tstepint)
    ...
    !! 0.1 Input arguments
    INTEGER, INTENT(in)   :: kjit           !! Time step of the restart file
    REAL, INTENT(in)      :: date0_loc      !! The date at which kjit=0
    REAL, INTENT(in)      :: dt_sechiba_loc !! Time step of sechiba component
...
!_ ==============================================================================================================================
  SUBROUTINE time_nextstep(kjit)
    ...
    !! 0.1 Input variables
    INTEGER, INTENT(in)   :: kjit       !! Current time step
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE time_interval(kjit, year_s, month_s, day_s, sec_s, &
    ...
    INTEGER, INTENT(out)                 :: year_s, month_s, day_s
    INTEGER, INTENT(out)                 :: year_e, month_e, day_e
    REAL, INTENT(out)                    :: sec_s
