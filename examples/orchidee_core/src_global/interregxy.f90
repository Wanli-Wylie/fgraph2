...
CONTAINS
  ...
  SUBROUTINE interregxy_aggr2d(nbpt, lalo, neighbours, resolution, contfrac, &
    ...
    INTEGER, INTENT(in)   :: iml, jml             ! Size of the source grid
    REAL, INTENT(in)      :: lon_rel(iml, jml)    ! Longitudes for the source grid
    REAL, INTENT(in)      :: lat_rel(iml, jml)    ! Latitudes for the souce grid
    ...
    INTEGER, INTENT(in)   :: incmax               ! Maximum point of the source grid we can store.
    !
    ! Output
    !
    INTEGER, INTENT(out)    :: indinc(nbpt,incmax,2)
    ...
    !
    INTEGER, PARAMETER  :: sbd=5
    INTEGER, PARAMETER  :: idom=1
    !
    INTEGER                                :: i, is, js, in
    INTEGER                                :: ilow, iup, jlow, jup
    REAL                                   :: dle, dlw, dls, dln
    !
    REAL, DIMENSION((sbd+1)*4+1)           :: lons, lats, ri, rj
    !
    INTEGER, ALLOCATABLE, DIMENSION(:,:)   :: nbov
    INTEGER, ALLOCATABLE, DIMENSION(:,:,:) :: sourcei, sourcej
    REAL, ALLOCATABLE, DIMENSION(:,:,:)    :: overlap
    LOGICAL                                       :: checkprint=.FALSE.
    !
    !
    REAL, DIMENSION(2)   :: BBlon, BBlat
    !
...
!
  SUBROUTINE interregxy_aggrve(nbpt, lalo, neighbours, resolution, contfrac, &
    ...
    REAL, INTENT(in)      :: resolution(nbpt,2)   ! The size in km of each grid-box in X and Y
    REAL, INTENT(in)      :: contfrac(nbpt)       ! Fraction of land in each grid box.
    INTEGER, INTENT(in)   :: iml                  ! Size of the source grid
    REAL, INTENT(in)      :: lon_rel(iml)         ! Longitudes for the source grid
    REAL, INTENT(in)      :: lat_rel(iml)         ! Latitudes for the source grid
    REAL, INTENT(in)      :: resol_lon, resol_lat ! Resolution in meters of the source grid
    CHARACTER(LEN=*), INTENT(in) :: callsign             ! Allows to specify which variable is beeing treated
    INTEGER, INTENT(in)   :: incmax               ! Maximum point of the source grid we can store.
    !
    ! Output
    ...
    !
    REAL, DIMENSION(2)   :: BBlon, BBlat
    REAL                 :: half_resol_lon, half_resol_lat, trtmp
    LOGICAL                     :: checkprint=.FALSE.
    !
...
!
  SUBROUTINE interregxy_findpoints(nb_we, nb_sn, lon_cen, lat_cen, is, js, ri, rj, &
    ...
    ! Arguments
    !
    INTEGER, INTENT(in)                      :: nb_we, nb_sn
    REAL, INTENT(in)                         :: lon_cen, lat_cen
    INTEGER, INTENT(in)                      :: is, js
    REAL, DIMENSION(:), INTENT(in)           :: ri, rj
    INTEGER, INTENT(in)                      :: nbpt_max
    LOGICAL, INTENT(in)                             :: checkprint
    INTEGER, DIMENSION(:,:), INTENT(out)     :: nbpt
    INTEGER, DIMENSION(:,:,:), INTENT(out)   :: sourcei, sourcej
    REAL, DIMENSION(:,:,:), INTENT(out)      :: overlap_area
    !
    ! Local
    ! 
    INTEGER                                  :: ilow, iup, jlow, jup, nbr
    INTEGER                                  :: ix, jx, i

    INTEGER                                  :: nbint, nbcross, nbsorted, nbfinal, nbtmpa, nbtmpb
    !
    INTEGER                                  :: nbdots=5, dimpoly = 200
    REAL, DIMENSION(4,2)                     :: poly_wrf
    REAL, ALLOCATABLE, DIMENSION(:,:)        :: poly_in, poly_tmpa, poly_tmpb, poly_int, poly_cross
    !
    REAL                                     :: ax, ay, overlaparea
    !
    INTEGER, PARAMETER                       :: icheck=50, jcheck=46
    !
    !
    !
    nbr = SIZE(ri)
