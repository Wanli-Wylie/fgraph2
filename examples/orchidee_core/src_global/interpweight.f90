...
!_================================================================================================================================
  ...
  PRIVATE
  PUBLIC interpweight_1D, interpweight_2D, interpweight_3D, interpweight_4D, interpweight_2Dcont,     &
    interpweight_4Dcont, interpweight_get_varNdims_file, interpweight_RangeR,                         &
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_1D(nbpt, Nvariabletypes, variabletypes, lalo, resolution, neighbours,       &
    ...
    !! 0.1 Input variables
    !
    INTEGER, INTENT(in)                 :: nbpt                    !! Number of points for which the data 
                                                                          !!   needs to be interpolated
    INTEGER, INTENT(in)                 :: Nvariabletypes          !! Number of types of the variable
    REAL, DIMENSION(Nvariabletypes),    &                          !! Vector of values of the types
      INTENT(in)    :: variabletypes                                      !!   (-1, not used)
    REAL, DIMENSION(nbpt,2), INTENT(in) :: lalo                    !! Vector of latitude and longitudes 
    ...
    REAL, DIMENSION(nbpt),  INTENT(in)  :: contfrac                !! Fraction of land in each grid box.
    ...
    REAL, INTENT(inout)                 :: varmin, varmax          !! min/max values to use for the
                                                                          !!   renormalization
! Transformations to apply to the original read values from the file
    ...
    CHARACTER(LEN=50), INTENT(in)              :: masktype                !! Type of masking
                                                                          ...
                                                                          !!     variable (>0)
    REAL, DIMENSION(3), INTENT(in)      :: maskvalues              !! values to use to mask (according to 
                                                                          !!   `masktype') 
    CHARACTER(LEN=250), INTENT(in)             :: maskvarname             !! name of the variable to use to mask 
    INTEGER, INTENT(in)                 :: dim1, dim2              !! 4D complementary dimension sizes
    CHARACTER(LEN=50), INTENT(in)              :: typefrac                !! Type of fraction retrieval:
                                                                          !!   'default': standard

    INTEGER, INTENT(in)                 :: initime                 !! Initial time to take (for input data 
                                                                          !!   with time values, -1 not used)
    REAL, INTENT(in)                    :: maxresollon,maxresollat !! lon,lat maximum resolutions (in m)
                                                                          !!   (-un, not used)
    !! 0.2 Modified variables
    !
    !! 0.3 output variables
    !
    REAL, DIMENSION(nbpt), INTENT(out) :: outvar1D                 !! 1D output variable and re-dimensioned
    REAL, DIMENSION(nbpt), INTENT(out)  :: aoutvar                 !! availability of input data to
                                                                          !!   interpolate output variable 
                                                                          !!   (on the nbpt space)
    !
    !! 0.4 Local variables
    !
    INTEGER                             :: iml, jml, lml, tml, fid, ib, ip, jp, it, jj, jv
    INTEGER                                    :: inNdims                 !! Number of dimensions of the input data
    INTEGER                                    :: inNLldims               !! Number of dimensions of the lon/lat input data
    REAL, ALLOCATABLE, DIMENSION(:)     :: lat_in, lon_in          !! latitude and
                                                                          !! longitude, extract from input map
    REAL, ALLOCATABLE, DIMENSION(:,:)   :: sub_area                !! the area of the fine grid in the 
                                                                          !!   model grid ???
                                                                          !! cf src_global/interpol_help.f90, 
                                                                          !!   line 377, called "areaoverlap"
    INTEGER, ALLOCATABLE, DIMENSION(:,:)  :: sub_index             !! the indexes from the grid boxes from the 
                                                                          !!   data that go into the 
                                                                          !!   model's boxes  
                                                                          !! cf src_global/interpol_help.f90,
                                                                          !!   line 300, called "ip"

    REAL, ALLOCATABLE, DIMENSION(:)       :: invar1D               !! 1D input values
    REAL, ALLOCATABLE, DIMENSION(:,:)     :: invar2D               !! 2D input values
    REAL, ALLOCATABLE, DIMENSION(:,:,:)   :: invar3D               !! 3D input values
    REAL, ALLOCATABLE, DIMENSION(:,:,:,:) :: invar4D               !! 4D input values

    REAL, ALLOCATABLE, DIMENSION(:,:,:)   :: resol_in
    INTEGER, ALLOCATABLE, DIMENSION(:)    :: mask
    REAL, ALLOCATABLE, DIMENSION(:)       :: maskvar
    INTEGER                             :: nix, njx
    INTEGER                             :: idi, nbvmax             !! nbvmax : number of maximum vegetation map
    ...
    INTEGER                                    :: ALLOC_ERR 
    CHARACTER(LEN=50)                          :: S1
    REAL, ALLOCATABLE, DIMENSION(:)     :: variableusetypes
...
!    REAL, INTENT(out)                   ::  laimap(nbpt,nvm,12)    !! lai read variable and re-dimensioned
    REAL, DIMENSION(nbpt,Nvariabletypes), INTENT(out) :: outvar2D  !! 2D output variable and re-dimensioned
    REAL, DIMENSION(nbpt), INTENT(out)  :: aoutvar                 !! availability of input data to
                                                                          !!   interpolate output variable 
                                                                          !!   (on the nbpt space)
    !
    ...
    REAL, ALLOCATABLE, DIMENSION(:)     :: lat_in, lon_in          !! latitude and
                                                                          !! longitude, extract from input map
    REAL, ALLOCATABLE, DIMENSION(:,:)   :: lat, lon                !! en 2D ???
    REAL, ALLOCATABLE, DIMENSION(:,:)   :: sub_area                !! the area of the fine grid in the 
                                                                          !!   model grid ???
                                                                          !! cf src_global/interpol_help.f90, 
                                                                          !!   line 377, called "areaoverlap"
...
! Transformations to apply to the original read values from the file
    ...
    REAL, DIMENSION(dim1), INTENT(inout) :: varmin, varmax          !! min/max values to use for the renormalization
    CHARACTER(LEN=50), INTENT(in)              :: typefrac                !! Type of fraction retrieval:
                                                                          !!   'default': standard
    INTEGER, INTENT(in)                 :: initime                 !! Initial time to take (for input data 
                                                                          !!   with time values, -1 not used)
    ...
    !  0.3 OUTPUT
    !
    REAL, DIMENSION(nbpt,dim1), INTENT(out) :: outvar3D            !! 3D output variable and re-dimensioned
...
! Transformations to apply to the original read values from the file
    ...
    !
!    REAL, INTENT(out)                   ::  laimap(nbpt,nvm,12)    !! lai read variable and re-dimensioned
    REAL, DIMENSION(nbpt,dim1,dim2), INTENT(out) :: outvar4D       !! 4D output variable and re-dimensioned
    REAL, DIMENSION(nbpt), INTENT(out)  :: aoutvar                 !! availability of input data to
                                                                          !!   interpolate output variable 
                                                                          !!   (on the nbpt space)
    !
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_2Dcont(nbpt, dim1, dim2, lalo, resolution, neighbours,                      &
    ...
    CHARACTER(LEN=80), INTENT(in)              :: varname                 !! name of the variable to interpolate
    CHARACTER(LEN=80), INTENT(in)              :: inlonname, inlatname    !! names of the longitude and latitude
    REAL, INTENT(in)                    :: varmin, varmax          !! min/max values to use for the
                                                                          !!   renormalization
    REAL, INTENT(in)                    :: defaultvalue            !! default interpolated value 
    REAL, INTENT(in)                    :: defaultNOvalue          !! default interpolated NO-value 
...
! Transformations to apply to the original read values from the file
    ...
    !! 0.3 Output variables
    !
    REAL, DIMENSION(nbpt), INTENT(out)  :: outvar2D                !! 2D output variable and re-dimensioned
    REAL, DIMENSION(nbpt), INTENT(out)  :: aoutvar                 !! availability of input data to
                                                                          !!   interpolate output variable 
                                                                          !!   (on the nbpt space)
    !
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_4Dcont(nbpt, dim1, dim2, lalo, resolution, neighbours,                      &
    ...
    REAL, INTENT(in)                    :: varmin, varmax          !! min/max values to use for the
                                                                          !!   renormalization
    REAL, DIMENSION(dim1,dim2), INTENT(in) :: defaultvalue         !! default interpolated value 
    REAL, INTENT(in)                    :: defaultNOvalue          !! default interpolated NO-value 
...
! Transformations to apply to the original read values from the file
    ...
    !! 0.3 Output variables
    !
    REAL, DIMENSION(nbpt,dim1,dim2), INTENT(out) :: outvar4D       !! 2D output variable and re-dimensioned
    REAL, DIMENSION(nbpt), INTENT(out)  :: aoutvar                 !! availability of input data to
                                                                          !!   interpolate output variable 
                                                                          !!   (on the nbpt space)
    !
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_calc_resolution_in(lons, lats, dx, dy, mcos, REarth, piv, resolin)
    ...
    !! 0.1 Input variables
    !
    INTEGER, INTENT(in)                           :: dx, dy        !! Resolution input data
    REAL, INTENT(in)                              :: mcos          !! cosinus
    REAL, INTENT(in)                              :: REarth        !! Earth Radius
    REAL, INTENT(in)                              :: piv           !! pi
    REAL, DIMENSION(dx,dy), INTENT(in)            :: lons, lats    !! longitude and latitudes (degrees)
    !! 0.2 Modified variables
    !
    !! 0.3 Output variables
    !
    REAL, DIMENSION(dx,dy,2), INTENT(out)         :: resolin       !! distance between grid points (km)
    !
    !! 0.4 Local variables
    !
    INTEGER                                              :: ip,jp
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_modifying_input1D(dim1, dim2, dim3, dim4, stime, etime, zeroval, ivar)
    ...
    REAL, INTENT(in)                              :: zeroval       !! zero value
    !! 0.2 Modified variables
    REAL, DIMENSION(dim1), INTENT(inout)          :: ivar          !! modified input data
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_modifying_input2D(dim1, dim2, dim3, dim4, stime, etime, zeroval, ivar)
    ...
    REAL, INTENT(in)                              :: zeroval       !! zero value
    !! 0.2 Modified variables
    REAL, DIMENSION(dim1,dim2), INTENT(inout)     :: ivar          !! modified input data
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_modifying_input3D(dim1, dim2, dim3, dim4, stime, etime, zeroval, ivar)
    ...
    REAL, INTENT(in)                              :: zeroval       !! zero value
    !! 0.2 Modified variables
    REAL, DIMENSION(dim1,dim2,dim3), INTENT(inout):: ivar          !! modified input data
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_modifying_input4D(dim1, dim2, dim3, dim4, stime, etime, zeroval, ivar)
    ...
    REAL, INTENT(in)                              :: zeroval       !! zero value
    !! 0.2 Modified variables
    REAL, DIMENSION(dim1,dim2,dim3,dim4), INTENT(inout) :: ivar          !! modified input data
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_masking_input1D(dx, dy, d1, d2, d3, d4, mtype, mvalues, var,                &
    ...
    CHARACTER(LEN=50), INTENT(in)                        :: mtype         !! Type of masking
                                                                          ...
                                                                          !!   'var': mask values are taken from a 
                                                                          !!     variable (>0)
    REAL, DIMENSION(3), INTENT(in)                :: mvalues       !! values to use to mask (according to `mtype') 
    !! 0.2 Modified variables
    REAL, DIMENSION(dx), INTENT(inout)            :: var           !! variable from which provide the mask
    INTEGER, DIMENSION(dx), INTENT(inout)         :: msk           !! mask to retrieve
    !! 0.3 Output variables
    !! 0.4 Local variables
    INTEGER                                              :: i,j,k,l
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_masking_input2D(dx, dy, d1, d2, d3, d4, mtype, mvalues, var,                &
    ...
    REAL, DIMENSION(3), INTENT(in)                :: mvalues       !! values to use to mask (according to `mtype') 
    !! 0.2 Modified variables
    REAL, DIMENSION(dx,dy), INTENT(inout)         :: var           !! variable from which provide the mask
    INTEGER, DIMENSION(dx,dy), INTENT(inout)      :: msk           !! mask to retrieve
    !! 0.3 Output variables
    !! 0.4 Local variables
    INTEGER                                              :: i,j,k,l
    REAL                                          :: sumval
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_masking_input3D(dx, dy, d1, d2, d3, d4, mtype, mvalues, var,                &
    ...
    REAL, DIMENSION(3), INTENT(in)                :: mvalues       !! values to use to mask (according to `mtype') 
    !! 0.2 Modified variables
    REAL, DIMENSION(dx,dy,d3), INTENT(inout)      :: var           !! variable from which provide the mask
    INTEGER, DIMENSION(dx,dy), INTENT(inout)      :: msk           !! mask to retrieve
    !! 0.3 Output variables
    !! 0.4 Local variables
    INTEGER                                              :: i,j,k,l
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_masking_input4D(dx, dy, d1, d2, d3, d4, mtype, mvalues, var,                &
    ...
    REAL, DIMENSION(3), INTENT(in)                :: mvalues       !! values to use to mask (according to `mtype') 
    !! 0.2 Modified variables
    REAL, DIMENSION(dx,dy,d3,d4), INTENT(inout)   :: var           !! variable from which provide the mask
    INTEGER, DIMENSION(dx,dy), INTENT(inout)      :: msk           !! mask to retrieve
    !! 0.3 Output variables
    !! 0.4 Local variables
    INTEGER                                              :: i,j,k,l
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_provide_fractions1D(tint, npt, Ntypes, valstypes,                           &
    ...
    INTEGER, INTENT(in)                                  :: nbmax         !! maximum input grid cells to use 
                                                                          !!   for in 1 grid point
    REAL, INTENT(in)                              :: zeroval, two  !! zero and 2. real values
    INTEGER, INTENT(in)                                  :: Ntypes        !! number of types
    REAL, DIMENSION(Ntypes), INTENT(in)           :: valstypes     !! value for each type 
    REAL, DIMENSION(dx), INTENT(in)               :: ivar1D        !! values of the input data
    REAL, DIMENSION(npt,nbmax), INTENT(in)        :: sarea         !! overlaping area of a given input 
                                                                          !!   grid point
    INTEGER, DIMENSION(npt,nbmax), INTENT(in)     :: sindex        !! index of the overlaping input 
                                                                          !!   grid point
    REAL, DIMENSION(npt,2), INTENT(in)            :: latlon        !! longitude and latitude of the destiny
                                                                          !!   grid points
    !! 0.2 Modified variables
    REAL, INTENT(inout)                           :: vmax, vmin    !! thresholds of the values
    !! 0.3 Output variables
    REAL, DIMENSION(npt,Ntypes), INTENT(out)      :: ovar          !! output variable (on the nbpt space)
    REAL, DIMENSION(npt), INTENT(out)             :: aovar         !! availability of input data to 
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_provide_fractions2D(tint, npt, Ntypes, valstypes,                           &
    ...
    INTEGER, INTENT(in)                                  :: Ntypes        !! number of types
    REAL, DIMENSION(Ntypes), INTENT(in)           :: valstypes     !! value for each type 
    REAL, DIMENSION(dx,dy), INTENT(in)            :: ivar2D        !! values of the input data
    REAL, DIMENSION(npt,nbmax), INTENT(in)        :: sarea         !! overlaping area of a given input 
                                                                          !!   grid point
    INTEGER, DIMENSION(npt,nbmax,2), INTENT(in)   :: sindex        !! index of the overlaping input 
                                                                          !!   grid point
    REAL, DIMENSION(npt,2), INTENT(in)            :: latlon        !! longitude and latitude of the destiny
                                                                          !!   grid points
    !! 0.2 Modified variables
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_provide_fractions3D(tint, npt, Ntypes, valstypes,                           &
    ...
    INTEGER, INTENT(in)                                  :: Ntypes        !! number of types
    REAL, DIMENSION(Ntypes), INTENT(in)           :: valstypes     !! value for each type 
    REAL, DIMENSION(dx,dy,d3), INTENT(in)         :: ivar3D        !! values of the input data
    REAL, DIMENSION(npt,nbmax), INTENT(in)        :: sarea         !! overlaping area of a given input 
                                                                          !!   grid point
    INTEGER, DIMENSION(npt,nbmax,2), INTENT(in)   :: sindex        !! index of the overlaping input 
                                                                          !!   grid point
    REAL, DIMENSION(npt,2), INTENT(in)            :: latlon        !! longitude and latitude of the destiny
                                                                          !!   grid points
    !! 0.2 Modified variables
    REAL, DIMENSION(Ntypes), INTENT(inout)        :: vmax, vmin    !! thresholds of the values
    !! 0.3 Output variables
    REAL, DIMENSION(npt,Ntypes), INTENT(out)      :: ovar          !! output variable (on the nbpt space)
    REAL, DIMENSION(npt), INTENT(out)             :: aovar         !! availability of input data to 
                                                                          !!   interpolate output variable (on 
    ...
    INTEGER                                              :: Nzeros
    INTEGER                                              :: ALLOC_ERR 
    REAL, ALLOCATABLE, DIMENSION(:)               :: subfrac
    REAL                                          :: sumpt
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_provide_fractions4D(tint, npt, Ntypes, valstypes,                           &
    ...
    INTEGER, INTENT(in)                                  :: Ntypes        !! number of types
    REAL, DIMENSION(Ntypes), INTENT(in)           :: valstypes     !! value for each type 
    REAL, DIMENSION(dx,dy,d3,d4), INTENT(in)      :: ivar4D        !! values of the input data
    REAL, DIMENSION(npt,nbmax), INTENT(in)        :: sarea         !! overlaping area of a given input 
                                                                          !!   grid point
    INTEGER, DIMENSION(npt,nbmax,2), INTENT(in)   :: sindex        !! index of the overlaping input 
                                                                          !!   grid point
    ...
    REAL, DIMENSION(Ntypes), INTENT(inout)        :: vmax, vmin    !! thresholds of the values
    !! 0.3 Output variables
    REAL, DIMENSION(npt,Ntypes,d4), INTENT(out)   :: ovar          !! output variable (on the nbpt space)
    REAL, DIMENSION(npt), INTENT(out)             :: aovar         !! availability of input data to 
                                                                          !!   interpolate output variable (on 
                                                                          !!   the nbpt space)
                                                                          !!     >0.: As the total sum of the 
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_provide_interpolation2D(tint, npt, d1, d2, dx, dy, d3, d4,                  &
    ...
    INTEGER, INTENT(in)                                  :: nbmax         !! maximum input grid cells to use 
                                                                          !!   for in 1 grid point
    REAL, INTENT(in)                              :: zeroval,one,two !! zero 1. and 2. real values
    REAL, DIMENSION(dx,dy), INTENT(in)            :: ivar2D        !! values of the input data
    REAL, DIMENSION(npt,nbmax), INTENT(in)        :: sarea         !! overlaping area of a given input 
                                                                          !!   grid point
    INTEGER, DIMENSION(npt,nbmax,2), INTENT(in)   :: sindex        !! index of the overlaping input 
    ...
    REAL, DIMENSION(npt,2), INTENT(in)            :: latlon        !! longitude and latitude of the destiny
                                                                          !!   grid points
    REAL, INTENT(in)                              :: defaultval    !! default interpolated value, only usef if tint='default'
    REAL, INTENT(in)                              :: defaultNOval  !! default interpolated NO value, only used if tint='slopecalc'
    !! 0.2 Modified variables
    REAL, INTENT(in)                              :: vmax, vmin    !! thresholds of the values
    !! 0.3 Output variables
    REAL, DIMENSION(npt), INTENT(out)             :: ovar          !! output variable (on the nbpt space)
    REAL, DIMENSION(npt), INTENT(out)             :: aovar         !! availability of input data to 
                                                                          !!   interpolate output variable (on 
                                                                          !!   the nbpt space)
                                                                          !!     >0.: As the total sum of the 
...
!_ ================================================================================================================================
  SUBROUTINE interpweight_provide_interpolation4D(tint, npt, d1, d2, dx, dy, d3, d4,                  &
    ...
    INTEGER, INTENT(in)                                  :: nbmax         !! maximum input grid cells to use 
                                                                          !!   for in 1 grid point
    REAL, INTENT(in)                              :: zeroval,two,one  !! zero 1. and 2. real values
    REAL, DIMENSION(dx,dy,d3,d4), INTENT(in)      :: ivar4D        !! values of the input data
    REAL, DIMENSION(npt,nbmax), INTENT(in)        :: sarea         !! overlaping area of a given input 
                                                                          !!   grid point
    INTEGER, DIMENSION(npt,nbmax,2), INTENT(in)   :: sindex        !! index of the overlaping input 
    ...
    REAL, DIMENSION(npt,2), INTENT(in)            :: latlon        !! longitude and latitude of the destiny
                                                                          !!   grid points
    REAL, DIMENSION(d1,d2), INTENT(in)            :: defaultval    !! default interpolated value
    REAL, INTENT(in)                              :: defaultNOval  !! default interpolated NO value
    !! 0.2 Modified variables
    REAL, INTENT(in)                              :: vmax, vmin    !! thresholds of the values
    !! 0.3 Output variables
    REAL, DIMENSION(npt,d1,d2), INTENT(out)       :: ovar          !! output variable (on the nbpt space)
    REAL, DIMENSION(npt), INTENT(out)             :: aovar         !! availability of input data to 
                                                                          !!   interpolate output variable (on 
                                                                          !!   the nbpt space)
                                                                          !!     >0.: As the total sum of the 
...
! Debug
    INTEGER                                              :: nbptlL
    REAL, DIMENSION(npt,d1,d2)                    :: ovartst
...
!_================================================================================================================================
  INTEGER FUNCTION interpweight_Index1DLonLat(lon, lat, d1, lonval, latval)
    ...
    !! 0.1 Input variables
    INTEGER, INTENT(in)                                  :: d1            !! size of the array 
    REAL, INTENT(in)                              :: lonval,latval !! longitude and latitude value
    REAL, DIMENSION(d1), INTENT(in)               :: lon, lat      !! longitude and latitudes to search in
    !! 0.2 Modified variables
    !! 0.3 Output variables
    !! 0.4 Local variables
    INTEGER                                              :: i
    REAL                                          :: mindist
    REAL, DIMENSION(d1)                           :: dist
...
!_================================================================================================================================
  FUNCTION interpweight_Index2DLonLat(lon, lat, d1, d2, lonval, latval)
    ...
    INTEGER, INTENT(in)                                  :: d1, d2        !! size of the array 
    REAL, INTENT(in)                              :: lonval,latval !! longitude and latitude value
    REAL, DIMENSION(d1,d2), INTENT(in)            :: lon, lat      !! longitude and latitudes to search in
    !! 0.2 Modified variables
    !! 0.3 Output variables
