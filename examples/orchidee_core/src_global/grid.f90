...
!!
!! @call sechiba_main
!! @Version : $Revision: 4717 $, $Date: 2017-10-26 14:32:36 +0200 (四, 2017-10-26) $
!! 
!< $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_global/grid.f90 $
...
!$OMP THREADPRIVATE(global)
  ...
  REAL, PARAMETER :: default_resolution = 250000.
  !
  ! VARIABLES
  !
  !-
  ...
  REAL, SAVE                                   :: limit_west, limit_east, &
...
!$OMP THREADPRIVATE(neighbours)
  ...
  !
  REAL, ALLOCATABLE, DIMENSION(:,:), SAVE      :: headings
!$OMP THREADPRIVATE(headings)
  !
  ! Length of segments of the polygon.
  !
  REAL, ALLOCATABLE, DIMENSION(:,:), SAVE      :: seglength
...
!$OMP THREADPRIVATE(area)
  ...
  !
  REAL, ALLOCATABLE, DIMENSION(:,:,:), SAVE    :: corners
...
!$OMP THREADPRIVATE(grid_dir)
  ...
  !
  REAL, SAVE, ALLOCATABLE, DIMENSION(:,:)   :: dxwrf, dywrf
  !
  !
...
!!
  SUBROUTINE grid_init ( npts, nbseg, gtype, gname, isglobal )
    ...
    !
    ! Domain size
    INTEGER, INTENT(in)                                 :: npts     !! Number of local continental points
    INTEGER, INTENT(in)                                 :: nbseg    !! number of segments of the polygone of the mesh
...
!!
  SUBROUTINE grid_set_glo(arg_nbp_lon,arg_nbp_lat,arg_nbp_glo)
    ...
    INTEGER, INTENT(IN) :: arg_nbp_lon
    INTEGER, INTENT(IN) :: arg_nbp_lat
    INTEGER, INTENT(IN),OPTIONAL :: arg_nbp_glo
    iim_g=arg_nbp_lon
    jjm_g=arg_nbp_lat
...
!!
  SUBROUTINE grid_stuff (npts_glo, iim, jjm, grid_lon, grid_lat, kindex, contfrac_tmp)
    ...
    ! Domain size
    INTEGER, INTENT(in)                                 :: npts_glo
    ! Size of cartesian grid
    INTEGER, INTENT(in)                                 :: iim, jjm
    ! Longitudes on cartesian grid
    REAL, DIMENSION(iim,jjm), INTENT(in)                :: grid_lon
    ! Latitudes on cartesian grid
    REAL, DIMENSION(iim,jjm), INTENT(in)                :: grid_lat
    ! Index of land point on 2D map (in local position)
    INTEGER, DIMENSION(npts_glo), INTENT(in)            :: kindex
    ! The fraction of continent in the grid box [0-1]
    REAL, DIMENSION(npts_glo), OPTIONAL, INTENT(in)     :: contfrac_tmp
    !
    !
    ! =========================================================================
...
!!
  SUBROUTINE grid_topolylist(gtype, nbseg, nland, iim, jjm, grid_lon, grid_lat, kindex, &
    ...
    REAL, DIMENSION(iim,jjm), INTENT(in)          :: grid_lat
    ! Index of land point on 2D map (in local position)
    INTEGER, DIMENSION(nland), INTENT(in)         :: kindex
    !
    ! 0.2 Output
    !
    LOGICAL, INTENT(out)                                  :: globalg
    !
    REAL, DIMENSION(nland,nbseg,2), INTENT(out)    :: corners_loc
    INTEGER, DIMENSION(nland,nbseg*2), INTENT(out) :: neighbours_loc
    REAL, DIMENSION(nland,nbseg*2), INTENT(out)    :: headings_loc
    REAL, DIMENSION(nland,nbseg), INTENT(out)      :: seglength_loc
    REAL, DIMENSION(nland), INTENT(out)            :: area_loc
    INTEGER, DIMENSION(nland), INTENT(out)         :: ilindex_loc, jlindex_loc
    !
    ! 0.3 Local variables
    !
    INTEGER                        :: i, is, iss
    REAL, DIMENSION(nland,2)       :: center
    REAL                           :: maxdellon, mindellon, maxlon, minlon
    REAL, DIMENSION(nland,nbseg*2) :: lonpoly, latpoly
    !
...
!!
  SUBROUTINE grid_initproj (fid, iim, jjm)
    ...
    ! 0.2 Local variables
    !
    INTEGER             :: current_proj, idom, iret, lonid, latid, numLons, numLats
    INTEGER, DIMENSION(nf90_max_var_dims) :: dimIDs
    REAL                :: user_stand_lon, user_truelat1, user_truelat2, user_dxkm, user_dykm
    REAL                :: user_dlat, user_dlon, user_known_x, user_known_y, user_known_lat, user_known_lon
    REAL, DIMENSION(16) :: corner_lons, corner_lats
    !
    INTEGER             :: iv, i, j
    CHARACTER(LEN=20)          :: varname
    REAL                :: dx, dy, dtx, dty, coslat
    REAL, ALLOCATABLE, DIMENSION (:)    :: LON, LAT
    REAL, ALLOCATABLE, DIMENSION (:,:)  :: mapfac_x, mapfac_y
    !
    !
...
!
  SUBROUTINE grid_tolola_1d (ri, rj, lon, lat)
    ...
    !
    ! Argument
    REAL, INTENT(in), DIMENSION(:)      :: ri, rj
...
!
  SUBROUTINE grid_toij_1d (lon, lat, ri, rj)
    ...
    REAL, INTENT(out), DIMENSION(:)    :: ri, rj
    !
    ! Local
    INTEGER                            :: i, imax
    !
