...
!
  SUBROUTINE polygones_pointinside(nvert_in, poly, point_x, point_y, inside)
    ...
    REAL, INTENT(in)                 :: point_x, point_y
...
!
  SUBROUTINE polygones_lineintersect(la, lb, intersection, point_x, point_y)
    ...
    !
    ! ARGUMENTS
    REAL, DIMENSION(2,2), INTENT(in)    :: la, lb
    LOGICAL, INTENT(out)                       :: intersection
    REAL, INTENT(out)                   :: point_x, point_y
    !
    ! Local
    REAL                                :: den, ua, ub
...
!
  SUBROUTINE polygones_extend(nvert_in, poly_in, nbdots, nvert_out, poly_out)
    ...
    ! Arguments
    INTEGER, INTENT(in)               :: nvert_in
    REAL, DIMENSION(:,:), INTENT(in)  :: poly_in
    INTEGER, INTENT(in)               :: nbdots
    INTEGER, INTENT(out)              :: nvert_out
    REAL, DIMENSION(:,:), INTENT(out) :: poly_out
    !
    ! Local
    !
    INTEGER                        :: nvert, nvert_tmp, i, j, id, ipos
    REAL                           :: xs, xe, ys, ye
    !
...
!
  SUBROUTINE polygones_intersection(nvert_a, poly_a, nvert_b, poly_b, nbpts, poly_out)
    ...
    !
    ! Arguments
    INTEGER, INTENT(in)               :: nvert_a
    REAL, DIMENSION(:,:), INTENT(in)  :: poly_a
    INTEGER, INTENT(in)               :: nvert_b
    REAL, DIMENSION(:,:), INTENT(in)  :: poly_b
    INTEGER, INTENT(out)              :: nbpts
    REAL, DIMENSION(:,:), INTENT(out) :: poly_out
    !
    ! Local
    INTEGER                            :: nvert_tmpa, nvert_tmpb, nvert_out, i, j
    LOGICAL                                   :: inside
    INTEGER                            :: in_a, in_b
    INTEGER, ALLOCATABLE, DIMENSION(:) :: ptin_a, ptin_b
    !
...
!
  SUBROUTINE polygones_cleanup(nvert_in, poly_in, nvert_out, poly_out)
    ...
    ! Local
    !
    INTEGER                           :: nvert, nvert_tmpin, nvert_tmp
    INTEGER                           :: i, j
    INTEGER, DIMENSION(1)             :: ismin
    REAL, ALLOCATABLE, DIMENSION(:)   :: dist
    REAL, ALLOCATABLE, DIMENSION(:,:) :: poly_tmp
    !
...
!
  SUBROUTINE polygones_crossing(nvert_a, poly_a, nvert_b, poly_b, nbpts, crossings)
    ...
    REAL, DIMENSION(:,:), INTENT(in)  :: poly_b
    INTEGER, INTENT(out)              :: nbpts
    REAL, DIMENSION(:,:), INTENT(out) :: crossings
    !
    ! Local
    !
    INTEGER                            :: nvert, ia, ja, ib, jb
    REAL, DIMENSION(2,2)               :: la, lb
    REAL                               :: x, y
    LOGICAL                                   :: intersect
    !
...
!
  SUBROUTINE polygones_convexhull(nvert_in, poly_in, nvert_out, poly_out)
    ...
    ! Local
    !
    INTEGER                           :: nvert, nvert_tmp
    INTEGER, ALLOCATABLE, DIMENSION(:):: iwk, next, vertex
    REAL                              :: xmin, temp, dy, dx2, dy1, dy2
    REAL                              :: x1, x2, y1, y2
    REAL                              :: xmax, ymin, ymax
    REAL                              :: dx, dx1
    REAL                              :: dmax, dmax1, dmax2
    REAL                              :: dist, dmin
    INTEGER                           :: i, i1, i2, j, jp1, jp2, i2save, i3, i2next
    LOGICAL                                  :: points_todo
    INTEGER, PARAMETER                :: nextinc=20
    !
