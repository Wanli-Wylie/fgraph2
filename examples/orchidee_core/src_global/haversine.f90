...
!_ ==============================================================================================================================
  SUBROUTINE haversine_reglatlontoploy(iim, jjm, lon, lat, nbpt, index_loc, global, &
    ...
    REAL, DIMENSION(iim,jjm), INTENT(in)                :: lat
    ! Size of the gathered domain
    INTEGER, INTENT(in)                                 :: nbpt
    ! Index of land point on 2D map (in local position)
    INTEGER, DIMENSION(nbpt), INTENT(in)                :: index_loc
    ! Is it a global grid ?
    LOGICAL, INTENT(in)                                        :: global
    ! Number of segments
    INTEGER, INTENT(in)                                 :: nbseg
    !
    ! 0.2 Ouput
    !
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(out)          :: lonpoly
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(out)          :: latpoly
    REAL, DIMENSION(nbpt,2), INTENT(out)                :: center
    INTEGER, DIMENSION(nbpt,nbseg*2), INTENT(out)       :: neighb_loc
    INTEGER, DIMENSION(nbpt), INTENT(out)               :: iorig, jorig
    !
    !
    ! 0.3 Local variables
    !
    INTEGER, DIMENSION(iim,jjm) :: correspondance
    INTEGER                     :: i, ip, jp
    INTEGER                     :: ipm1, ipp1, jpm1, jpp1
    REAL                        :: dlonm1, dlonp1, dlatm1, dlatp1
    !
    !
    correspondance(:,:) = -1
    DO i = 1, nbpt      
...
!!
  SUBROUTINE haversine_polyheadings(nbpt, nbseg, lonpoly, latpoly, center, headings)
    ...
    INTEGER, INTENT(in)                                 :: nbseg
    !
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(in)           :: lonpoly
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(in)           :: latpoly
    REAL, DIMENSION(nbpt,2), INTENT(in)                 :: center
    !
    ! 0.2 Output variables
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(out)          :: headings
    !
    ! 0.3 Local variables
    !
    INTEGER :: i, ns
...
!!
  SUBROUTINE haversine_polysort(nbpt, nbseg, lonpoly, latpoly, headings, neighb)
    ...
    INTEGER, INTENT(in)                                 :: nbseg
    !
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(inout)        :: lonpoly
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(inout)        :: latpoly
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(inout)        :: headings
    INTEGER, DIMENSION(nbpt,nbseg*2), INTENT(inout)     :: neighb
    !
    ! 0.2 Local variables
    !
    INTEGER :: i, ic, startindex
    INTEGER, DIMENSION(nbseg*2)     :: reindex
    REAL                            :: starthead
    REAL, DIMENSION(nbseg*2)        :: lon_loc
    REAL, DIMENSION(nbseg*2)        :: lat_loc
    REAL, DIMENSION(nbseg*2)        :: head_loc
    REAL, DIMENSION(nbseg*2)        :: negb_loc
    !
    DO i=1,nbpt
       head_loc(:) = headings(i,:)
       lon_loc(:) = lonpoly(i,:)
...
!!
  SUBROUTINE haversine_polyseglen(nbpt, nbseg, lonpoly, latpoly, seglength)
    ...
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(in)           :: lonpoly
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(in)           :: latpoly
    REAL, DIMENSION(nbpt,nbseg), INTENT(out)            :: seglength
    !
    ! 0.2 Local variables
    !
    INTEGER :: i, iv, istart, iend, iseg, ioff
    REAL    :: slpm1, slpp1
    !
    DO i=1,nbpt
       iseg = 0
...
!!
  SUBROUTINE haversine_laloseglen(nbpt, nbseg, lonpoly, latpoly, seglength)
    ...
    ! 0.2 Local variables
    !
    INTEGER :: i, iv, istart, iend, ioff, iseg
    REAL    :: coslat, slpm1, slpp1
    !
    DO i=1,nbpt
       iseg = 0
       !
...
!!
  SUBROUTINE haversine_clockwise(nbseg, heading, start, sortindex)
    ...
    ! Number of segments
    INTEGER, INTENT(in)                                 :: nbseg
    REAL, DIMENSION(nbseg*2), INTENT(in)                :: heading
    REAL, INTENT(in)                                    :: start
    ! 
    ! 0.2 Output variable
    !
    INTEGER, DIMENSION(nbseg*2), INTENT(out)            :: sortindex
    !
    ! 0.3 Local variables
    !
    INTEGER :: is, js, imin(1)
    REAL    :: delang
    REAL    :: undef = 9999999999.99999
    REAL, DIMENSION(nbseg*2) :: workhead
    !
...
!!
  SUBROUTINE haversine_polyarea(nbpt, nbseg, lonpoly, latpoly, area)
    ...
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(in)           :: lonpoly
    REAL, DIMENSION(nbpt,nbseg*2), INTENT(in)           :: latpoly
    REAL, DIMENSION(nbpt), INTENT(out)                  :: area
    !
    ! 0.2 Local variables
    !
    INTEGER :: i, ia, ib1, ib2, iseg
    REAL    :: beta1, beta2
    REAL, DIMENSION(nbseg) :: angles
    !
    DO i=1,nbpt
       iseg=0
       DO ia=1,nbseg*2,2
...
!!
  SUBROUTINE haversine_laloarea(nbpt, nbseg, seglen, area)
    ...
    INTEGER, INTENT(in)                                 :: nbseg
    !
    REAL, DIMENSION(nbpt,nbseg), INTENT(in)             :: seglen
    REAL, DIMENSION(nbpt), INTENT(out)                  :: area
    !
    ! 0.2 Local variables
    !
...
!!
  SUBROUTINE haversine_xyarea(nbpt, nbseg, ilandtoij, jlandtoij, dx, dy, area)
    ...
    INTEGER, INTENT(in)                                 :: nbseg
    !
    INTEGER, DIMENSION(nbpt), INTENT(in)                :: ilandtoij, jlandtoij
...
!! \n
!_ ==============================================================================================================================
  REAL FUNCTION haversine_heading(lon_start, lat_start, lon_end, lat_end)
    !
    ! The heading is an angle in degrees between 0 and 360.
    !
    IMPLICIT NONE
    REAL, INTENT(IN) :: lon_start, lat_start, lon_end, lat_end
    !
    REAL :: dlon, lat1, lat2, y, x
    !
  ...
  !!
  !!
  REAL FUNCTION haversine_distance(lon_start, lat_start, lon_end, lat_end)
    IMPLICIT NONE
    REAL, INTENT(IN) :: lon_start, lat_start, lon_end, lat_end
    !
    REAL :: dlon, dlat, lat1, lat2, a, c
    !
  ...
  FUNCTION haversine_radialdis(lon_start, lat_start, head, dis)
    IMPLICIT NONE
    REAL, INTENT(IN)   :: lon_start, lat_start, head, dis
    REAL, DIMENSION(2) :: haversine_radialdis
    !
    REAL :: lonout, latout, lat1, lon1, lat, lon, dlon, tc
    !
  ...
  ! --------------------------------------------------------------------
  REAL FUNCTION  haversine_dtor(Degree)
    IMPLICIT  NONE
  ...
  ! --------------------------------------------------------------------
  REAL FUNCTION  haversine_rtod(Radian)
    IMPLICIT  NONE
