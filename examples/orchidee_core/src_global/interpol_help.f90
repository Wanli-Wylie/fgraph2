...
!
!
!< $HeadURL: svn://forge.ipsl.jussieu.fr/orchidee/branches/ORCHIDEE-CN-P/ORCHIDEE/src_global/interpol_help.f90 $
...
CONTAINS
  ...
  SUBROUTINE aggregate_2d (nbpt, lalo, neighbours, resolution, contfrac, &
    ...
    INTEGER, INTENT(in)   :: neighbours(nbpt,NbNeighb)! Vector of neighbours for each grid point (1=N, 2=E, 3=S, 4=W)
    ...
    INTEGER, INTENT(in)   :: iml, jml             ! Size of the finer grid
    REAL, INTENT(in)      :: lon_rel(iml, jml)    ! Longitudes for the finer grid
    REAL, INTENT(in)      :: lat_rel(iml, jml)    ! Latitudes for the finer grid
    INTEGER, INTENT(in)   :: mask(iml, jml)       ! Mask which retains only the significative points
                                                         ! of the fine grid.
    CHARACTER(LEN=*), INTENT(in) :: callsign             ! Allows to specify which variable is beeing treated
    INTEGER, INTENT(in)   :: incmax              ! Maximum point of the fine grid we can store.
    INTEGER, OPTIONAL, INTENT(in)    :: opt_nbpt_start            ! Input Start grid cell interpolation 
    INTEGER, OPTIONAL, INTENT(in)    :: opt_nbpt_end              ! Input End grid cell interpolation
    !
    ! Output
    !
    INTEGER, INTENT(out)  :: indinc(:,:,:)
    REAL, INTENT(out)      :: areaoverlap(:,:)
    LOGICAL, OPTIONAL, INTENT(out)      :: ok            ! return code
    !
    ! Local Variables
    !
    REAL, ALLOCATABLE, DIMENSION(:,:) :: lat_ful, lon_ful
    REAL, ALLOCATABLE, DIMENSION(:,:) :: loup_rel, lolow_rel, laup_rel, lalow_rel
    INTEGER, ALLOCATABLE, DIMENSION(:,:) :: searchind
    REAL :: lon_up, lon_low, lat_up, lat_low
    REAL :: coslat, ax, ay, sgn, lonrel, lolowrel, louprel
    INTEGER :: fopt, fopt_max, ip, jp, ib, i, itmp, iprog, nbind
    REAL :: domain_minlon,domain_maxlon,domain_minlat,domain_maxlat
    INTEGER :: minLon(1), maxLon(1)
    INTEGER :: nbpt_start            ! Start grid cell interpolation 
    INTEGER :: nbpt_end              ! End grid cell interpolation
    INTEGER :: landpoint_idx 
   ...
   &                   '(',domain_maxlat,',',domain_maxlon,')'
    ...
    IF ( global ) THEN
       ...
       IF (printlev >= 3) WRITE(numout,*) 'In aggregate_p : do interpolation to global model domain'
       nbind=0
       DO jp=1,jml
          DO ip=1,iml
    ...
    ELSE
       ...
       IF (printlev >= 3) WRITE(numout,*) 'In aggregate_p : do interpolation to regional model domain'
       nbind=0
       DO jp=1,jml
          DO ip=1,iml
    ...
    ENDIF
    !
    IF (printlev >= 3) WRITE(numout,*) 'We will work with ', nbind, ' points of the fine grid'
    !
    IF (printlev >= 3) WRITE(numout,*) 'Aggregate_2d : ', callsign
          ...
          IF ( lonrel > lon_low .AND. lonrel < lon_up .OR. &
             ...
             IF ( lat_rel(ip,jp) > lat_low .AND. lat_rel(ip,jp) < lat_up .OR. &
                ...
                ELSE
                   ...
                   ! 
                   IF ( lon_low >= -180.0 .AND. lon_up <= 180.0 ) THEN
                      lolowrel = lolow_rel(ip,jp)
                      louprel = loup_rel(ip,jp)
                   ELSE IF ( lon_low < -180.0 ) THEN
                      lolowrel = MOD( lolow_rel(ip,jp) - 360.0, 360.0)
    ...
    IF (printlev >= 3) WRITE(numout,*) "aggregate_2D nbvmax = ",incmax, "max used = ",fopt_max
    !
  ...
  SUBROUTINE aggregate_vec (nbpt, lalo, neighbours, resolution, contfrac, &
    ...
    REAL, INTENT(in)      :: resolution(nbpt,2)   ! The size in km of each grid-box in X and Y
    REAL, INTENT(in)      :: contfrac(nbpt)       ! Fraction of land in each grid box.
    INTEGER, INTENT(in)   :: iml                  ! Size of the finer grid
    REAL, INTENT(in)      :: lon_rel(iml)         ! Longitudes for the finer grid
    REAL, INTENT(in)      :: lat_rel(iml)         ! Latitudes for the finer grid
    REAL, INTENT(in)      :: resol_lon, resol_lat ! Resolution in meters of the fine grid
    CHARACTER(LEN=*), INTENT(in) :: callsign             ! Allows to specify which variable is beeing treated
    INTEGER, INTENT(in)   :: incmax              ! Maximum point of the fine grid we can store.
    !
    ! Output
    !
    INTEGER, INTENT(out)  :: indinc(nbpt,incmax)
    REAL, INTENT(out)      :: areaoverlap(nbpt,incmax)
    LOGICAL, OPTIONAL, INTENT(out)      :: ok            ! return code
    !
    ! Local Variables
    !
    INTEGER, ALLOCATABLE, DIMENSION(:) :: searchind
    REAL :: lon_up, lon_low, lat_up, lat_low
    REAL :: coslat, ax, ay, lonrel, lolowrel, louprel
    REAL :: latrel, lauprel, lalowrel
    INTEGER, DIMENSION(nbpt) :: fopt
    INTEGER :: fopt_max, not_found_fopt
    INTEGER :: ip, ib, i, j, itmp, iprog, nbind, pp, ipp
    REAL :: domain_minlon,domain_maxlon,domain_minlat,domain_maxlat
    REAL :: minlon, minlat, mini
    INTEGER :: ff(1), incp
    INTEGER, ALLOCATABLE, DIMENSION(:,:) :: fine_ind
    INTEGER :: pos_pnt(5)
    INTEGER                  :: ALLOC_ERR
    !
    LOGICAL :: err_fopt
    ...
    ENDDO
    !
    IF (printlev >= 3) WRITE(numout,*) 'Domaine LON range : ', domain_minlon, domain_maxlon
    IF (printlev >= 3) WRITE(numout,*) 'Domaine LAT range : ', domain_minlat, domain_maxlat
    !
    ! we list a first approximation of all point we will need to 
    ! scan to fill our coarse grid.
    !
    ...
    ENDIF
    !
    IF (printlev >= 3) WRITE(numout,*) 'We will work with ', nbind, ' points of the fine grid and ', nbpt, 'for the coarse grid'
    !
    IF (printlev >= 3) WRITE(numout,*) 'Aggregate_vec : ', callsign
    !
    !   Now we take each grid point and find out which values from the forcing we need to average
    !
    fopt(:) = zero
    ...
    IF (printlev >= 3) WRITE(numout,*) "aggregate_vec : ",not_found_fopt, &
         & "did not find any corresponding data in the input file."
    IF (printlev >= 3) WRITE(numout,*) "aggregate_vec : This is ", not_found_fopt/REAL(nbpt, r_std)*100., &
         & " % of the grid"
    IF (printlev >= 3) WRITE(numout,*) "aggregate_vec : nbvmax = ",incmax, "max used = ",fopt_max
    !
    ! Do some memory management.
    !
    DEALLOCATE (searchind)
...
!
  SUBROUTINE aggregate_vec_p(nbpt, lalo, neighbours, resolution, contfrac,          &
    ...
    REAL, INTENT(in)      :: lalo(nbpt,2)
    INTEGER, INTENT(in)   :: neighbours(nbpt,NbNeighb)   
    REAL, INTENT(in)      :: resolution(nbpt,2)   
    REAL, INTENT(in)      :: contfrac(nbpt)       
    INTEGER, INTENT(in)   :: iml                 
    REAL, INTENT(in)      :: lon_ful(iml)         
    REAL, INTENT(in)      :: lat_ful(iml)         
    REAL, INTENT(in)      :: resol_lon, resol_lat 
    CHARACTER(LEN=*), INTENT(in) :: callsign             
    INTEGER, INTENT(in)   :: nbvmax             
    INTEGER, INTENT(out)  :: sub_index(nbpt,nbvmax)
    REAL, INTENT(out)     :: sub_area(nbpt,nbvmax) 
    LOGICAL, OPTIONAL, INTENT(out)  :: ok            ! return code

    INTEGER  :: sub_index_g(nbp_glo,nbvmax)
    REAL       :: sub_area_g(nbp_glo,nbvmax)

    IF ( INDEX(GridType,"RegLonLat") > 0) THEN
       IF (is_root_prc) CALL aggregate_vec(nbp_glo, lalo_g, neighbours_g, resolution_g, contfrac_g, &
            &                                  iml, lon_ful, lat_ful, resol_lon, resol_lat, callsign,   &
            &                                  nbvmax, sub_index_g, sub_area_g, ok)
    ELSE IF ( INDEX(GridType,"RegXY") > 0) THEN
       IF ( proj_stack(1)%code > undef_int-1 ) THEN
          CALL ipslerr(3, "aggregate_vec_p", "RegXY projection was not intialized.", &
               &       "proj_stack(1)%code is undefined","")
       ENDIF
       IF (is_root_prc) CALL interregxy_aggrve(nbp_glo, lalo_g, neighbours_g, resolution_g, contfrac_g, &
            &                                  iml, lon_ful, lat_ful, resol_lon, resol_lat, callsign,   &
            &                                  nbvmax, sub_index_g, sub_area_g, ok)
    ELSE
       CALL ipslerr(3, "aggregate_vec_p", "Interpolation is only possible for regular lat/lon grids for the moment.", &
            &       "More general interpolation will be implemented soon.","")
    ENDIF
    !
    CALL BCAST(ok)
  ...
  SUBROUTINE aggregate_2d_p(nbpt, lalo, neighbours, resolution, contfrac,          &
    ...
    REAL, INTENT(in)        :: lon_ful(iml,jml)         
    REAL, INTENT(in)        :: lat_ful(iml,jml)         
    INTEGER, INTENT(in)     :: mask(iml, jml)
    CHARACTER(LEN=*), INTENT(in)   :: callsign             
    INTEGER, INTENT(in)     :: nbvmax             
    INTEGER, INTENT(out)    :: sub_index(nbpt,nbvmax,2)
    REAL, INTENT(out)       :: sub_area(nbpt,nbvmax) 
    LOGICAL, OPTIONAL, INTENT(out) :: ok            ! return code
    INTEGER                 :: nbp_start, nbp_end


    INTEGER, ALLOCATABLE, DIMENSION(:,:,:)  :: sub_index_g 
    REAL, ALLOCATABLE, DIMENSION(:,:)       :: sub_area_g 
    ...
    ELSE IF ( INDEX(GridType,"RegXY") > 0) THEN
       ...
       IF ( proj_stack(1)%code > undef_int-1 ) THEN
          CALL ipslerr(3, "aggregate_2d_p", "RegXY projection was not intialized.", &
               &       "proj_stack(1)%code is undefined","")
       ENDIF
       IF (is_root_prc) CALL interregxy_aggr2d(nbp_glo, lalo_g, neighbours_g, resolution_g, contfrac_g, &
            &                               iml, jml, lon_ful, lat_ful, mask, callsign,   &
            &                               nbvmax, sub_index_g, sub_area_g, ok)
