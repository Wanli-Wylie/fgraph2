...
!_ ================================================================================================================================
    ...
    SUBROUTINE gauss_jordan_method(n,matrix_a,vector_b)
      ...
      INTEGER, INTENT(in) :: n  !! size of the system (1-N, unitless)

      !! 0.3 Modified variables

      REAL, DIMENSION(n,n), INTENT(inout) :: matrix_a   !! Matrix A of the linear system A.X = B
      REAL, DIMENSION(n), INTENT(inout)   :: vector_b   !! Vector B in the linear system A.X = B
      
      !! 0.4 Local Variables

      INTEGER :: i,col,row,j,k,ii,jj        !! index (unitless)
      INTEGER, DIMENSION(n) :: index_pivot  !! vector containing the pivot index
      INTEGER, DIMENSION(n) :: index_col    !! vector containing the columns index
      INTEGER, DIMENSION(n) :: index_row    !! vector containing the rows index
      REAL :: pivot_max,inv_pivot,temp      !! temporary variables
...
!_ ================================================================================================================================
 ...
 SUBROUTINE error_L1_passive(npts,nb_veget, nb_pools, current_value, previous_value, veget_max, criterion, flag)
    ...
    INTEGER, INTENT(in) :: npts                                             !! Number of continental grid cells (unitless)
    INTEGER, INTENT(in) :: nb_veget                                         !! Number of vegetation types (2-N, unitless)
    INTEGER, INTENT(in) :: nb_pools                                         !! Number of carbon pools (unitless)
    REAL, DIMENSION(npts,nb_veget,nb_pools), INTENT(in) :: current_value    !! Previous values of carbon pools obtained
                                                                                   !! by matrix resolution (gC.m^{-2})
    REAL, DIMENSION(npts,nb_veget,nb_pools), INTENT(in) :: previous_value   !! Current values of carbon pools  obtained
                                                                                   !! by matrix resolution (gC.m^{-2})   
    REAL, DIMENSION(npts,nb_veget), INTENT(in) :: veget_max                 !! Fraction of vegetation (0-1, uniless)
    REAL, INTENT(in) :: criterion                                           !! Threshold for the relativ error (0-1, unitless) 
    ...
    REAL, DIMENSION(npts) :: previous_passive_stock   !! Previous value of total passive carbon (gC) 
    REAL, DIMENSION(npts) :: current_passive_stock    !! Current value of total passive carbon (gC)
    REAL, DIMENSION(npts) :: error_global             !! Temporary arrays containing the relative error for each grid cell 
                                                             !! (unitless)         
    REAL, DIMENSION(npts) :: temp_diff                !! Working array storing difference values between previous_passive_stock
